  ...
  implements ProtocolFactory
{
    ...
    throws ProtocolCreationException
  {
    ...
    if ((message.getOperation() instanceof UpnpRequest))
    {
      ...
      {
      case NOTIFY: 
        return (isByeBye(incomingRequest)) || (isSupportedServiceAdvertisement(incomingRequest)) ? createReceivingNotification(incomingRequest) : null;
      case MSEARCH: 
    ...
    throws ProtocolCreationException
  {
    ...
    if (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.GET)) {
      return createReceivingRetrieval(message);
    }
    if (getUpnpService().getConfiguration().getNamespace().isControlPath(message.getUri()))
    {
      if (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.POST)) {
    ...
    {
      if (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.SUBSCRIBE)) {
        return createReceivingSubscribe(message);
      }
      if (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.UNSUBSCRIBE)) {
    ...
    {
      if (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.NOTIFY)) {
    ...
    else if (message.getUri().getPath().contains("/event/cb"))
    {
      ...
      message.setUri(
        URI.create(invalid.substring(0, invalid
        .indexOf("/cb") + "/cb".length())));
      if ((getUpnpService().getConfiguration().getNamespace().isEventCallbackPath(message.getUri())) && 
        (((UpnpRequest)message.getOperation()).getMethod().equals(UpnpRequest.Method.NOTIFY))) {
        return createReceivingEvent(message);
      }
    }
    ...
    throws ProtocolCreationException
  {
    try
    {
      List<NetworkAddress> activeStreamServers = getUpnpService().getRouter().getActiveStreamServers(
        ((RemoteDeviceIdentity)((RemoteDevice)((RemoteService)subscription.getService()).getDevice()).getIdentity()).getDiscoveredOnLocalAddress());
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\org\fourthline\cling\protocol\ProtocolFactoryImpl.class
