  ...
  implements MonetaryConstants
{
  ...
  private static void loadReimb(String name)
    throws IOException
  {
    if (Servers.localServer.id == Servers.loginServer.id)
    {
  ...
  public static String addReimb(String changerName, String name, int numMonths, int _silver, int _daysLeft, boolean setbok)
  {
    ...
    if (r != null)
    {
      ...
      catch (IOException ex)
      {
        ...
        return Servers.localServer.name + " - Error - problem saving to database. Player was awarded though and may withdraw anyway. " + ex
  ...
  public static boolean withDraw(String retriever, String name, String _email, int _months, int _silvers, boolean titlebok, int _daysLeft)
  {
    ...
    if (r != null) {
      if (r.email.toLowerCase().equals(_email.toLowerCase()))
      {
        ...
        logger.log(Level.WARNING, name + " does not match email: " + r.email.toLowerCase() + " with submitted email " + _email
          .toLowerCase());
      }
    }
    return false;
  ...
  private boolean withDraw(String aName, int _months, int _silvers, boolean titlebok, int _daysLeft)
  {
    ...
    if (!this.deleted) {
      if ((_months > 0) || (_silvers > 0) || (titlebok) || (_daysLeft > 0))
      {
        if ((_months < 0) || (_silvers < 0) || (_daysLeft < 0)) {
          return false;
        }
        if ((_months > this.months) || (_silvers > this.silver) || ((titlebok == true) && (!this.titleAndBok)) || (_daysLeft > this.daysLeft)) {
          return false;
        }
  ...
  private static Reimbursement[] getReimbsForEmail(String _email)
  {
    ...
    for (Iterator<Reimbursement> it = reimbursements.values().iterator(); it.hasNext();)
    {
      Reimbursement r = (Reimbursement)it.next();
      if ((!r.deleted) && (r.email.toLowerCase().equals(_email))) {
        if ((r.months > 0) || (r.silver > 0) || (r.titleAndBok) || (r.daysLeft > 0)) {
          set.add(r);
        }
      }
    }
    return (Reimbursement[])set.toArray(new Reimbursement[set.size()]);
  }
  ...
  public static String getReimbursementInfo(String _email)
  {
    ...
    if (reimbs.length > 0)
    {
      ...
      buf
        .append("text{type='italic';text='Note that within a few months the kingdom of Mol-Rehan may emerge. You may want to save some if you want to try a MR character.'}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\players\Reimbursement.class
