  ...
  implements TimeConstants, MiscConstants
{
  ...
  public static final byte getPathFor(int tilex, int tiley, int layer, int performerHeight)
  {
    ...
    for (lCloseTile : closeTiles) {
      for (int y = 0; y < closeTiles.length; y++)
      {
        ...
        Integer nums = (Integer)dominatingType.get(Byte.valueOf(type));
        if (nums == null) {
        ...
        dominatingType.put(Byte.valueOf(type), nums);
        ...
        {
          if (theTile.isThorn(Tiles.decodeData(lCloseTile[y]))) {
            thornTiles++;
          }
        }
      }
    ...
    int maxCount = 0;
    for (Map.Entry<Byte, Integer> e : dominatingType.entrySet())
    {
      int count = ((Integer)e.getValue()).intValue();
      if (count > maxCount)
      {
        secondType = domType;
        domType = ((Byte)e.getKey()).byteValue();
        maxCount = count;
      }
    }
    ...
    {
      if (!belowRockfall) {
        if ((domType == Tiles.Tile.TILE_ROCK.id) || (domType == Tiles.Tile.TILE_CLIFF.id) || (quickDropoff)) {
          return 5;
        }
      }
      return 0;
    }
    if ((Tiles.getTile(domType).isMycelium()) && (!nearWater)) {
      if ((hateEnabler) || (thornTiles > 3)) {
        return 2;
      }
    }
    ...
    if ((!nearWater) && (farWater) && (secondType == Tiles.Tile.TILE_GRASS.id)) {
      if (flowerTiles > 4) {
        path = 1;
      }
    }
  ...
  public static final boolean meditate(Creature performer, int layer, Action aAct, float counter, Item rug)
  {
    ...
    if ((performer.getPower() == 0) && (Math.abs(performer.getMeditateX() - performer.getTileX()) < 10) && 
    ...
    if (cultist == null) {
      if (meditation.getKnowledge(0.0D) >= 20.0D)
      {
        increaseSkill = false;
        if (counter == 1.0F) {
    ...
    if (counter == 1.0F)
    {
      ...
      performer.sendActionControl(com.wurmonline.server.behaviours.Actions.actionEntrys['ƀ'].getVerbString(), true, time);
    ...
    else if (aAct.currentSecond() == 5)
    {
      ...
      if (meditation.skillCheck(difficulty, rug, 0.0D, true, 1.0F + rug.getRarity() * 0.1F) < 0.0D)
      {
      ...
      performer.getCommunicator().sendNormalServerMessage("You fall into a trance. " + 
        getMeditationString(path, level, tilex, tiley));
    }
    if (counter * 10.0F > time)
    {
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " finishes " + performer.getHisHerItsString() + " meditation.", performer, 5);
      if (cultist != null) {
        if (meditation.getKnowledge(0.0D) >= 20.0D)
        {
          long timeSinceLast = System.currentTimeMillis() - cultist.getLastMeditated();
          if ((increaseSkill) && 
            (cultist.getSkillgainCount() > 0)) {
            if (timeSinceLast > (Servers.localServer.isChallengeServer() ? 1800000L : 10800000L)) {
      ...
      if (meditation.skillCheck(95.0D, rug, 0.0D, !increaseSkill, 1.0F + rug.getRarity() * 0.1F) > 0.0D)
      {
      ...
      if (path != 0)
      {
        ...
        if (cultist != null)
        {
          timeToNextLevel = cultist.getTimeLeftToIncreasePath(System.currentTimeMillis(), meditation.getKnowledge(0.0D));
          if (timeToNextLevel <= 0L)
          {
            if ((cultist.getLevel() * 10 - meditation.getKnowledge(0.0D) < 30.0D) || (meditation.getKnowledge(0.0D) > 90.0D))
            {
        ...
        else if (meditation.getKnowledge(0.0D) >= 15.0D)
        {
          mayIncreaseLevel = true;
        }
        else
      ...
      power = meditation.skillCheck(difficulty, rug, 0.0D, !increaseSkill, counter / 3.0F * (1.0F + rug.getRarity() * 0.1F));
  ...
  static String getWaitForProgressMessage(long timeToNextLevel)
  {
    ...
    {
      String message;
      if (timeToNextLevel > 1209600000L)
      {
  ...
  public static final String getMeditationString(byte path, byte level, int tilex, int tiley)
  {
    ...
    {
      int pathForMessage = 0;
      
      int baseRand = random.nextInt(60);
      max = 60;
    }
    else
    {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\players\Cults.class
