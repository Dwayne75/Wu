  ...
  implements TimeConstants
{
  ...
  {
    if ((usedItem.isRiftLoot()) && (isTimed(usedItem))) {
      if (WurmCalendar.currentTime - usedItem.getLastMaintained() > 28800L)
      {
  ...
  private static final void addBonus(Item item, Creature creature, int bonusType, float value, boolean isStacking)
  {
    ...
    Map<Integer, ItemBonus> curr = (Map)playerBonuses.get(Long.valueOf(creature.getWurmId()));
    if (curr == null)
    {
      curr = new ConcurrentHashMap();
      playerBonuses.put(Long.valueOf(creature.getWurmId()), curr);
    }
    ItemBonus cbonus = (ItemBonus)curr.get(Integer.valueOf(bonusType));
    if (cbonus == null)
    {
      cbonus = new ItemBonus(item, creature.getWurmId(), bonusType, value, isStacking);
      curr.put(Integer.valueOf(bonusType), cbonus);
    }
    else
    {
  ...
  public final void sendNewBonusToClient(Item item, Creature creature)
  {
    if (item != null)
    {
      if ((!isTimed(item)) || (getSecondsLeft() > 0))
      {
        SpellEffectsEnum senum = SpellEffectsEnum.getEnumForItemTemplateId(item.getTemplateId(), item.getData1());
        creature.getCommunicator().sendAddSpellEffect(senum, getSecondsLeft(), getBonusVal(0.0F));
      }
    }
    else {
  ...
  {
    Map<Integer, ItemBonus> curr = (Map)playerBonuses.get(Long.valueOf(player.getWurmId()));
    if (curr != null) {
  ...
  public static final void removeBonus(Item item, Creature creature)
  {
    ...
    SpellEffectsEnum senum = SpellEffectsEnum.getEnumForItemTemplateId(item.getTemplateId(), item.getData1());
    ItemBonus cbonus = (ItemBonus)curr.get(Integer.valueOf(senum.getTypeId()));
    if (cbonus != null)
    {
      ...
      if (cbonus.getItemOne() == item) {
        if (cbonus.getItemTwo() != null)
        {
          ...
          curr.put(Integer.valueOf(cbonus.getBonusType()), newBonus);
          newBonus.sendNewBonusToClient(item, creature);
        }
        else
        {
          curr.remove(Integer.valueOf(cbonus.getBonusType()));
  ...
  {
    playerBonuses.remove(Long.valueOf(playerid));
  }
  ...
  {
    Map<Integer, ItemBonus> curr = (Map)playerBonuses.get(Long.valueOf(playerId));
    if (curr == null) {
      return null;
    }
    return (ItemBonus)curr.get(Integer.valueOf(bonusType));
  }
  
  public final float getBonusVal(float damageDealt)
  {
    if (isStacking()) {
      return getItemOneBonusValue(damageDealt) + getItemTwoBonusValue(damageDealt);
    }
    if (getItemOneBonusValue(0.0F) > getItemTwoBonusValue(0.0F)) {
      return getItemOneBonusValue(damageDealt);
    }
    return getItemTwoBonusValue(damageDealt);
  }
  
  public final int getSecondsLeft()
  {
    if (isStacking()) {
      return Math.min(getSeconds1Left(), getSeconds2Left());
    }
    if (getItemOneBonusValue(0.0F) > getItemTwoBonusValue(0.0F)) {
      return getSeconds1Left();
    }
    return getSeconds2Left();
  }
  ...
  public static final float getGlanceBonusFor(ArmourTemplate.ArmourType armourType, byte woundType, Item weapon, Creature creature)
  {
    ...
    else if ((armourType == ArmourTemplate.ARMOUR_TYPE_STUDDED) && 
      (weapon.isTwoHanded()))
    {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\players\ItemBonus.class
