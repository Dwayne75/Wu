  ...
  implements MiscConstants, TimeConstants
{
  ...
  private static final boolean ascend(Creature responder, byte deityId, int data)
  {
    ...
    try
    {
      Players.getInstance().sendGlobalNonPersistantTimedEffect(0L, (short)19, responder.getTileX(), responder.getTileY(), responder
        .getPositionZ(), System.currentTimeMillis() + 600000L);
      Players.getInstance().sendGlobalNonPersistantTimedEffect(0L, (short)4, responder.getTileX(), responder.getTileY(), responder
        .getPositionZ(), System.currentTimeMillis() + 600000L);
      Players.getInstance().sendGlobalNonPersistantTimedEffect(0L, (short)22, responder.getTileX(), responder.getTileY(), responder
        .getPositionZ(), System.currentTimeMillis() + 600000L);
    ...
    for (int x = 0; x < 3; x++)
    {
      ...
      if (responder.getSex() == 0) {
        ...
        default: 
          ...
          throw new AssertionError(num + " is not possible");
        }
      } else {
        switch (num)
        {
    ...
    float bodyStr = (float)(s.getSkillOrLearn(102).getKnowledge(0.0D) - 20.0D);
    float bodySta = (float)(s.getSkillOrLearn(103).getKnowledge(0.0D) - 20.0D);
    float bodyCon = (float)(s.getSkillOrLearn(104).getKnowledge(0.0D) - 20.0D);
    float mindLog = (float)(s.getSkillOrLearn(100).getKnowledge(0.0D) - 20.0D);
    float mindSpe = (float)(s.getSkillOrLearn(101).getKnowledge(0.0D) - 20.0D);
    float soulStr = (float)(s.getSkillOrLearn(105).getKnowledge(0.0D) - 20.0D);
    float soulDep = (float)(s.getSkillOrLearn(106).getKnowledge(0.0D) - 20.0D);
    
    responder.getCommunicator().sendNormalServerMessage(lsw
      .ascend(Deities.getNextDeityNum(), responder.getName(), responder.getWurmId(), deityId, responder.getSex(), (byte)2, bodyStr, bodySta, bodyCon, mindLog, mindSpe, soulStr, soulDep));
  ...
  public static final boolean useItem(Creature performer, Item item, Action act, float counter)
  {
    if (!item.isAbility())
    {
      performer.getCommunicator().sendNormalServerMessage("The " + item
        .getName() + " makes no sense to you.");
      return true;
    }
    if (item.isStreetLamp())
    {
      performer.getCommunicator().sendNormalServerMessage("You need to plant the " + item
        .getName() + " for it to have effect.");
      return true;
    }
    if (alreadyHasAbilityForItem(item, performer))
    {
      performer.getCommunicator().sendNormalServerMessage("You already know the secrets that the " + item
        .getName() + " contains.");
      return true;
    }
    ...
    if (counter == 1.0F)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You try to solve the puzzle of the " + item
          .getName() + ".");
        Server.getInstance().broadCastAction(performer
          .getName() + " starts solving the puzzle of the " + item.getName() + ".", performer, 5);
      }
      else
      {
        performer.getCommunicator().sendNormalServerMessage("You start using the " + item
          .getName() + ".");
        Server.getInstance().broadCastAction(performer
          .getName() + " starts using " + item.getNameWithGenus() + ".", performer, 5);
      }
      performer.sendActionControl(com.wurmonline.server.behaviours.Actions.actionEntrys[118].getVerbString(), true, time);
    ...
    else if (act.currentSecond() > time / 10)
    {
      ...
      if (item.getTemplateId() == 794)
      {
        try
        {
          if (performer.getSkills().getSkill(100).skillCheck(25.0D, 0.0D, false, 30.0F) > 0.0D)
          {
  ...
  private static final void sendUseMessage(Item item, Creature performer)
  {
    ...
    case 796: 
      ...
      performer.getStatus().modifyThirst(65535.0F);
      performer.getStatus().modifyHunger(65535, 0.0F);
      performer.addWoundOfType(null, (byte)6, 21, false, 1.0F, true, 10000.0D, 10.0F, 0.0F, false, false);
    ...
    case 809: 
      ...
      if (!performer.addWoundOfType(null, (byte)4, 14, false, 1.0F, true, 5000.0D, 0.0F, 0.0F, false, false)) {
        performer.addWoundOfType(null, (byte)4, 13, false, 1.0F, true, 5000.0D, 0.0F, 0.0F, false, false);
      }
      break;
    case 797: 
      if (!performer.addWoundOfType(null, (byte)10, 1, false, 1.0F, false, 10000.0D, 0.0F, 0.0F, false, false)) {
        performer.toggleFrozen(performer);
      }
      break;
    case 799: 
      performer.getCommunicator().sendNormalServerMessage("The pages all seem to be water... waves.. flowing..");
      if (performer.addWoundOfType(null, (byte)7, 2, false, 1.0F, false, 20000.0D, 0.0F, 0.0F, false, false)) {
        return;
      }
      performer.toggleFrozen(performer);
      performer.getStatus().modifyStamina(-65535.0F);
    ...
    case 810: 
      ...
      performer.addWoundOfType(null, (byte)4, 1, false, 1.0F, false, 10000.0D, 0.0F, 0.0F, false, false);
    ...
    case 802: 
      ...
      CombatEngine.addWound(null, performer, (byte)1, 5, 10000.0D, 1.0F, "", null, 0.0F, 1.0F, false, false, false, false);
  ...
  public static final void sendEffectsToCreature(Creature c)
  {
    ...
    if (c.getFireResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.FIRE_RESIST, 100000, c
        .getFireResistance() * 100.0F);
    }
    if (c.getColdResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.COLD_RESIST, 100000, c
        .getColdResistance() * 100.0F);
    }
    if (c.getDiseaseResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.DISEASE_RESIST, 100000, c
      
        .getDiseaseResistance() * 100.0F);
    }
    if (c.getPhysicalResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.PHYSICAL_RESIST, 100000, c
        .getPhysicalResistance() * 100.0F);
    }
    if (c.getPierceResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.PIERCE_RESIST, 100000, c
        .getPierceResistance() * 100.0F);
    }
    if (c.getSlashResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.SLASH_RESIST, 100000, c
        .getSlashResistance() * 100.0F);
    }
    if (c.getCrushResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.CRUSH_RESIST, 100000, c
        .getCrushResistance() * 100.0F);
    }
    if (c.getBiteResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.BITE_RESIST, 100000, c
        .getBiteResistance() * 100.0F);
    }
    if (c.getPoisonResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.POISON_RESIST, 100000, c
        .getPoisonResistance() * 100.0F);
    }
    if (c.getWaterResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.WATER_RESIST, 100000, c
        .getWaterResistance() * 100.0F);
    }
    if (c.getAcidResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.ACID_RESIST, 100000, c
        .getAcidResistance() * 100.0F);
    }
    if (c.getInternalResistance() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.INTERNAL_RESIST, 100000, c
        .getInternalResistance() * 100.0F);
    }
    if (c.getFireVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.FIRE_VULNERABILITY, 100000, -100.0F + c
        .getFireVulnerability() * 100.0F);
    }
    if (c.getColdVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.COLD_VULNERABILITY, 100000, -100.0F + c
        .getColdVulnerability() * 100.0F);
    }
    if (c.getDiseaseVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.DISEASE_VULNERABILITY, 100000, -100.0F + c
        .getDiseaseVulnerability() * 100.0F);
    }
    if (c.getPhysicalVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.PHYSICAL_VULNERABILITY, 100000, -100.0F + c
      
        .getPhysicalVulnerability() * 100.0F);
    }
    if (c.getPierceVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.PIERCE_VULNERABILITY, 100000, -100.0F + c
        .getPierceVulnerability() * 100.0F);
    }
    if (c.getSlashVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.SLASH_VULNERABILITY, 100000, -100.0F + c
        .getSlashVulnerability() * 100.0F);
    }
    if (c.getCrushVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.CRUSH_VULNERABILITY, 100000, -100.0F + c
        .getCrushVulnerability() * 100.0F);
    }
    if (c.getBiteVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.BITE_VULNERABILITY, 100000, -100.0F + c
        .getBiteVulnerability() * 100.0F);
    }
    if (c.getPoisonVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.POISON_VULNERABILITY, 100000, -100.0F + c
        .getPoisonVulnerability() * 100.0F);
    }
    if (c.getWaterVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.WATER_VULNERABILITY, 100000, -100.0F + c
        .getWaterVulnerability() * 100.0F);
    }
    if (c.getAcidVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.ACID_VULNERABILITY, 100000, -100.0F + c
        .getAcidVulnerability() * 100.0F);
    }
    if (c.getInternalVulnerability() > 0.0F) {
      c.getCommunicator().sendAddSpellEffect(SpellEffectsEnum.INTERNAL_VULNERABILITY, 100000, -100.0F + c
      
        .getInternalVulnerability() * 100.0F);
    }
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\players\Abilities.class
