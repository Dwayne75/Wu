  ...
  implements MiscConstants, ItemMaterials
{
  ...
  {
    return (AwardLadder)resetLadder.get(Integer.valueOf(months));
  }
  ...
  public static final AwardLadder[] getLadderStepsForTotal(int months)
  {
    ...
    return (AwardLadder[])steps.toArray(new AwardLadder[steps.size()]);
  }
  
  public static final AwardLadder getLadderStepForSilver(int silver)
  {
    return (AwardLadder)silverLadder.get(Integer.valueOf(silver));
  }
  ...
  public static final AwardLadder getNextTotalAward(int totalMonthsSinceReset)
  {
    ...
    for (Map.Entry<Integer, AwardLadder> entry : resetLadder.entrySet()) {
      if (next == null)
      {
        if (((Integer)entry.getKey()).intValue() > totalMonthsSinceReset) {
          next = (AwardLadder)entry.getValue();
        }
      }
      else if ((((Integer)entry.getKey()).intValue() > totalMonthsSinceReset) && 
        (((Integer)entry.getKey()).intValue() < next.getMonthsRequiredReset())) {
        next = (AwardLadder)entry.getValue();
      }
    }
    return next;
  ...
  {
    for (Iterator localIterator1 = itemsToAward.entrySet().iterator(); localIterator1.hasNext();)
    {
      entry = (Map.Entry)localIterator1.next();
      for (Item i : (Set)entry.getValue())
      {
        ((Player)entry.getKey()).getInventory().insertItem(i, true);
        if (((Player)entry.getKey()).getCommunicator() != null) {
          ((Player)entry.getKey()).getCommunicator().sendSafeServerMessage("You receive " + i.getNameWithGenus() + " as premium bonus!");
        }
      }
    }
    Map.Entry<Player, Set<Item>> entry;
    itemsToAward.clear();
  }
  ...
  public static final void awardTotalLegacy(PlayerInfo p)
  {
    if (p.awards != null) {
      ...
      {
        logger.log(Level.WARNING, ex.getMessage() + " " + p.getName() + " " + p.awards.getMonthsPaidSinceReset() + ": " + p.awards
          .getMonthsPaidInARow(), ex);
      }
    }
  }
      ...
      try
      {
        if (tickedMonth)
        {
          ...
          try
          {
            ...
            player.getCommunicator().sendSafeServerMessage("You receive " + i
              .getNameWithGenus() + " at ql " + ql + " for staying premium!");
          }
          catch (NoSuchPlayerException nsp)
          {
            long inventoryId = DbCreatureStatus.getInventoryIdFor(p.wurmId);
          ...
          if (sinceReset != null)
          {
            if (sinceReset.getTitleAwarded() != null)
            {
              ...
              {
                Player player = Players.getInstance().getPlayer(p.wurmId);
                player.getCommunicator().sendSafeServerMessage("You receive the title " + sinceReset
                
                  .getTitleAwarded().getName(player.getSex() == 0) + " for staying premium!");
              }
            ...
            if (sinceReset.getItemNumberAwarded() > 0) {
              try
              {
                ...
                Item i = ItemFactory.createItem(sinceReset.getItemNumberAwarded(), sinceReset
                  .getQlOfItemAward(), "");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\players\AwardLadder.class
