  ...
  implements TimeConstants, MiscConstants
{
  ...
  Tracks()
  {
    this.tracks = new ConcurrentLinkedDeque();
  }
  ...
  final Track[] getTracksFor(int tilex, int tiley)
  {
    ...
    return (Track[])matches.toArray(toReturn);
  }
  ...
  final Track[] getTracksFor(int tilex, int tiley, int dist)
  {
    ...
    for (Track track : this.tracks)
    {
      ...
      else if ((tx == minx) && (ty > miny) && (ty < maxy)) {
        if ((dir >= 5) && (dir <= 7)) {
          add = true;
        }
      }
    ...
    Track[] toReturn = new Track[matches.size()];
    return (Track[])matches.values().toArray(toReturn);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\Tracks.class
