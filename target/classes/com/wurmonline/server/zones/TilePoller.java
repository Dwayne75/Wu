  ...
  implements TimeConstants, SoundNames, MiscConstants
{
  ...
  private static int MAX_KELP_HEIGHT = 65136;
  ...
  private static final long HIVE_PRIME = 102700L + WurmCalendar.getYear();
  ...
  private static void pollNextTile()
  {
    ...
    else
    {
      ...
      else if (tempbyte1 == Tiles.Tile.TILE_CAVE_WALL.id)
      {
        ...
        if (state == 0)
        {
          r.setSeed((temptilex + temptiley * Zones.worldTileSizeY) * 102533L);
          if (r.nextInt(100) == 0)
          {
            Server.caveMesh.setTile(temptilex, temptiley, 
            
              Tiles.encode(Tiles.decodeHeight(temptile1), Tiles.Tile.TILE_CAVE_WALL_ROCKSALT.id, 
              Tiles.decodeData(temptile1)));
            Players.getInstance().sendChangedTile(temptilex, temptiley, false, true);
          }
          r.setSeed((temptilex + temptiley * Zones.worldTileSizeY) * 123307L);
          if (r.nextInt(64) == 0)
          {
            Server.caveMesh.setTile(temptilex, temptiley, 
            
              Tiles.encode(Tiles.decodeHeight(temptile1), Tiles.Tile.TILE_CAVE_WALL_SANDSTONE.id, 
              Tiles.decodeData(temptile1)));
            Players.getInstance().sendChangedTile(temptilex, temptiley, false, true);
          }
        }
  ...
  private static void checkCaveDecay(int tile, int tilex, int tiley, byte type, int _data)
  {
    ...
    if (Tiles.decodeType(tile) == Tiles.Tile.TILE_CAVE_EXIT.id)
    {
      if (Server.rand.nextInt(60) == 0) {
        if (!Tiles.isMineDoor(Tiles.decodeType(Server.surfaceMesh.getTile(tilex, tiley))))
        {
          ...
          if ((decay) && (Tiles.decodeType(currentMesh.getTile(tilex, tiley + 1)) != Tiles.Tile.TILE_CAVE_WALL.id) && 
            (Tiles.decodeType(currentMesh.getTile(tilex, tiley + 1)) != Tiles.Tile.TILE_CAVE.id)) {
            decay = false;
          }
          if ((decay) && (Tiles.decodeType(currentMesh.getTile(tilex + 1, tiley)) != Tiles.Tile.TILE_CAVE_WALL.id) && 
    ...
    else
    {
      ...
      if (Tiles.decodeType(temptile1) == Tiles.Tile.TILE_CAVE_WALL.id) {
        if (Server.rand.nextFloat() <= 0.002F) {
          decay = true;
        }
      }
      if ((decay) && (Tiles.isReinforcedFloor(Tiles.decodeType(tile)))) {
        if (Server.rand.nextInt(10) < 8) {
          decay = false;
        }
      }
    }
  ...
  public static void checkEffects(int tile, int tilex, int tiley, byte type, byte aData)
  {
      ...
      if (hives != null) {
        for (Item hive : hives)
        {
          if (hive.hasTwoQueens()) {
            if ((hive.removeQueen()) && (Servers.isThisATestServer())) {
    ...
    {
      if ((!Features.Feature.CROP_POLLER.isEnabled()) && (Zones.protectedTiles[tilex][tiley] == 0)) {
    ...
    else if ((theTile.isMyceliumTree()) || (theTile.isMyceliumBush()))
    {
      if (!Servers.isThisAPvpServer())
      {
        ...
        currentMesh.setTile(tilex, tiley, 
          Tiles.encode(Tiles.decodeHeight(tile), newType, aData));
        Players.getInstance().sendChangedTile(tilex, tiley, pollingSurface, false);
      }
      else
      {
    ...
    {
      if (Zones.walkedTiles[tilex][tiley] == 0)
      {
        ...
        Zones.walkedTiles[tilex][tiley] = 0;
      }
    }
  ...
  static boolean checkForGrassGrowth(int tile, int tilex, int tiley, byte type, byte aData, boolean andFlowers)
  {
    ...
    {
      if (logger.isLoggable(Level.FINER)) {
        logger.log(Level.FINER, "tile [" + tilex + "," + tiley + "] changed: " + height + " type=" + 
          Tiles.getTile(type).getName() + " stage=" + growthStage
          
          .toString() + " flower=" + flowerType.toString().toLowerCase() + ".");
      }
  ...
  static boolean checkForSeedGrowth(int tile, int tilex, int tiley)
  {
    ...
    r.setSeed((tilex + tiley * Zones.worldTileSizeY) * 102563L);
    
    boolean canHaveHerb = (r.nextInt(herbChance) == 0) && (theTile.canBotanize());
    
    r.setSeed((tilex + tiley * Zones.worldTileSizeY) * 101531L);
  ...
  {
    r.setSeed((tilex + tiley * Zones.worldTileSizeY) * 104149L);
    boolean canHaveInvestigate = r.nextInt(investigateChance) == 0;
    if (containsStructure(tilex, tiley)) {
      return;
    }
    if (canHaveInvestigate) {
      if (!Server.isInvestigatable(tilex, tiley)) {
  ...
  {
    r.setSeed((tilex + tiley * Zones.worldTileSizeY) * 103025L);
  ...
  public static void checkForFarmGrowth(int tile, int tilex, int tiley, byte type, byte aData)
  {
    ...
    {
      if (((tileAge == 5) || (tileAge == 6)) && 
        (Server.rand.nextInt(tileAge) != 0)) {
        return;
      }
      tileAge += 1;
      ...
      {
        currentMesh.setTile(tilex, tiley, 
          Tiles.encode(Tiles.decodeHeight(tile), type, Crops.encodeFieldData(false, tileAge, crop)));
        Server.modifyFlagsByTileType(tilex, tiley, type);
        if (logTilePolling) {
      ...
      for (int xx = -1; xx <= 1; xx++) {
        for (int yy = -1; yy <= 1; yy++) {
          if ((tilex + xx >= 0) && (tiley + yy >= 0) && (tilex + xx < 1 << Constants.meshSize) && (tiley + yy < 1 << Constants.meshSize))
          {
            ...
            byte tempbyte1 = Tiles.decodeType(temptile1);
            if ((tempbyte1 != Tiles.Tile.TILE_LAVA.id) && (tempbyte1 != Tiles.Tile.TILE_HOLE.id) && 
              (!Tiles.isMineDoor(tempbyte1)))
            {
              ...
              if (Tiles.isSolidCave(tempbyte2))
              {
                ...
                for (int xn = 0; xn <= 1; xn++) {
                  for (int yn = 0; yn <= 1; yn++) {
                    try
                    {
                      ...
                      Server.rockMesh.setTile(tilex + xx + xn, tiley + yy + yn, 
                ...
                Server.caveMesh.setTile(tilex + xx, tiley + yy, 
                  Tiles.encode(Tiles.decodeHeight(temptile2), Tiles.Tile.TILE_CAVE_WALL_LAVA.id, 
                  Tiles.decodeData(temptile2)));
  ...
  {
    r.setSeed((tilex + tiley * Zones.worldTileSizeY) * 103591L);
              ...
              if ((tilex + xx >= 0) && (tiley + yy >= 0) && (tilex + xx < 1 << Constants.meshSize) && (tiley + yy < 1 << Constants.meshSize))
              {
                ...
                if (tempint1 < currHeight)
                {
                  ...
                  if (Server.rand.nextInt(2) == 0) {
                    if ((type2 != Tiles.Tile.TILE_LAVA.id) && (type2 != Tiles.Tile.TILE_HOLE.id) && 
                      (!Tiles.isMineDoor(type2)))
                    {
                ...
                else if ((Tiles.decodeData(tile) & 0xFF) == 255)
                {
                  int t = currentMesh.getTile(tilex + xx, tiley + yy);
                  byte type2 = Tiles.decodeType(t);
                  if (type2 == Tiles.Tile.TILE_ROCK.id)
                  {
                    ...
                    if (Tiles.isSolidCave(tempbyte2))
                    {
                      ...
                      currentMesh.setTile(tilex + xx, tiley + yy, temptile1);
                      Server.rockMesh.setTile(tilex + xx, tiley + yy, 
                        Tiles.encode((short)tempint2, Tiles.Tile.TILE_ROCK.id, (byte)0));
    ...
    if ((theight > MAX_KELP_HEIGHT) && (pollingSurface))
    {
      if (theight < 0)
      {
        ...
        if (kelpRandom.nextInt(20) == 0)
        {
          ...
          if (kelpRandom.nextBoolean()) {
            if (theight <= MINIMUM_REED_HEIGHT)
            {
      ...
      for (int xx = -1; xx <= 1; xx++) {
        ...
        {
          if (((xx == 0) && (yy != 0)) || ((yy == 0) && (xx != 0))) {
            if ((tilex + xx >= 0) && (tiley + yy >= 0) && (tilex + xx < Zones.worldTileSizeX) && (tiley + yy < Zones.worldTileSizeY))
            {
              if ((xx >= 0) && (yy >= 0))
              {
                float height = Tiles.decodeHeight(currentMesh.getTile(tilex + xx, tiley + yy));
                if (height < 0.0F) {
                  return false;
                }
              }
    ...
    if (pollingSurface) {
      if (Server.rand.nextInt(10) == 0) {
        try
        {
          ...
          VolaTile tempvtile1 = zone.getTileOrNull(tilex, tiley);
          if (tempvtile1 != null) {
            if ((!containsHouse(tempvtile1)) && (tempvtile1.getVillage() == null))
            {
              boolean foundMarsh = false;
              for (int xx = -1; xx <= 1; xx++) {
                for (int yy = -1; yy <= 1; yy++) {
                  if ((tilex + xx >= 0) && (tiley + yy >= 0) && (tilex + xx < 1 << Constants.meshSize) && (tiley + yy < 1 << Constants.meshSize)) {
                    if (Tiles.decodeType(currentMesh.getTile(tilex + xx, tiley + yy)) == Tiles.Tile.TILE_MARSH.id)
                    {
                      foundMarsh = true;
                      break;
                    }
              ...
              {
                currentMesh.setTile(tilex, tiley, 
                  Tiles.encode(Tiles.decodeHeight(tile), Tiles.Tile.TILE_MARSH.id, (byte)0));
                Server.modifyFlagsByTileType(tilex, tiley, Tiles.Tile.TILE_MARSH.id);
              }
              else
              {
                currentMesh.setTile(tilex, tiley, 
                  Tiles.encode(Tiles.decodeHeight(tile), Tiles.Tile.TILE_DIRT.id, (byte)0));
                Server.modifyFlagsByTileType(tilex, tiley, Tiles.Tile.TILE_DIRT.id);
              }
              Players.getInstance().sendChangedTile(tilex, tiley, pollingSurface, false);
            }
    ...
    if ((Tiles.decodeHeight(tile) > 0) && (pollingSurface))
    {
      ...
      if (foundMycel)
      {
        if ((type == Tiles.Tile.TILE_GRASS.id) || (type == Tiles.Tile.TILE_STEPPE.id) || (type == Tiles.Tile.TILE_DIRT.id) || (type == Tiles.Tile.TILE_LAWN.id) || (type == Tiles.Tile.TILE_MOSS.id))
        {
          ...
          currentMesh.setTile(tilex, tiley, 
            Tiles.encode(Tiles.decodeHeight(tile), newType, (byte)0));
          Server.modifyFlagsByTileType(tilex, tiley, newType);
          Players.getInstance().sendChangedTile(tilex, tiley, pollingSurface, false);
        }
        ...
        for (int xx = -3; xx <= 3; xx++) {
          for (int yy = -3; yy <= 3; yy++) {
            if (((xx == 0) && (yy != 0)) || ((xx != 0) && (yy == 0))) {
              if (GeneralUtilities.isValidTileLocation(newtilex + xx, newtiley + yy))
              {
      ...
      if (age != 15)
      {
        ...
        if (chance <= (16 - age) * (16 - age))
        {
          ...
          if ((!isOak) || (Server.rand.nextInt(5) == 0))
          {
            if (theTile.isMycelium()) {
              if (Kingdoms.getKingdomTemplateFor(Zones.getKingdom(tilex, tiley)) != 3) {
                if (Server.rand.nextInt(3) == 0)
                {
                  ...
                  currentMesh.setTile(tilex, tiley, 
                    Tiles.encode(Tiles.decodeHeight(tile), newType, newData));
                  Server.modifyFlagsByTileType(tilex, tiley, newType);
                  Players.getInstance().sendChangedTile(tilex, tiley, pollingSurface, false);
                  return;
                }
            ...
            currentMesh
              .setTile(tilex, tiley, Tiles.encode(Tiles.decodeHeight(tile), newType, newData));
            Server.modifyFlagsByTileType(tilex, tiley, newType);
            Players.getInstance().sendChangedTile(tilex, tiley, pollingSurface, false);
            if (age >= 15) {
              ...
              for (int y = Math.max(tiley - rad, 0); y <= maxY; y++) {
                if ((x != tilex) || (y != tiley))
                {
                  ...
                  if ((ttile.isTree()) || (ttile.isBush()))
                  {
                    ...
                    currentMesh.setTile(x, y, 
                      Tiles.encode(Tiles.decodeHeight(tt), newType, (byte)0));
      ...
      else
      {
        ...
        if (chance == 1)
        {
          ...
          else
          {
            ...
            newType = convertToNewType(theTile, aData);
  ...
  public static void growMushroom(int tilex, int tiley)
  {
    ...
    if (num % 128 == 0)
    {
      ...
      float posx = (tilex << 2) + Server.rand.nextFloat() * 4.0F;
      float posy = (tiley << 2) + Server.rand.nextFloat() * 4.0F;
      try
      {
        ItemFactory.createItem(templ, 80.0F + Server.rand.nextInt(20), posx, posy, Server.rand.nextInt(180), pollingSurface, (byte)0, -10L, null);
      }
  ...
  private static void addWildBeeHives(int tilex, int tiley, Tiles.Tile theTile, byte aData)
  {
    if (Zones.isFarFromAnyHive(tilex, tiley, true))
    {
      ...
      if ((age > 7) && (age < 13))
      {
        ...
        switch (TilePoller.1.$SwitchMap$com$wurmonline$mesh$TreeData$TreeType[treeType.ordinal()])
        {
        case 1: 
        case 2: 
        case 3: 
        ...
        if (canHaveHive)
        {
          r.setSeed((tilex + tiley * Zones.worldTileSizeY) * HIVE_PRIME);
          if (r.nextInt(500) == 0)
          {
            boolean ok = true;
            for (int x = -1; x <= 1; x++) {
            ...
            if (ok)
            {
              ...
              try
              {
                ...
                Item wildHive = ItemFactory.createItem(1239, ql, treeType
                  .getMaterial(), (byte)0, null);
                ...
                if (Servers.isThisATestServer()) {
                  Players.getInstance().sendGmMessage(null, "System", "Debug: Adding Hive (" + wildHive.getWurmId() + ") @ " + tilex + "," + tiley + " ql:" + ql + " rot:" + p
                    .getRot() + " ht:" + p.getPosZ() + " material:" + treeType.getName(), false);
                }
              }
              catch (FailedException fe)
              {
  ...
  private static void checkHoneyProduction(HiveZone hiveZone, int tilex, int tiley, byte type, byte data, Tiles.Tile theTile)
  {
    ...
    {
      if (Servers.isThisATestServer()) {
        Players.getInstance().sendGmMessage(null, "System", "Debug: Queen bee died as hive is underground @ " + hive.getTileX() + "," + hive.getTileY() + " from " + hive
          .getWurmId() + " to " + hive.getWurmId(), false);
      }
      hive.removeQueen();
      return;
    }
    if (hiveZone.hasHive(tilex, tiley)) {
    ...
    if (hive.hasQueen())
    {
      if (Server.rand.nextInt(3) == 0)
      {
        ...
        if (emptyHive != null)
        {
          ...
          {
            distX = Math.abs(hive.getPosX() - emptyHive.getPosX());
            distY = Math.abs(hive.getPosY() - emptyHive.getPosY());
          ...
          {
            if (Servers.isThisATestServer()) {
              Players.getInstance().sendGmMessage(null, "System", "Debug: Queen bee migrated @ " + hive.getTileX() + "," + hive.getTileY() + " from " + hive
                .getWurmId() + " to " + emptyHive.getWurmId(), false);
            }
            hive.removeQueen();
            emptyHive.addQueen();
            if (!hive.hasQueen())
            {
              ...
              {
                distX = hive.getItemsAsArray();distY = distX.length;
                for (dist = 0; dist < distY; dist++)
                {
                  Item item = distX[dist];
      ...
      if (Server.rand.nextInt(3) == 0)
      {
        ...
        if ((hive.hasQueen()) && (!hive.hasTwoQueens())) {
          if (((WurmCalendar.isSeasonSpring()) || (WurmCalendar.isSeasonSummer())) && (hiveZone.hasHive(tilex, tiley))) {
            if (honey != null) {
              if ((honey.getWeightGrams() > 1000) && (Server.rand.nextInt(5) == 0))
              {
                if (Servers.isThisATestServer()) {
  ...
  private static Item addHoney(int tilex, int tiley, Item hive, byte type, byte data, Tiles.Tile theTile)
  {
    ...
    if ((addedHoneyWeight > 0) && (hive.getFreeVolume() > addedHoneyWeight))
    {
      ...
      if (honey != null)
      {
        float totalQL = honey.getWeightGrams() * honey.getCurrentQualityLevel() + addedHoneyWeight * hive.getCurrentQualityLevel();
        newHoneyWeight = honey.getWeightGrams() + addedHoneyWeight;
        float newQL = totalQL / newHoneyWeight;
      ...
      if (nectarProduced > 10.0F) {
        if ((waxcount < 20) && (Server.rand.nextInt(40) == 0)) {
          try
          {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\TilePoller.class
