  ...
  implements MiscConstants
{
  ...
  private static void addToMap(AreaSpellEffect sp)
  {
    ...
    {
    case 0: 
      LAYER_0.put(Long.valueOf(sp.id), sp);
      break;
    case -1: 
      LAYER_MINI.put(Long.valueOf(sp.id), sp);
      break;
    default: 
      LAYER_0.put(Long.valueOf(sp.id), sp);
    }
  ...
  {
    AreaSpellEffect[] eff = (AreaSpellEffect[])map.values().toArray(new AreaSpellEffect[map.size()]);
    long now = System.currentTimeMillis();
    for (int as = 0; as < eff.length; as++) {
      if (eff[as].expireTime < now)
      {
        map.remove(Long.valueOf(eff[as].getId()));
  ...
  public static void removeAreaEffect(int tilex, int tiley, int layer)
  {
    ...
    AreaSpellEffect sp = (AreaSpellEffect)map.remove(Long.valueOf(calculateId(tilex, tiley)));
    if (sp != null)
    {
  ...
  public static AreaSpellEffect getEffect(int tilex, int tiley, int layer)
  {
    Map<Long, AreaSpellEffect> map = getMap(layer);
    if (map != null) {
      return (AreaSpellEffect)map.get(Long.valueOf(calculateId(tilex, tiley)));
    }
    return null;
  }
}
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\AreaSpellEffect.class
