    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++)
      {
        ...
        if (height > 0)
        {
          if (type == 4)
          {
            float tenth = height / 10.0F;
            float percent = tenth / maxH;
    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++)
      {
        ...
        if ((vill != null) && (vill.isHighwayFound())) {
          if ((x == vill.getStartX()) || (x == vill.getEndX()) || (y == vill.getStartY()) || (y == vill.getEndY())) {
      ...
      case 1114: 
        ...
        default: 
          ...
          else
          {
            ...
            continue;
            if ((colour != catseye0Surface) && (colour != catseye2Surface) && (colour != catseye2Surface) && (colour != catseye3Surface)) {
              switch (MethodsHighways.numberOfSetBits(marker.getAuxData()))
              {
              case 0: 
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\ZonesUtility.class
