  ...
  implements CreatureTemplateIds
{
  ...
  {
    dens.remove(Integer.valueOf(templateId));
  }
  
  public static Den getDen(int templateId)
  {
    return (Den)dens.get(Integer.valueOf(templateId));
  }
  ...
  private static void checkTemplate(int templateId, boolean whileRunning)
  {
    try
    {
      ...
      if ((Constants.respawnUniques) || (whileRunning))
      {
        ...
        if (!creatureExists)
        {
          Den d = (Den)dens.get(Integer.valueOf(templateId));
          if (d != null) {
      ...
      if (!dens.containsKey(Integer.valueOf(templateId)))
      {
        Den den = null;
        ...
        if (den != null)
        {
          ...
          if (!template.isUnique())
          {
            try
            {
              ...
              zone.den = den;
              logger.log(Level.INFO, "Zone at " + den.getTilex() + ", " + den.getTiley() + " now spawning " + template
                .getName() + " (" + den.getTemplateId() + ")");
            }
            catch (NoSuchZoneException nsz)
            {
              logger.log(Level.WARNING, "Den at " + den
                .getTilex() + ", " + den.getTiley() + " surf=" + den.isSurfaced() + " - zone does not exist.");
            }
          }
          else if (!creatureExists)
          {
            ...
            try
            {
              Creature.doNew(templateId, ctype, (den.getTilex() << 2) + 2, (den.getTiley() << 2) + 2, 180.0F, den
                .isSurfaced() ? 0 : -1, template.getName(), template.getSex());
              logger.log(Level.INFO, "Created " + template
                .getName() + " at " + den.getTilex() + "," + den.getTiley() + "!");
            }
            catch (Exception ex)
            {
      ...
      else if (!template.isUnique())
      {
        ...
        try
        {
          Zone zone = Zones.getZone(den.getTilex(), den.getTiley(), den.isSurfaced());
          zone.den = den;
          logger.log(Level.INFO, "Zone at " + den
            .getTilex() + ", " + den.getTiley() + " now spawning " + template.getName() + " (" + den
            .getTemplateId() + ")");
        }
        catch (NoSuchZoneException nsz)
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\Dens.class
