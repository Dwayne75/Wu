  ...
  implements MiscConstants
{
  ...
  private static final boolean fillTile(byte[][] map, int x, int y, byte checkType, byte setType, boolean looping)
  {
    if (map[x][y] == checkType) {
      if ((map[(x - 1)][y] == setType) || (map[(x + 1)][y] == setType) || (map[x][(y - 1)] == setType) || (map[x][(y + 1)] == setType))
      {
        map[x][y] = setType;
        return true;
      }
  ...
  private static final byte checkShallows(byte[][] map, int x, int y, int ii, byte closest, byte wType)
  {
    for (int jj = -ii; (jj < ii) && (closest == 5); jj++)
    {
      ...
      if ((x + jj < mapSize) && (x - jj >= 1) && (y + jj < mapSize) && (y - jj >= 1)) {
        if ((x + ii < mapSize) && (x - ii >= 1) && (y + ii < mapSize) && (y - ii >= 1))
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\WaterType.class
