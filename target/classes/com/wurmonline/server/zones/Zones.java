  ...
  implements MiscConstants, EffectConstants, CombatConstants, TimeConstants
{
  ...
  {
    landPercent.put(Byte.valueOf(kingdom), Float.valueOf(percent));
  }
  
  public static float getPercentLandForKingdom(byte kingdom)
  {
    Float f = (Float)landPercent.get(Byte.valueOf(kingdom));
    if (f != null) {
      return f.floatValue();
    }
    return 0.0F;
  ...
  public static final void addGuardTowerInfluence(Item tower, boolean silent)
  {
    if (Features.Feature.NEW_KINGDOM_INF.isEnabled())
    {
      ...
      HashMap<Item, InfluenceZone> thisZone = (HashMap)influenceZones.get(actualZone);
      if (thisZone == null) {
    ...
    if (Features.Feature.NEW_KINGDOM_INF.isEnabled())
    {
      ...
      for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++)
        {
          ...
          {
            thisZone = (HashMap)influenceZones.get(actualZone);
            if (thisZone != null) {
  ...
  public static void loadTowers()
  {
    ...
    for (ListIterator<Item> it = guardTowers.listIterator(); it.hasNext();)
    {
    ...
    logger.log(Level.INFO, "Loaded " + Kingdoms.getNumberOfGuardTowers() + " Guard towers. That took " + 
      (float)(System.nanoTime() - now) / 1000000.0F + " ms.");
  }
  ...
  private static long lastCounted = 0L;
  ...
  public static void calculateZones(boolean overRide)
  {
    if ((System.currentTimeMillis() - lastCounted > 60000L) || (overRide))
    {
      ...
      for (int x = 0; x < 255; x++) {
        if (zoneControl[x] > 0) {
          setLandPercent((byte)x, zoneControl[x] * 100.0F / (float)numberOfTiles);
        }
      }
  ...
  static void addAltar(Item altar, boolean silent)
  {
    if (Features.Feature.NEWDOMAINS.isEnabled())
    {
      ...
      HashMap<Item, FaithZone> thisZone = (HashMap)altarZones.get(actualZone);
      if (thisZone == null) {
        thisZone = new HashMap();
      }
  ...
  static void removeAltar(Item altar, boolean silent)
  {
    if (Features.Feature.NEWDOMAINS.isEnabled())
    {
      ...
      FaithZone oldZone = (FaithZone)thisZone.remove(altar);
          ...
          for (int i = oldZone.getStartX(); i < oldZone.getEndX(); i++) {
            for (int j = oldZone.getStartY(); j < oldZone.getEndY(); j++)
            {
              ...
              if (tempTile != null)
              {
                ...
                if (newZone == null) {
                  tempTile.broadCast(oldZone.getCurrentRuler().getName() + " has had to lose " + oldZone
                    .getCurrentRuler().getHisHerItsString() + " hold over this area!");
                } else {
                  tempTile.broadCast(newZone.getCurrentRuler().getName() + "'s domain is now the strongest here!");
                }
              }
  ...
  public static Item[] getAltars(int deityId)
  {
        ...
        if (m != null) {
          for (Item altar : m.keySet()) {
            if (altar != null)
            {
              ...
              if (((deity == null) && (deityId == 0)) || ((deity != null) && 
                (deity.getNumber() == deityId))) {
                lAltars.add(altar);
              }
            }
          }
    ...
    return (Item[])lAltars.toArray(new Item[lAltars.size()]);
  }
      ...
      while ((!found) && (tries < 1000))
      {
        ...
        if (Tiles.decodeHeight(tile) > 5) {
          try
          {
            ...
            Item target1 = ItemFactory.createItem(type, 100.0F, tx * 4 + 2, ty * 4 + 2, Server.rand
              .nextInt(360), true, (byte)0, -10L, "");
            ...
            logger.log(Level.INFO, "Created " + target1.getName() + " at " + target1.getTileX() + " " + target1
              .getTileY() + " sizes " + target1.getSizeX() + "," + target1.getSizeY() + "," + target1
              .getSizeZ() + ")");
  ...
  private static final void createCampsOnLine(int x, int y)
  {
    ...
    while ((!found) && (tries < 1000))
    {
      ...
      {
        int tile = Server.surfaceMesh.getTile(tx, ty);
        if (Tiles.decodeHeight(tile) > 2) {
          if (Terraforming.isAllCornersInsideHeightRange(tx, ty, true, (short)(Tiles.decodeHeight(tile) + 5), 
            (short)(Tiles.decodeHeight(tile) - 5))) {
            try
            {
              ...
              new FocusZone(tx - 60, tx + 60, ty - 60, ty + 60, (byte)7, target1
    ...
    if (item.getData1() != 0)
    {
      ...
      if (deity != null)
      {
        ...
        if (lCoveredFaithZones != null) {
          ...
          {
            int dist = Math.max(Math.abs(tilex - lCoveredFaithZones[x].getCenterX()), 
              Math.abs(tiley - lCoveredFaithZones[x].getCenterY()));
            if (100 - dist > 0) {
    ...
    for (int x = 0; x < max; x++) {
      for (int y = 0; y < max; y++)
      {
        ...
        if ((type == Tiles.Tile.TILE_TREE.id) || (type == Tiles.Tile.TILE_BUSH.id) || (type == Tiles.Tile.TILE_ENCHANTED_TREE.id) || (type == Tiles.Tile.TILE_ENCHANTED_BUSH.id) || (type == Tiles.Tile.TILE_MYCELIUM_TREE.id) || (type == Tiles.Tile.TILE_MYCELIUM_BUSH.id))
        {
          ...
          {
            byte newType;
            if (type == Tiles.Tile.TILE_ENCHANTED_TREE.id)
            {
            ...
            {
              byte newType;
              if (type == Tiles.Tile.TILE_MYCELIUM_TREE.id)
              {
              ...
              {
                byte newType;
                if (type == Tiles.Tile.TILE_BUSH.id)
                {
                ...
                {
                  byte newType;
                  if (type == Tiles.Tile.TILE_ENCHANTED_BUSH.id) {
  ...
  public static final Item isWithinDuelRing(int tileX, int tileY, boolean surfaced)
  {
    ...
    for (Item ring : duelRings) {
      if ((ring.getZoneId() > 0) && (!ring.deleted)) {
        if ((ring.getTileX() < tileX + 20) && (ring.getTileX() > tileX - 20) && (ring.getTileY() < tileY + 20) && 
  ...
  public static final FaithZone getFaithZone(int tilex, int tiley, boolean surfaced)
    ...
    if (Features.Feature.NEWDOMAINS.isEnabled())
    {
      ...
      for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++)
        {
          ...
          {
            thisZone = (HashMap)altarZones.get(actualZone);
            if (thisZone != null) {
  ...
  public static final ArrayList<HashMap<Item, FaithZone>> getCoveredZones(int startx, int starty, int endx, int endy)
  {
    ...
    for (int y = Math.min(0, starty / 64); y <= Math.min(0, endy / 64); y++) {
      ...
      {
        HashMap<Item, FaithZone> thisZone = (HashMap)altarZones.get(y * domainSizeX + x);
  ...
  public static final FaithZone[] getFaithZonesCoveredBy(int startx, int starty, int endx, int endy, boolean surfaced)
  {
    ...
    if (Features.Feature.NEWDOMAINS.isEnabled())
    {
      ArrayList<HashMap<Item, FaithZone>> coveredZones = getCoveredZones(startx, starty, endx, endy);
      for (HashMap<Item, FaithZone> z : coveredZones) {
        for (FaithZone f : z.values()) {
          if ((f.getCurrentRuler() != null) && 
            (areaOverlapsFaithZone(f, startx, starty, endx, endy))) {
            zoneList.add(f);
          }
        }
      }
    ...
    return (FaithZone[])zoneList.toArray(toReturn);
  }
  ...
  public static final FaithZone[] getFaithZones()
  {
    ...
    return (FaithZone[])allZones.toArray(toReturn);
  }
  ...
  public static void pollNextZones(long sleepTime)
    throws IOException
  {
    int bonusZone;
    if (rest > 0)
  ...
  public static void saveNextZone()
    throws IOException
  {
  ...
  {
    if (logger.isLoggable(Level.FINEST)) {
      logger.finest("Creating virtual zone " + startX + ", " + startY + ", size: " + size + ", surface: " + surface + " for " + watcher
        .getName());
    }
    VirtualZone zone = new VirtualZone(watcher, startX, startY, centerX, centerY, size, surface);
    virtualZones.put(Integer.valueOf(zone.getId()), zone);
  ...
  {
    virtualZones.remove(Integer.valueOf(id));
  }
  
  static VirtualZone getVirtualZone(int number)
    throws NoSuchZoneException
  {
    VirtualZone toReturn = (VirtualZone)virtualZones.get(Integer.valueOf(number));
    if (toReturn == null) {
  ...
  public static int getZoneIdFor(int x, int y, boolean surfaced)
    throws NoSuchZoneException
  {
  ...
  public static Zone getZone(@Nonnull TilePos tilePos, boolean surfaced)
    throws NoSuchZoneException
  {
    return getZone(tilePos.x, tilePos.y, surfaced);
  }
  
  public static Zone getZone(int tilex, int tiley, boolean surfaced)
    throws NoSuchZoneException
  {
    Zone toReturn = null;
    if (surfaced) {
      try
      {
        ...
        if (!toReturn.covers(tilex, tiley)) {
          logger.log(Level.WARNING, "Error in the way zones are fetched. Doesn't work for " + (tilex >> 6) + " to cover " + tilex + " or " + (tiley >> 6) + " to cover " + tiley);
        }
      }
    ...
    try
    {
      ...
      {
        logger.log(Level.WARNING, "THIS SHOULD NOT HAPPEN - zone: x=" + (tilex >> 6) + ", y=" + (tiley >> 6) + " - surfaced: " + surfaced, new Exception());
    ...
    {
      logger.log(Level.WARNING, "Failed to load zone " + (tilex >> 6) + ", " + (tiley >> 6) + ". Zone will be empty.", ex);
    }
    return toReturn;
  }
  ...
  public static Zone[] getZonesCoveredBy(VirtualZone zone)
  {
    ...
    if (logger.isLoggable(Level.FINEST)) {
      logger.finest("Getting zones covered by " + zone.getId() + ": " + zone.getStartX() + "," + zone.getEndX() + ",y:" + zone
        .getStartY() + "," + zone.getEndY() + " which starts at " + (zone.getStartX() >> 6) + "," + (zone
        .getStartY() >> 6) + " and ends at " + (zone.getEndX() >> 6) + "," + (zone
        .getEndY() >> 6));
    }
    for (int x = zone.getStartX() >> 6; x <= zone.getEndX() >> 6; x++) {
      for (int y = zone.getStartY() >> 6; y <= zone.getEndY() >> 6; y++) {
        try
        {
          ...
          if (logger.isLoggable(Level.FINEST)) {
            logger.finest("Adding " + zone2.getId() + ": " + zone2.getStartX() + "," + zone2.getStartY() + "-" + zone2
              .getEndX() + "," + zone2.getEndY() + " which is at " + x + "," + y);
          }
          zoneList.add(zone2);
        }
        catch (NoSuchZoneException localNoSuchZoneException) {}
    ...
    return (Zone[])zoneList.toArray(toReturn);
  }
  ...
  private static final double getDir(float x, float y)
  {
    ...
    if (x != 0.0F)
    {
      if (y != 0.0F)
      {
        ...
        degree = degree * 57.29577951308232D + 90.0D;
        while (degree < 0.0D) {
          degree += 360.0D;
        }
        while (degree >= 360.0D) {
  ...
  public static final float calculateRockHeight(float posX, float posY)
    throws NoSuchZoneException
  {
    float height = 0.0F;
    try
  ...
  public static final float calculateHeight(float posX, float posY, boolean surfaced)
    throws NoSuchZoneException
  {
    float height = 0.0F;
    try
  ...
  public static final float calculatePosZ(float posx, float posy, VolaTile tile, boolean isOnSurface, boolean floating, float currentPosZ, @Nullable Creature creature, long bridgeId)
  {
    try
    {
      ...
      if (bridgeId > 0L)
      {
        ...
        float xa = posx / 4.0F - xx;
        float ya = posy / 4.0F - yy;
      ...
      if (tile != null)
      {
        ...
        float h = basePosZ + Math.max(0, dfl * 3) + (f.length > 0 ? 0.25F : 0.0F);
      ...
      if (vt != null) {
        for (BridgePart bp : vt.getBridgeParts()) {
          if (bp.getStructureId() == bridgeId) {
            if (bp.getBridgePartState() == BridgeConstants.BridgeState.COMPLETED)
            {
  ...
  public static VolaTile[] getTilesSurrounding(int tilex, int tiley, boolean surfaced, int distance)
  {
    ...
    return (VolaTile[])tiles.toArray(new VolaTile[tiles.size()]);
  }
  ...
  public static final boolean willEnterStructure(Creature creature, float startx, float starty, float endx, float endy, boolean surfaced)
  {
    ...
    if ((Math.abs(endtilex - starttilex) > max) || (Math.abs(endtiley - starttiley) > max))
    {
      if (creature != null) {
        logger.log(Level.WARNING, creature
          .getName() + " checking more than his maxdist of " + creature.getMaxHuntDistance(), new Exception());
      } else {
    ...
    double xPosMod = Math.sin(rot * 0.01745329238474369D) * 2.0D;
    double yPosMod = -Math.cos(rot * 0.01745329238474369D) * 2.0D;
    ...
    for (;;)
    {
      if ((Math.abs(endtilex - currTileX) <= 1) && (Math.abs(endtiley - currTileY) <= 1))
      {
        ...
        {
          if (endTile != null) {
            if ((startTile.getStructure() == null) && (endTile.getStructure() != null) && 
              (endTile.getStructure().isFinished())) {
              return true;
            }
          }
        }
  ...
  public static Structure[] getStructuresInArea(int startX, int startY, int endX, int endY, boolean surfaced)
  {
    ...
    Structure[] toReturn;
    if (set.size() > 0) {
      toReturn = (Structure[])set.toArray(new Structure[set.size()]);
    } else {
  ...
  private static final double getRotation(float startx, float starty, float endx, float endy)
  {
    ...
    return newrot * 57.29577951308232D + 90.0D;
  }
  
  public static final LongPosition getEndTile(float startPosX, float startPosY, float rot, int tiledist)
  {
    float xPosMod = (float)Math.sin(rot * 0.017453292F) * (4 * tiledist);
    float yPosMod = -(float)Math.cos(rot * 0.017453292F) * (4 * tiledist);
  ...
  static final Den getNorthTop(int templateId)
  {
    for (int tries = 0; tries < 1000; tries++)
    {
      ...
      for (int x = 0; x < zones.length; x++) {
        if (zones[x].highest > top) {
          if ((top == 0.0F) || (Server.rand.nextInt(2) == 0))
          {
            top = zones[x].highest;
            highest = zones[x];
          }
      ...
      if (highest != null)
      {
        ...
        if (h > 0)
        {
          ...
          if (t == null) {
            if ((Villages.getVillageWithPerimeterAt(safeTileX(tx), safeTileY(ty), true) == null) && 
              ...
              (Villages.getVillageWithPerimeterAt(safeTileX(tx + 20), 
              safeTileY(ty - 20), true) == null)) {
              if (Villages.getVillageWithPerimeterAt(safeTileX(tx + 20), 
                safeTileY(ty + 20), true) == null) {
                if (Villages.getVillageWithPerimeterAt(safeTileX(tx), 
                  safeTileY(ty + 20), true) == null) {
                  if (Villages.getVillageWithPerimeterAt(safeTileX(tx - 20), 
                    safeTileY(ty + 20), true) == null)
                  {
  ...
  public static final Den getRandomTop()
  {
    for (int a = 0; a < 100; a++)
    {
      ...
      for (int x = startx; x < endx; x++) {
        for (int y = starty; y < endy; y++) {
          if (surfaceZones[x][y].highest > top)
          {
            logger.info("Zone " + x + "," + y + " now highest for top.");
            top = surfaceZones[x][y].highest;
            if (top > 200.0F) {
      ...
      for (int y = 0; y < worldSize >> 6; y++)
      {
        ...
        for (int itx = 0; itx < its.length; itx++) {
          if (its[itx].getTemplateId() == 272) {
            if (its[itx].getName().equals("corpse of " + creature.getName())) {
  ...
  {
    Map<Integer, Byte> tab = (Map)miningTiles.get(Integer.valueOf(x));
    if (tab == null) {
      return 0;
    }
    Byte b = (Byte)tab.get(Integer.valueOf(y));
    if (b == null) {
      return 0;
    }
    return b.byteValue();
  ...
  public static final void setMiningState(int x, int y, byte state, boolean load)
  {
    ...
    if (tab == null)
    {
      ...
      {
        tab = new Hashtable();
        tab.put(Integer.valueOf(y), Byte.valueOf(state));
        miningTiles.put(Integer.valueOf(x), tab);
        save = true;
      }
    }
    else if ((state > 0) || (state == -1) || (tab.get(Integer.valueOf(y)) != null))
    {
      tab.put(Integer.valueOf(y), Byte.valueOf(state));
      save = true;
    }
  ...
  private static final void saveMiningTile(int tilex, int tiley, byte state)
  {
    ...
    {
      logger.log(Level.WARNING, "Failed to save miningtile " + tilex + ", " + tiley + ", " + state + ":" + sqex
        .getMessage(), sqex);
    }
    finally
    {
  ...
  private static final boolean exists(Connection dbcon, int tilex, int tiley)
    throws SQLException
  {
    PreparedStatement ps = null;
    ResultSet rs = null;
  ...
  private static final void spawnSanta(byte kingdom)
  {
    ...
    try
    {
      evilsanta = Creature.doNew(temp, tilex * 4 + 1, tiley * 4 + 1, 154.0F, 0, name, (byte)0, kingdom);
    }
    catch (Exception ex)
    {
  ...
  public static final void loadChristmas()
  {
    if (!isHasLoadedChristmas())
    {
      ...
      if (!Servers.localServer.HOMESERVER)
      {
        ...
        santa = addSanta(v, tilex, tiley, 154, 46, "Santa Claus", (byte)1);
        ...
        if (v != null)
        {
          ...
          santas.put(Long.valueOf(santa.getWurmId()), santa);
        }
      }
      else
      {
        ...
        try
        {
          santa = Creature.doNew(46, tilex * 4 + 1, tiley * 4 + 1, 90.0F, 0, "Santa Claus", (byte)0, Servers.localServer.KINGDOM);
        }
        catch (Exception ex)
        {
          logger.log(Level.WARNING, "Failed to create santa! " + ex.getMessage(), ex);
  ...
  public static void sendChristmasEffect(Creature creature, Item item)
  {
    ...
    for (int x = 0; x < players.length; x++) {
      if (item != null) {
        players[x].getCommunicator().sendAddEffect(creature.getWurmId(), (short)4, item.getPosX(), item
          .getPosY(), item.getPosZ(), (byte)0);
      } else {
        players[x].getCommunicator().sendAddEffect(creature.getWurmId(), (short)4, creature.getPosX(), creature
          .getPosY(), creature.getPositionZ(), (byte)0);
      }
    }
  }
  ...
  public static void removeNewYear()
  {
    ...
    {
      Player[] players = Players.getInstance().getPlayers();
      LongPosition l = (LongPosition)posmap.removeFirst();
  ...
  public static void sendNewYearsEffectsToPlayer(Player p)
  {
    ...
    for (Iterator<LongPosition> it = posmap.iterator(); it.hasNext();) {
      try
      {
        LongPosition l = (LongPosition)it.next();
        p.getCommunicator().sendAddEffect(l.getId(), l.getEffectType(), l.getTilex() << 2, l.getTiley() << 2, 
          calculateHeight(l.getTilex() << 2, l.getTiley() << 2, true), (byte)0);
      }
      catch (NoSuchZoneException localNoSuchZoneException) {}
    }
  }
  ...
  public static final boolean interruptedRange(Creature performer, Creature defender)
  {
    ...
    {
      BlockingResult blockers = Blocking.getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), defender
        .getPosX(), defender.getPosY(), performer.getPositionZ(), defender.getPositionZ(), performer
        .isOnSurface(), defender.isOnSurface(), true, 4, -1L, performer.getBridgeId(), performer
        .getBridgeId(), false);
      if ((blockers != null) && (
        (blockers.getTotalCover() >= 100.0F) || (
        ((!performer.isOnPvPServer()) || (!defender.isOnPvPServer())) && 
    ...
    if (!defender.isWithinDistanceTo(performer, 4.0F)) {
    ...
    for (int x = safeTileX(tilex - 2); x <= safeTileX(tilex + 2); x++) {
      for (int y = safeTileY(tiley - 2); y <= safeTileY(tiley + 2); y++)
      {
        if (Features.Feature.SURFACEWATER.isEnabled()) {
          if (Water.getSurfaceWater(x, y) > 0) {
            return 2;
          }
        }
  ...
  public static void addHive(Item hive, boolean silent)
  {
    ...
    ConcurrentHashMap<Item, HiveZone> thisZone = (ConcurrentHashMap)hiveZones.get(actualZone);
    if (thisZone == null) {
  ...
  public static final boolean isFarFromAnyHive(int tilex, int tiley, boolean surfaced)
  {
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++)
      {
        ...
        {
          thisZone = (ConcurrentHashMap)hiveZones.get(actualZone);
          if (thisZone != null) {
              ...
              if (hz.containsTile(tilex, tiley)) {
                if (hz.getStrengthForTile(tilex, tiley, surfaced) > 0) {
                  if (toReturn == null) {
                    ...
                    if (hz.getCurrentHive().getCurrentQualityLevel() > toReturn.getCurrentHive().getCurrentQualityLevel()) {
                      toReturn = hz;
                    }
                  }
                }
  ...
  public static final Item[] getHives(int hiveType)
  {
    ...
    return (Item[])hiveZoneSet.toArray(new Item[hiveZoneSet.size()]);
  }
  ...
  public static boolean isGoodTileForSpawn(int tilex, int tiley, boolean surfaced, boolean canBeOccupied)
  {
    ...
    if ((canBeOccupied) || (t == null)) {}
    return 
      (Tiles.decodeHeight(getTileIntForTile(tilex, tiley, surfaced ? 0 : -1)) > 0) && ((steepness[0] < 23) || (steepness[1] < 23));
  }
  
  public static boolean isVillagePremSpawn(Village village)
  {
    return (Servers.localServer.testServer) || (isPremSpawnZoneAt(village.getStartX(), village.getStartY())) || 
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\Zones.class
