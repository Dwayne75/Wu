  ...
  implements TimeConstants, SoundNames
{
  ...
  private static void logCropFields()
  {
    ...
    for (Iterator<CropTile> it = tiles.values().iterator(); it.hasNext();)
    {
      tile = (CropTile)it.next();
      Integer crop = Integer.valueOf(tile.getCropType());
      Integer count = (Integer)map.get(crop);
      if (count == null)
      {
        map.put(crop, Integer.valueOf(1));
      }
      else
      {
    ...
    {
      Integer count = (Integer)map.get(crop);
      String cropName = Crops.getCropName(crop.intValue());
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\CropTilePoller.class
