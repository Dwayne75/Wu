...
public final class FaithZone
{
  ...
  {
    return 
      (int)(this.altar.getCurrentQualityLevel() / (surfaced == this.altar.isOnSurface() ? 1 : 2) - Math.max(Math.abs(this.altar.getTileX() - tileX), Math.abs(this.altar.getTileY() - tileY)));
  }
  ...
  boolean poll()
  {
    ...
    for (int f = 0; f < 4; f++)
    {
      if (this.activeGods[f] != null) {
        ...
        else if (this.activeGods[f] == getCurrentRuler())
        {
    ...
    if (Tiles.decodeHeight(tile) > 0)
    {
      ...
      else if (theTile.isNormalTree())
      {
        ...
        Server.setSurfaceTile(tilex, tiley, 
          Tiles.decodeHeight(tile), newType, data);
        Players.getInstance().sendChangedTile(tilex, tiley, true, false);
      }
  ...
  void addToFaith(Deity aDeity, int aStrength)
  {
    for (int f = 0; f < 4; f++) {
      if ((this.activeGods[f] == null) || (this.activeGods[f] == aDeity))
      {
        ...
        this.newFaiths[f] = ((byte)Math.min(100, aStrength)); break;
      }
    }
  }
}
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\zones\FaithZone.class
