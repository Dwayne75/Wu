  ...
  implements MiscConstants, TimeConstants
{
  ...
  {
    allAppointments.put(Integer.valueOf(era), app);
  }
  ...
  {
    return (Appointments)allAppointments.get(Integer.valueOf(era));
  }
  
  void addAppointment(Appointment app)
  {
    this.appointments.put(Integer.valueOf(app.getId()), app);
  }
  
  public Appointment getAppointment(int _id)
  {
    return (Appointment)this.appointments.get(Integer.valueOf(_id));
  }
  ...
  public int getAppointmentLevels(long data, long wurmid)
  {
    ...
    for (int x = 0; x < 36; x++) {
      if ((data >> x & 1L) == 1L)
      {
        Appointment tempapp = getAppointment(x);
    ...
    for (int x = 0; x < this.officials.length; x++) {
      if (this.officials[x] == wurmid)
      {
        Appointment tempapp = getAppointment(x + 1500);
        levels += tempapp.getLevel();
      }
    }
    return levels;
  ...
  public String getTitles(long data, boolean male)
  {
    ...
    for (Iterator<Appointment> it = tempset.iterator(); it.hasNext();)
    {
      if (male) {
    ...
    if (this.era > 0)
    {
      ...
      try
      {
        ...
        if (rs.next())
        {
          ...
          logger.log(Level.INFO, "Loaded lastChecked for Era " + this.era + ". Last checked was " + 
          
            (System.currentTimeMillis() - this.lastChecked) / 3600000L + " hours ago.");
        }
  ...
  {
    int newAvail = this.availableTitles[id] -= 1;
    Connection dbcon = null;
    PreparedStatement ps = null;
    try
    {
    ...
    {
      logger.log(Level.WARNING, "Failed to set avail titles for era " + this.era + " " + id + ", " + 
      
        Kingdoms.getNameFor(this.kingdom) + sqex
        .getMessage(), sqex);
    }
    finally
    {
  ...
  {
    int newAvail = this.availableOrders[(id - 30)] -= 1;
    Connection dbcon = null;
    PreparedStatement ps = null;
    try
    {
    ...
    {
      logger.log(Level.WARNING, "Failed to set avail orders for era " + this.era + " " + id + ", " + 
  ...
  public void resetAppointments(byte kingdomId)
  {
    ...
    try
    {
      ...
      logger.log(Level.INFO, "Resetting lastChecked for Era " + this.era + ". Last checked was " + 
    ...
    {
      logger.log(Level.WARNING, "Failed to reset appointments for era " + this.era + " kingdom " + 
      
        Kingdoms.getNameFor(kingdomId) + ", " + sqex
        .getMessage(), sqex);
    }
    finally
    {
  ...
  private void resetOfficials()
  {
    ...
    {
      logger.log(Level.WARNING, "Failed to reset officials for era " + this.era + " kingdom " + Kingdoms.getNameFor(this.kingdom) + ", " + sqex
        .getMessage(), sqex);
    }
    finally
    {
  ...
  private void resetOrders(float percentOwned)
  {
    ...
    {
      int avail = (int)((percentOwned + 10.0F) / (Math.max(a, 1) * 5.0F));
      if (avail == 0) {
        break;
      }
    ...
    {
      logger.log(Level.WARNING, "Failed to reset orders for era " + this.era + " kingdom " + Kingdoms.getNameFor(this.kingdom) + ", " + sqex
        .getMessage(), sqex);
    }
    finally
    {
  ...
  private void resetTitles(float percentOwned)
  {
    ...
    {
      logger.log(Level.WARNING, "Failed to reset titles for era " + this.era + " kingdom " + Kingdoms.getNameFor(this.kingdom) + ", " + sqex
        .getMessage(), sqex);
    }
    finally
    {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\kingdom\Appointments.class
