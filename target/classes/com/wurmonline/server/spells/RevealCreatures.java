  ...
  void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset)
  {
    ...
    for (Zone lZone : zones)
    {
      ...
      for (Creature cret : crets) {
        if (cret.getPower() <= performer.getPower()) {
          if (cret != performer) {
            if (cret.getBonusForSpellEffect((byte)29) <= 0.0F)
            {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\RevealCreatures.class
