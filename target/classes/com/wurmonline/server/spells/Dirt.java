  ...
  extends ReligiousSpell
{
  ...
  boolean precondition(Skill castSkill, Creature performer, Item target)
  {
    ...
    if ((target.isLockable()) && (target.getLockId() != -10L)) {
      if ((!MethodsItems.mayUseInventoryOfVehicle(performer, target)) && 
        (!MethodsItems.hasKeyForContainer(performer, target)))
      {
        performer.getCommunicator().sendNormalServerMessage("You must be able to open the " + target
          .getName() + " to create dirt inside of it.", (byte)3);
    ...
    if (target.isTent()) {
      if (target.getParentOrNull() != null)
      {
    ...
    if (target.isCrate())
    {
      ...
      if (nums <= 0)
      {
        performer.getCommunicator().sendNormalServerMessage("The " + target
          .getName() + " will not be able to contain all that dirt.", (byte)3);
    ...
    for (int x = 0; x >= -1; x--) {
      for (int y = 0; y >= -1; y--) {
        try
        {
          ...
          if (Tiles.decodeHeight(tile) < 62536)
          {
          ...
          if (vtile != null)
          {
            ...
            if (fences.length > 0)
            {
              if ((x == 0) && (y == 0))
              {
                performer.getCommunicator().sendNormalServerMessage("The " + fences[0]
                  .getName() + " is in the way.", (byte)3);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Dirt.class
