  ...
  private void sendOres(Creature performer, double power, int tilex, int tiley)
  {
    ...
    for (int dx = maxRadius * -1; dx < maxRadius; dx++) {
      for (int dy = maxRadius * -1; dy < maxRadius; dy++)
      {
        ...
        if ((type == 205) && (power > 40.0D)) {
          ores.add("slate");
        } else if ((type == 206) && (power > 40.0D)) {
          ores.add("marble");
        } else if ((type == 220) && (power > 60.0D)) {
          ores.add("gold");
        } else if ((type == 221) && (power > 50.0D)) {
          ores.add("silver");
        } else if ((type == 227) && (power > 70.0D)) {
          ores.add("adamantine");
        } else if ((type == 228) && (power > 80.0D)) {
  ...
  void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset)
  {
    ...
    try
    {
      ...
      if ((diff >= 0.0F) && (diff < 20.0F))
      {
        ...
        if (diff > 0.0F) {
          sendOres(performer, power, tilex, tiley);
        }
      }
      else if (diff > 20.0F)
      {
    ...
    int numtiles = (int)Math.max(1.0D, power / 10.0D) + performer.getNumLinks() * 2;
    for (int nt = 1; nt <= numtiles; nt++)
    {
      ...
      if (Tiles.isOreCave(t.id)) {
        performer.getCommunicator().sendNormalServerMessage("You sniff " + t.tiledesc
          .toLowerCase() + " " + nt + " tiles away in your facing direction.");
      }
    }
  }
}
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\MoleSenses.class
