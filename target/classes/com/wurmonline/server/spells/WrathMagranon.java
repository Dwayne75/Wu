  ...
  implements AttitudeConstants
{
  ...
  void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset)
  {
    ...
    int radiusBonus = (int)(power / 80.0D);
          ...
          if (ring == null)
          {
            for (Creature lCret : t.getCreatures()) {
              if ((!lCret.isInvulnerable()) && (lCret.getAttitude(performer) == 2))
              {
                ...
                double damage = calculateDamage(lCret, power, 3000.0D, 60.0D);
            ...
            if (Servers.isThisAPvpServer()) {
              for (Wall wall : t.getWalls()) {
                if (wall.getType() != StructureTypeEnum.PLAN)
                {
                  ...
                  if ((v != null) && 
                    (!v.isEnemy(performer)) && 
                    (!MethodsStructure.mayModifyStructure(performer, structure, wall.getTile(), (short)82))) {
                    dealDam = false;
                  }
                  if (dealDam)
                  {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\WrathMagranon.class
