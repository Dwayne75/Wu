  ...
  extends ReligiousSpell
{
  ...
  {
    if ((!target.isMailBox()) && (!target.isSpringFilled()) && (!target.isUnenchantedTurret()) && 
      (!target.isPuppet()) && (!target.isEnchantedTurret()))
    {
  ...
  {
    if (((!target.isMailBox()) && (!target.isSpringFilled()) && (!target.isPuppet()) && 
      (!target.isUnenchantedTurret()) && (!target.isEnchantedTurret())) || (
    ...
    if (eff == null)
    {
      performer.getCommunicator().sendNormalServerMessage("You summon nearby spirits into the " + target
        .getName() + ".", (byte)2);
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " looks pleased as " + performer.getHeSheItString() + " summons some spirits into the " + target
        .getName() + ".", performer, 5);
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("You frown as you fail to summon more spirits into the " + target
        .getName() + ".", (byte)3);
  ...
  void doNegativeEffect(Skill castSkill, double power, Creature performer, Item target)
  {
    performer.getCommunicator().sendNormalServerMessage("The " + target
      .getName() + " emits a deep worrying sound of resonance, but stays intact.", (byte)3);
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Courier.class
