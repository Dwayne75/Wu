      ...
      if (!skip) {
        try
        {
          if (Zones.calculateHeight((x << 2) + 2, (y << 2) + 2, performer.isOnSurface()) > 0.0F)
          {
            ...
            Creature.doNew(69, true, (x << 2) + 2, (y << 2) + 2, Server.rand
              .nextFloat() * 360.0F, performer.getLayer(), "Zombie", sex, performer.getKingdomId(), ctype, true);
          }
        }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\ZombieInfestation.class
