  ...
  extends KarmaSpell
{
  ...
  void doEffect(Skill castSkill, double power, Creature performer, Creature target)
  {
    ...
    for (int x = Zones.safeTileX(target.getTileX() - 5); x < Zones.safeTileX(target.getTileX() + 5); x++)
    {
      ...
      createTrapAt(x, y, layer, villid, performer.getKingdomId());
    }
    for (int x = Zones.safeTileX(target.getTileX() - 5); x < Zones.safeTileX(target.getTileX() + 5); x++)
    {
      int y = Zones.safeTileY(target.getTileY() + 5);
    ...
    performer.getCommunicator().sendNormalServerMessage("You predict a grim future for " + target
      .getNameWithGenus() + " by placing deadly traps around " + target
      .getHimHerItString() + ".");
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Forecast.class
