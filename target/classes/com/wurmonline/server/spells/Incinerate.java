  ...
  extends KarmaEnchantment
{
  ...
  void doEffect(Skill castSkill, double power, Creature performer, Creature target)
  {
    if ((!target.isUnique()) || (power > 99.0D))
    {
      ...
      if (eff == null)
      {
        if (target != performer) {
          performer.getCommunicator().sendNormalServerMessage(target
            .getNameWithGenus() + " is engulfed in a wave of extreme heat.", (byte)4);
        }
        ...
        Server.getInstance().broadCastAction(performer
          .getNameWithGenus() + " looks pleased as " + performer.getHeSheItString() + " engulfs " + target
          .getNameWithGenus() + " in a wave of heat.", performer, 5);
      }
      ...
      {
        if (target != performer) {
          performer.getCommunicator().sendNormalServerMessage("You succeed in improving the heat around " + target
        ...
        Server.getInstance().broadCastAction(performer
          .getNameWithGenus() + " looks pleased as " + performer.getHeSheItString() + " increases the heat around " + target
          .getNameWithGenus() + ".", performer, 5);
      }
    }
    else
    {
      performer.getCommunicator().sendNormalServerMessage("You try to incinerate " + target
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Incinerate.class
