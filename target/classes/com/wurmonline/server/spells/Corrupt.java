        ...
        try
        {
          ...
          if (ok)
          {
            ...
            if ((t == null) || (t.getVillage() == null) || (t.getVillage().kingdom == performer.getKingdomId()))
            {
              ...
              if ((type == Tiles.Tile.TILE_FIELD.id) || (type == Tiles.Tile.TILE_FIELD2.id) || (type == Tiles.Tile.TILE_GRASS.id) || (type == Tiles.Tile.TILE_REED.id) || (type == Tiles.Tile.TILE_DIRT.id) || (type == Tiles.Tile.TILE_LAWN.id) || (type == Tiles.Tile.TILE_STEPPE.id) || 
                ...
                (theTile.isNormalBush()))
              {
                if (theTile.isNormalTree()) {
                  ...
                  Server.setSurfaceTile(x, y, 
                    Tiles.decodeHeight(tile), theTile.getBushType(data).asMyceliumBush(), data);
                  ...
                  Server.setSurfaceTile(x, y, 
                    Tiles.decodeHeight(tile), Tiles.Tile.TILE_MYCELIUM_LAWN.id, (byte)0);
                } else {
                  Server.setSurfaceTile(x, y, 
                    Tiles.decodeHeight(tile), Tiles.Tile.TILE_MYCELIUM.id, (byte)0);
                }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Corrupt.class
