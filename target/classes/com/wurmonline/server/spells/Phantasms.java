  ...
  extends ReligiousSpell
{
  ...
  void doEffect(Skill castSkill, double power, Creature performer, Creature target)
  {
    ...
    else
    {
      ...
      byte casterAtt;
      for (Iterator<Creature> it = nearbyCreatures.iterator(); it.hasNext(); testCret = (Creature)it.next()) {
        if (testCret != null) {
          ...
          else
          {
            ...
            {
              casterAtt = performer.getAttitude(testCret);
      ...
      {
        currentCret = 0;
        targetCret = Server.rand.nextInt(nearbyCreatures.size());
        for (Creature c : nearbyCreatures) {
          if (currentCret == targetCret)
          {
            ...
            performer.getCommunicator().sendNormalServerMessage(target.getName() + " starts to see phantasms  and turns towards " + c
              .getName() + " in anger.", (byte)2);
      ...
      {
        performer.getCommunicator().sendNormalServerMessage(target.getName() + " starts to see phantasms  but can only see " + target
          .getTarget().getName() + " as being the cause.", (byte)2);
      }
    }
  }
}
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Phantasms.class
