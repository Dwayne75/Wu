  ...
  void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset)
  {
    try
    {
      ...
      if (Tiles.isReinforcedCave(type))
      {
        ...
        else
        {
          ...
          Server.caveMesh.setTile(tilex, tiley, 
            Tiles.encode(Tiles.decodeHeight(tile), Tiles.Tile.TILE_CAVE_WALL.id, Tiles.decodeData(tile)));
          Players.getInstance().sendChangedTile(tilex, tiley, false, false);
          TileEvent.log(performer.getTileX(), performer.getTileY(), performer.getLayer(), performer
            .getWurmId(), 449);
          Tiles.Tile t = Tiles.getTile(type);
          if (t != null) {
            logger.info(performer.getName() + " (" + performer.getWurmId() + ") disintegrated a " + t
              .getName() + " at [" + tilex + "," + tiley + ",false]");
          }
      ...
      if ((dis) && (TileRockBehaviour.createInsideTunnel(tilex, tiley, tile, performer, 145, dir, true, act))) {
        performer.getCommunicator().sendNormalServerMessage("You disintegrate the " + 
          Tiles.getTile(Tiles.decodeType(tile)).tiledesc.toLowerCase() + ".", (byte)2);
      }
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\Disintegrate.class
