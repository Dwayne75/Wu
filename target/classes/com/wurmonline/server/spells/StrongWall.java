  ...
  extends ReligiousSpell
{
  ...
  void doEffect(Skill castSkill, double power, Creature performer, int tilex, int tiley, int layer, int heightOffset)
  {
    ...
    if (Tiles.isSolidCave(type))
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("Nothing happens to the " + 
          Tiles.getTile(type).tiledesc.toLowerCase() + ".", (byte)3);
      }
      return;
    }
    if ((Tiles.decodeType(tile) == Tiles.Tile.TILE_CAVE_EXIT.id) || (Tiles.decodeType(tile) == Tiles.Tile.TILE_CAVE.id) || 
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\spells\StrongWall.class
