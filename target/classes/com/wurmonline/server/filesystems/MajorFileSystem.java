  ...
  public File[] getAllFiles()
  {
    ...
    for (int x = 0; x < dirFiles.length; x++) {
      if (dirFiles[x].isDirectory()) {
        files.addAll(getAllFiles(dirFiles[x]));
      } else {
        files.add(dirFiles[x]);
      }
    }
    File[] toReturn = new File[files.size()];
    return (File[])files.toArray(toReturn);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\filesystems\MajorFileSystem.class
