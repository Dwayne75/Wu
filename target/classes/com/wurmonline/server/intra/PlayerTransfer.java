  ...
  implements MiscConstants, TimeConstants
{
  ...
  public PlayerTransfer(Server server, Player wurmplayer, String ip, int port, String serverpass, int _targetServerId, int posx, int posy, boolean aSurfaced, boolean toOrFromEpicCluster, byte targetKingdom)
    throws Exception
  {
  ...
  public static final boolean willItemsTransfer(Player player, boolean setTransferFlag, int targetServer, boolean changingCluster)
  {
    ...
    for (int x = 0; x < items.length; x++) {
      ...
      {
        numitems++;
        if ((player.getPower() == 0) && (changingCluster)) {
          if (numitems - stayBehind - 12 > 200) {
            if (!items[x].isInventory())
            {
              if (setTransferFlag) {
  ...
  public boolean poll()
  {
      ...
      if (!this.copiedToLoginServer) {
        if (!Servers.isThisLoginServer())
        {
          ...
          else
          {
            ...
            {
              this.copiedToLoginServer = true;
              logger.log(Level.INFO, "Copy to login server for " + this.player
                .getName() + " took " + (float)(System.nanoTime() - time) / 1000000.0F + " millis.");
            }
            else
            {
              logger.log(Level.INFO, "Failed copy to login server for " + this.player
                .getName() + " took " + 
                (float)(System.nanoTime() - time) / 1000000.0F + " millis.");
      ...
      if (this.copiedToLoginServer)
      {
        ...
        if (entry != null) {
          try
          {
            ...
            {
              logger.log(Level.INFO, "Copy to target server for " + entry
                .getName() + " (" + entry.id + ") " + this.player.getName() + " took " + (
              ...
              {
                logger.log(Level.WARNING, "Failed to set target server=" + this.targetServerId + " for " + this.player
                  .getName() + ".", iox);
              }
              this.player.getCommunicator().sendReconnect(entry.EXTERNALIP, Integer.parseInt(entry.EXTERNALPORT), this.player
                .getSaveFile().getPassword());
            ...
            {
              new LoginServerWebConnection(Servers.loginServer.id).setCurrentServer(this.player.getName(), 
                Servers.getLocalServerId());
              
              logger.log(Level.INFO, "Failed copy to target server for " + this.player
    ...
    if (this.data == null)
    {
      ...
      if (!this.toOrFromEpic) {
        if (this.player.getBody().getWounds() != null) {
      ...
      {
        if (this.toOrFromEpic) {
          this.data = createPlayerData(this.player.getSaveFile(), this.player.getStatus(), this.player.getSkills().getSkillsNoTemp(), this.targetServerId, this.targetKingdomId, 
            System.currentTimeMillis());
        } else {
          this.data = createPlayerData(wounds, this.player.getSaveFile(), this.player.getStatus(), this.player.getAllItems(), this.player
            .getSkills().getSkillsNoTemp(), this.player.getDraggedItem(), this.targetServerId, 
            System.currentTimeMillis(), this.targetKingdomId);
        }
      }
      catch (IOException iox)
      {
  ...
  public void dataReceived(IntraClient aClient)
  {
    ...
    {
      commandFailed(aClient);
    }
  }
  
  public static final void sendItem(Item item, DataOutputStream dos, boolean dragged)
    throws UnsupportedEncodingException, IOException
  {
    ...
    if (item.isUnfinished()) {
      if (item.getTemplateId() == 179)
      {
        Object doneSet = ItemRequirement.getRequirements(item.getWurmId());
        if (doneSet != null)
        {
          nums = ((Set)doneSet).size();
          if (nums > 0)
          {
            hasMoreItems = true;
            dos.writeBoolean(true);
            dos.writeInt(nums);
            for (player = ((Set)doneSet).iterator(); player.hasNext();)
            {
              next = (ItemRequirement)player.next();
    ...
    PermissionsByPlayer[] arrayOfPermissionsByPlayer1 = pbps;Player player = arrayOfPermissionsByPlayer1.length;
    for (ItemRequirement next = 0; next < player; next++)
    {
      PermissionsByPlayer pbp = arrayOfPermissionsByPlayer1[next];
  ...
  private static void sendSpellEffects(CreatureStatus status, DataOutputStream dos)
    throws IOException
  {
  ...
  public static byte[] createPlayerData(PlayerInfo pinf, CreatureStatus status, Skill[] skills, int targServId, byte targetKingdomId, long clientTimeDifference)
    throws IOException
  {
        ...
        if (pinf.realdeath > 0) {
          ...
          case 10068: 
            if (skills[s].getNumber() == 10067) {
              actualKnowledge = Math.max(pinf.champChanneling, actualKnowledge - 50.0D);
            } else {
              actualKnowledge = Math.max(10.0D, actualKnowledge - 50.0D);
            }
            actualMin = Math.max(actualKnowledge, actualMin - 50.0D);
            break;
          case 100: 
          case 101: 
          case 102: 
  ...
  public static byte[] createPlayerData(Wound[] wounds, PlayerInfo pinf, CreatureStatus status, Item[] items, Skill[] skills, @Nullable Item draggedItem, int targServId, long clientTimeDifference, byte targetKingdom)
    throws IOException
  {
    ByteArrayOutputStream bos = new ByteArrayOutputStream();
    try
    {
      ...
      if (cd != null)
      {
        ...
        if (cd.cooldowns.size() > 0) {
          ...
          {
            dos.writeInt(((Integer)ent.getKey()).intValue());
            dos.writeLong(((Long)ent.getValue()).longValue());
          }
        }
      }
      else
  ...
  private static final void sendPrivateMapAnnotations(long playerId, DataOutputStream dos)
    throws IOException
  {
    try
    {
    ...
    {
      logger.log(Level.WARNING, "Unable to find the player during transfer, no map annotations will be sent. " + nsp
        .getMessage(), nsp);
      dos.writeBoolean(false);
    }
  }
  
  private static final void sendAchievements(long wurmid, DataOutputStream dos)
    throws IOException
  {
  ...
  private static final void sendPMList(PlayerInfo pinf, DataOutputStream dos)
    throws IOException
  {
    ...
    {
      dos.writeUTF((String)x.getKey());
      dos.writeLong(((Long)x.getValue()).longValue());
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\intra\PlayerTransfer.class
