  ...
  implements SimpleConnectionListener, MiscConstants, TimeConstants
{
  ...
  public void reallyHandle(int num, ByteBuffer byteBuffer)
  {
    ...
    else if (cmd == 9)
    {
      ...
      try
      {
        ...
        pinf.load();
        sendPlayerVersion(pinf.version);
      }
    ...
    else if (cmd == 11)
    {
      ...
      try
      {
        ...
        PlayerInfo pinf = PlayerInfoFactory.createPlayerInfo(name);
        pinf.load();
        sendPlayerPaymentExpire(pinf.getPaymentExpire());
      }
      catch (NoSuchPlayerException nsp)
      {
        try
    ...
    else if (cmd == 3)
    {
      ...
      if (unpackPlayerData(posx, posy, surfaced)) {
        try
        {
          sendCommandDone();
        }
    ...
    else if (cmd == 16)
    {
      ...
      if ((info != null) && (info.wurmId > 0L))
      {
        if (info.currentServer != Servers.localServer.id) {
          logger.warning("Received a CMD_SET_PLAYER_MONEY for player " + info.getName() + " (id: " + wurmid + ") but their currentserver (id: " + info
            .getCurrentServer() + ") is not this server (id: " + Servers.localServer.id + ")");
        }
        try
        {
          ...
          try
          {
            ...
            p.getCommunicator().sendNormalServerMessage("Your available money in the bank is now " + c
              .getChangeString() + ".");
            if (referred)
            {
      ...
      if (System.currentTimeMillis() - check > 1000L) {
        logger.log(Level.INFO, "Lag detected at CMD_SET_PLAYER_MONEY: " + 
    ...
    else if (cmd == 17)
    {
      ...
      if (info != null) {
        ...
        catch (IOException iox)
        {
          ...
          this.conn.ticksToDisconnect = 200;
          if (System.currentTimeMillis() - check > 1000L) {
            logger.log(Level.INFO, "Lag detected at CMD_SET_PLAYER_PAYMENTEXPIRE IOEXCEPTION: " + 
          ...
          catch (NoSuchPlayerException exp)
          {
            if (dealItems) {
              try
              {
                ...
                logger.log(Level.INFO, "Inserted " + months + " sleep powder in offline " + info
      ...
      if (System.currentTimeMillis() - check > 1000L) {
        logger.log(Level.INFO, "Lag detected at CMD_SET_PLAYER_PAYMENTEXPIRE: " + 
          (int)((System.currentTimeMillis() - check) / 1000L));
      }
    }
    else if (cmd == 10)
    ...
    else if (cmd == 18)
    {
      ...
      if (System.currentTimeMillis() - check > 1000L) {
        logger.log(Level.INFO, "Lag detected at CMD_SET_PLAYER_PASSWORD: " + 
          (int)((System.currentTimeMillis() - check) / 1000L));
      }
    }
    else if (cmd == 15)
  ...
  public static final boolean setNewPassword(long playerId, String newHashedPassword)
  {
    ...
    if (info != null)
    {
      ...
      {
        logger.log(Level.WARNING, "Failed to load info for wurmid " + playerId + ". Password unchanged." + eex
          .getMessage(), eex);
      }
      if (info.wurmId <= 0L)
      {
  ...
  public static final void deletePlayer(long id)
    throws IOException
  {
  ...
  private static final void deletePlayer(String name, long id)
    throws IOException
  {
    Connection dbcon = null;
    PreparedStatement ps = null;
  ...
  public static final void deleteItem(long id, boolean frozen)
    throws IOException
  {
  ...
  public static final void readCultist(DataInputStream dis, String name, long wurmId)
  {
    try
    {
      ...
      if (cultist == null)
      {
        ...
        {
          logger.log(Level.WARNING, "Failed to save cultist " + name + " level=" + clevel + " path=" + cpath + " " + iox
            .getMessage(), iox);
        }
    ...
    try
    {
      ...
      if (pinf != null)
      {
        ...
        if (!password.equals(pinf.getPassword())) {
          logger.log(Level.WARNING, name + " after transfer but before loading: password now is " + pinf
            .getPassword() + ". Sent " + password);
        }
        pinf.loaded = false;
        try
        {
  ...
  private static final void unpackAchievements(long wurmId, DataInputStream dis)
    throws IOException
  {
    ...
    for (int x = 0; x < nums; x++)
    {
      ...
      new Achievement(achievement, ts, wurmId, counter, -1).create();
    }
  }
  
  private static final void unpackPMList(PlayerInfo pinf, DataInputStream dis)
    throws IOException
  {
  ...
  private static final void unpackPrivateMapAnnotations(long playerID, DataInputStream dis)
    throws IOException
  {
  ...
  public static long savePlayerToDisk(byte[] bytes, int posx, int posy, boolean surfaced, boolean newPlayer)
  {
    ...
    try
    {
      ...
      catch (NoSuchPlayerException localNoSuchPlayerException) {}catch (Exception ex)
      {
        logger.log(Level.WARNING, ex.getMessage(), ex);
      }
      PlayerInfo info = PlayerInfoFactory.getPlayerInfoWithWurmId(wurmId);
      ...
      for (Iterator<Long> it = itemIds.iterator(); it.hasNext();) {
        deleteItem(((Long)it.next()).longValue(), (info != null) && (info.hasMovedInventory()));
      }
      ...
      if (pet != -10L)
      {
        if (!Creatures.getInstance().isCreatureOffline(Long.valueOf(pet))) {
          try
          {
      ...
      if (Servers.localServer.PVPSERVER)
      {
        ...
        if (info != null) {
          if ((info.getDeity() != null) && (info.getDeity().getNumber() == 4))
          {
      ...
      if (!Servers.localServer.LOGINSERVER) {
        try
        {
          ...
          zoneId = zone.getId();
          posz = 0.0F;
          tile = Server.surfaceMesh.getTile(posx, posy);
          if (!surfaced)
          {
      ...
      for (int seff = 0; seff < numspeffects; ???++) {
      ...
      if (info != null)
      {
        ...
        if (!password.equals(info.getPassword())) {
          logger.log(Level.WARNING, name + " after transfer but before loading: password now is " + info
            .getPassword() + ". Sent " + password);
        }
        info.loaded = false;
        try
  ...
  public static void createItem(DataInputStream dis, float posx, float posy, float posz, Set<ItemMetaData> metadataset, boolean frozen)
    throws IOException
  {
    try
    {
  ...
  private void validate(ByteBuffer byteBuffer)
  {
    ...
    if (!password.equals(Servers.localServer.INTRASERVERPASSWORD))
    {
      try
      {
        sendLoginAnswer(false, "Wrong password: " + password, 0);
      }
      catch (IOException localIOException1) {}
      return;
    }
  ...
  private void sendLoginAnswer(boolean ok, String message, int retrySeconds)
    throws IOException
  {
    try
    {
  ...
  private void sendTransferUserRequestAnswer(boolean ok, String sessionKey, int retrySeconds)
    throws IOException
  {
    try
    {
  ...
  private void sendCommandFailed()
    throws IOException
  {
  ...
  private void sendPlayerVersion(long playerversion)
    throws IOException
  {
    try
    {
  ...
  private void sendPlayerPaymentExpire(long paymentExpire)
    throws IOException
  {
    try
    {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\intra\IntraServerConnection.class
