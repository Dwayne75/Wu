  ...
  implements Runnable
{
  ...
  public final void run()
  {
    ...
    try
    {
      if (logger.isLoggable(Level.FINEST)) {
        logger.finest("Starting DatabaseLogger.run() " + this.iLoggerDescription + " for WurmLoggable type: " + this.iLoggableClass
          .getName());
      }
      if (!this.queue.isEmpty())
      {
        ...
        {
          T object = (WurmLoggable)this.queue.remove();
          objectsRemoved++;
          if (logger.isLoggable(Level.FINEST)) {
          ...
          addLoggableToBatch(logsStatement, object);
        }
        logsStatement.executeBatch();
        if ((logger.isLoggable(Level.FINER)) || ((!this.queue.isEmpty()) && (logger.isLoggable(Level.FINE)))) {
          logger.fine("Removed " + this.iLoggableClass.getName() + ' ' + objectsRemoved + " objects from FIFO queue, which now contains " + this.queue
            .size() + " objects");
        }
      }
    }
    ...
    {
      if (logger.isLoggable(Level.FINEST)) {
        logger.finest("Ending DatabaseLogger.run() " + this.iLoggerDescription + " for WurmLoggable type: " + this.iLoggableClass
          .getName());
      }
  ...
  abstract void addLoggableToBatch(PreparedStatement paramPreparedStatement, T paramT)
    throws SQLException;
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\utils\logging\DatabaseLogger.class
