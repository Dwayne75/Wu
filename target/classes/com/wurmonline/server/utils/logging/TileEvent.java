  ...
  implements WurmLoggable, TimeConstants
{
  ...
  public static void log(int _tileX, int _tileY, int _layer, long _performer, int _action)
  {
    if (Constants.useTileEventLog)
    {
      ...
      try
      {
        TileEvent oEvent = (TileEvent)playersLog.get(Long.valueOf(_performer));
        if ((oEvent != null) && (oEvent.tilex == _tileX) && (oEvent.tiley == _tileY) && (oEvent.layer == _layer) && (oEvent.action == _action) && 
          (oEvent.date > System.currentTimeMillis() - 300000L)) {
          return;
        }
        lEvent = new TileEvent(_tileX, _tileY, _layer, _performer, _action);
        playersLog.put(Long.valueOf(_performer), lEvent);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\utils\logging\TileEvent.class
