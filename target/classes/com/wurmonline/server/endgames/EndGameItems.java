  ...
  implements MiscConstants, ItemTypes, Enchants, TimeConstants
{
  ...
  public static final float findPlacementTile(int tx, int ty)
  {
    ...
    for (int x = 0; x < 20; x++) {
      for (int y = 0; y < 20; y++)
      {
        ...
        if ((ttype != Tiles.Tile.TILE_ROCK.id) && (ttype != Tiles.Tile.TILE_CLIFF.id) && (ttype != Tiles.Tile.TILE_HOLE.id)) {
          if ((z > 0.0F) && (z > maxZ) && (z < 700.0F))
          {
            tileX = tx + x;
            tileY = ty + y;
            maxZ = z;
  ...
  public static final Item[] getArtifactDugUp(int x, int y, float height, boolean allCornersRock)
  {
    ...
    for (EndGameItem artifact : artifacts.values()) {
      if ((artifact.getItem().getZoneId() == -10L) && (artifact.getItem().getOwnerId() == -10L)) {
        if (((int)artifact.getItem().getPosX() >> 2 == x) && ((int)artifact.getItem().getPosY() >> 2 == y) && (
  ...
  public static final void loadEndGameItems()
  {
    ...
    logger.log(Level.INFO, "Loaded " + numberOfAltars + " altars and " + numberOfArtifacts + " artifacts. That took " + 
  ...
  {
    if (item.isHugeAltar()) {
      return (EndGameItem)altars.get(new Long(item.getWurmId()));
    }
    if (item.isArtifact()) {
      return (EndGameItem)artifacts.get(new Long(item.getWurmId()));
    }
    return null;
  }
  ...
  {
    EndGameItem eg = (EndGameItem)altars.get(new Long(altar.getWurmId()));
    if (eg != null)
    {
      Server.getInstance().broadCastAlert("The " + altar
        .getName() + " has fallen to the hands of " + destroyer.getName() + "!", true, (byte)4);
      ...
      if (destroyer.isPlayer())
      {
        ...
        ex = altar.getPosX() + 100.0F;
        sy = altar.getPosY() - 100.0F;
        ey = altar.getPosY() + 100.0F;
        for (Player p : Players.getInstance().getPlayers()) {
          if ((p.getPosX() > sx) && (p.getPosX() < ex) && 
            (p.getPosY() > sy) && (p.getPosY() < ey) && 
            (p.getKingdomId() == destroyer.getKingdomId()))
          {
      ...
      float ex = players;float sy = ex.length;
      for (float ey = 0; ey < sy; ey++)
      {
        Player lPlayer = ex[ey];
        if (eg.isHoly())
        {
          ...
          {
            lPlayer.getCommunicator().sendCombatAlertMessage("Your life force is drained, as it is used to heal the " + altar
              .getName() + "!");
            lPlayer.addWoundOfType(null, (byte)9, 1, false, 1.0F, false, 5000.0D, 0.0F, 0.0F, false, false);
          }
        }
        else if ((lPlayer.getDeity() != null) && (!lPlayer.getDeity().isHateGod()))
        {
  ...
  private static final void healAndTeleportAltar(EndGameItem altar)
  {
    ...
    try
    {
      ...
      if (altar.isHoly()) {
        for (Player lPlayer : p) {
          lPlayer.getCommunicator().sendAddEffect(altar.getWurmid(), (short)2, altar
            .getItem().getPosX(), altar.getItem().getPosY(), altar.getItem().getPosZ(), (byte)0);
        }
      } else {
        for (Player lPlayer : p) {
          lPlayer.getCommunicator().sendAddEffect(altar.getWurmid(), (short)3, altar
            .getItem().getPosX(), altar.getItem().getPosY(), altar.getItem().getPosZ(), (byte)0);
        }
      }
    }
      ...
      if (artifact.getOwnerId() != -10L)
      {
        ...
        if ((stat != null) && 
          ...
          try
          {
            ...
            Zone z = Zones.getZone((int)stat.getPosX() >> 2, (int)stat.getPosY() >> 2, stat
              .getLayer() >= 0);
            artifact.setPosXY(stat.getPosX(), stat.getPosY());
            z.addItem(artifact);
            logger.log(Level.INFO, "Zone " + z
              .getId() + " added " + artifact.getName() + " at " + (
              (int)stat.getPosX() >> 2) + "," + ((int)stat.getPosY() >> 2));
          }
          catch (NoSuchItemException nsi)
          {
  ...
  public static final void pollAll()
  {
    ...
    for (EndGameItem lArt : arts) {
      if (lArt.isInWorld()) {
        if (System.currentTimeMillis() - lArt.getLastMoved() > (Servers.isThisATestServer() ? 60000L : 604800000L))
        {
          ...
          else
          {
            ...
            catch (NoSuchCreatureException localNoSuchCreatureException) {}catch (NoSuchPlayerException localNoSuchPlayerException) {}catch (NotOwnedException localNotOwnedException) {}
          }
        }
      }
    }
  ...
  private static final void moveArtifact(Item artifact)
  {
    try
    {
      ...
      {
        Creature owner = Server.getInstance().getCreature(artifact.getOwner());
        owner.getCommunicator().sendNormalServerMessage(artifact
          .getName() + " disappears. It has fulfilled its mission.");
      }
    }
    catch (NoSuchCreatureException localNoSuchCreatureException) {}catch (NoSuchPlayerException localNoSuchPlayerException) {}catch (NotOwnedException localNotOwnedException) {}
    String act;
    ...
    if (templateId == -1)
    {
      if (Server.rand.nextBoolean())
      {
        ...
        if (missingCrowns.size() > 0)
        {
          ...
          Kingdom toLookFor = (Kingdom)missingCrowns.get(crownToLookFor);
          Item[] _items = Items.getAllItems();
          for (Item lItem : _items) {
            if (lItem.isRoyal()) {
              if (lItem.getKingdom() == toLookFor.getId()) {
      ...
      if (itemsearched == null)
      {
        ...
        if (s > 0)
        {
          num = Server.rand.nextInt(s);
          x = 0;
          for (it = artifacts.values().iterator(); it.hasNext();)
          {
            itemsearched = (EndGameItem)it.next();
            if (x == num) {
              break;
            }
            x++;
      ...
      else if (itemsearched.isInWorld())
      {
        ...
        if (t != null)
        {
          ...
          if (ct != null)
          {
            int ctx = ct.tilex;
            cty = ct.tiley;
            mindist = Math.max(Math.abs(tilex - ctx), Math.abs(tiley - cty));
            dir = MethodsCreatures.getDir(performer, tilex, tiley);
        ...
        else
        {
          try
          {
            ...
            else
            {
              ...
              if (structs.length > 0)
              {
                mindist = structs;dir = mindist.length;
                for (lVillage = 0; lVillage < dir; lVillage++)
                {
                  Structure lStruct = mindist[lVillage];
              ...
              else
              {
                ...
                if (ct != null)
                {
                  ...
                  String direction = MethodsCreatures.getLocationStringFor(performer.getStatus()
                    .getRotation(), dir, "you");
                  toReturn = toReturn + getDistanceString(mindist, name, direction, true);
                }
              }
          ...
          {
            logger.log(Level.WARNING, "No Zone At " + tilex + ", " + tiley + " surf=true for item " + itemsearched
              .getItem().getName() + ".", nsz);
          }
        }
      }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\endgames\EndGameItems.class
