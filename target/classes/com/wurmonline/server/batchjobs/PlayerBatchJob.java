  ...
  implements TimeConstants, MiscConstants
{
  ...
  private static final String getPlayers = "SELECT WURMID FROM PLAYERS WHERE LASTLOGOUT<" + (
    System.currentTimeMillis() - 2419200000L);
      ...
      while (rs.next())
      {
        ...
        if (toCreateOn != null)
        {
          ...
          try
          {
            ...
            byte[] playerData = lsw.createAndReturnPlayer(name, password, challengePhrase, answer, email, kingdom, power, Server.rand
              .nextLong(), gender, true, true, true);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\batchjobs\PlayerBatchJob.class
