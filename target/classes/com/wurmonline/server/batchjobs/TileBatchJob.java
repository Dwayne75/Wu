    ...
    for (int x = 0; x < max; x++) {
      for (int y = 0; y < max; y++)
      {
        ...
        if ((tileType == Tiles.Tile.TILE_TREE.id) || (tileType == Tiles.Tile.TILE_ENCHANTED_TREE.id) || (tileType == Tiles.Tile.TILE_MYCELIUM_TREE.id))
        {
          ...
          if (((treeType < 0) || (treeType >= 10)) && (treeType != 16) && (treeType != 17)) {
  ...
  public static final void clearFlowers()
  {
    ...
    for (int x = 0; x < max; x++) {
      for (int y = 0; y < max; y++)
      {
        tile = Server.surfaceMesh.getTile(x, y);
        if (Tiles.decodeType(tile) == Tiles.Tile.TILE_GRASS.id) {
          if (Tiles.decodeData(tile & 0xF) > 0)
          {
            fixed++;
  ...
  public static final void fixCorruptBushData()
  {
    ...
    for (int x = 0; x < max; x++) {
      for (int y = 0; y < max; y++)
      {
        ...
        if (type == 31) {
          if ((Tiles.decodeData(tile) & 0xF) == 15)
          {
            fixed++;
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\batchjobs\TileBatchJob.class
