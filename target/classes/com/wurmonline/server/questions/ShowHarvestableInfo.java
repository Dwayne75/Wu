  ...
  extends Question
{
  ...
  {
    return LoginHandler.raiseFirstLetter(harvestable.getName() + " report (" + df
      .format(paper.getCurrentQualityLevel()) + ")");
  }
  ...
  public void answer(Properties answers)
  {
    ...
    {
      boolean show = getBooleanProp("show");
      String sbyid = getAnswer().getProperty("by");
      int byid = Integer.parseInt(sbyid);
      if (show)
      {
        ShowHarvestableInfo shi = new ShowHarvestableInfo(getResponder(), this.almanac, byid);
        shi.sendQuestion();
      }
    }
  }
  ...
  public void sendQuestion()
  {
    ...
    buf.append("border{border{size=\"20,20\";null;null;label{type='bold';text=\"" + this.question + "\"};harray{" + dropdown + "label{text=\" \"};button{text=\"Close\";id=\"close\"};label{text=\" \"}};null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\"" + 
    ...
    if (this.harvestable != null)
    {
      ...
      {
      case 1: 
        buf.append(showOliveInfo());
        break;
      case 2: 
        buf.append(showGrapeInfo());
        break;
      case 3: 
        buf.append(showCherryInfo());
        break;
      case 4: 
        buf.append(showAppleInfo());
        break;
      case 5: 
        buf.append(showLemonInfo());
        break;
      case 6: 
        buf.append(showOleanderInfo());
        break;
      case 7: 
        buf.append(showCamelliaInfo());
        break;
      case 8: 
        buf.append(showLavenderInfo());
        break;
      case 9: 
        buf.append(showMapleInfo());
        break;
      case 10: 
        buf.append(showRoseInfo());
        break;
      case 11: 
        buf.append(showChestnutInfo());
        break;
      case 12: 
        buf.append(showWalnutInfo());
        break;
      case 13: 
        buf.append(showPineInfo());
        break;
      case 14: 
        buf.append(showHazelInfo());
        break;
      case 15: 
        buf.append(showHopsInfo());
        break;
      case 16: 
        buf.append(showOakInfo());
        break;
      case 17: 
        buf.append(showOrangeInfo());
        break;
      case 18: 
        buf.append(showRaspberryInfo());
        break;
      case 19: 
        buf.append(showBlueberryInfo());
        break;
      case 20: 
        buf.append(showLingonberryInfo());
      }
      buf.append(showHarvestableTimes());
    }
    else if (this.almanac != null)
    {
      buf.append(showSummary());
    }
    buf.append("label{text=\"\"}");
    
    buf.append("}};null;null;}");
    getResponder().getCommunicator().sendBml(500, 450, true, true, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  private String showSummary()
  {
    StringBuilder buf = new StringBuilder();
    buf.append("label{text=\"\"}");
    Item[] reports = Methods.getBestReports(getResponder(), this.almanac);
    if (reports.length == 0)
    {
    ...
    {
      if ((this.almanac.getOwnerId() == getResponder().getWurmId()) && (reports.length == 20)) {
        getResponder().achievement(575);
      }
      ...
      for (Item report : reports)
      {
        ...
        if (harvestable != null)
        {
          ...
          long howGood = (report.getCurrentQualityLevel() * 86400.0F);
          String harvestStart = "";
          String harvestEnd = "";
          String colourStart = "";
          String colourState = "";
          ...
          {
            if (knowHarvestStart(harvestable, howGood))
            {
            ...
            else if (knowHarvestEnd(harvestable, howGood))
            {
              harvestEnd = WurmCalendar.getDaysFrom(harvestable.getSeasonEnd());
            }
          ...
          else if ((harvestable.isAlmostRipe()) && (showSummaryAlmostRipe()))
          {
            ...
            if (knowHarvestEnd(harvestable, howGood))
            {
              harvestEnd = WurmCalendar.getDaysFrom(harvestable.getSeasonEnd());
            }
            else
          ...
          if (output)
          {
            buf.append("label{" + colourState + "text=\"" + harvestable.getName() + "\"};label{" + colourState + "text=\"" + df
              .format(report.getCurrentQualityLevel()) + "\"};label{" + colourState + "text=\"" + harvestable
              .getState() + "\"};label{" + colourStart + "text=\"" + harvestStart + "\"};label{" + colourState + "text=\"" + harvestEnd + "\"};");
  ...
  private String showHarvestableTimes()
  {
    ...
    long howGood = (this.paper.getCurrentQualityLevel() * 86400.0F);
    if (this.harvestable.isHarvestable())
    {
      buf.append("text{text=\"It is currently harvestable.\"}");
      if (knowHarvestStart(this.harvestable, howGood)) {
        buf.append("text{text=\"The harvest season started " + 
          WurmCalendar.getDaysFrom(this.harvestable.getSeasonStart()) + "\"}");
      }
      if (knowHarvestEnd(this.harvestable, howGood)) {
        buf.append("text{text=\"The harvest season should end in " + 
          WurmCalendar.getDaysFrom(this.harvestable.getSeasonEnd()) + "\"}");
      }
    }
    else if (this.harvestable.isAlmostRipe())
    {
      if (knowHarvestStart(this.harvestable, howGood)) {
        buf.append("text{text=\"The harvest season will start in " + 
          WurmCalendar.getDaysFrom(this.harvestable.getSeasonStart()) + "\"}");
      } else {
        buf.append("text{text=\"The default harvest season is in " + 
    ...
    else if (knowHarvestStart(this.harvestable, howGood))
    {
      buf.append("text{text=\"The harvest season will start in " + 
        WurmCalendar.getDaysFrom(this.harvestable.getSeasonStart()) + "\"}");
    }
    ...
    {
      buf.append("text{text=\"The default harvest season is in " + 
        WurmCalendar.getDaysFrom(this.harvestable.getDefaultSeasonStart()) + " This is only approximate as it can be plus or minus 2 weeks from that date. \"}");
    }
    return buf.toString();
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ShowHarvestableInfo.class
