  ...
  public void answer(Properties aAnswer)
  {
    ...
    if (this.type == 114) {
      ...
      case 0: 
        ...
        {
          InGameVoteSetupQuestion igvsq = new InGameVoteSetupQuestion(getResponder(), "In game voting setup", "Add New Question", (byte)1);
        ...
        else
        {
          ...
          {
            nextPart = 2;
            String windowTitle = "Edit Question";
            header = "";
          }
          else
          {
            String header;
            if (vq.getVoteEnd() > System.currentTimeMillis())
            {
              nextPart = 3;
          ...
          InGameVoteSetupQuestion igvsq = new InGameVoteSetupQuestion(getResponder(), windowTitle, header, nextPart, this.questionId);
      ...
      case 2: 
        ...
        {
          strServer = aAnswer.getProperty("s" + entries[x].getId());
          if ((strServer != null) && (Boolean.parseBoolean(strServer))) {
            this.serverIds.add(Integer.valueOf(entries[x].getId()));
          }
        }
        if (fail.length() == 0) {
        ...
        if (fail.length() > 0)
        {
          InGameVoteSetupQuestion igvsq = new InGameVoteSetupQuestion(getResponder(), this.title, "Error: " + fail, this.part, this.questionId, fail);
        ...
        else
        {
          ...
          for (Integer serverId : this.serverIds) {
            servers.add(new VoteServer(this.questionId, serverId.intValue()));
          }
      ...
      case 3: 
        ...
        if ((remove != null) && (Boolean.parseBoolean(remove)))
        {
          ...
          VoteQuestions.closeVoteing(this.questionId, calNow.getTimeInMillis()); return;
        }
        break;
      }
    }
      ...
      try
      {
        ...
        case 0: 
          ...
          for (int x = 0; x < vqs.length; x++)
          {
            ...
            buf.append("radio{group=\"qid\";id=\"" + vqs[x].getQuestionId() + "\"};label{text=\"" + vqs[x]
              .getQuestionTitle() + "\"" + colour + "};label{text=\"" + 
              Tickets.convertTime(vqs[x].getVoteStart()) + "\"" + colour + "};label{text=\"" + 
              Tickets.convertTime(vqs[x].getVoteEnd() - 1000L) + "\"" + colour + "};");
            
            vsl = vqs[x].getServers();
            ServerEntry[] arrayOfServerEntry3 = servers;se = arrayOfServerEntry3.length;
            for (ServerEntry localServerEntry1 = 0; localServerEntry1 < se; localServerEntry1++)
            {
              ServerEntry se = arrayOfServerEntry3[localServerEntry1];
              
              String sss = " o";
              for (VoteServer vs : (List)vsl) {
        ...
        case 4: 
          ...
          buf.append(showSummaryRow(vq.getOption1Text(), vq.getOption1Count(), vq.getVoteCount()));
          buf.append(showSummaryRow(vq.getOption2Text(), vq.getOption2Count(), vq.getVoteCount()));
          buf.append(showSummaryRow(vq.getOption3Text(), vq.getOption3Count(), vq.getVoteCount()));
          buf.append(showSummaryRow(vq.getOption4Text(), vq.getOption4Count(), vq.getVoteCount()));
          
          buf.append("label{type=\"bold\";text=\"Total Players who voted\"};label{type=\"bold\";text=\"" + vq
            .getVoteCount() + "\"};label{text=\"\"}");
        ...
        getResponder().getCommunicator().sendBml(this.width, this.height, true, true, buf.toString(), 200, 200, 200, this.title);
      }
      catch (Exception e)
      {
    ...
    for (int x = 0; x < entries.length; x++)
    {
      ...
      if (vq != null) {
        ...
        for (Integer sid : this.serverIds) {
          if (sid.intValue() == entries[x].getId())
          {
            selected = ";selected=\"true\"";
            break;
          }
      ...
      if (entries[x].isAvailable(getResponder().getPower(), getResponder().isPaying())) {
        buf.append("harray{label{color=\"100,255,100\";text=\"" + entries[x].getName() + "?\"}checkbox{id=\"s" + entries[x]
          .getId() + "\"" + selected + disabled + "}}");
      } else {
        buf.append("harray{label{color=\"255,100,100\";text=\"" + entries[x].getName() + "?\"}checkbox{id=\"s" + entries[x]
          .getId() + "\"" + selected + disabled + "}}");
      }
    }
    String textMonth = new java.text.DateFormatSymbols().getMonths()[this.startMonth];
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\InGameVoteSetupQuestion.class
