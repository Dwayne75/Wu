    ...
    if (village != null)
    {
      ...
      if (pvpAll != null)
      {
        ...
        if (FocusZone.getHotaZone() != null) {
          buf.append("text{text=\"" + pvpAll.getName() + " has won the Hunt of the Ancients " + pvpAll
            .getNumberOfWins() + " times.\"}");
        }
        if (village.getId() == village.getAllianceNumber())
        {
          if (village.getMayor().getId() == getResponder().getWurmId())
          {
            buf.append("text{text=\"" + village
              .getName() + " is the capital in the alliance which means your diplomats are responsible for ousting other settlements. The mayor may change name, disband or set another village as the alliance capital:\"};");
            ...
            for (x = 0; x < this.allies.length; x++) {
          ...
          for (Village ally : this.allies) {
            if (ally != village) {
              buf.append("harray{label{text=\"Check to break alliance with " + ally.getName() + ":\"}checkbox{id=\"break" + ally
        ...
        {
          buf.append("harray{label{text=\"Check to break alliance with " + pvpAll.getName() + ":\"}checkbox{id=\"break" + pvpAll
        ...
        for (WarDeclaration declaration : village.warDeclarations.values()) {
          if (declaration.declarer == village)
          {
            if ((Servers.isThisAChaosServer()) && 
              (System.currentTimeMillis() - declaration.time > 86400000L))
            {
              declaration.accept();
            ...
            {
              buf.append("harray{label{text=\"Check to withdraw declaration to " + declaration.receiver.getName() + ":\"}checkbox{id'decl" + declaration.receiver
                .getId() + "';text=' '}}");
            }
          }
          else if (Servers.isThisAChaosServer())
          {
            ...
            {
              buf.append("harray{label{text=\"You have " + 
                Server.getTimeFor(System.currentTimeMillis() - declaration.time) + " until you automatically accept the declaration of war.\"}}");
              
              buf.append("harray{label{text=\"Check to accept declaration from " + declaration.declarer
                .getName() + ":\"}checkbox{id='recv" + declaration.declarer
            ...
            {
              declaration.accept();
              buf.append("harray{label{text=\"" + declaration.receiver.getName() + " has now automatically accepted the war declaration from " + declaration.declarer
              
                .getName() + ".\"}}");
            }
          }
          else {
            buf.append("harray{label{text=\"Check to accept declaration from " + declaration.declarer.getName() + ":\"}checkbox{id='recv" + declaration.declarer
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ManageAllianceQuestion.class
