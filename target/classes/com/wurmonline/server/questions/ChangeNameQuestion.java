  ...
  extends Question
{
  ...
  public void answer(Properties answers)
  {
    ...
    if (newname.equalsIgnoreCase(getResponder().getName()))
    {
    ...
    if (Deities.isNameOkay(newname))
    {
      ...
      else
      {
        ...
        String toReturn = lsw.renamePlayer(oldname, newname, newpw, 
          getResponder().getPower());
        ...
        if (!toReturn.contains("Error."))
        {
          ...
          logger.info(oldname + " (" + getResponder().getWurmId() + ") changed " + 
            getResponder().getHisHerItsString() + " name to " + newname + '.');
          
          Server.getInstance().broadCastSafe(oldname + " changed " + 
            getResponder().getHisHerItsString() + " name to " + newname + '.');
  ...
  public void sendQuestion()
  {
    ...
    buf.append("input{id=\"answer\";maxchars=\"40\";text=\"" + getResponder().getName() + "\"}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ChangeNameQuestion.class
