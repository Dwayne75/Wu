  ...
  implements TimeConstants
{
  ...
  public static void dismissMerchant(Creature dismisser, long target)
  {
    try
    {
      ...
      if (trader != null)
      {
        if (!trader.isTrading())
        {
          Server.getInstance().broadCastAction(trader
            .getName() + " grunts, packs " + trader.getHisHerItsString() + " things and is off.", trader, 5);
          if (dismisser != null)
          {
            dismisser.getCommunicator().sendNormalServerMessage("You dismiss " + trader
              .getName() + " from " + trader.getHisHerItsString() + " post.");
          ...
          for (int x = 0; x < items.length; x++) {
            if (items[x].getTemplateId() == 300) {
              if (items[x].getData() == target)
              {
                items[x].setData(-1, -1);
                break;
              }
          ...
          if (shop != null) {
            try
            {
              Item backPack = ItemFactory.createItem(1, 10.0F + Server.rand
                .nextFloat() * 10.0F, trader.getName());
              backPack.setDescription("Due to poor business I have moved on. Thank you for your time. " + trader
                .getName());
              
              ArrayList<Item> largeItems = new ArrayList();
              for (Item realItem : trader.getInventory().getAllItems(false)) {
                if ((realItem.getTemplate() != null) && (!realItem.getTemplate().isComponentItem()) && 
                  (!backPack.insertItem(realItem, false))) {
            ...
            {
              logger.log(Level.WARNING, e.getMessage() + " " + trader.getName() + " at " + trader
                .getTileX() + ", " + trader.getTileY(), e);
            }
          } else {
            logger.log(Level.WARNING, "No shop when dismissing trader " + trader
              .getName() + " " + trader.getWurmId());
          }
          trader.destroy();
        }
  ...
  public void sendQuestion()
  {
    ...
    if (this.isDismissing) {
      buf.append(mayorDismissingQuestion());
    } else {
      buf.append(contractQuestion());
    }
    buf.append(createAnswerButton3());
    if (this.isDismissing) {
      getResponder().getCommunicator().sendBml(250, 200, true, true, buf.toString(), 200, 200, 200, this.title);
    } else {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\TraderManagementQuestion.class
