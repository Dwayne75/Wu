  ...
  implements VillageStatus, ItemTypes, MonetaryConstants, CounterTypes
{
  ...
  private boolean answersFail()
  {
    ...
    String val = getAnswer().getProperty("cancel");
    ...
    if (this.expanding) {
      try
      {
        ...
        if ((oldvill.plan != null) && 
          (oldvill.plan.isUnderSiege()))
        {
    ...
    if (!checkBlockingCreatures()) {
      return true;
    }
    if (!checkStructuresInArea()) {
      return true;
    }
    if (!checkBlockingItems()) {
      return true;
    }
    setSize();
    if (!checkTile()) {
  ...
  {
    VillageFoundationQuestion vf = new VillageFoundationQuestion(getResponder(), "Settlement Application Form", "Welcome to the Settlement Application Form", this.target);
    if (vf != null)
    {
      copyValues(vf);
      vf.sequence = 0;
  ...
  {
    VillageFoundationQuestion vf = new VillageFoundationQuestion(getResponder(), "Settlement Size", "Stage One - The size of your settlement", this.target);
    if (vf != null)
    {
      copyValues(vf);
      vf.sequence = 1;
  ...
  {
    VillageFoundationQuestion vfq = new VillageFoundationQuestion(getResponder(), "The Deed Perimeter", "Congratulations! You have chosen a deed that will be " + this.diameterX + " tiles by " + this.diameterY + " tiles.", this.target);
    if (vfq != null)
    {
      copyValues(vfq);
      vfq.sequence = 2;
  ...
  {
    VillageFoundationQuestion vfq = new VillageFoundationQuestion(getResponder(), "Naming Your Deed", "Your survey was a success!", this.target);
    if (vfq != null)
    {
      copyValues(vfq);
      vfq.sequence = 3;
  ...
  {
    VillageFoundationQuestion vfq = new VillageFoundationQuestion(getResponder(), "Guards", "Congratulations! You have reserved the name '" + this.villageName + "'", this.target);
    if (vfq != null)
    {
      copyValues(vfq);
      vfq.sequence = 4;
  ...
  {
    VillageFoundationQuestion vfq = new VillageFoundationQuestion(getResponder(), "Check your settings", "Check your settings and Found your settlement!", this.target);
    if (vfq != null)
    {
      copyValues(vfq);
      vfq.sequence = 5;
  ...
  private void createQuestion6()
  {
    if (!this.expanding)
    {
      SoundPlayer.playSong("sound.music.song.foundsettlement", getResponder());
      VillageFoundationQuestion vfq = new VillageFoundationQuestion(getResponder(), "CONGRATULATIONS!", "CONGRATULATIONS!", this.target);
      if (vfq != null)
      {
        copyValues(vfq);
        vfq.sequence = 6;
  ...
  boolean parseVillageFoundationQuestion1()
  {
    ...
    if (((this.diameterX / this.diameterY <= 4.0F) && (this.diameterY / this.diameterX <= 4.0F) ? 1 : 0) == 0) {
      ...
      for (int x = xa; x <= xe; x++) {
        for (int y = ya; y <= ye; y++)
        {
          ...
          if (create) {
            try
            {
              Item i = ItemFactory.createItem(671, 80.0F, getResponder().getName());
        ...
        for (int x = xa; x <= xe; x++) {
          for (int y = ya; y <= ye; y++)
          {
            ...
            if (create) {
              try
              {
                Item i = ItemFactory.createItem(673, 80.0F, getResponder().getName());
                i.setPosXYZ((x << 2) + 2, (y << 2) + 2, 
                  Zones.calculateHeight((x << 2) + 2, (y << 2) + 2, true) + 5.0F);
                Zones.getZone(x, y, true).addItem(i);
              }
              catch (Exception ex)
              {
  ...
  boolean parseVillageFoundationQuestion3()
  {
    ...
    if (getResponder().getPower() >= 3)
    {
      ...
      val = getAnswer().getProperty("kingdomid");
      if (val != null) {
  ...
  private long getFoundingCharge()
  {
    ...
    return getFoundingCost() + 30000L - (this.deed.getTemplateId() == 862 ? 0L : 100000L);
  }
  
  boolean parseVillageFoundationQuestion5()
  {
    String key = "back";
    String val = getAnswer().getProperty("back");
    if ((val != null) && (val.equals("true")))
    {
      createQuestion4();
      return false;
    }
    long moneyNeeded = getFoundingCost();
    ...
    if (this.expanding) {
      moneyNeeded = getResizeCostDiff();
    }
    try
    {
      if (!this.expanding)
      {
        long toCharge = getFoundingCharge();
        boolean charge = toCharge > 0L;
        long left = this.deed.getTemplateId() == 862 ? 30000L : 0L;
        if (toCharge < 0L) {
          left = 100000L - getFoundingCost();
        }
        if ((!charge) || (((Player)getResponder()).chargeMoney(toCharge)))
        {
          try
          {
            Village v = Villages.createVillage(Zones.safeTileX(this.tokenx - this.selectedWest), 
              Zones.safeTileX(this.tokenx + this.selectedEast), Zones.safeTileY(this.tokeny - this.selectedNorth), 
              Zones.safeTileY(this.tokeny + this.selectedSouth), this.tokenx, this.tokeny, this.villageName, 
              getResponder(), this.target, this.surfaced, this.democracy, this.motto, this.permanent, this.spawnKingdom, this.initialPerimeter);
            
            logger.log(Level.INFO, getResponder().getName() + " founded " + this.villageName + " for " + toCharge + " irons.");
            Server.getInstance().broadCastSafe(WurmCalendar.getTime(), false);
            Server.getInstance().broadCastSafe("The settlement of " + this.villageName + " has just been founded by " + 
          ...
          {
            getResponder().getCommunicator().sendSafeServerMessage(fe.getMessage());
            return false;
          }
        ...
        else
        {
          ...
          getResponder().getCommunicator().sendAlertServerMessage("You do not have the required " + funds
            .getChangeString() + " available in your bank account.");
          removeSettlementMarkers();
          removePerimeterMarkers();
          return false;
        }
      ...
      else
      {
        ...
        if ((!this.permanent) && (moneyNeeded > 0L))
        {
          ...
          if (rest > 0L)
          {
            if (Servers.localServer.testServer) {
              getResponder().getCommunicator().sendNormalServerMessage("We need " + moneyNeeded + ". " + rest + " must be taken from the bank.");
            }
            if (!((Player)getResponder()).chargeMoney(rest))
            {
              change = false;
            ...
            if (Servers.localServer.testServer) {
              getResponder().getCommunicator().sendNormalServerMessage("We also take " + oldvill
                .getAvailablePlanMoney() + " from upkeep.");
            }
          ...
          {
            if (Servers.localServer.testServer) {
              getResponder().getCommunicator().sendNormalServerMessage("We charge " + moneyNeeded + " from the plan which has " + oldvill.plan.moneyLeft);
            }
        ...
        else if (moneyNeeded < 0L)
        {
          ...
          if (!lsw.addMoney(getResponder().getWurmId(), getResponder().getName(), Math.abs(moneyNeeded), "Resize " + oldvill
            .getName()))
          {
            change = false;
            logger.log(Level.INFO, "Changing values did not yield money for " + oldvill.getName() + " to " + 
              getResponder().getName() + ": " + Math.abs(moneyNeeded) + "?");
            getResponder()
              .getCommunicator()
              .sendAlertServerMessage("You try to change the settlement size, but you could not be reimbursed because we could not reach your bank account. The action was aborted.");
          }
        }
        if (change)
        {
            ...
            (oldvill.getEndY() != this.tokeny + this.selectedSouth))
          {
            ...
            oldvill.setNewBounds(Zones.safeTileX(oldvill.getTokenX() - this.selectedWest), 
          ...
          {
            oldvill.addHistory(getResponder().getName(), "New motto: " + this.motto);
          ...
          oldvill.addHistory(getResponder().getName(), builder.toString());
  ...
  private boolean checkSize()
  {
    ...
    if (getResponder().getPower() < 3) {
      ...
      {
        InfluenceChain chain = InfluenceChain.getInfluenceChain(getResponder().getKingdomId());
        boolean found = false;
        for (Item marker : chain.getChainMarkers()) {
          if (((marker.isChained()) || (getResponder().getKingdomId() == 4)) && 
          
            (marker.isGuardTower())) {
            if (Math.abs(this.tokenx - marker.getTileX()) <= 50) {
              if (Math.abs(this.tokeny - marker.getTileY()) <= 50)
              {
                found = true;
                break;
              }
        ...
        if (!found)
        {
          getResponder().getCommunicator().sendSafeServerMessage("You must found the settlement within 50 tiles of an allied tower linked to the kingdom influence chain.");
      ...
      else
      {
        ...
        for (int x = this.tokenx - 50; x <= this.tokenx + 50; x += 10) {
          for (y = this.tokeny - 50; y <= this.tokeny + 50; y += 10) {
    ...
    Map<Village, String> decliners = Villages.canFoundVillage(this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, this.initialPerimeter, true, oldvill, 
      getResponder());
    if (!decliners.isEmpty())
    {
      ...
      {
        String reason = (String)decliners.get(vill);
        if (reason.startsWith("has perimeter")) {
          getResponder().getCommunicator().sendSafeServerMessage(vill.getName() + " " + reason);
        } else {
          getResponder().getCommunicator().sendSafeServerMessage("Some settlement nearby " + reason);
        }
      }
      return false;
    }
    ...
    if (checkFocusZones)
    {
      ...
      {
        getResponder().getCommunicator().sendSafeServerMessage(focusZoneReject);
        return false;
      }
    }
    return true;
  ...
  public boolean checkDeedItem()
  {
    try
    {
      ...
      if ((!this.deed.isNewDeed()) && (!Servers.localServer.testServer) && 
        (this.deed.getTemplateId() != 862))
      {
  ...
  private boolean checkBlockingCreatures()
  {
    ...
    Object cret = Villages.isAggOnDeed(currVill, getResponder(), this.selectedWest, this.selectedEast, this.selectedNorth, this.selectedSouth, this.tokenx, this.tokeny, true);
    if (cret != null)
    {
  ...
  private boolean checkBlockingKingdoms()
  {
    if (getResponder().getPower() < 2)
    {
      ...
      return Zones.isKingdomBlocking(this.tokenx - this.selectedWest - 5 - this.initialPerimeter - mindist, this.tokeny - this.selectedNorth - 5 - this.initialPerimeter - mindist, this.tokenx + this.selectedEast + 5 + this.initialPerimeter + mindist, this.tokeny + this.selectedSouth + 5 + this.initialPerimeter + mindist, 
      
        getResponder().getKingdomId(), eZoneStartX, eZoneStartY, eZoneEndX, eZoneEndY);
    }
    return true;
  }
  ...
  private boolean checkBlockingItems()
  {
    if (getResponder().getPower() < 2)
    {
      int maxnorth = Math.max(0, this.tokeny - this.selectedNorth - (Servers.localServer
        .isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
      int maxsouth = Math.min(Zones.worldTileSizeY, this.tokeny + this.selectedSouth + (Servers.localServer
        .isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
      
      int maxwest = Math.max(0, this.tokenx - this.selectedWest - (Servers.localServer
        .isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
      int maxeast = Math.min(Zones.worldTileSizeX, this.tokenx + this.selectedEast + (Servers.localServer
        .isChallengeServer() ? 20 : Kingdoms.minKingdomDist));
      int maxcnorth = Math.max(0, this.tokeny - this.selectedNorth - (Servers.localServer
        .isChallengeServer() ? 20 : 60));
      int maxcsouth = Math.min(Zones.worldTileSizeY, this.tokeny + this.selectedSouth + (Servers.localServer
      
        .isChallengeServer() ? 20 : 60));
      
      int maxcwest = Math.max(0, this.tokenx - this.selectedWest - (Servers.localServer
        .isChallengeServer() ? 20 : 60));
      int maxceast = Math.min(Zones.worldTileSizeX, this.tokenx + this.selectedEast + (Servers.localServer
      
        .isChallengeServer() ? 20 : 60));
      
      Rectangle2D rectangleToCheck = new Rectangle2D.Float(maxwest, maxnorth, maxeast - maxwest, maxsouth - maxnorth);
      for (Item targ : Items.getWarTargets()) {
        if ((targ.getTileX() > maxcwest) && (targ.getTileX() < maxceast) && 
          (targ.getTileY() < maxcsouth) && (targ.getTileY() > maxcnorth))
        {
      ...
      if (altar != null) {
        if ((altar.isEpicTargetItem()) && (Servers.localServer.PVPSERVER))
        {
          ...
          {
            getResponder().getCommunicator().sendSafeServerMessage("You cannot found the settlement here, since the " + altar
              .getName() + " is currently required for an active mission.");
            return false;
          }
        }
        else
  ...
  {
    ArrayList<Structure> oldStructures;
    Structure c;
    Object surfStructuresOld;
    boolean ok;
    if (getResponder().getPower() <= 1)
    {
      ArrayList<Structure> newStructures = new ArrayList();
      oldStructures = new ArrayList();
      ...
      if (this.expanding) {
        ...
        {
          Village oldvill = Villages.getVillage(this.deed.getData2());
          surfStructuresOld = Zones.getStructuresInArea(oldvill.getStartX(), oldvill
            .getStartY(), oldvill.getEndX(), oldvill.getEndY(), true);
          Structure[] caveStructuresOld = Zones.getStructuresInArea(oldvill.getStartX(), oldvill
            .getStartY(), oldvill.getEndX(), oldvill.getEndY(), true);
          for (Structure c : (Structure)surfStructuresOld) {
            oldStructures.add(c);
          }
      ...
      {
        ok = false;
        for (surfStructuresOld = newStructures.iterator(); ((Iterator)surfStructuresOld).hasNext();)
        {
          Structure lStructure = (Structure)((Iterator)surfStructuresOld).next();
          ...
          if (!ok) {
            if ((lStructure.isTypeHouse()) && (!lStructure.mayManage(getResponder())))
            {
              getResponder().getCommunicator().sendSafeServerMessage("You need to have manage permissions for the structure " + lStructure
                .getName() + " to found the settlement here.");
  ...
  private void addTileCost(StringBuilder buf)
  {
    ...
    {
      if (!Servers.localServer.isFreeDeeds()) {
        buf.append("text{text=\"The Purchase price for these tiles are " + new Change(this.tiles * Villages.TILE_COST)
          .getChangeString() + ".\"}");
      }
    ...
    else
    {
      try
      {
        ...
        if ((diff > 0) && (!Servers.localServer.isFreeDeeds())) {
          buf.append("text{text=\"The initial cost for the tiles will be " + new Change(diff * Villages.TILE_COST)
            .getChangeString() + ".\"}");
        }
        if (Servers.localServer.isUpkeep()) {
          buf.append("text{text=\"The new monthly upkeep cost for the tiles will be " + new Change(this.tiles * Villages.TILE_UPKEEP)
            .getChangeString() + ".\"}");
        }
        buf.append("text{text=\"\"}");
      }
  ...
  private void addPerimeterCost(StringBuilder buf)
  {
    if (!this.expanding)
    {
      buf.append("text{text=\"You have selected a perimeter of 5" + (Servers.localServer
        .isFreeDeeds() ? "" : " free") + " tiles plus " + this.initialPerimeter + " additional tiles from your settlement boundary.\"}");
      if (this.initialPerimeter > 0)
      {
        if (!Servers.localServer.isFreeDeeds()) {
          buf.append("text{text=\"The initial cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_COST)
            .getChangeString() + ".\"}");
        }
        if (Servers.localServer.isUpkeep()) {
          buf.append("text{text=\"The monthly upkeep cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_UPKEEP)
            .getChangeString() + ".\"}");
        }
      }
      buf.append("text{text=\"\"}");
    ...
    else
    {
      try
      {
        ...
        if ((diff > 0) && (!Servers.localServer.isFreeDeeds())) {
          buf.append("text{text=\"The additional cost for the extra perimeter tiles will be " + new Change(diff * Villages.PERIMETER_COST)
            .getChangeString() + ".\"}");
        }
        if (Servers.localServer.isUpkeep()) {
          if (this.initialPerimeter > 0) {
            buf.append("text{text=\"The new monthly upkeep cost for the perimeter tiles will be " + new Change(this.perimeterTiles * Villages.PERIMETER_UPKEEP)
              .getChangeString() + ".\"}");
          } else if (diff < 0) {
    ...
    if (!this.expanding)
    {
      if (this.selectedGuards > 0)
      {
        if (Servers.localServer.isFreeDeeds()) {
          ...
          buf.append("text{text=\"You will hire " + this.selectedGuards + " guards for a cost of " + new Change(this.selectedGuards * Villages.GUARD_COST)
            .getChangeString() + ".\"}");
        }
        if (Servers.localServer.isUpkeep()) {
          buf.append("text{text=\"The guard upkeep will be " + new Change(
            GuardPlan.getCostForGuards(this.selectedGuards)).getChangeString() + ".\"}");
        }
      }
      else
      {
      ...
      try
      {
        ...
        if (diff > 0)
        {
          if (Servers.localServer.isFreeDeeds()) {
            ...
            buf.append("text{text=\"You will hire " + diff + " new guards for a cost of " + new Change(diff * Villages.GUARD_COST)
              .getChangeString() + ".\"}");
          }
          if (Servers.localServer.isUpkeep()) {
            buf.append("text{text=\"The new guard upkeep will be " + new Change(
              GuardPlan.getCostForGuards(this.selectedGuards)).getChangeString() + ".\"}");
          }
        }
        else
  ...
  public void sendIntro()
  {
    ...
    getResponder().getCommunicator().sendBml(600, 500, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  public void sendQuestion()
  {
    ...
    getResponder().getCommunicator().sendBml(600, 600, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  public void sendQuestion2()
  {
    ...
    buf.append("text{text=\"\"}");
    buf.append("text{text=\"Please enter the number of tiles BEYOND the 5" + (Servers.localServer
    
      .isFreeDeeds() ? "" : " initial tiles that you get for free") + ". You can simply leave the number at zero if you are happy with the " + 5 + (Servers.localServer
      
      .isFreeDeeds() ? "" : " free") + " tiles or extend the perimeter at a later date.\"}");
    
    buf.append("text{text=\"\"}");
    
    buf.append("harray{label{text=\"Perimeter Size: 5" + (Servers.localServer
    
      .isFreeDeeds() ? "" : " free") + " tiles plus: \"}");
  ...
  {
    StringBuilder buf = new StringBuilder(getBmlHeader());
    buf.append("text{type=\"italic\";text=\"Your deed size will be " + this.diameterX + " by " + this.diameterY + " and the perimeter will extend beyond the border by " + (5 + this.initialPerimeter) + " tiles (including the required " + 5 + (Servers.localServer
    
      .isFreeDeeds() ? "" : " free") + " tiles)\"}");
    buf.append("text{text=\"\"}");
    if ((this.error != null) && (this.error.length() > 0))
    {
      buf.append("text{color=\"255,0,0\";type=\"bold\";text=\"" + this.error + "\"}");
    ...
    if (this.expanding) {
      try
      {
        ...
        if (village.mayChangeName())
        {
          ...
          buf.append("text{type=\"bold\";color=\"255,50,0\";text=\"NOTE: Changing name will" + (Servers.localServer
            .isFreeDeeds() ? "" : " cost 5 silver,") + " remove all faith bonuses, and lock the name for 6 months.\"}");
        }
        else
        {
  ...
  public void sendQuestion4()
  {
    ...
    addTileCost(buf);
    addPerimeterCost(buf);
    addChangeNameCost(buf);
    buf.append("text{text=\"\"}");
    ...
    {
      buf.append("text{text=\"The only guard type is heavy guards. There is no cost for hiring guards here" + (Servers.localServer
        .isUpkeep() ? ", but they have an upkeep of " + Villages.GUARD_UPKEEP_STRING + " per month" : ".") + "\"};");
  ...
  public void sendQuestion5()
  {
    ...
    if (!hasError)
    {
      ...
      if (this.expanding)
      {
        long resCost = getResizeCostDiff();
        if (resCost > 0L)
        {
    ...
    getResponder().getCommunicator().sendBml(600, 700, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  public void sendQuestion6()
  {
    ...
    getResponder().getCommunicator().sendBml(600, 430, true, false, buf.toString(), 200, 200, 200, this.title);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\VillageFoundationQuestion.class
