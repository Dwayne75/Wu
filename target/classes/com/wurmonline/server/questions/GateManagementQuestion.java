  ...
  public void answer(Properties answers)
  {
    ...
    if ((val != null) && (val.length() > 0)) {
      for (int x = 0; x < 10; x++) {
        if (val.equals("g" + x))
        {
          GateManagementQuestion vs = new GateManagementQuestion(getResponder(), "Manage gates", "Managing gates.", this.target);
          if (vs != null)
          {
            vs.confirmed = true;
    ...
    try
    {
      ...
      if (gates != null)
      {
        ...
        if ((this.confirmed) || (sz < 100))
        {
          ...
          catch (NoSuchRoleException localNoSuchRoleException) {}
          FenceGate[] gs = (FenceGate[])gates.toArray(new FenceGate[gates.size()]);
          int max = gs.length;
          int start = 0;
          ...
          for (int g = start; g < max; g++)
          {
            ...
            buf.append("label{text=\"Gate\"}input{id=\"gate" + gateId + "\";maxchars=\"40\";text=\"" + gs[g]
              .getName() + "\"}label{text=\"Open to " + "everybody" + " from:\"}input{id=\"open" + gateId + "\";maxchars=\"2\";text=\"" + gs[g]
              
              .getOpenTime() + "\"}label{text=\"(0-23) to:\"}input{id=\"close" + gateId + "\";maxchars=\"2\"; text=\"" + gs[g]
              
              .getCloseTime() + "\"}label{text=\"(0-23)\"}");
          }
          buf.append("}");
        }
        else
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\GateManagementQuestion.class
