  ...
  implements CounterTypes
{
  ...
  public void sendQuestion()
  {
    ...
    if (getResponder().getPower() >= 5)
    {
      ...
      buf.append(getServerEntryData("Current", Servers.localServer));
      if (Servers.localServer.serverNorth != null) {
        buf.append(getServerEntryData("NORTH", Servers.localServer.serverNorth));
      }
      if (Servers.localServer.serverEast != null) {
        buf.append(getServerEntryData("EAST", Servers.localServer.serverEast));
      }
      if (Servers.localServer.serverSouth != null) {
        buf.append(getServerEntryData("SOUTH", Servers.localServer.serverSouth));
      }
      if (Servers.localServer.serverWest != null) {
        buf.append(getServerEntryData("WEST", Servers.localServer.serverWest));
      }
      if (Servers.loginServer != null) {
        buf.append(getServerEntryData("LOGIN", Servers.loginServer));
      }
      buf.append("}");
  ...
  {
    StringBuilder buf = new StringBuilder();
    buf.append("label{text=\"" + serverType + "\"}label{text=\"" + server.name + "\"}label{text=\"" + server.PVPSERVER + "\"}label{text=\"" + server.EPIC + "\"}label{text=\"" + server.HOMESERVER + "\"}label{text=\"" + server
  ...
  {
    return (ServerEntry)this.serverEntries.get(aPosition);
  }
  
  ServerEntry getTransferEntry(int aPosition)
  {
    return (ServerEntry)this.transferEntries.get(aPosition);
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ServerQuestion.class
