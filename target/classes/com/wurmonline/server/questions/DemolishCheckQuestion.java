  ...
  extends Question
{
      ...
      if (val.equals("true"))
      {
        if (getResponder().getPower() >= 2) {
          getResponder().getLogger().log(Level.INFO, 
            ...
            getResponder().getName() + " destroyed structure " + this.building.getName() + " at " + this.building
            .getCenterX() + ", " + this.building.getCenterY());
        }
        if (this.building.isOnSurface()) {
          Zones.flash(this.building.getCenterX(), this.building.getCenterY(), false);
        }
        if ((!this.building.hasWalls()) || (getResponder().getPower() >= 2) || 
          (this.building.isOwner(getResponder()))) {
  ...
  public void sendQuestion()
  {
    ...
    getResponder().getCommunicator().sendBml(200, 200, true, true, buf.toString(), 200, 200, 200, this.title);
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\DemolishCheckQuestion.class
