  ...
  public void sendQuestion()
  {
    ...
    try
    {
      ...
      if (oldVill != -1)
      {
        try
        {
          getResponder().getCommunicator().sendSafeServerMessage("This is the deed for " + 
            Villages.getVillage(oldVill).getName() + "! You cannot use it to expand a settlement!");
        }
      ...
      if (village != null)
      {
        ...
        buf.append("text{text='The expansion will set the size of the settlement to " + size + " tiles out in all directions from the " + this.token
          .getName() + ".'}");
        
        buf.append("text{text='You will require all the house deeds for any houses in the new area.'}");
        buf
          .append("text{text='Also note that in the case that the allowed number of citizens is decreased any surplus will be kicked from the settlement automatically and in no particular order so you may want to do that manually instead.'}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\VillageExpansionQuestion.class
