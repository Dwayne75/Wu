    ...
    if (getResponder().isKing())
    {
      ...
      if (k != null)
      {
        ...
        if (decline)
        {
          ...
          {
            getResponder().getCommunicator().sendAlertServerMessage("The people of " + 
              Kingdoms.getNameFor(k.kingdom) + " may now vote you from the throne at the duelling ring.");
            ...
            Server.getInstance().broadCastNormal(
              getResponder().getName() + " may now be voted away from the throne within one week at the duelling stone.");
        ...
        if (!accept)
        {
          ...
          getResponder().getCommunicator().sendNormalServerMessage("Unless you answer this challenge within " + tl + " you will automatically have declined " + (k
          
            .getDeclinedChallengesNumber() + 1) + " challenges.");
          ...
          try
          {
            ...
            if ((valhour != null) && (valhour.length() > 0)) {
              try
              {
                ...
                {
                  getResponder().getCommunicator().sendSafeServerMessage("You have accepted the challenge and must be at the duelling ring exactly in " + day + " days and " + hour + " hours (which is " + (day * 24 + hour) + " hours away). You must stay there a bit more than half an hour until you receive a message.");
                }
                k.setChallengeAccepted(time);
                
                Message mess = new Message(getResponder(), (byte)10, Kingdoms.getChatNameFor(getResponder().getKingdomId()), "<" + getResponder().getName() + "> has accepted the challenge and must be at the duelling ring exactly in " + day + " days and " + hour + " hours");
                
                Player[] playarr = Players.getInstance().getPlayers();
                
                byte windowKingdom = getResponder().getKingdomId();
  ...
  public void sendQuestion()
  {
    ...
    if (getResponder().isKing())
    {
      ...
      if (k != null)
      {
        ...
        buf.append("text{text='In case you leave the proximity or die within the proximity during this time you may be removed from the throne of " + 
        ...
        else if (k.getChallengeDate() < System.currentTimeMillis())
        {
          long timeLeft = 604800000L + k.getChallengeDate() - System.currentTimeMillis();
          String tl = Server.getTimeFor(timeLeft);
          buf.append("text{text=\"Unless you answer this challenge within " + tl + " you will have declined " + (k
            .getDeclinedChallengesNumber() + 1) + " challenges.\"}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\RoyalChallenge.class
