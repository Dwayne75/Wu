  ...
  implements TimeConstants
{
  ...
  public void answer(Properties answers)
  {
    if (getResponder().isKing())
    {
      ...
      if (cap != null)
      {
        ...
        for (int x = 0; x < vills.length; x++) {
          if (vills[x].getName().equalsIgnoreCase(cap)) {
            if (vills[x].kingdom == getResponder().getKingdomId())
            {
              King k = King.getKing(getResponder().getKingdomId());
              if ((!cap.equalsIgnoreCase(k.capital)) && 
                (!k.setCapital(cap, false))) {
      ...
      if ((val != null) && (val.length() > 0)) {
        if (!val.equals(kingd.getFirstMotto())) {
      ...
      if ((val != null) && (val.length() > 0)) {
        if (!val.equals(kingd.getSecondMotto())) {
          if (!QuestionParser.containsIllegalVillageCharacters(val))
          {
      ...
      if ((kingd.isCustomKingdom()) && (getResponder().isKing()))
      {
        String ostra = answers.getProperty("expel");
        if ((ostra != null) && (ostra.length() > 0)) {
          kingd.expelMember(getResponder(), ostra);
        }
        for (Village v : this.villages)
        {
          ...
          if ((val != null) && (val.equals("true")))
          {
            ...
            for (Village v2 : Villages.getVillages()) {
              if (v2.kingdom == getResponder().getKingdomId()) {
      ...
      if (mayAlly(getResponder().getKingdomId())) {
        checkAlliances(answers, kingd);
      }
      if (changed)
      {
        kingd.update();
        WcKingdomInfo wck = new WcKingdomInfo(WurmId.getNextWCCommandId(), true, getResponder().getKingdomId());
        wck.encode();
  ...
  public void checkAlliances(Properties answers, Kingdom kingd)
  {
    ...
    for (Kingdom kingdz : kingdoms) {
      if (kingdz.existsHere())
      {
        ...
        if ((val != null) && (val.equals("true"))) {
          if (kingdz.isAllied(kingd.getId()))
          {
            kingdz.setAlliance(kingd.getId(), (byte)0);
            kingd.setAlliance(kingdz.getId(), (byte)0);
            getResponder().getCommunicator().sendNormalServerMessage("You break the alliance with " + kingdz
              .getName() + ".");
            King other = King.getKing(kingdz.getId());
            try
            {
              Player otherKing = Players.getInstance().getPlayer(other.kingid);
              otherKing.getCommunicator().sendNormalServerMessage("Your alliance with " + kingd
                .getName() + " has ended.");
            }
        ...
        if ((val != null) && (val.equals("true"))) {
          if (kingdz.hasSentRequestingAlliance(kingd.getId()))
          {
            kingdz.setAlliance(kingd.getId(), (byte)1);
            kingd.setAlliance(kingdz.getId(), (byte)1);
            getResponder().getCommunicator().sendNormalServerMessage("You accept the request for alliance with " + kingdz
              .getName() + ".");
            King other = King.getKing(kingdz.getId());
            try
            {
              Player otherKing = Players.getInstance().getPlayer(other.kingid);
              otherKing.getCommunicator().sendNormalServerMessage("You are now allied to " + kingd
                .getName() + "!");
            }
        ...
        if ((val != null) && (val.equals("true"))) {
          if (kingd.hasSentRequestingAlliance(kingdz.getId()))
          {
            kingdz.setAlliance(kingd.getId(), (byte)0);
            kingd.setAlliance(kingdz.getId(), (byte)1);
            getResponder().getCommunicator().sendNormalServerMessage("You revoke your request for alliance with " + kingdz
              .getName() + ".");
            King other = King.getKing(kingdz.getId());
            try
            {
              Player otherKing = Players.getInstance().getPlayer(other.kingid);
              otherKing.getCommunicator().sendNormalServerMessage(kingd
                .getName() + " has withdrawn their request for an alliance.");
            }
    ...
    if ((val != null) && (val.length() > 0)) {
      try
      {
        ...
        if (index > 0)
        {
          Kingdom kingdz = (Kingdom)this.askedAllies.get(index - 1);
          if ((!kingd.hasSentRequestingAlliance(kingdz.getId())) && 
            (!kingdz.hasSentRequestingAlliance(kingd.getId())))
          {
            getResponder().getCommunicator().sendNormalServerMessage("You invite " + kingdz
              .getName() + " to join you in an alliance.");
            King other = King.getKing(kingdz.getId());
            if (other != null) {
              try
              {
                Player otherKing = Players.getInstance().getPlayer(other.kingid);
                otherKing.getCommunicator().sendNormalServerMessage(kingd
                  .getName() + " invites you to enter a mutual alliance.");
              }
              catch (NoSuchPlayerException localNoSuchPlayerException3) {}
            } else {
              getResponder().getCommunicator().sendNormalServerMessage("Seems " + kingdz
                .getName() + " has no ruler to ally with.");
            }
  ...
  public void append(byte kingdom, StringBuilder buf)
  {
    ...
    buf.append("text{text=\"" + Kingdoms.getNameFor(kingdom) + " '" + motone + " " + mottwo + "' (" + twoDecimals
      .format(Zones.getPercentLandForKingdom(kingdom)) + "% land)\"}");
    if (k != null)
    {
      buf.append("text{text=\" under the rule of " + k.getRulerTitle() + " " + k.kingName + ":\"}");
      float sperc = k.getLandSuccessPercent();
      if ((kingd != null) && (k.kingid == getResponder().getWurmId()))
      {
        ...
        if (kingd.isCustomKingdom())
        {
          ...
          buf.append("checkbox{id='allowPortal';text='Allow people to join the kingdom via portals?';selected=\"" + kingd
            .acceptsTransfers() + "\"}");
          for (Village v : Villages.getVillages()) {
            if ((v.kingdom == k.kingdom) && 
              (!v.isCapital())) {
              this.villages.add(v);
            }
          }
          ...
          if (this.villages.size() > 0) {
            for (??? = this.villages.iterator(); ((Iterator)???).hasNext();)
            {
              Village v = (Village)((Iterator)???).next();
              if (v.pmkKickDate > 0L)
              {
                buf.append("text{text=\"\"}");
                buf.append("checkbox{id=\"revRej" + v.getId() + "\";text=\"" + v.getName() + " is being kicked out in " + 
                
                  Server.getTimeFor(v.pmkKickDate - System.currentTimeMillis()) + ". Remove kicking? \";selected=\"false\"};");
              }
            }
          }
        }
        ...
        if (sperc == 100.0F) {
          buf.append("text{text=\"Your kingdom share lies steady at " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land.\"}");
        } else if (sperc > 100.0F) {
          buf.append("text{text=\"You have increased your kingdom share from " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(k.startLand) }) + " to " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land, which is an increase by " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(sperc - 100.0F) }) + ".\"}");
        } else {
          buf.append("text{text=\"Your share of the available lands has decreased from " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(k.startLand) }) + " to " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land, so you have lost " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(100.0F - sperc) }) + ".\"}");
        }
        if (Servers.localServer.isChallengeOrEpicServer()) {
          if (getResponder().hasCustomKingdom()) {
        ...
        buf.append("text{text=\"Your subjects have slain " + k.levelskilled + " appointment levels of enemy nobles, and your enemies have slain " + k.levelslost + " levels of your nobles, which equals " + 
        
          String.format("%d%%", new Object[] {Integer.valueOf((int)k.getAppointedSuccessPercent()) }) + " success.\"}");
        ...
        if (getResponder().getPower() >= 3) {
          buf.append("text{text=\" Challenges: " + k.getChallengeSize() + " Declined: " + k
            .getDeclinedChallengesNumber() + " Votes: " + k.getVotes() + ".\"}");
        }
        buf.append("text{text=\"\"}");
        if (mayAlly(kingdom))
        {
          ...
          for (Kingdom kingdz : kingdoms) {
            if ((kingdz.existsHere()) && (mayAlly(kingdz.getId()))) {
              if (kingdz.isAllied(kingdom)) {
                ...
                buf.append("checkbox{id=\"rem" + kingdz.getId() + "\";text=\" You are asking " + kingdz
                  .getName() + " to become an ally. Revoke? \";selected=\"false\"};");
              } else {
                allAllies = false;
              }
            }
          ...
          if (!allAllies) {
            for (Kingdom kingdz : kingdoms) {
              if ((kingdz.existsHere()) && (kingdz.getId() != 0)) {
                if ((!kingdz.isAllied(kingdom)) && (kingdz.getId() != kingdom) && (mayAlly(kingdz.getId())))
                {
      ...
      else
      {
        ...
        if (sperc == 100.0F) {
          buf.append("text{text=\"Kingdom share lies steady at " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land.\"}");
        } else if (sperc > 100.0F) {
          buf.append("text{text=\"" + k.kingName + " has increased the kingdom share from " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(k.startLand) }) + " to " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land, which is an increase by " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(sperc - 100.0F) }) + ".\"}");
        } else {
          buf.append("text{text=\"During the reign of " + k.kingName + ", the share of the available lands has decreased from " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(k.startLand) }) + " to " + String.format("%.2f%%", new Object[] { Float.valueOf(k.currentLand) }) + " of the available land, so the kingdom has lost " + 
            String.format("%.2f%%", new Object[] {Float.valueOf(100.0F - sperc) }) + ".\"}");
        }
        buf.append("text{text=\"" + k.kingName + " has appointed " + k.appointed + " levels of titles, orders and offices.\"}");
        
        buf.append("text{text=\"The subjects of " + Kingdoms.getNameFor(kingdom) + " have slain " + k.levelskilled + " appointment levels of enemy nobles. Their enemies have slain " + k.levelslost + " levels of " + 
        
          Kingdoms.getNameFor(kingdom) + " nobles, which equals " + String.format("%d%%", new Object[] { Integer.valueOf((int)k.getAppointedSuccessPercent()) }) + " success.\"}");
        buf.append("text{text=\"This ranks " + k.kingName + " as " + k.getFullTitle() + ".\"}");
      }
      buf.append("text{text=\"\"}");
      
      addApps(buf, kingdom, false);
    }
    else if (kingdom != 0)
    {
  ...
  private void addApps(StringBuilder buf, byte kingdom, boolean isResponder)
  {
    ...
    if (apps != null)
    {
      if (isResponder)
      {
        String titles = apps.getOffices(getResponder().getWurmId(), getResponder().getSex() == 0);
        if (titles.length() > 0)
        {
          buf.append("text{text=\"");
          ...
          buf.append(titles);
          buf.append(" of ");
          buf.append(Kingdoms.getNameFor(getResponder().getKingdomId()));
          buf.append(".");
          buf.append("\"}");
          buf.append("text{text=\"\"}");
        }
        titles = apps.getTitles(getResponder().getAppointments(), getResponder().getSex() == 0);
        if (titles.length() > 0)
        {
          buf.append("text{text=\"");
          buf.append(" You are ");
        ...
        titles = apps.getOrders(getResponder().getAppointments(), getResponder().getSex() == 0);
        if (titles.length() > 0)
        {
          buf.append("text{text=\"");
      ...
      else
      {
        ...
        Appointments rap = King.getCurrentAppointments(getResponder().getKingdomId());
        if (rap != null)
        {
          long secretP = rap.getOfficialForId(1500);
          if (secretP == getResponder().getWurmId()) {
            isSecretPolice = true;
          }
  ...
  public void sendQuestion()
  {
    ...
    buf.append("text{text=\"\"}");
    addApps(buf, getResponder().getKingdomId(), true);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\KingdomStatusQuestion.class
