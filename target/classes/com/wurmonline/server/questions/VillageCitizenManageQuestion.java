  ...
  public void sendQuestion()
  {
    try
    {
      ...
      for (int x = 0; x < citizens.length; x++) {
        ...
        {
          this.idMap.put(Integer.valueOf(x), new Long(citizens[x].getId()));
          ...
          if ((role.getStatus() != 4) && (role.getStatus() != 5) && (role.getStatus() != 6))
          {
            ...
            for (int r = 0; r < roles.length; r++) {
              if ((roles[r].getStatus() != 4) && (roles[r].getStatus() != 5) && 
                (roles[r].getStatus() != 1) && 
                (roles[r].getStatus() != 6))
              {
            ...
            if ((!village.isDemocracy()) && (citizens[x].getId() != getResponder().getWurmId()) && 
              (citizens[x].getRole().getStatus() != 2)) {
          ...
          else if (citizens[x].getRole().getStatus() == 6)
          {
            ...
            buf.append("label{text=\"" + citizens[x].getName() + "\"}label{text=\"role:\"};label{text=\"" + citizens[x]
            
              .getRole().getName() + "\"};label{text=\"" + wagoner
              .getStateName() + "\"};label{text=\"\"};label{text=\"\"};label{text=\"\"}");
          }
        }
      }
      buf.append("}text{text=\"\"}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\VillageCitizenManageQuestion.class
