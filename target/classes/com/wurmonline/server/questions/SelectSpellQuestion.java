  ...
  implements TimeConstants
{
  ...
  private String addSpell(int id, Spell spell)
  {
    ...
    buf.append("row{id=\"e" + id + "\";hover=\"" + 
      StringUtilities.raiseFirstLetterOnly(spell.getDescription()) + "\";name=\"" + spell
      .getName() + "\";rarity=\"0\";children=\"0\";col{text=\"" + spell.level + "\"};col{text=\"" + spell
      
      .getCost() + "\"};col{text=\"" + spell
      .getDifficulty(false) + "\"};col{text=\"" + 
      getTargets(spell) + "\"};col{text=\"" + 
  ...
  public void sendQuestion()
  {
    ...
    if (getResponder().getDeity() != null)
    {
      ...
      int level = (int)getResponder().getFaith();
      Set<Spell> spellset = getResponder().getDeity().getSpells();
      Spell[] spells = (Spell[])spellset.toArray(new Spell[spellset.size()]);
      
      Arrays.sort(spells, new SelectSpellQuestion.1(this));
      
      int available = 0;
      Spell s;
      for (s : spells) {
        if ((s.level <= level) && (
          (!s.isRitual) || (deity.getFavor() > 1000) || (Servers.isThisATestServer()))) {
          available++;
        }
      }
      ...
      {
        deityConnection = "You have a " + getDeityConnection(level) + " connection to " + deity.getName() + ".";
      }
      else
      {
        Deity templateGod = Deities.getDeity(deity.getTemplateDeity());
        deityConnection = "You have a " + getDeityConnection(level) + " connection to " + deity.getName() + ", demigod of " + templateGod.getName() + ".";
      }
      buf.append("text{text=\"" + deityConnection + "\";type=\"bold\"}");
      buf.append("text{text=\"\"}");
      buf.append(getDeityPassives(deity, level));
      buf.append("text{text=\"\"}");
      ...
      for (Spell s : spells) {
        if (s.level <= level) {
          if ((!s.isRitual) || (deity.getFavor() > 1000) || (Servers.isThisATestServer()))
          {
            rowNumb++;
            buf.append(addSpell(rowNumb, s));
          }
        }
      }
      buf.append("}");
    ...
    getResponder().getCommunicator().sendBml(700, 800, true, true, buf.toString(), 200, 200, 200, this.title);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\SelectSpellQuestion.class
