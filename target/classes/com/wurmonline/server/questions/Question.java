  ...
  implements MiscConstants, QuestionTypes
{
  ...
  public static String itemNameWithColorByRarity(Item litem)
  {
    ...
    if (litem.isBulkItem())
    {
      ...
      {
        logger.log(Level.WARNING, litem.getWurmId() + " bulk nums=" + nums + " but template is " + litem
          .getBulkTemplateId());
      }
    }
    else
  ...
  String colHeader(String aName, int numb, int sortBy)
  {
    ...
    buf.append("button{text=\"" + aName + (sortBy == -numb ? " \\/" : sortBy == numb ? " /\\" : "") + "\";id=\"sort" + (sortBy == numb ? "-" + numb : new StringBuilder().append("").append(numb).toString()) + "\"};");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\Question.class
