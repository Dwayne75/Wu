  ...
  public void sendQuestion()
  {
    try
    {
      ...
      else
      {
          ...
          for (int x = 0; x < reputations.length; x++) {
            if (ids < 100)
            {
              ...
              try
              {
                ...
                this.itemMap.put(new Long(wid), Integer.valueOf(ids));
              }
        ...
        getResponder().getCommunicator().sendBml(500, szy, true, true, buf.toString(), 200, 200, 200, this.title);
      }
    }
    catch (NoSuchItemException nsi)
    {
      getResponder().getCommunicator().sendNormalServerMessage("No such item.");
      logger.log(Level.WARNING, getResponder().getName(), nsi);
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ReputationQuestion.class
