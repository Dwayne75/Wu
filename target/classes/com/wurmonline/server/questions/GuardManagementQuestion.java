  ...
  implements TimeConstants, MonetaryConstants
{
  ...
  public void sendQuestion()
  {
    ...
    if (getResponder().getCitizenVillage() != null)
    {
      ...
      {
        GuardPlan plan = getResponder().getCitizenVillage().plan;
        if (getResponder().getCitizenVillage().isCitizen(getResponder()))
        {
          buf.append("text{text=\"The size of " + getResponder().getCitizenVillage().getName() + " is " + 
            getResponder().getCitizenVillage().getDiameterX() + " by " + 
            getResponder().getCitizenVillage().getDiameterY() + ".\"}");
          buf.append("text{text=\"The perimeter is " + (5 + 
            getResponder().getCitizenVillage().getPerimeterSize()) + " and it has " + plan
        ...
        else
        {
          ...
          {
            buf.append("text{text=\"A drain would cost " + 
              Economy.getEconomy().getChangeFor(plan.getMoneyDrained()).getChangeString() + ".\"};");
        ...
        buf.append("text{text=\"How many guards do you wish to have? You currently have " + plan.getNumHiredGuards() + " and may hire up to " + 
          GuardPlan.getMaxGuards(getResponder().getCitizenVillage()) + ".\"};input{text=\"" + plan
          .getNumHiredGuards() + "\";id=\"hired\"}; ");
        buf.append("text{text=\"\"};");
      }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\GuardManagementQuestion.class
