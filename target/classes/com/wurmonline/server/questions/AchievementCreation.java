  ...
  extends Question
{
  ...
  public void answer(Properties answers)
  {
      ...
      try
      {
        if ((edita != null) && (edita.length() > 0)) {
          if (this.myAchievements != null)
          {
            ...
            {
              this.toWorkOn = ((AchievementTemplate)this.myAchievements.get(index));
              
              AchievementCreation m = new AchievementCreation(getResponder(), "Edit Achievement", "Achievement management", getTarget());
    ...
    if (this.toWorkOn == null)
    {
      ...
      {
        getResponder().getCommunicator().sendAlertServerMessage("The name " + name + " needs at least 2 characters.");
        
        return;
      }
      if ((description == null) || (description.length() < 10))
      {
        getResponder().getCommunicator().sendAlertServerMessage("The description " + description + " needs at least 10 characters.");
        
        return;
      }
      String creator = getResponder().getName();
      if (getResponder().getPower() > 0) {
        creator = "GM " + creator;
      } else {
        try
        {
          Item ruler = Items.getItem(getTarget());
          if (ruler.getAuxData() < 10)
          {
            getResponder().getCommunicator().sendAlertServerMessage("The " + ruler
            
              .getName() + " needs at least 10 charges in order to create an Achievement. It currently contains " + ruler
      ...
      getResponder()
        .getCommunicator()
        .sendSafeServerMessage("You successfully create the Achievement " + this.toWorkOn.getName() + ".");
      return;
    }
    ...
    if ((delete != null) && (delete.equals("true"))) {
      if (changing) {
        if (this.toWorkOn != null)
        {
          ...
          getResponder()
            .getCommunicator()
            .sendSafeServerMessage("You successfully delete the Achievement " + this.toWorkOn.getName() + ".");
          return;
        }
      }
    }
    ...
    if ((name != null) && (name.length() > 0) && 
      (!name.equals(this.toWorkOn.getName())))
    {
      changing = true;
      this.toWorkOn.setName(name);
    }
    if ((description != null) && (description.length() > 0) && 
      (!description.equals(this.toWorkOn.getDescription())))
    {
      changing = true;
    ...
    if (changing) {
      getResponder().getCommunicator().sendSafeServerMessage("You successfully update the Achievement " + this.toWorkOn.getName() + ".");
    } else {
      getResponder().getCommunicator().sendSafeServerMessage("You change nothing on the Achievement " + this.toWorkOn.getName() + ".");
    }
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\AchievementCreation.class
