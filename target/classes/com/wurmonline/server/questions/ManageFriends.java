    ...
    if (this.type == 118)
    {
      for (int i = 0; i < this.friends.length; i++)
      {
        ...
        if ((remove != null) && (remove.equalsIgnoreCase("true")))
        {
          ...
          ManageFriends mf = new ManageFriends(getResponder());
          mf.sendQuestion();
          return;
        }
      }
      ...
      if ((update != null) && (update.equalsIgnoreCase("true")))
      {
        ...
        for (int i = 0; i < this.friends.length; i++)
        {
          ...
          if (cat != null)
          {
            ...
            {
              ((Player)getResponder()).updateFriendData(this.friends[i].getFriendId(), catId, note);
              if (this.friends[i].getCatId() != catId) {
                getResponder().getCommunicator().sendNormalServerMessage(this.friends[i].getName() + " is now in your category " + 
                
                  Friend.Category.catFromInt(catId).name() + ".");
              }
              if (!this.friends[i].getNote().equals(note)) {
                getResponder().getCommunicator().sendNormalServerMessage("You added a note for " + this.friends[i]
                  .getName() + ".");
              }
              didChange = true;
            }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ManageFriends.class
