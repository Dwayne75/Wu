  ...
  implements ItemMaterials
{
  ...
  public void sendQuestion()
  {
    ...
    try
    {
      ...
      if (it.usesFoodState())
      {
        ...
        buf.append(addRaux(0, raux, "(Raw)"));
        buf.append(addRaux(1, raux, "Fried"));
        buf.append(addRaux(2, raux, "Grilled"));
        buf.append(addRaux(3, raux, "Boiled"));
        buf.append(addRaux(4, raux, "Roasted"));
        buf.append(addRaux(5, raux, "Steamed"));
        buf.append(addRaux(6, raux, "Baked"));
        buf.append(addRaux(7, raux, "Cooked"));
        buf.append(addRaux(8, raux, "Candied"));
        buf.append(addRaux(9, raux, "Choc Coated"));
        buf.append("label{text=\"\"};label{text=\"\"}");
        buf.append("label{text=\"\"};label{text=\"\"}");
        if ((it.isHerb()) || (it.isVegetable()) || (it.isFish()) || (it.isMushroom())) {
          buf.append(addLaux("chopped", it.isChopped(), "Chopped"));
        } else if (it.isMeat()) {
          buf.append(addLaux("chopped", it.isDiced(), "Diced"));
        } else if (it.isSpice()) {
          buf.append(addLaux("chopped", it.isGround(), "Ground"));
        } else if (it.canBeFermented()) {
          buf.append(addLaux("chopped", it.isUnfermented(), "Unfermented"));
        } else if (it.getTemplateId() == 1249) {
          buf.append(addLaux("chopped", it.isWhipped(), "Whipped"));
        } else {
          buf.append(addLaux("chopped", it.isZombiefied(), "Zombiefied"));
        }
        if (it.isVegetable()) {
          buf.append(addLaux("mashed", it.isMashed(), "Mashed"));
        } else if (it.isMeat()) {
          buf.append(addLaux("mashed", it.isMinced(), "Minced"));
        } else if (it.canBeFermented()) {
          buf.append(addLaux("mashed", it.isFermenting(), "Fermenting"));
        } else if (it.isFish()) {
          buf.append(addLaux("mashed", it.isUnderWeight(), "Underweight"));
        } else {
          buf.append(addLaux("mashed", it.isClotted(), "Clotted"));
        }
        if (it.canBeDistilled()) {
          buf.append(addLaux("wrap", it.isUndistilled(), "Undistilled"));
        } else {
          buf.append(addLaux("wrap", it.isWrapped(), "Wrapped"));
        }
        if ((it.isHerb()) || (it.isSpice())) {
          buf.append(addLaux("fresh", it.isFresh(), "Fresh"));
        } else if ((it.isDish()) || (it.isLiquid())) {
          buf.append(addLaux("fresh", it.isSalted(), "Salted"));
        } else if (it.isFish()) {
          buf.append(addLaux("fresh", it.isLive(), "live"));
        } else {
          buf.append(addLaux("fresh", false, "n/a"));
        }
        buf.append("}");
      ...
      if (it.getTemplate().usesRealTemplate())
      {
        ...
        if (it.getTemplate().isRune()) {
          for (int x = 0; x < templates.length; x++) {
            if ((templates[x].getTemplateId() == 1102) || (templates[x].getTemplateId() == 1104) || 
              (templates[x].getTemplateId() == 1103))
            {
    ...
    getResponder().getCommunicator().sendBml(320, height, true, true, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  {
    return (ItemTemplate)this.itemplates.get(fruitId);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\ItemDataQuestion.class
