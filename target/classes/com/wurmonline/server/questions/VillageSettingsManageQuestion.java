  ...
  implements VillageStatus, TimeConstants
{
  ...
  public void sendQuestion()
  {
    try
    {
      ...
      if ((!village.isDisbanding()) && 
        (village.mayChangeName()) && 
        (village.getRoleFor(getResponder()).mayResizeSettlement()))
      {
        buf.append("header{text=\"Settlement Name:\"};input{maxchars=\"40\";id=\"vname\";text=\"" + village
          .getName() + "\"}");
        buf.append("text{type=\"bold\";color=\"255,50,0\";text=\"NOTE: Changing the name will" + (Servers.localServer
          .isFreeDeeds() ? "" : " cost 5 silver,") + " remove all the faith bonuses, and lock the name for 6 months.\"}");
      }
      else
      {
        buf.append("header{text=\"Settlement Name\"};text{text=\"" + village
          .getName() + "\"}");
      ...
      if (((village.isDemocracy()) && (village.getFounderName().equals(getResponder().getName())) && 
        (village.getMayor().getId() == getResponder().getWurmId())) || 
        (getResponder().getPower() >= 4)) {
      ...
      buf.append("checkbox{id=\"unlimitC\"" + ch + ";text=\"Mark this if you want to be able to recruit more than " + village
        .getMaxCitizens() + " citizens.\"}label{text=\"Your upkeep costs are doubled as long as you have more than that amount of citizens.\"}");
      ...
      getResponder().getCommunicator().sendBml(535, 430, true, true, buf.toString(), 200, 200, 200, this.title);
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\VillageSettingsManageQuestion.class
