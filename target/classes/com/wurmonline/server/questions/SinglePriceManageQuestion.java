    ...
    else
    {
      try
      {
        ...
        if (item.getOwnerId() == getResponder().getWurmId())
        {
          ...
          buf.append("label{text=\"" + item.getName().replace("\"", "''") + "\"};harray{label{text=\" \"};input{maxchars=\"3\";id=\"" + wid + "gold\";text=\"" + change
            .getGoldCoins() + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\"" + wid + "silver\";text=\"" + change
            .getSilverCoins() + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\"" + wid + "copper\";text=\"" + change
            .getCopperCoins() + "\"};};harray{label{text=\" \"};input{maxchars=\"3\";id=\"" + wid + "iron\";text=\"" + change
            .getIronCoins() + "\"};};");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\SinglePriceManageQuestion.class
