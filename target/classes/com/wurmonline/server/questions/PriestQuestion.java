  ...
  extends Question
{
  ...
  public void sendQuestion()
  {
    ...
    buf.append("text{text='You may choose to become a priest of " + getResponder().getDeity().name + ".'}text{text=''}");
    
    buf
      .append("text{text='If you answer yes, you will receive special powers from your deity, such as the ability to cast spells.'}text{text=''}");
    if (Servers.localServer.PVPSERVER) {
      buf.append("text{text='You must also walk this path if you strive to become a real champion of " + 
        getResponder().getDeity().name + ".'}text{text=''}");
    }
    buf.append("text{text='You will however be very limited in what you can do.'}");
    buf
      .append("text{text='You will for instance not be able to do such things as create, repair or improve items or use alchemy.'}");
    ...
    if (Servers.localServer.PVPSERVER)
    {
      buf.append("text{text='If you later decide to become a champion of " + getResponder().getDeity().name + " these restrictions will be lifted.'}");
      
      buf
        .append("text{text='As a champion, you may only escape death a few times though. After that your life ends permanently.'}text{text=''}");
    }
    buf.append("text{text='If your faith ever fails you, you will lose your priesthood.'}text{text=''}");
    buf.append("text{type='italic';text='Do you want to become a priest of " + getResponder().getDeity().name + " despite the severe limitations it will have on your actions?'}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\PriestQuestion.class
