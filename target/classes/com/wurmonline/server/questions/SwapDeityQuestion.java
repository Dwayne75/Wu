    ...
    else
    {
      ...
      for (Deity d : deitys) {
        if ((Servers.isThisAPvpServer()) || (d.getNumber() != 4)) {
          buf.append("radio{group=\"did\";id=\"" + d.getNumber() + "\"};label{text=\"" + d
            .getName() + (d
            .getNumber() == this.playerDeity.getNumber() ? " (No Change)" : "") + "\"};");
        }
      }
      buf.append("}");
      buf.append("label{text=\"\"}");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\SwapDeityQuestion.class
