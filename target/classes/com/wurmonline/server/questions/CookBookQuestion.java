  ...
  extends Question
{
  ...
  {
    super(aResponder, aResponder.getName() + "'s CookBook", 
      makeTitle(aResponder, (byte)0, aTargetId), 135, aTargetId);
    this.sortBy = 1;
    if (aTargetId == -10L) {
      this.displayType = 0;
    } else {
  ...
  {
    super(aResponder, aResponder.getName() + "'s Cookbook", 
      makeTitle(aResponder, aDisplayType, aTargetId), 135, aTargetId);
    this.sortBy = 1;
  ...
  {
    super(aResponder, aResponder.getName() + "'s Cookbook", "List of recipes that use " + ingredient
      .getName(false) + ".", 135, -10L);
    this.sortBy = 1;
    this.displayType = aDisplayType;
  ...
  {
    super(aResponder, aResponder.getName() + "'s Cookbook", "Recipe: " + recipe
      .getName(), 135, aTarget);
    this.sortBy = 1;
    this.displayType = aDisplayType;
    this.recip = recipe;
    ...
    if (justRecipe) {
      this.history.add(aDisplayType + "," + recipe
        .getRecipeId() + "," + justRecipe + "," + aTarget + "," + signedBy);
    }
  }
  
  public CookBookQuestion(Creature aResponder, String searchFor, int sortId)
  {
    super(aResponder, aResponder.getName() + "'s Cookbook", searchFor
      .length() > 0 ? "List of recipes that have a name with " + searchFor + " in it." : "List of all your known recipes.", 135, -10L);
  ...
  public void answer(Properties aAnswer)
  {
    ...
    if (this.type == 135)
    {
      ...
      boolean add = getBooleanProp("add");
      if ((add) && (this.target != -10L))
      {
        try
        {
          ...
          {
            if (RecipesByPlayer.addRecipe(getResponder(), this.recip))
            {
              getResponder().getCommunicator().sendNormalServerMessage("You finish adding the " + this.recip
                .getName() + " into your cookbook, just in time, as the recipe has decayed away.");
              Server.getInstance().broadCastAction(
                getResponder().getName() + " stops writing.", getResponder(), 5);
      ...
      for (String key : getAnswer().stringPropertyNames()) {
        if (key.startsWith("sort"))
        {
          ...
          {
            CookBookQuestion cbq = new CookBookQuestion(getResponder(), this.displayType, this.target, newSort);
            cbq.history = this.history;
            cbq.ingred = this.ingred;
            cbq.sendQuestion();
            return;
          }
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), this.searchFor, newSort);
          cbq.history = this.history;
          cbq.ingred = this.ingred;
          cbq.sendQuestion();
          return;
      ...
      if (getBooleanProp("find"))
      {
        ...
        if (srch != null)
        {
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), srch, 0);
          cbq.history = this.history;
      ...
      else if (getBooleanProp("remove"))
      {
        RecipesByPlayer.removeRecipeForPlayer(getResponder().getWurmId(), this.recip.getRecipeId());
        getResponder().getCommunicator().sendCookbookRecipe(this.recip);
      }
      else if (getBooleanProp("back"))
      {
        if (this.history.size() < 2)
        {
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), 0L);
          cbq.history.add("0");
          cbq.sendQuestion();
          return;
        }
        this.history.remove(this.history.size() - 1);
        String last = (String)this.history.get(this.history.size() - 1);
        ...
        case 11: 
          ...
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), type, id);
          cbq.history = this.history;
          cbq.sendQuestion();
          return;
        case 12: 
          ...
          try
          {
            ...
            CookBookQuestion cbq = new CookBookQuestion(getResponder(), type, ingredient);
            cbq.history = this.history;
            cbq.sendQuestion();
            return;
          }
        ...
        case 13: 
          int id = Integer.parseInt(parts[1]);
          Recipe recipe = RecipesByPlayer.getRecipe(getResponder().getWurmId(), id);
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), (byte)13, recipe, false, -10L, "");
        ...
        case 14: 
          ...
          CookBookQuestion cbq = new CookBookQuestion(getResponder(), sFor, 0);
          cbq.history = this.history;
          cbq.sendQuestion();
          return;
        }
      }
      else if (getBooleanProp("show"))
      {
        ...
        else
        {
          if (type == 13)
          {
            ...
            cbq.history = this.history;
            this.history.add("13," + recipe
              .getRecipeId() + "," + false + "," + id + ",");
  ...
  public void sendInfo()
  {
    ...
    buf.append("border{border{size=\"20,20\";null;null;center{varray{header{text=\"" + this.question + "\"}}};" + "harray{label{text=\" \"};button{text=\"Close\";id=\"close\";hover=\"Close the cookbook.\"};label{text=\" \"}};" + "null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\"" + 
    ...
    buf.append("label{text=\"\"};");
    Recipe[] knownrecipes = RecipesByPlayer.getSearchRecipesFor(getResponder().getWurmId(), "");
    buf.append("label{text=\"You know a total of " + knownrecipes.length + " recipes\"};");
    
    buf.append("}};null;null;}");
    
    getResponder().getCommunicator().sendBml(500, 480, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  public void sendRecipes()
  {
    ...
    {
    case 1: 
      buf.append(sendTargetActionRecipes());
      break;
    case 2: 
      buf.append(sendContainerActionRecipes());
      width = 730;
      break;
    case 3: 
      buf.append(sendHeatRecipes());
      width = 730;
      break;
    case 4: 
      buf.append(sendTimeRecipes());
      break;
    case 6: 
      buf.append(sendCookerRecipes());
      break;
    case 8: 
      buf.append(sendContainerRecipes());
      break;
    case 10: 
      buf.append(sendToolRecipes());
      break;
    case 12: 
      buf.append(sendIngredientRecipes());
      break;
    case 14: 
      buf.append(sendSearchRecipes());
    ...
    getResponder().getCommunicator().sendBml(width, height, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  private void sendList()
  {
    ...
    {
    case 5: 
      buf.append(sendCookersList());
      break;
    case 7: 
      buf.append(sendContainersList());
      break;
    case 9: 
      buf.append(sendToolsList());
      
      break;
    case 11: 
      buf.append(sendIngredientsList());
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getTargetActionRecipesFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.1(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.2(this, upDown));
      
      break;
    case 2: 
      Arrays.sort(recipes, new CookBookQuestion.3(this, upDown));
      
      break;
    case 3: 
      Arrays.sort(recipes, new CookBookQuestion.4(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"6\";label{text=\" \"};" + 
      colHeader("Recipe Name", 1, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Active Item", 2, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Target Item", 3, this.sortBy));
    for (Recipe recipe : recipes) {
      buf.append("radio{group=\"sel\";id=\"13," + recipe.getRecipeId() + "\";text=\"\"}" + 
        colourRecipeName(recipe, recipe.getName(), false) + (
        recipe
        .getActiveItem().getTemplate().isCookingTool() ? "radio{group=\"sel\";id=\"10," + recipe
        .getActiveItem().getTemplateId() + "\";text=\"\"}" : recipe.getActiveItem() == null ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"12,")
        
        .append(recipe.getActiveItem().getTemplateId()).append(",")
        .append(recipe.getActiveItem().getCState()).append(",")
        .append(recipe.getActiveItem().getPState()).append(",")
        .append(recipe.getActiveItem().getMaterial()).append(",")
        .append(recipe.getActiveItem().hasRealTemplate()).append(",")
        .append(recipe.getActiveItem().getRealTemplateId()).append(",")
        .append(recipe.getActiveItem().getCorpseData()).append("\";text=\"\"}").toString()) + "label{text=\"" + 
        
        Recipes.getIngredientName(recipe.getActiveItem()) + ((this.showExtra) && 
        (recipe.getActiveItem() != null) ? " - " + recipe.getActiveItem().getTemplateId() : "") + "\"};" + (
        recipe
        .getTargetItem().getTemplate().isFoodMaker() ? "radio{group=\"sel\";id=\"8," + recipe
        .getTargetItem().getTemplateId() + "\";text=\"\"}" : recipe.getTargetItem() == null ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"12,")
        
        .append(recipe.getTargetItem().getTemplateId()).append(",")
        .append(recipe.getTargetItem().getCState()).append(",")
        .append(recipe.getTargetItem().getPState()).append(",")
        .append(recipe.getTargetItem().getMaterial()).append(",")
        .append(recipe.getTargetItem().hasRealTemplate()).append(",")
        .append(recipe.getTargetItem().getRealTemplateId()).append(",")
        .append(recipe.getTargetItem().getCorpseData()).append("\";text=\"\"}").toString()) + "label{text=\"" + 
        
        Recipes.getIngredientName(recipe.getTargetItem()) + ((this.showExtra) && 
        (recipe.getTargetItem() != null) ? " - " + recipe.getTargetItem().getTemplateId() : "") + "\"};");
    }
    buf.append("}");
    if (recipes.length == 0) {
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getContainerActionRecipesFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.5(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.6(this, upDown));
      
      break;
    case 2: 
      Arrays.sort(recipes, new CookBookQuestion.7(this, upDown));
      
      break;
    case 3: 
      Arrays.sort(recipes, new CookBookQuestion.8(this, upDown));
      
      break;
    case 4: 
      Arrays.sort(recipes, new CookBookQuestion.9(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"8\";label{text=\" \"};" + 
      colHeader("Recipe Name", 1, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Active Item", 2, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Containers", 3, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Ingredients", 4, this.sortBy));
    for (Recipe recipe : recipes) {
      buf.append("radio{group=\"sel\";id=\"13," + recipe.getRecipeId() + "\";text=\"\"}" + 
        colourRecipeName(recipe, recipe.getName(), false) + (
        recipe
        .getActiveItem().getTemplate().isCookingTool() ? "radio{group=\"sel\";id=\"9," + recipe
        .getActiveItem().getTemplateId() + "\";text=\"\"}" : recipe.getActiveItem() == null ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"12,")
        ...
        .append(recipe.getActiveItem().hasRealTemplate()).append(",")
        .append(recipe.getActiveItem().getRealTemplateId()).append(",")
        .append(recipe.getActiveItem().getCorpseData()).append("\";text=\"\"}").toString()) + "label{text=\"" + recipe
        
        .getActiveItemName() + ((this.showExtra) && 
        (recipe.getActiveItem() != null) ? " - " + recipe.getActiveItem().getTemplateId() : "") + "\"};" + (
        !recipe.hasOneContainer() ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"7,")
        .append(recipe.getContainerId()).append("\";text=\"\"}").toString()) + "label{text=\"" + recipe
        .getContainersAsString() + "\"};label{text=\" \"};label{text=\"" + recipe
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getHeatRecipesFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.10(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.11(this, upDown));
      
      break;
    case 2: 
      Arrays.sort(recipes, new CookBookQuestion.12(this, upDown));
      
      break;
    case 3: 
      Arrays.sort(recipes, new CookBookQuestion.13(this, upDown));
      
      break;
    case 4: 
      Arrays.sort(recipes, new CookBookQuestion.14(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"8\";label{text=\" \"};" + 
      colHeader("Recipe Name", 1, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Cookers List", 2, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Containers List", 3, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Ingredients List", 4, this.sortBy));
    for (Recipe recipe : recipes) {
      buf.append("radio{group=\"sel\";id=\"13," + recipe.getRecipeId() + "\";text=\"\"}" + 
        colourRecipeName(recipe, recipe.getName(), false) + (
        !recipe.hasOneCooker() ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"6,")
        .append(recipe.getCookerId()).append("\";text=\"\"}").toString()) + "label{text=\"" + recipe
        .getCookersAsString() + "\"};" + (
        !recipe.hasOneContainer() ? "label{text=\"\"}" : new StringBuilder().append("radio{group=\"sel\";id=\"8,")
        .append(recipe.getContainerId()).append("\";text=\"\"}").toString()) + "label{text=\"" + recipe
        .getContainersAsString() + "\"};label{text=\" \"};label{text=\"" + recipe
        .getIngredientsAsString() + "\"};");
    }
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getTimeRecipesFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.15(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.16(this, upDown));
      
      break;
    case 2: 
      Arrays.sort(recipes, new CookBookQuestion.17(this, upDown));
      
      break;
    case 3: 
      Arrays.sort(recipes, new CookBookQuestion.18(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"6\";label{text=\" \"};" + 
      colHeader("Recipe Name", 1, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Containers", 2, this.sortBy) + "label{text=\" \"};" + 
      colHeader("Ingredients", 3, this.sortBy));
    for (Recipe recipe : recipes) {
      buf.append("radio{group=\"sel\";id=\"13," + recipe.getRecipeId() + "\";text=\" \"}" + 
        colourRecipeName(recipe, recipe.getName(), false) + (
        !recipe.hasOneContainer() ? "label{text=\" \"}" : new StringBuilder().append("radio{group=\"sel\";id=\"7,")
  ...
  {
    StringBuilder buf = new StringBuilder();
    ItemTemplate[] templates = RecipesByPlayer.getKnownCookersFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(templates, new CookBookQuestion.19(this));
      
      break;
    case 1: 
      Arrays.sort(templates, new CookBookQuestion.20(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"3\";label{text=\" \"};" + 
      colHeader("Cooker Name", 1, this.sortBy) + "label{text=\" \"};");
    for (ItemTemplate itemTemplate : templates) {
      buf.append("radio{group=\"sel\";id=\"6," + itemTemplate.getTemplateId() + "\";text=\" \"}label{text=\"" + itemTemplate
        .getName() + (this.showExtra ? " - " + itemTemplate
        .getTemplateId() : "") + "\"};label{text=\"\"};");
    }
    buf.append("}");
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getCookerRecipesFor(getResponder().getWurmId(), (int)this.target);
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.21(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.22(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + colHeader("Recipe Name", 1, this.sortBy));
  ...
  {
    StringBuilder buf = new StringBuilder();
    ItemTemplate[] templates = RecipesByPlayer.getKnownContainersFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(templates, new CookBookQuestion.23(this));
      
      break;
    case 1: 
      Arrays.sort(templates, new CookBookQuestion.24(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"3\";label{text=\" \"};" + 
      colHeader("Container Name", 1, this.sortBy) + "label{text=\" \"};");
    for (ItemTemplate itemTemplate : templates) {
      buf.append("radio{group=\"sel\";id=\"8," + itemTemplate.getTemplateId() + "\";text=\"\"}label{text=\"" + itemTemplate
        .getName() + (this.showExtra ? " - " + itemTemplate
        .getTemplateId() : "") + "\"};label{text=\"\"};");
    }
    buf.append("}");
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getContainerRecipesFor(getResponder().getWurmId(), (int)this.target);
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.25(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.26(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + colHeader("Recipe Name", 1, this.sortBy));
    buf.append("table{rows=\"1\";cols=\"6\";");
    for (Recipe recipe : recipes) {
  ...
  {
    StringBuilder buf = new StringBuilder();
    ItemTemplate[] templates = RecipesByPlayer.getKnownToolsFor(getResponder().getWurmId());
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(templates, new CookBookQuestion.27(this));
      
      break;
    case 1: 
      Arrays.sort(templates, new CookBookQuestion.28(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"3\";label{text=\" \"};" + 
      colHeader("Tool Name", 1, this.sortBy) + "label{text=\" \"};");
    for (ItemTemplate itemTemplate : templates) {
      buf.append("radio{group=\"sel\";id=\"10," + itemTemplate.getTemplateId() + "\";text=\"\"}label{text=\"" + itemTemplate
        .getName() + (this.showExtra ? " - " + itemTemplate
        .getTemplateId() : "") + "\"};label{text=\"\"};");
    }
    buf.append("}");
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getToolRecipesFor(getResponder().getWurmId(), (int)this.target);
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.29(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.30(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + colHeader("Recipe Name", 1, this.sortBy));
    buf.append("table{rows=\"1\";cols=\"6\";");
    for (Recipe recipe : recipes) {
  ...
  public String sendIngredientsList()
  {
    ...
    case 0: 
    case 1: 
      Arrays.sort(this.ingredients, new CookBookQuestion.31(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + 
      colHeader("Ingredient Name", 1, this.sortBy));
    
    buf.append("table{rows=\"1\";cols=\"9\";");
    for (Ingredient ingredient : this.ingredients) {
      buf.append("radio{group=\"sel\";id=\"12," + ingredient
        .getTemplateId() + "," + ingredient
        .getCState() + "," + ingredient
        .getPState() + "," + ingredient
        .getMaterial() + "," + ingredient
        .hasRealTemplate() + "," + ingredient
        .getRealTemplateId() + "," + ingredient
        .getCorpseData() + "\";text=\"\"}label{text=\"" + 
        
        Recipes.getIngredientName(ingredient) + (this.showExtra ? " - " + ingredient
        .getTemplateId() : "") + "\"};label{text=\"\"};");
    }
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getIngredientRecipesFor(getResponder().getWurmId(), this.ingred);
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.32(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.33(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + colHeader("Recipe Name", 1, this.sortBy));
  ...
  {
    StringBuilder buf = new StringBuilder();
    Recipe[] recipes = RecipesByPlayer.getSearchRecipesFor(getResponder().getWurmId(), this.searchFor);
    int absSortBy = Math.abs(this.sortBy);
    int upDown = Integer.signum(this.sortBy);
    switch (absSortBy)
    {
    case 0: 
      Arrays.sort(recipes, new CookBookQuestion.34(this));
      
      break;
    case 1: 
      Arrays.sort(recipes, new CookBookQuestion.35(this, upDown));
    }
    buf.append("table{rows=\"1\";cols=\"1\";" + colHeader("Recipe Name", 1, this.sortBy));
  ...
  public void sendRecipe()
  {
    ...
    else
    {
      ...
      this.ingredients = ((Ingredient[])knownIngredients.values().toArray(new Ingredient[knownIngredients.size()]));
    }
    Arrays.sort(this.ingredients, new CookBookQuestion.36(this));
    
    Arrays.sort(this.ingredients, new CookBookQuestion.37(this));
    
    String name = colourRecipeName(this.recip, this.question, true);
    StringBuilder buf = new StringBuilder();
    ...
    buf.append("border{border{size=\"20,25\";null;null;" + name + closeBtn + "null;}null;scroll{vertical=\"true\";horizontal=\"false\";varray{rescale=\"true\";passthrough{id=\"id\";text=\"" + 
    ...
    if (this.recip.getActiveItem() != null)
    {
      ...
      else
      {
        ...
        String link = "label{color=\"255,127,127\";text=\"Unknown recipe" + (this.showExtra ? " - " + recipe.getRecipeId() : "") + "\"}";
        buf.append("label{text=\"Active Item:\"}");
        buf.append(this.showLinks ? "radio{group=\"sel\";id=\"12," + this.recip
          .getActiveItem().getTemplateId() + "," + this.recip
          .getActiveItem().getCState() + "," + this.recip
          .getActiveItem().getPState() + "," + this.recip
          .getActiveItem().getMaterial() + "," + this.recip
          .getActiveItem().hasRealTemplate() + "," + this.recip
          .getActiveItem().getRealTemplateId() + "," + this.recip
          .getActiveItem().getCorpseData() + "\";text=\"\"}" : "label{text=\"\"}");
    ...
    if (this.recip.getTargetItem() != null)
    {
      ...
      link = "label{color=\"255,127,127\";text=\"Unknown recipe" + (this.showExtra ? " - " + recipe.getRecipeId() : "") + "\"}";
      buf.append("label{text=\"Target Item:\"}");
      buf.append(this.showLinks ? "radio{group=\"sel\";id=\"12," + this.recip
        .getTargetItem().getTemplateId() + "," + this.recip
        .getTargetItem().getCState() + "," + this.recip
        .getTargetItem().getPState() + "," + this.recip
        .getTargetItem().getMaterial() + "," + this.recip
        .getTargetItem().hasRealTemplate() + "," + this.recip
        .getTargetItem().getRealTemplateId() + "," + this.recip
        .getTargetItem().getCorpseData() + "\";text=\"\"}" : "label{text=\"\"}");
    ...
    if (this.recip.hasCooker())
    {
      ...
      for (ItemTemplate template : this.recip.getCookerTemplates())
      {
        ...
        buf.append("label{text=\"" + (this.showExtra ? Integer.valueOf(template.getTemplateId()) : "") + "\"}");
        buf.append("label{text=\"\"}");
        cooker = "";
      }
    }
    ...
    {
      container = "Container:";
      for (link = this.recip.getContainerTemplates().iterator(); link.hasNext();)
      {
        template = (ItemTemplate)link.next();
    ...
    if (this.ingredients.length > 0)
    {
      ...
      for (Ingredient ingredient : this.ingredients)
      {
        ...
        if (ingredient.getGroupId() >= 0)
        {
          ...
          buf.append(this.showLinks ? "radio{group=\"sel\";id=\"12," + ingredient
            .getTemplateId() + "," + ingredient
            .getCState() + "," + ingredient
            .getPState() + "," + ingredient
            .getMaterial() + "," + ingredient
          ...
          buf.append("label{text=\"" + (this.showExtra ? ingredient.getGroupId() + "," + ingredient
            .getTemplateId() + "," + ingredient
            .getCState() + "," + ingredient
            .getPState() + "," + ingredient
            .getRatio() + "," + ingredient
    ...
    } else if ((this.target != -10L) && (!knownRecipe) && (paper != null) && (paper.getTopParent() == getResponder().getInventory().getWurmId())) {
    ...
    buf.append("}");
    int height = lines * 20;
    getResponder().getCommunicator().sendBml(this.showExtra ? 470 : 400, height, true, false, buf.toString(), 200, 200, 200, this.title);
  }
  ...
  private String colourRecipeName(Recipe recipe, String name, boolean isBold)
  {
    StringBuilder buf = new StringBuilder();
    if (recipe.isKnown()) {
      buf.append("label{" + (isBold ? "type=\"bold\";" : "") + "color=\"127,255,127\"" + "text=\"" + name.replace("\"", "''") + (this.showExtra ? " - " + recipe
        .getRecipeId() : "") + "\"};");
    } else {
      buf.append(nameColoredByRarity(name + (this.showExtra ? " - " + recipe.getRecipeId() : ""), "", recipe
        .getLootableRarity(), isBold));
    }
    return buf.toString();
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\CookBookQuestion.class
