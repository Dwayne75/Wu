  ...
  extends Question
{
  ...
  public void sendQuestion()
  {
    ...
    if ((surfTile.isBush()) || (surfTile.isTree()))
    {
      ...
      if (surfTile.isBush()) {
        if ((surfData >>> 4 & 0xE) == 14)
        {
          noGUnused = "";
          canHaveGrubs = true;
        }
      }
      if ((surfType == Tiles.Tile.TILE_TREE_BIRCH.id) || (surfType == Tiles.Tile.TILE_ENCHANTED_TREE_BIRCH.id) || (surfType == Tiles.Tile.TILE_MYCELIUM_TREE_BIRCH.id)) {
        if ((surfData >>> 4 & 0xE) == 14)
        {
          noGUnused = "";
          canHaveGrubs = true;
    ...
    buf.append("harray{label{text=\"Cave Flags:\"};label{text=\" Server:\"}input{id=\"caveserverflag\";maxchars=\"3\";text=\"" + (
    
      Server.getServerCaveFlags(this.tilex, this.tiley) & 0xFF) + "\"}label{text=\" Client:\"}input{id=\"caveclientflag\";maxchars=\"3\";text=\"" + (
      
      Server.getClientCaveFlags(this.tilex, this.tiley) & 0xFF) + "\"}};");
    ...
    getResponder().getCommunicator().sendBml(420, 400, true, true, buf.toString(), 200, 200, 200, this.title);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\questions\TileDataQuestion.class
