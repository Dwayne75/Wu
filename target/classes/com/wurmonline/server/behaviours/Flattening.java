  ...
  implements MiscConstants, SoundNames
{
  ...
  private static double skill = 0.0D;
  ...
  private static final boolean isTileUntouchable(Creature performer, int tilex, int tiley)
  {
    for (int x = 0; x >= -1; x--) {
      for (int y = 0; y >= -1; y--)
      {
        if (Zones.protectedTiles[(tilex + x)][(tiley + y)] != 0) {
          return true;
        }
  ...
  private static final boolean flatten(long borderId, Creature performer, Item source, int tile, int tilex, int tiley, int endX, int endY, int numbCorners, float counter, Action act)
  {
    ...
    else
    {
      ...
      if (!Terraforming.isNonDiggableTile(type))
      {
        ...
        if (((act.currentSecond() % 10 == 0) && (act.getNumber() == 150)) || 
          ((act.currentSecond() % 5 == 0) && (act.getNumber() != 150)) || (insta) || (counter == 1.0F))
        {
          flattenSkill = 0;
          flattenImmutable = 0;
          flattenRock = 0;
          ...
          for (int x = 1; x <= endX; x++) {
            for (int y = 1; y <= endY; y++)
            {
              ...
              if (performer.getStrengthSkill() < 21.0D) {
                if (Terraforming.isRoad(Tiles.decodeType(flattenTiles[x][y])))
                {
          ...
          if (!insta)
          {
            if (((!isTooDeep) && (!source.isDiggingtool())) || ((isTooDeep) && 
              (!source.isDredgingTool())))
            {
              ...
              catch (NoSuchSkillException nse)
              {
                if (performer.getPower() <= 0) {
                  logger.log(Level.WARNING, performer
                    ...
                    .getName() + " trying to " + action + " with an item with no primary skill: " + source
                    
                    .getName());
                }
              }
            }
            skill = digging.getKnowledge(0.0D);
          }
          else
          {
            skill = 99.0D;
          ...
          if ((counter == 1.0F) && (!insta))
          {
            ...
            Server.getInstance().broadCastAction(performer
              .getName() + " starts to " + action + " " + btype + ".", performer, 5);
          ...
          if ((act.currentSecond() % tickTimes == 0) || (insta))
          {
            ...
            if ((act.getNumber() == 533) || (act.getNumber() == 865))
            {
              if (checkBorderCorners(performer, tilex, tiley, tilex, tiley, 1, 1, endX, endY, 
                (int)Math.max(3.0D, skill * 3.0D), lAverageHeight, act)) {
                ddone++;
              }
              if (checkBorderCorners(performer, tilex, tiley, tilex + endX - 1, tiley + endY - 1, endX, endY, 1, 1, 
                (int)Math.max(3.0D, skill * 3.0D), lAverageHeight, act)) {
                ddone++;
              }
            }
            else
            {
              for (int xx = 1; xx <= endX; xx++) {
                for (int yy = 1; yy <= endY; yy++) {
                  if (checkFlattenCorner(performer, tilex, tiley, tilex + xx - 1, tiley + yy - 1, xx, yy, 
                    (int)Math.max(3.0D, skill * 3.0D), lAverageHeight, act)) {
                    ddone++;
                  }
                }
            ...
            if (ddone + flattenSkill + flattenImmutable + flattenRock + flattenSlope >= numbCorners)
            {
              ...
              if (ddone == numbCorners)
              {
                ...
                if ((flattenSkill == 0) && (flattenImmutable == 0) && (flattenSlope == 0)) {
                  checkUseDirt(tilex, tiley, endX, endY, performer, source, 
                    (int)Math.max(10.0D, skill * 3.0D), lAverageHeight, act, (insta) && 
                    (source.getAuxData() == 1));
                }
                if (needsDirt > 0) {
              ...
              checkChangedTiles(tile, tilex, tiley, performer);
              if ((act.getNumber() == 533) || (act.getNumber() == 865)) {
                if ((performer.getVisionArea() != null) && (borderId != -10L)) {
            ...
            if (!insta)
            {
              ...
              if (shovel != null) {
                shovel.skillCheck(30.0D, source, 0.0D, false, skilltimes);
              }
              digging.skillCheck(30.0D, source, 0.0D, false, skilltimes);
            }
          }
        }
      }
      else
      {
        ...
        performer.getCommunicator().sendNormalServerMessage("You can't " + action + " that place, as " + source
          .getNameWithGenus() + " just won't do.");
      }
    }
    return done;
  }
  ...
  public static void checkChangedTiles(int tile, int tilex, int tiley, Creature performer)
  {
    for (int x = 0; x < 4; x++) {
      for (int y = 0; y < 4; y++)
      {
        ...
        {
          changedTiles[x][y] = 1;
          flattenTiles[x][y] = Tiles.encode(Tiles.decodeHeight(flattenTiles[x][y]), Tiles.Tile.TILE_ROCK.id, 0);
        }
        if (changedTiles[x][y] != 0)
        {
          changedTiles[x][y] = 0;
  ...
  private static short calcAverageHeight(Creature performer, int tile, int tilex, int tiley, int endX, int endY, int numbCorners, Action act)
  {
    ...
    else if (act.getNumber() == 865)
    {
      ...
      if (dist < dist2) {
        lAverageHeight = Tiles.decodeHeight(flattenTiles[1][1]);
      } else {
        lAverageHeight = Tiles.decodeHeight(flattenTiles[endX][endY]);
      }
    }
    else
    {
      for (int x = 1; x <= endX; x++) {
        for (int y = 1; y <= endY; y++) {
          lAverageHeight += Tiles.decodeHeight(flattenTiles[x][y]);
        }
      }
  ...
  private static final boolean checkFlattenCorner(Creature performer, int initx, int inity, int tilex, int tiley, int x, int y, int maxDiff, int preferredHeight, Action act)
  {
    ...
    if ((!raise) && (!mayLowerCorner(initx, inity, flattenTiles[x][y], rockTiles[x][y], x, y, maxDiff))) {
      return true;
    }
    if ((x == 1) && (y == 1))
    {
      if (changeCorner(performer, tilex, tiley, x, y, x + 1, y, maxDiff, preferredHeight, raise, act))
      {
        ...
        {
          if (changeCorner(performer, tilex, tiley, x, y, x + 1, y + 1, maxDiff, preferredHeight, raise, act)) {
            return true;
          }
  ...
  private static final boolean checkBorderCorners(Creature performer, int initx, int inity, int tilex, int tiley, int x1, int y1, int x2, int y2, int maxDiff, int preferredHeight, Action act)
  {
    ...
    if ((!raise) && (!mayLowerCorner(initx, inity, flattenTiles[x1][y1], rockTiles[x1][y1], x1, y1, maxDiff))) {
      return true;
    }
    if (changeCorner(performer, tilex, tiley, x1, y1, x2, y2, maxDiff, preferredHeight, raise, act)) {
      return true;
    }
    return isCornerDone(x1, y1, preferredHeight);
  }
  ...
  private static final boolean changeCorner(Creature performer, int tilex, int tiley, int x, int y, int targx, int targy, int maxDiff, int preferredHeight, boolean raise, Action act)
  {
    if (raise)
    {
      if (Tiles.decodeHeight(flattenTiles[targx][targy]) > preferredHeight) {
        if (mayLowerCorner(tilex, tiley, flattenTiles[targx][targy], rockTiles[targx][targy], targx, targy, maxDiff)) {
          if (!changeFlattenCorner(performer, targx, targy, maxDiff, preferredHeight, false, act)) {
      ...
      if (mayRaiseCorner(tilex, tiley, flattenTiles[targx][targy], targx, targy, maxDiff)) {
        if (!changeFlattenCorner(performer, targx, targy, maxDiff, preferredHeight, true, act)) {
  ...
  private static final boolean mayRaiseCorner(int tilex, int tiley, int tile, int x, int y, int maxDiff)
  {
    ...
    if ((Terraforming.isImmutableTile(Tiles.decodeType(flattenTiles[x][(y - 1)]))) || 
      (Tiles.decodeType(flattenTiles[x][(y - 1)]) == Tiles.Tile.TILE_HOLE.id))
    {
      flattenImmutable += 1;
      return false;
    }
    if ((Terraforming.isImmutableTile(Tiles.decodeType(flattenTiles[(x - 1)][y]))) || 
      (Tiles.decodeType(flattenTiles[(x - 1)][y]) == Tiles.Tile.TILE_HOLE.id))
    {
      flattenImmutable += 1;
      return false;
    }
    if ((Terraforming.isImmutableTile(Tiles.decodeType(flattenTiles[(x - 1)][(y - 1)]))) || 
    ...
    for (int a = 0; a >= -1; a--) {
      for (int b = 0; b >= -1; b--)
      {
        ...
        if (Terraforming.isImmutableOrRoadTile(newType))
        {
          if ((a == 0) && (b == 0) && (!raise))
          {
            if (immutableTiles[modx][mody] != 0) {
              logger.log(Level.WARNING, "Does this really change anything? Changing " + modx + "," + mody + " from " + 
                Tiles.decodeHeight(flattenTiles[modx][mody]) + " to " + newFHeight + " at  protected=" + immutableTiles[modx][mody] + " xy=" + x + "," + y + " ab=" + a + "," + b, new Exception());
            }
            changedTiles[modx][mody] = 1;
        ...
        else
        {
          ...
          flattenTiles[modx][mody] = Tiles.encode(newFHeight, newType, 0);
          changedTiles[modx][mody] = 1;
        }
      }
    }
  ...
  private static final void checkUseDirt(int tilex, int tiley, int endX, int endY, Creature performer, Item source, int maxdiff, int preferredHeight, Action act, boolean quickLevel)
  {
    ...
    raising = false;
    if (((lowerFlatten >= 2) && (act.getNumber() == 150)) || ((lowerFlatten >= 1) && 
      (act.getNumber() == 533)))
    {
      if ((performer.getCarriedItem(26) != null) || 
        (performer.getCarriedItem(298) != null))
      {
        preferredHeight++;
        raising = true;
      }
    ...
    if (raising)
    {
      for (int x = 1; x <= endX; x++) {
        for (int y = 1; y <= endY; y++)
        {
          ...
          if (diff >= 1) {
            if (mayRaiseCorner(tilex, tiley, flattenTiles[x][y], x, y, maxdiff)) {
      ...
      if ((needsDirt == 3) && (act.getNumber() != 532))
      {
        ...
        for (int x = 1; x <= endX; x++) {
          for (int y = 1; y <= endY; y++)
          {
            ...
            if (diff >= 1) {
              if (mayLowerCorner(tilex, tiley, flattenTiles[x][y], rockTiles[x][y], x, y, maxdiff)) {
                if (dirtX == -1)
                {
                  dirtX = x;
                  dirtY = y;
  ...
  private static final void useDirt(Creature performer, int x, int y, int maxDiff, int preferredHeight, boolean quickLevel, Action act)
  {
    ...
    if (dirt != null)
    {
      ...
      try
      {
        ...
        if (quickLevel)
        {
          performer.getCommunicator().sendNormalServerMessage("You have a ghost " + dirt
            .getActualName() + " in inventory.");
          return;
        }
        performer.getCommunicator().sendNormalServerMessage("One corner could not be raised by a " + dirt
          .getActualName() + " you are carrying.");
        needsDirt += 1;
        return;
      }
  ...
  private static final void getDirt(Creature performer, Item source, int x, int y, int maxDiff, int preferredHeight, boolean quickLevel, Action act)
  {
    try
    {
      ...
      if ((performer.getInventory().getNumItemsNotCoins() < 100) && 
        (performer.canCarry(ItemTemplateFactory.getInstance().getTemplate(26).getWeightGrams()))) {
        try
        {
      ...
      if ((source.isDredgingTool()) && (!source.isWand()))
      {
        ...
        if ((ivehic != null) && (ivehic.getFreeVolume() < dirtVol))
        {
          ...
          {
            needsDirt -= 1;
            performer.getCommunicator().sendNormalServerMessage("The " + ivehic.getName() + " and the " + source
              .getName() + " are both full.");
          }
        }
        ...
        (performer.canCarry(ItemTemplateFactory.getInstance().getTemplate(26).getWeightGrams())))
      {
        if (!changeFlattenCorner(performer, x, y, maxDiff, preferredHeight, false, act))
        {
          ...
          if (!quickLevel)
          {
            Item dirt = ItemFactory.createItem(template, Server.rand.nextFloat() * maxDiff / 3.0F, performer
              .getName());
            if ((source.isDredgingTool()) && (!source.isWand()))
            {
              boolean addedToBoat = false;
              if ((performer.getVehicle() != -10L) && (ivehic != null)) {
                if (ivehic.isBoat()) {
                  if (ivehic.testInsertItem(dirt))
                  {
                    ivehic.insertItem(dirt);
                    performer.getCommunicator().sendNormalServerMessage("You put the " + dirt.getName() + " in the " + ivehic
                      .getName() + ".");
                    addedToBoat = true;
                  }
                }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Flattening.class
