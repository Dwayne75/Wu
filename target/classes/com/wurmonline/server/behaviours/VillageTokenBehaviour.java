  ...
  extends ItemBehaviour
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item target)
  {
    ...
    toReturn.addAll(getVillageBehaviours(performer, target));
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item target)
  {
    ...
    if ((performer.getPower() > 0) && (
      ...
      toReturn.add(Actions.actionEntrys['ȁ']);
    }
    if ((performer.getCurrentVillage() != null) && (performer.getCurrentVillage().kingdom == performer.getKingdomId())) {
      if (!performer.getCurrentVillage().isEnemy(performer)) {
        if (performer.isWithinTileDistanceTo(target.getTileX(), target.getTileY(), 0, 2)) {
          if ((!source.isNoDiscard()) && (!source.isInstaDiscard()) && 
            (!source.isTemporary())) {
  ...
  public boolean action(Action act, Creature performer, Item target, short action, float counter)
  {
    ...
    else if (action == 80)
    {
      ...
      if (village.mayDoDiplomacy(performer)) {
        ...
        performer.getCommunicator().sendNormalServerMessage("You may not perform diplomacy for " + village
          .getName() + ".");
      }
    }
    else if (action == 67)
    {
      ...
      try
      {
        ...
        if (currVill != null) {
          if (currVill.isActionAllowed((short)67, performer)) {
    ...
    else if (action == 513)
    {
      ...
      {
        Village currVill = Villages.getVillage(villageId);
        if ((currVill.getGuards().length == 1) || (
          (performer.getPower() >= 2) && (currVill.getGuards().length >= 1)))
        {
      ...
      catch (NoSuchVillageException localNoSuchVillageException2) {}
    }
    else if (action == 69)
    {
      done = true;
    ...
    else if (action == 1)
    {
      ...
      if (target.getData2() > 0) {
        ...
        {
          Village currVill = Villages.getVillage(target.getData2());
          performer.getCommunicator().sendNormalServerMessage("You spot " + currVill
            .getGuards().length + " guards in the vicinity.");
        ...
        {
          logger.log(Level.WARNING, performer.getName() + " managing token with id " + target.getWurmId() + " but villageid=" + target
            .getData2());
        }
      }
    }
        ...
        try
        {
          ...
          if (guards.length == 0) {
            ...
            for (int x = 0; x < guards.length; x++) {
              performer.getCommunicator().sendNormalServerMessage(guards[x]
                .getCreature().getName() + " (" + guards[x].getCreature().getWurmId() + ") at " + (
                (int)guards[x].getCreature().getPosX() >> 2) + ", " + (
                (int)guards[x].getCreature().getPosY() >> 2) + ", surfaced=" + guards[x]
                .getCreature().isOnSurface());
            }
          }
        }
        catch (NoSuchVillageException nsv)
        ...
        try
        {
          ...
          for (int x = currVill.getStartX(); x <= currVill.getEndX(); x++) {
            for (int y = currVill.getStartY(); y <= currVill.getEndY(); y++) {
              ...
              {
                com.wurmonline.server.zones.Zones.protectedTiles[x][y] = (action == 381 ? 1 : 0);
              }
            }
          }
          if (action == 118) {
    ...
    else if (action == 481)
    {
      ...
      else
      {
        ...
        {
          Village currVill = Villages.getVillage(villageId);
          if ((currVill.isActionAllowed((short)68, performer)) || 
        ...
        try
        {
          ...
          if (currVill != null) {
            if (performer.getCitizenVillage().mayDeclareWarOn(currVill)) {
              ...
              performer.getCommunicator().sendAlertServerMessage(currVill
  ...
  private List<ActionEntry> getVillageBehaviours(Creature performer, Item target)
  {
    ...
    try
    {
      ...
      toReturn.addAll(getSettlementMenu(performer, performer
        .isWithinDistanceTo(target.getPosX(), target.getPosY(), target.getPosZ(), 4.0F), village, currVill));
      
      size = -3;
      toReturn.add(new ActionEntry((short)size, "Bank", "banking"));
      toReturn.add(Actions.actionEntrys['Þ']);
      ...
      if ((performer.getPower() >= 4) && (currVill.isPermanent))
      {
        ...
        if (com.wurmonline.server.zones.Zones.protectedTiles[target.getTileX()][target.getTileY()] != 0) {
    ...
    {
      logger.log(Level.WARNING, "No settlement for token with id " + target.getWurmId() + " at " + target.getPosX() + ", " + target
      
        .getPosY() + ", surf=" + target.isOnSurface());
    }
    return toReturn;
  }
  ...
  static List<ActionEntry> getSettlementMenu(Creature performer, boolean closeToToken, Village citizenVillage, Village currentVillage)
  {
    ...
    if ((citizenVillage != null) && (citizenVillage.mayDoDiplomacy(performer)) && (currentVillage != null)) {
      if (citizenVillage != currentVillage)
      {
    ...
    if ((citizenVillage != null) && (citizenVillage.equals(currentVillage)) && 
      (currentVillage.isActionAllowed((short)67, performer))) {
      guards = true;
    }
    ...
    if (((performer.isPaying()) && (currentVillage.kingdom != performer.getKingdomId())) || 
      (currentVillage.isEnemy(performer.getCitizenVillage()))) {
      enemy = true;
    }
    List<ActionEntry> actionMenu = new LinkedList();
    
    actionMenu.add(Actions.actionEntrys['ʞ']);
    actionMenu.add(Actions.actionEntrys[72]);
    if ((destroy) && (closeToToken)) {
      actionMenu.add(Actions.actionEntrys['Ŝ']);
    }
    if (enemy) {
      actionMenu.add(Actions.actionEntrys['Ş']);
    }
    if (findguards) {
    ...
    if (King.currentEra > 0) {
      actionMenu.add(Actions.actionEntrys['Ť']);
    }
    if (King.currentEra > 0) {
      actionMenu.add(Actions.actionEntrys['ţ']);
    }
    if (citizens) {
    ...
    if ((enemy) && (currentVillage.getGuards().length == 1) && (closeToToken)) {
      actionMenu.add(Actions.actionEntrys['ȁ']);
    }
    if ((twitter) || (performer.getPower() >= 2)) {
      actionMenu.add(Actions.actionEntrys['ǡ']);
    }
    if (warmonger) {
      actionMenu.add(Actions.actionEntrys['Ñ']);
    }
    List<ActionEntry> toReturn = new LinkedList();
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\VillageTokenBehaviour.class
