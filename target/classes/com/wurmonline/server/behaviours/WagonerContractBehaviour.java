  ...
  extends ItemBehaviour
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item target)
  {
    ...
    toReturn.addAll(getBehavioursForWagonerContract(performer, null, target));
    return toReturn;
  }
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item target)
  {
    ...
    toReturn.addAll(getBehavioursForWagonerContract(performer, source, target));
    return toReturn;
  }
  
  public boolean action(Action act, Creature performer, Item target, short action, float counter)
  {
    boolean[] ans = wagonerContractActions(act, performer, null, target, action, counter);
    if (ans[0] != 0) {
      return ans[1];
    }
  ...
  {
    boolean[] ans = wagonerContractActions(act, performer, source, target, action, counter);
    if (ans[0] != 0) {
      return ans[1];
    }
  ...
  private List<ActionEntry> getBehavioursForWagonerContract(Creature performer, @Nullable Item source, Item contract)
  {
    ...
    if ((Features.Feature.WAGONER.isEnabled()) && (contract.getData() != -1L))
    {
      ...
      if ((wagoner != null) && (wagoner.getVillageId() != -1))
      {
        ...
        waglist.add(Actions.actionEntrys['Θ']);
        if (!waglist.isEmpty())
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\WagonerContractBehaviour.class
