  ...
  implements ItemMaterials, MiscConstants
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Wound target)
  {
    ...
    if ((performer.getCultist() != null) && (performer.getCultist().mayCleanWounds())) {
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Wound target)
  {
    ...
    if (!target.isDrownWound())
    {
      if ((source.getMaterial() == 17) && (!target.isInternal())) {
        toReturn.add(Actions.actionEntrys['Ä']);
      } else if (source.isHealingSalve()) {
        if ((target.isInternal()) || (target.isBruise()) || (target.isPoison())) {
          toReturn.add(Actions.actionEntrys['Ä']);
        }
      }
      ...
      else if (source.getTemplateId() == 128) {
        if (target.getType() == 10) {
          toReturn.add(Actions.actionEntrys['Ĝ']);
        }
      }
  ...
  public boolean action(Action act, Creature performer, Wound target, short action, float counter)
  {
    ...
    if (owner != null) {
      if (!performer.isWithinDistanceTo(owner.getPosX(), owner.getPosY(), owner.getPositionZ() + owner.getAltOffZ(), 8.0F)) {
        return true;
      }
    }
    if (action == 1)
    {
      performer.getCommunicator().sendNormalServerMessage("You see " + target
        .getWoundString() + " at the " + performer
        .getBody().getWoundLocationString(target.getLocation()) + ".");
      if (target.getHealEff() > 0) {
        performer.getCommunicator().sendNormalServerMessage("It is covered with some healing plants (" + target
          .getHealEff() + ").");
      }
    }
    else if (action == 395)
    {
      ...
      else if ((target.getInfectionSeverity() <= 0.0F) && (target.isInternal()))
      {
        ...
        (performer.getCultist() != null) && (performer.getCultist().mayCleanWounds()))
      {
        ...
        if (target.getCreature() == performer)
        {
          Server.getInstance().broadCastAction(performer
            .getName() + " sucks on " + performer.getHisHerItsString() + " wounds.", performer, 5);
        }
        else
        {
          target.getCreature().getCommunicator().sendNormalServerMessage(performer.getName() + " sucks on your wounds.");
          Server.getInstance().broadCastAction(performer.getName() + " sucks on " + targetName + "'s wounds.", performer, target
            .getCreature(), 5);
        }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\WoundBehaviour.class
