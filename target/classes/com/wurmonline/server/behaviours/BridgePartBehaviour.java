  ...
  implements QuestionTypes
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, boolean onSurface, BridgePart bridgePart)
  {
    ...
    if (MethodsStructure.mayModifyStructure(performer, structure, floorTile, (short)169)) {
      if (bpState.isBeingBuilt()) {
        ...
        toReturn.add(new ActionEntry((short)(20000 + bridgePart.getMaterial().getCode()), "Build " + bridgePart
          .getMaterial().getName().toLowerCase() + " " + bridgePart
          .getType().getName(), bridgePart.getMaterial().getName() + " " + bridgePart
          .getType().getName(), emptyIntArr));
      }
    }
    if (!source.isTraded())
    {
      ...
      toReturn.addAll(Actions.getDefaultItemActions());
      if (!bridgePart.isIndestructible()) {
      ...
      if (((source.getTemplateId() == 176) || (source.getTemplateId() == 315)) && 
        (WurmPermissions.mayUseGMWand(performer)) && (performer.getPower() >= 2))
      {
      ...
      if ((bridgePart.isFinished()) && (bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.ROPE) && (bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.WOOD))
      {
        ...
        if (((source.getTemplateId() == 492) || 
          ...
          (WurmPermissions.mayUseGMWand(performer)) && (source.getTemplateId() == 315))) && (roadType == 0)) {
          toReturn.add(new ActionEntry((short)155, "Prepare", "preparing the floor"));
        } else if (((source.getTemplateId() == 97) || 
          ((WurmPermissions.mayUseDeityWand(performer)) && (source.getTemplateId() == 176)) || (
          (WurmPermissions.mayUseGMWand(performer)) && (source.getTemplateId() == 315))) && (roadType == Tiles.Tile.TILE_PREPARED_BRIDGE.id)) {
        ...
        } else if (((source.getTemplateId() == 1115) || 
          ((WurmPermissions.mayUseDeityWand(performer)) && (source.getTemplateId() == 176)) || (
        ...
        if ((MethodsHighways.onHighway(bridgePart)) && (source.isPaveable()) && (roadType != 0) && (roadType != Tiles.Tile.TILE_PREPARED_BRIDGE.id)) {
          if ((source.isPaveable()) && (roadType != 0)) {
  ...
  private boolean buildAction(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter)
  {
    if (bridgePart.getBridgePartState() == BridgeConstants.BridgeState.PLANNED)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You Cannot build a " + bridgePart
          .getName() + " made of " + 
          BridgeConstants.BridgeMaterial.fromByte((byte)(action - 20000)).getName().toLowerCase() + " as It's already planned to be made of " + bridgePart
  ...
  {
    switch (BridgePartBehaviour.1.$SwitchMap$com$wurmonline$shared$constants$BridgeConstants$BridgeMaterial[material.ordinal()])
    {
    case 1: 
      return 1013;
    case 2: 
  ...
  {
    BuildAllMaterials toReturn = getRequiredMaterials(bridgePart.getType(), bridgePart
      .getMaterial(), bridgePart.getNumberOfExtensions());
  ...
  public static BuildAllMaterials getRequiredMaterials(BridgeConstants.BridgeType bridgeType, BridgeConstants.BridgeMaterial bridgeMaterial, int bridgeExtensions)
  {
    ...
    try
    {
      switch (BridgePartBehaviour.1.$SwitchMap$com$wurmonline$shared$constants$BridgeConstants$BridgeMaterial[bridgeMaterial.ordinal()])
      {
      case 9: 
      ...
      {
        logger.log(Level.WARNING, "Someone tried to make a bridge but the material choice was not supported (" + bridgeMaterial
  ...
  static final boolean isOkToBuild(Creature performer, Item tool, BridgePart bridgePart)
  {
    ...
    if (buildSkill.getKnowledge(0.0D) < bridgePart.minRequiredSkill())
    {
    ...
    boolean ok = false;
    if ((performer.getTileX() == bridgePart.getTileX()) && (performer.getTileY() + 1 == bridgePart.getTileY()) && 
      (bridgePart.hasNorthExit())) {
      ok = true;
    } else if ((performer.getTileX() == bridgePart.getTileX()) && (performer.getTileY() - 1 == bridgePart.getTileY()) && 
      (bridgePart.hasSouthExit())) {
      ok = true;
    } else if ((performer.getTileX() + 1 == bridgePart.getTileX()) && (performer.getTileY() == bridgePart.getTileY()) && 
      (bridgePart.hasWestExit())) {
      ok = true;
    } else if ((performer.getTileX() - 1 == bridgePart.getTileX()) && (performer.getTileY() == bridgePart.getTileY()) && 
      (bridgePart.hasEastExit())) {
      ok = true;
    }
    if (!ok)
    {
  ...
  private static final boolean bridgePartBuilding(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter)
  {
    ...
    boolean insta = ((Servers.isThisATestServer()) && (performer.getPower() > 1)) || ((performer.getPower() >= 4) && ((source.getTemplateId() == 315) || (source.getTemplateId() == 176)));
    if (bridgePart.isFinished())
    {
    ...
    if (counter == 1.0F)
    {
      ...
      if ((!advanceNextState(performer, bridgePart, act, true)) && (!insta))
      {
        ...
        performer.getCommunicator().sendNormalServerMessage("You need " + message + " to start building the " + bridgePart
          .getName() + " of " + bridgePart
          .getMaterial().getName().toLowerCase() + ".");
        performer.getCommunicator().sendActionResult(false);
        return true;
      }
    ...
    else
    {
      ...
      {
        SoundPlayer.playSound(bridgePart.getSoundByMaterial(), bridgePart.getTileX(), bridgePart.getTileY(), performer
          .isOnSurface(), 1.6F);
        performer.getStatus().modifyStamina(-1000.0F);
        damageTool(performer, bridgePart, source);
      }
    ...
    if ((counter * 10.0F > time) || (insta))
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You need " + message + " to build the " + bridgePart
          .getName() + ".");
        performer.getCommunicator().sendActionResult(false);
        return true;
      }
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " attaches " + message + " to a " + bridgePart.getName() + ".", performer, 5);
      performer.getCommunicator()
        .sendNormalServerMessage("You attach " + message + " to a " + bridgePart.getName() + ".");
      
      float divider = bam.getTotalQuantityRequired();
      float oldql = bridgePart.getQualityLevel();
      float qlevel = Math.max(1.0F, 
        (float)Math.min(act.getPower() + oldql, oldql + buildSkill.getKnowledge(0.0D) / divider));
      ...
      if (bsm.isStageComplete(bridgePart))
      {
        ...
        else
        {
          if (insta)
          {
            ...
            if (!Servers.isThisATestServer()) {
              logger.info("Insta Bridge: " + performer.getName() + " completed bridge part at " + bridgePart
                .getTileX() + "," + bridgePart.getTileY());
            }
          }
        ...
        {
          Server.getInstance().broadCastAction(performer
            .getName() + " completes a " + bridgePart.getName() + ".", performer, 5);
      ...
      try
      {
        ...
        if (bridgePart.getState() == BridgeConstants.BridgeState.COMPLETED.getCode())
        {
          ...
          if (!structure.isFinished()) {
            if (structure.updateStructureFinishFlag()) {
  ...
  private static boolean hasValidTool(BridgeConstants.BridgeMaterial bridgeMaterial, Item source)
  {
    ...
    case 8: 
    default: 
      logger.log(Level.WARNING, "Enum value '" + bridgeMaterial
      
        .toString() + "' added to BridgeMaterial but not to a switch statement in method BridgePartBehaviour.hasValidTool()");
    }
    if (tid == 315) {
      return true;
    }
  ...
  private static boolean maybeDestroy(Creature performer, BridgePart bridgePart)
  {
    ...
    try
    {
      ...
      performer.getCommunicator().sendNormalServerMessage("You revert the " + bridgePart
        .getName() + " back to planning stage" + wand + ".");
      Server.getInstance().broadCastAction(performer
        .getName() + effort + " reverts the " + bridgePart.getName() + wand + ".", performer, 3);
    ...
    {
      if (performer.getPower() > 1) {
        performer.getCommunicator().sendNormalServerMessage("Could not find the bridge that " + bridgePart
          .getName() + " belongs to.");
      }
    }
    return true;
  ...
  {
    if (action == 1) {
      return examine(performer, bridgePart);
    }
    if (action == 607)
    {
      if (!bridgePart.isFinished()) {
    ...
    return super.action(act, performer, bridgePart.getTileX(), bridgePart.getTileY(), onSurface, 
      Zones.getTileIntForTile(bridgePart.getTileX(), bridgePart.getTileY(), 0), action, counter);
  }
  ...
  public boolean action(Action act, Creature performer, Item source, boolean onSurface, BridgePart bridgePart, int encodedTile, short action, float counter)
  {
    ...
    if (action == 179)
    {
      if (((source.getTemplateId() == 176) || (source.getTemplateId() == 315)) && 
        ...
        Methods.sendSummonQuestion(performer, source, bridgePart.getTileX(), bridgePart.getTileY(), bridgePart
          .getStructureId());
      }
      return true;
    }
    ...
    if ((bridgePart.isFinished()) && (bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.ROPE) && (bridgePart.getMaterial() != BridgeConstants.BridgeMaterial.WOOD))
    {
      if ((action == 155) && 
        ...
        ((WurmPermissions.mayUseDeityWand(performer)) && (source.getTemplateId() == 176)) || (
        (WurmPermissions.mayUseGMWand(performer)) && (source.getTemplateId() == 315))) && (roadType == 0)) {
        return changeFloor(act, performer, source, bridgePart, action, counter);
      }
    ...
    if (!isConstructionAction(action)) {
      return super.action(act, performer, source, bridgePart.getTileX(), bridgePart.getTileY(), onSurface, bridgePart
        .getHeightOffset(), 
        Zones.getTileIntForTile(bridgePart.getTileX(), bridgePart.getTileY(), 0), action, counter);
    }
    ...
    {
      if (buildAction(act, performer, source, bridgePart, action, counter)) {
        return true;
      }
      return false;
    }
      ...
      if (((source.getTemplateId() == 176) || (source.getTemplateId() == 315)) && 
        ...
        try
        {
          ...
          performer.getLogger().log(Level.INFO, performer
            ...
            .getName() + " destroyed bridge " + struct.getName() + " at " + bridgePart.getTileX() + ", " + bridgePart
  ...
  private boolean examine(Creature performer, BridgePart bridgePart)
  {
    ...
    if (bpState.isBeingBuilt())
    {
      ...
      performer.getCommunicator().sendNormalServerMessage("You see a " + bridgePart
        .getName() + " under construction. The " + bridgePart.getName() + " requires " + allmaterials + " to be finished.");
      
      performer.getCommunicator().sendNormalServerMessage("The current stage being constructed is " + bridgePart
        .getFloorStageAsString() + "of " + bam
        .getStageCountAsString() + " and requires " + stagematerials + " to be completed.");
    ...
    else if (bpState == BridgeConstants.BridgeState.PLANNED)
    {
      ...
      String allmaterials = bam.getRequiredMaterialString(true);
      String stagematerials = bsm.getRequiredMaterialString(true);
      performer.getCommunicator().sendNormalServerMessage("You see plans for a " + bridgePart
        .getName() + ". The " + bridgePart.getName() + " requires " + allmaterials + " to be finished.");
      
      performer.getCommunicator().sendNormalServerMessage("There are " + bam
        .getStageCountAsString() + " stages and the initial stage requires " + stagematerials + " to be completed.");
      
      sendQlString(performer, bridgePart);
    }
    else
    {
      performer.getCommunicator().sendNormalServerMessage("It is a " + bridgePart
        .getName() + ".");
      sendQlString(performer, bridgePart);
      ...
      try
      {
        ...
        if (bridgePart.getDamage() > 0.0F) {
          performer.getCommunicator().sendNormalServerMessage("It needs to be repaired with " + template
            .getNameWithGenus() + ".");
        } else {
          performer.getCommunicator().sendNormalServerMessage("It can be improved with " + template
            .getNameWithGenus() + ".");
        }
      }
  ...
  static final void sendQlString(Creature performer, BridgePart bridgePart)
  {
    ...
    if (performer.getPower() > 0) {
      performer.getCommunicator().sendNormalServerMessage("id: " + bridgePart
        .getId() + " @" + bridgePart.getTileX() + "," + bridgePart.getTileY() + " height: " + bridgePart
        .getHeightOffset() + " " + bridgePart
        .getMaterial().getName() + " " + bridgePart.getType().getName() + " (" + bridgePart
        .getBridgePartState().toString().toLowerCase() + ").");
    }
  }
  ...
  private boolean changeFloor(Action act, Creature performer, Item source, BridgePart bridgePart, short action, float counter)
  {
    ...
    if ((pavingItem == 492) || ((insta) && (roadType == 0)))
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("It takes " + source
          .getTemplate().getWeightGrams() / 1000 + "kg of " + source.getName() + " to prepare the bridge part.");
        return true;
      }
    ...
    else if (pavingItem == 153)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("It takes " + source
          .getTemplate().getWeightGrams() / 1000 + "kg of " + source.getName() + " to colour the floorboards.");
        return true;
      }
    ...
    if ((counter == 1.0F) && (!insta))
    {
      ...
      if ((checkSkill == 1009) && (paving.getRealKnowledge() < 10.0D))
      {
      ...
      if ((pavingItem == 1115) || (pavingItem == 97)) {
        time = Actions.getDestroyActionTime(performer, paving, source, 0.0D);
      } else {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\BridgePartBehaviour.class
