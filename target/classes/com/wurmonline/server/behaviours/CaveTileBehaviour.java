  ...
  extends TileBehaviour
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, int tilex, int tiley, boolean onSurface, int tile, int dir)
  {
    ...
    if ((Features.Feature.CAVE_DWELLINGS.isEnabled()) && ((Tiles.isReinforcedFloor(type)) || (Tiles.isRoadType(type)))) {
      if (dir == 0) {
    ...
    if ((source.isMiningtool()) && ((dir == 1) || (type == Tiles.Tile.TILE_CAVE.id) || (type == Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id)))
    {
      ...
      if (((dir == 0) && (type == Tiles.Tile.TILE_CAVE.id)) || ((dir == 1) && ((type == Tiles.Tile.TILE_CAVE.id) || 
    ...
    if ((source.getTemplateId() == 601) || 
      ...
      if ((Tiles.decodeType(tile) == Tiles.Tile.TILE_CAVE_EXIT.id) || (Tiles.decodeType(tile) == Tiles.Tile.TILE_CAVE.id) || 
  ...
  public boolean action(Action act, Creature performer, Item source, int tilex, int tiley, boolean onSurface, int heightOffset, int tile, int dir, short action, float counter)
  {
    ...
    case 145: 
      ...
      {
        handled = true;
        done = handle_MINE(act, performer, source, tilex, tiley, action, counter, dir);
      }
      if ((source.isMiningtool()) && (type == Tiles.Tile.TILE_CAVE_EXIT.id) && (fType == Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id))
      {
        handled = true;
    ...
    case 156: 
      ...
      {
        handled = true;
        done = handle_PROSPECT(performer, source, tilex, tiley, tile, counter, dir);
      }
      break;
    case 150: 
      if ((source.isMiningtool()) && ((dir == 1) || (type == Tiles.Tile.TILE_CAVE.id) || (type == Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id)))
      {
        handled = true;
        done = handle_FLATTEN(act, performer, source, tilex, tiley, tile, counter, dir);
      }
      break;
    case 532: 
      if ((source.isMiningtool()) && ((dir == 1) || (type == Tiles.Tile.TILE_CAVE.id) || (type == Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id)))
      {
        handled = true;
        done = handle_LEVEL(act, performer, source, tilex, tiley, tile, counter, dir);
      }
      break;
    case 229: 
      ...
      {
        handled = true;
        done = handle_REINFORCE(performer, source, tilex, tiley, tile, counter, dir);
      }
      break;
    case 141: 
      if ((performer.getDeity() != null) && (performer.getDeity().isMountainGod()))
    ...
    case 193: 
      ...
      handled = true;
      handle_REPAIR_STRUCT(performer, source, tilex, tiley, tile);
      break;
    case 176: 
      handled = true;
    ...
    case 155: 
      ...
      {
        handled = true;
        done = changeFloor(act, performer, source, tilex, tiley, tile, action, counter);
      }
      ...
      if ((source.isCavePaveable()) && (dir == 0) && ((Tiles.isRoadType(type)) || ((type == Tiles.Tile.TILE_CAVE_EXIT.id) && 
        (Tiles.isRoadType(fType)))))
      {
        handled = true;
        done = changeFloor(act, performer, source, tilex, tiley, tile, action, counter);
      }
      break;
    case 191: 
      if ((source.getTemplateId() == 1115) && (dir == 0) && ((Tiles.isRoadType(type)) || ((type == Tiles.Tile.TILE_CAVE_EXIT.id) && 
        (Tiles.isRoadType(fType)))))
      {
        handled = true;
    ...
    case 1: 
      handled = true;
      done = action(act, performer, source, tilex, tiley, onSurface, heightOffset, tile, dir, action, counter);
      break;
    default: 
      handled = false;
    }
  ...
  private void handle_REPAIR_STRUCT(Creature performer, Item source, int tilex, int tiley, int tile)
  {
    ...
    boolean hasSpecialPermission = ((WurmPermissions.mayUseDeityWand(performer)) && (source.getTemplateId() == 176)) || ((WurmPermissions.mayUseGMWand(performer)) && (source.getTemplateId() == 315));
    if ((!shakerOrb) && (!hasSpecialPermission)) {
      return;
    }
    byte decodedType = Tiles.decodeType(tile);
    if ((decodedType != Tiles.Tile.TILE_CAVE_EXIT.id) && (decodedType != Tiles.Tile.TILE_CAVE.id)) {
      if (!Tiles.isReinforcedFloor(decodedType)) {
        return;
      }
    }
    ...
    comm.sendNormalServerMessage("You throw the " + source
      .getName() + " on the ground. The earth suddenly shakes and the rock falls in!");
    
    Server.getInstance().broadCastAction(performer
      .getName() + " throws " + source.getNameWithGenus() + " on the ground. The earth suddenly shakes and the rock falls in!", performer, 5);
  ...
  private boolean handle_REINFORCE(Creature performer, Item source, int tilex, int tiley, int tile, float counter, int dir)
  {
    ...
    if ((!Methods.isActionAllowed(performer, (short)229, false, tilex, tiley, tile, dir)) || 
    ...
    if (counter == 1.0F)
    {
      ...
      Server.getInstance()
        .broadCastAction(performer.getName() + " starts to reinforce the cave " + floorexit + ".", performer, 5);
    ...
    if ((counter * 10.0F > time) || (insta))
    {
      ...
      {
        int encodedValue = Tiles.encode(Tiles.decodeHeight(tile), Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id, 
  ...
  private boolean handle_LEVEL(Action act, Creature performer, Item source, int tilex, int tiley, int tile, float counter, int dir)
  {
    ...
    boolean done = flatten(performer, source, tile, tilex, tiley, counter, act, dir);
    return done;
  }
  ...
  private boolean handle_PROSPECT(Creature performer, Item source, int tilex, int tiley, int tile, float counter, int dir)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      Server.getInstance()
        .broadCastAction(performer.getName() + " starts gathering fragments of the rock.", performer, 5);
  ...
  public static boolean mine(Action act, Creature performer, Item source, int tilex, int tiley, short action, float counter, int dir, TilePos digTilePos)
  {
    ...
    if (Tiles.decodeData(digCorner) == 0) {
      if (Tiles.decodeHeight(digCorner) == Tiles.decodeHeight(Server.surfaceMesh.getTile(digTilePos)))
      {
    ...
    catch (Exception ex)
    {
      ...
      catch (NoSuchSkillException nse)
      {
        logger.log(Level.WARNING, performer
          .getName() + " trying to mine with an item with no primary skill: " + source
          .getName());
      }
    }
    if (dir == 0)
    {
      ...
      for (int x = 0; x >= -1; x--) {
        for (y = 0; y >= -1; y--)
        {
          vt = Zones.getTileOrNull(digTilex + x, digTiley + y, false);
          if ((vt != null) && (vt.getStructure() != null))
          {
    ...
    if ((dir == 0) && (tileType == Tiles.Tile.TILE_CAVE_FLOOR_REINFORCED.id))
    {
      ...
      if (actualType == Tiles.Tile.TILE_CAVE_EXIT.id) {
        ...
        Server.caveMesh.setTile(tilex, tiley, 
          Tiles.encode(Tiles.decodeHeight(meshTile), Tiles.Tile.TILE_CAVE.id, 
          Tiles.decodeData(meshTile)));
      }
    ...
    if (dir == 1)
    {
      ...
      if (!createOutside)
      {
        ...
        Server.caveMesh.setTile(digTilex, digTiley, 
        
          Tiles.encode(Tiles.decodeHeight(digCorner), 
          Tiles.decodeType(digCorner), (byte)(cceil + 1)));
    ...
    else
    {
      ...
      Server.caveMesh.setTile(digTilex, digTiley, Tiles.encode(
        (short)(Tiles.decodeHeight(digCorner) - 1), Tiles.decodeType(digCorner), (byte)(cceil + 1)));
    ...
    if (tool != null) {
      bonus = tool.skillCheck(diff, source, 0.0D, false, tickCounter) / 5.0D;
    }
    power = Math.max(1.0D, mining
      .skillCheck(diff, source, bonus, false, tickCounter));
    try
    {
      ...
      int max = Math.min(m, (int)(20.0D + r.nextInt(80) * imbueEnhancement));
      power = Math.min(power, max);
      if (source.isCrude()) {
        power = 1.0D;
      }
      float orePower = GeneralUtilities.calcOreRareQuality(power * modifier, act.getRarity(), source
        .getRarity());
  ...
  public static final boolean raiseRockLevel(Creature performer, Item source, int tilex, int tiley, float counter, Action act)
  {
    ...
    if ((counter == 1.0F) || (counter == 0.0F) || (act.justTickedSecond()))
    {
      ...
      if ((source.getTemplateId() != 176) && (source.getWeightGrams() < source.getTemplate().getWeightGrams()))
      {
        performer.getCommunicator().sendNormalServerMessage("The " + source
          .getName() + " contains too little material to be usable.");
        return true;
      }
    }
    boolean done = true;
    ...
    if ((h >= -25) || (source.getTemplateId() == 176))
    {
      ...
      if (counter == 1.0F)
      {
        time = (int)Math.max(30.0D, 100.0D - masonry.getKnowledge(source, 0.0D));
        try
        {
          performer.getCurrentAction().setTimeLeft(time);
        }
        ...
        performer.sendActionControl(Actions.actionEntrys['Ȇ'].getVerbString(), true, time);
      }
      else
      {
        try
      ...
      if ((counter * 10.0F > time) || (source.getTemplateId() == 176))
      {
        if (source.getTemplateId() != 176)
        {
          ...
          masonry.skillCheck(1.0D, source, 0.0D, false, counter);
          source.setDamage(source.getDamage() + 5.0E-4F * source.getDamageModifier());
        }
        done = true;
        if (performer.getLayer() < 0)
        {
          Server.caveMesh.setTile(tilex, tiley, 
            ...
            Tiles.encode((short)(Tiles.decodeHeight(tile) + 1), Tiles.decodeType(tile), 
            (byte)(CaveTile.decodeCeilingHeight(tile) - 1)));
          tile = Server.caveMesh.getTile(tilex, tiley);
        }
        else
        {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The water is too deep and would only dissolve the " + source
        .getName() + ".");
    }
    return done;
  }
  ...
  private boolean flatten(Creature performer, Item source, int tile, int tilex, int tiley, float counter, Action act, int dir)
  {
    ...
    if ((act.getNumber() == 532) && (dir == 0) && 
      (!Terraforming.isFlat(performer.getTileX(), performer.getTileY(), performer.isOnSurface(), 0)))
    {
      performer.getCommunicator().sendNormalServerMessage("You need to be standing on flat ground to be able to level.");
      return true;
    }
    if ((act.getNumber() == 532) && (dir == 1) && 
      (!isCeilingFlat(performer.getTileX(), performer.getTileY())))
    {
    ...
    if (dir == 0) {
      if ((Tiles.isReinforcedFloor(Tiles.decodeType(Server.caveMesh.getTile(tilex - 1, tiley - 1)))) || 
    ...
    if (counter == 1.0F)
    {
      ...
      for (int x = 0; x <= 1; x++) {
        for (int y = 0; y <= 1; y++)
        {
          short ht = getHeight(tilex + x, tiley + y, dir);
      ...
      if (dir == 1) {
        ...
        requiredHeight = getHeight(performer.getTileX(), performer.getTileY(), dir);
      }
      int totalup = 0;
      int totaldown = 0;
      for (int x = 0; x <= 1; x++) {
      ...
      if (((totalup > 0) && (dir == 0)) || ((totaldown > 0) && (dir == 1)))
      {
        ...
        int tickMining = Actions.getStandardActionTime(performer, mining, source, 0.0D) + 5;
        if (dir == 0) {
      ...
      if ((totaldown > 0) && (dir == 0))
      {
        ...
        int tickMasonry = Actions.getStandardActionTime(performer, masonry, null, 0.0D) + 5;
      ...
      if (!insta)
      {
        ...
        if (isBlocked(tilex, tiley, tilex, tiley, performer, act
          .getNumber(), counter, dir, h, cceil, digTile)) {
          return true;
        }
        ...
        Server.getInstance().broadCastAction(performer
          .getName() + " starts to " + action + " the " + gc + ".", performer, 5);
    ...
    if ((counter * 10.0F >= act.getNextTick()) || (insta))
    {
      ...
      for (int x = 0; x <= 1; x++) {
        for (int y = 0; y <= 1; y++)
        {
          int diff = getHeight(tilex + x, tiley + y, dir) - requiredHeight;
          if (dir == 0)
          {
            if (diff > highCorner)
            {
      ...
      if (highCorner != 0)
      {
        if ((dir == 0) && (anyReinforcedFloorTiles(highx, highy)))
        {
        ...
        if ((dir == 1) && (act.getNumber() == 150) && 
          (cceil > 60 + performer.getFloorLevel() * 30))
        {
        ...
        if ((dir == 1) && 
          (Tiles.decodeHeight(highTile) + cceil + 2 >= Tiles.decodeHeight(Server.rockMesh.getTile(highx, highy))))
        {
        ...
        int tickTime = Actions.getStandardActionTime(performer, mining, source, 0.0D);
        double bonus = 0.0D;
        double power = 0.0D;
        ...
        if (!insta)
        {
          ...
          if (tool != null) {
            bonus = tool.skillCheck(diff, source, 0.0D, false, tickTime / 10) / 5.0D;
          }
          power = Math.max(1.0D, mining.skillCheck(diff, source, bonus, false, tickTime / 10));
        }
        if (dir == 0) {
          Server.caveMesh.setTile(highx, highy, Tiles.encode(
            (short)(Tiles.decodeHeight(highTile) - 1), Tiles.decodeType(highTile), (byte)(cceil + 1)));
        } else {
          Server.caveMesh.setTile(highx, highy, Tiles.encode(
            Tiles.decodeHeight(highTile), Tiles.decodeType(highTile), (byte)(cceil + 1)));
        }
      ...
      else if (dir == 0)
      {
        ...
        if (lowCorner > 0)
        {
          if (anyReinforcedFloorTiles(lowx, lowy))
          {
            performer.getCommunicator().sendNormalServerMessage("You cannot modify one of the corners due to reinforced floors.");
          ...
          {
            Skill masonry = performer.getSkills().getSkillOrLearn(1013);
            int tickMasonry = Actions.getStandardActionTime(performer, masonry, null, 0.0D);
            try
            {
              ...
              if (concrete != null)
              {
                ...
                masonry.skillCheck(1.0D, source, 0.0D, false, tickMasonry / 10);
          ...
          Server.caveMesh.setTile(lowx, lowy, 
          
            Tiles.encode((short)(Tiles.decodeHeight(lowTile) + 1), Tiles.decodeType(lowTile), 
            (byte)(CaveTile.decodeCeilingHeight(lowTile) - 1)));
      ...
      if (!done)
      {
        ...
        {
          Skill mining = performer.getSkills().getSkillOrLearn(1008);
          tickNextTime = Actions.getStandardActionTime(performer, mining, source, 0.0D);
        }
        else
        {
          Skill masonry = performer.getSkills().getSkillOrLearn(1013);
          tickNextTime = Actions.getStandardActionTime(performer, masonry, null, 0.0D);
        }
        act.incTickCount();
  ...
  private static boolean anyReinforcedFloors(Creature performer)
  {
    ...
    return anyAdjacentReinforcedFloors(digTilex, digTiley, false);
  }
  ...
  {
    short cht = getRealCeilingHeight(tilex, tiley);
    if (cht != getRealCeilingHeight(tilex + 1, tiley)) {
      return false;
    }
    if (cht != getRealCeilingHeight(tilex + 1, tiley + 1)) {
      return false;
    }
    if (cht != getRealCeilingHeight(tilex, tiley + 1)) {
      return false;
    }
    return true;
  }
  ...
  private short getHeight(int tilex, int tiley, int dir)
  {
    ...
    return getRealCeilingHeight(tilex, tiley);
  }
  ...
  private boolean changeFloor(Action act, Creature performer, Item source, int tilex, int tiley, int tile, short action, float counter)
  {
    ...
    if (pavingItem == 492)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("It takes 2kg of " + source
          .getName() + " to prepare the " + floorexit + ".");
        return true;
      }
    }
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("The amount of " + source
    ...
    if (counter == 1.0F)
    {
      ...
      String prepared = (repaving ? "repave the " : "pave the prepared ") + floorexit;
      if (pavingItem == 519) {
    ...
    String prepared = pavingItem == 97 ? "back to plain reinforcement" : pavingItem == 492 ? "prepared" : "paved";
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\CaveTileBehaviour.class
