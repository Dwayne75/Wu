  ...
  implements MiscConstants
{
  ...
  public static short checkForSpecialFish(Creature performer, int tilex, int tiley)
  {
    ...
    if (canCatch.size() == 1) {
      return ((Integer)canCatch.get(0)).shortValue();
    }
    int randomfish = Server.rand.nextInt(canCatch.size());
    return ((Integer)canCatch.get(randomfish)).shortValue();
  }
  ...
  public static ArrayList<Integer> getSpecialFishList(Creature performer, int tilex, int tiley)
  {
    ...
    for (Point point : points) {
      if (performer.isWithinTileDistanceTo(point.getX(), point.getY(), 0, 5)) {
        canCatch.add(Integer.valueOf(point.getH()));
      }
    }
    return canCatch;
  }
  ...
  public static Point[] getRareSpots(int tilex, int tiley)
  {
    ...
    return (Point[])rareSpots.toArray(new Point[rareSpots.size()]);
  }
  ...
  private static boolean isFishSpotValid(Point point)
  {
    ...
    if (Tiles.decodeHeight(tile) < 65286) {
      return true;
    }
    return false;
  }
  ...
  {
    float toreturn = getDamModFor(f.getTemplateId()) * Math.max(0.1F, f.getWeightGrams() / 3000);
  ...
  public static double getMeatFor(Item fish)
  {
    ...
    double weight = fish.getTemplate().getWeightGrams() * ql;
    return weight;
  }
  ...
  static boolean fish(Creature performer, Item source, int tilex, int tiley, int tile, float counter, Action act)
  {
    ...
    if ((source.getTemplateId() == 23) || (source.getTemplateId() == 780))
    {
      performer.getCommunicator().sendNormalServerMessage("The " + source
        .getName() + " will do a poor job of catching fish.");
      done = true;
    }
    if ((source.getTemplateId() != 94) && (source.getTemplateId() != 152) && 
        ...
        if (number < 2000)
        {
          if ((act.currentSecond() * 10 > number) || (source.getTemplateId() == 176))
          {
            ...
            if (waterFound > 1) {
              if (waterFound < 9) {
                waterType = 20000;
              } else if ((waterFound > 20) && (maxDepth < 65286)) {
                waterType = 5000;
              } else {
                waterType = 7500;
              }
          ...
          else if ((act.justTickedSecond()) && (act.currentSecond() % 2 == 0))
          {
            ...
            fishing.skillCheck(50.0D, source, 0.0D, false, 2.0F);
          }
        }
        else if ((act.justTickedSecond()) && (act.currentSecond() % 2 == 0))
        {
          ...
          if (waterType != 0)
          {
            ...
            if (fishOnHook == 0)
            {
              double result = fishing.skillCheck(waterType / 250.0F, source, bonus, false, 1.0F);
              if (result > 0.0D)
              {
                int perfTileX = performer.getTileX();
                int perfTileY = performer.getTileY();
                
                fishOnHook = getFishFor(performer, (short)waterType, (short)(int)result, perfTileX, perfTileY, source);
                try
                {
                  if (act.getRarity() != 0) {
                    performer.playPersonalSound("sound.fx.drumroll");
                  }
                  double fishresult = 10.0D + 0.9D * Math.max(Server.rand.nextFloat() * 10.0F, fishing
                    .skillCheck(getDifficultyFor(fishOnHook), source, bonus, true, 1.0F));
                  
                  fish = ItemFactory.createItem(fishOnHook, (float)fishresult, (byte)2, act
                    .getRarity(), null);
                  ...
                  if (weight <= 50)
                  {
                    ...
                    Server.getInstance().broadCastAction(performer
                      .getName() + "'s line twitches but nothing is on it.", performer, 5);
                    done = true;
                  ...
                  else
                  {
                    ...
                    Server.getInstance().broadCastAction("Something bites on " + performer
                      .getName() + "'s hook.", performer, 5);
                    performer.getCurrentAction().setDestroyedItem(fish);
                  }
                }
                catch (NoSuchTemplateException nse)
                {
                  logger.log(Level.INFO, "Failed to create item with id " + fishOnHook + " for performer " + performer
                    .getName(), nse);
                }
              ...
              if (((act.currentSecond() % 5 == 0) || (source.getTemplateId() == 176)) && (act.justTickedSecond())) {
                if (result > 0.0D)
                {
                  ...
                  if (source.getTemplateId() != 176) {
                    source.setDamage(source.getDamage() + 
                      Math.min(
                      (100.0F - source.getCurrentQualityLevel()) / 50.0F, 
                      Math.max(0.1F, source
                      
                      .getDamageModifier(false, true) / (10.0F * 
                      Math.max(10.0F, source.getQualityLevel()))) * 
                      getDamModFor(fish)));
                  }
                  ...
                  performer.getCommunicator().sendNormalServerMessage("You catch " + fish
                    .getNameWithGenus() + ".");
                  Server.getInstance().broadCastAction(performer
                    .getName() + " catches " + fish.getNameWithGenus() + ".", performer, 5);
                  fishOnHook = 0;
                  if ((fish.getTemplateId() == 570) || 
                    (fish.getTemplateId() == 571))
                  {
                    result = fishing.skillCheck(difficulty, source, bonus, false, 1.0F);
                    if ((result < 0.0D) && (performer.getPower() <= 1)) {
                      try
                      {
                        performer.getCommunicator().sendNormalServerMessage("The " + fish
                          .getName() + " bites you!");
                        Server.getInstance().broadCastAction(performer
                          .getName() + " is bit by the " + fish.getName() + "!", performer, 5);
                        
                        performer.addWoundOfType(null, (byte)3, 0, true, 1.0F, false, 
                          Math.max(3000, fish.getWeightGrams() / 10), 0.0F, 3.0F, false, false);
                      }
                      catch (Exception ex)
                      {
                ...
                {
                  if ((result < -80.0D) && (!source.isWand()) && 
                    (Server.rand.nextInt(Math.max(1, 30 - (int)fishing.getKnowledge(0.0D))) == 0))
                  {
                    done = true;
                    performer.getCommunicator().sendNormalServerMessage("The line snaps, and the fish escapes!");
                    
                    Server.getInstance().broadCastAction(performer
                      .getName() + "s line snaps and the fish escapes.", performer, 5);
                  ...
                  else if (result < -60.0D)
                  {
                    ...
                    source.setDamage(source.getDamage() + 
                      ...
                      Math.min(
                      (100.0F - source.getCurrentQualityLevel()) / 100.0F, source
                      .getDamageModifier() / (10.0F * 
                      Math.max(10.0F, source.getQualityLevel()))) * damMod));
                  }
                }
                else
                {
                  ...
                  if (mess == 0)
                  {
                    ...
                    Server.getInstance().broadCastAction("The fish pulls hard on " + performer
                      .getName() + "'s rod and bends it.", performer, 5);
                  }
                  else if (mess == 1)
                  {
                    ...
                    Server.getInstance().broadCastAction(performer
                      .getName() + " pulls in the line a bit.", performer, 5);
                  }
                  else if (mess == 2)
                  {
                    performer.getCommunicator().sendNormalServerMessage("The water splashes as the fish fights in the water.");
                    
                    Server.getInstance().broadCastAction("The water splashes as " + performer
                      .getName() + "'s fish fights in the water", performer, 5);
                  }
                }
              }
            }
          ...
          {
            fishing.skillCheck(50.0D, source, 0.0D, false, 1.0F);
          }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Fish.class
