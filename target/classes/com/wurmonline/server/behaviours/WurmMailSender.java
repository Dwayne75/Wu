            ...
            if ((mailbox.hasDarkMessenger()) || (mailbox.hasCourier()))
            {
              ...
              if (containedItems.length != 0)
              {
                for (Item lContainedItem : containedItems) {
                  if ((lContainedItem.isNoDrop()) || (lContainedItem.isArtifact()) || (lContainedItem.isBodyPart()) || 
                    (lContainedItem.isTemporary()) || (lContainedItem.isLiquid()))
                  {
                    performer.getCommunicator().sendNormalServerMessage("You may not send the " + lContainedItem
                      .getName() + ".");
                    ok = false;
                  }
                  else if (lContainedItem.lastOwner != performer.getWurmId())
                  {
                    performer.getCommunicator().sendNormalServerMessage("You must possess the " + lContainedItem
                      .getName() + " in order to send it.");
                    ok = false;
                  }
                }
                if (ok) {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\WurmMailSender.class
