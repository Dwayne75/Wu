  ...
  implements MiscConstants, ActionTypes
{
  ...
  public static final int getStandardActionTime(Creature performer, Skill skill, @Nullable Item source, double bonus)
  {
    double knowl = 1.0D;
    if (source == null) {
      knowl = skill.getKnowledge(0.0D);
    } else {
      knowl = skill.getKnowledge(source, 0.0D);
    }
    float multiplier = 1.3F / Servers.localServer.getActionTimer();
    double time = (100.0D - knowl) * multiplier;
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = 30.0D + time * (1.0D - 0.2D * source.getSpellSpeedBonus() / 100.0D);
    } else {
      time = 30.0D + time;
    }
    if (source != null)
  ...
  public static final int getQuickActionTime(Creature performer, Skill skill, @Nullable Item source, double bonus)
  {
    ...
    double time = (100.0D - knowl) / Servers.localServer.getActionTimer();
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = 30.0D + time * (1.0D - 0.5D * source.getSpellSpeedBonus() / 100.0D);
    } else {
      time = 30.0D + time;
    }
    if (source != null)
  ...
  public static final int getVariableActionTime(Creature performer, Skill skill, @Nullable Item source, double bonus, int max, int min, int staminaNeeded)
  {
    ...
    float multiplier = (max - min) / 100.0F / Servers.localServer.getActionTimer();
    double time = (100.0D - skillVal) * multiplier;
    if (source != null)
    {
  ...
  public static final int getSlowActionTime(Creature performer, Skill skill, @Nullable Item source, double bonus)
  {
    ...
    double time = Math.max(20.0D, (100.0D - knowl) * multiplier);
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = 25.0D + time * (1.0D - 0.5D * source.getSpellSpeedBonus() / 100.0D);
    } else {
      time = 25.0D + time;
    }
    if (skill.getNumber() == 1008) {
      time = Math.min(250.0D, time);
    }
    if (source != null)
    {
      time -= getRarityTimeModifier(source.getRarity());
  ...
  public static final int getPickActionTime(Creature performer, Skill skill, Item source, double bonus)
  {
    ...
    int time = (int)(Math.max(300.0D, (100.0D - knowl) * 5.0D) / Servers.localServer.getActionTimer());
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = (int)Math.max(200.0D, time * (1.0D - 0.5D * source.getSpellSpeedBonus() / 100.0D));
    }
    if (source != null)
    {
      time = (int)(time - getRarityTimeModifier(source.getRarity()) * 10.0F);
      if (source.getSpellEffects() != null)
      {
        float modifier = 1.0F / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
    ...
    time = (int)(time * getStaminaModiferFor(performer, 5000));
  ...
  public static final int getPlantActionTime(Creature performer, Item item)
  {
    ...
    else if (item.isRoadMarker())
    {
      ...
      double knowledge = paving.getKnowledge(0.0D);
      int newTime = (int)(120.0D - knowledge);
  ...
  public static final boolean canReceiveDeedSpeedBonus(Creature performer)
  {
    ...
    return (currentVillage != null) && (currentVillage.isActionAllowed((short)233, performer)) && 
  ...
  public static final int getImproveActionTime(Creature performer, Item source)
  {
    ...
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = (int)Math.max(50.0D, time * (1.0D - 0.5D * source.getSpellSpeedBonus() / 100.0D));
    }
    if (source != null)
    {
      time = (int)(time - getRarityTimeModifier(source.getRarity()) * 10.0F);
    ...
    time = (int)(time * getStaminaModiferFor(performer, 20000));
    time = Math.min(65535, time);
  ...
  {
    int time = (int)(80.0D * (1.0D + (100.0D - skill.getKnowledge(bonus)) / 100.0D));
    time = (int)(time * getStaminaModiferFor(performer, 20000));
    time = (int)(time / Servers.localServer.getActionTimer());
  ...
  public static final int getDestroyActionTime(Creature performer, Skill skill, Item source, double bonus)
  {
    ...
    int time = (int)(Math.max(300.0D, (100.0D - knowl) * 5.0D) / Servers.localServer.getActionTimer());
    if ((source != null) && (source.getSpellSpeedBonus() > 0.0F)) {
      time = (int)(Math.max(50.0D, time * (1.0D - 0.5D * source.getSpellSpeedBonus() / 100.0D)) / Servers.localServer.getActionTimer());
    }
    if (source != null)
    {
      time = (int)(time - getRarityTimeModifier(source.getRarity()));
      if (source.getSpellEffects() != null)
      {
        float modifier = 1.0F / source.getSpellEffects().getRuneEffect(RuneUtilities.ModifierEffect.ENCH_USESPEED);
        time = (int)(time * modifier);
  ...
  public static final int getItemCreationTime(int startTime, Creature performer, Skill primSkill, CreationEntry entry, Item realSource, Item realTarget, boolean isMassProduction)
    throws NoSuchTemplateException
  {
    float standardTime = 100.0F / Servers.localServer.getActionTimer();
    
    float difficultyTimeMod = Math.max(0.9F, 
      (startTime - entry.getDifficultyFor(realTarget, realSource, performer)) / 100.0F);
    
    float minTime = 60.0F;
    if (isMassProduction) {
      minTime = 50.0F;
    }
    double time = minTime + (standardTime - primSkill.getKnowledge(realSource, 0.0D)) * difficultyTimeMod;
    if ((realSource != null) && (realSource.getSpellSpeedBonus() > 0.0F)) {
      time *= (1.0D - 0.2D * realSource.getSpellSpeedBonus() / 100.0D);
    } else if ((realTarget != null) && (realTarget.getSpellSpeedBonus() > 0.0F)) {
      time *= (1.0D - 0.2D * realTarget.getSpellSpeedBonus() / 100.0D);
    }
    if (realTarget != null)
    {
    ...
    time *= getStaminaModiferFor(performer, 20000);
    time = Math.min(65535.0D, time);
    time = Math.max(15.0D, time);
    return (int)time;
  }
  
  public static final int getRecipeCreationTime(int startTime, Creature performer, Skill primSkill, Recipe entry, Item realSource, Item realTarget, boolean isMassProduction)
    throws NoSuchTemplateException
  {
    float standardTime = 100.0F / Servers.localServer.getActionTimer();
    
    float difficultyTimeMod = Math.max(0.9F, 
      (startTime - entry.getChanceFor(realSource, realTarget, performer)) / 100.0F);
  ...
  public static final boolean isMultipleItemAction(short action, long[] targets)
  {
    ...
    if ((action == 7) || (action == 638) || (action == 59) || (action == 86)) {
      return true;
    }
    return false;
  }
  ...
  {
    return (action == 116) || (action == 231) || (action == 232) || (action == 363) || (action == 169) || (action == 170) || (action == 516) || (action == 517) || (action == 521) || 
  ...
  {
    return (action == 85) || ((action == 863) && 
      (Features.Feature.WAGONER.isEnabled()));
  }
  ...
  public static final short getDefaultActionForTarget(long actionItemId, long target)
  {
    ...
    case 3: 
      ...
      if (actionItem.isPresent())
      {
        Item actionTool = (Item)actionItem.get();
    ...
    case 2: 
      ...
      {
        Item targetItem = (Item)targetOptional.get();
        Optional<Item> actionItem = Items.getItemOptional(actionItemId);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Actions.class
