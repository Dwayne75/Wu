      ...
      if (creature != null)
      {
        ...
        if (this.type == 0)
        {
          creature.getCommunicator().sendNormalServerMessage(
            StringUtil.format("You %s on the %s as the %s.", new Object[] { vehicle.embarkString, vehicleName, vehicle.pilotName }));
          
          Server.getInstance().broadCastAction(
            StringUtil.format("%s %s on the %s as the %s.", new Object[] {creature
            .getName(), vehicle.embarksString, vehicleName, vehicle.pilotName }), creature, 5);
        }
        else if ((vehicle.isChair()) || (vehicle.isBed()))
        {
          creature.getCommunicator().sendNormalServerMessage(
            StringUtil.format("You %s on the %s.", new Object[] { vehicle.embarkString, vehicleName }));
          
          Server.getInstance().broadCastAction(
            StringUtil.format("%s %s on the %s.", new Object[] {creature
            .getName(), vehicle.embarksString, vehicleName }), creature, 5);
        }
        else
        {
          creature.getCommunicator().sendNormalServerMessage(
            StringUtil.format("You %s on the %s as a passenger.", new Object[] { vehicle.embarkString, vehicleName }));
          
          Server.getInstance().broadCastAction(
            StringUtil.format("%s %s on the %s as a passenger.", new Object[] {creature
            .getName(), vehicle.embarksString, vehicleName }), creature, 5);
        }
        return true;
      }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Seat.class
