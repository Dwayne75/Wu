  ...
  extends ItemBehaviour
{
  ...
  public static final boolean mayUseItem(Item target, @Nullable Creature performer)
  {
    ...
    if (target.getTemplateId() == 339)
    {
      if (WurmCalendar.currentTime - target.getData() < 28800L)
      {
        if (performer != null) {
          performer.getCommunicator().sendNormalServerMessage("The " + target
            .getName() + " is still fading from the last activation.");
        }
        return false;
      }
      return true;
  ...
  public static final void spawnCreature(int deity, float posx, float posy, boolean onSurface, byte kingdom)
  {
    ...
    try
    {
      ...
      Creature cret = Creature.doNew(creatureTemplate, posx, posy, Server.rand.nextInt(360), onSurface ? 0 : -1, "", ctemplate
        .getSex(), kingdom);
      cret.setDeity(Deities.getDeity(deity));
    }
    catch (Exception ex)
    {
      logger.log(Level.WARNING, "Problem spawning new Creature with Template ID: " + creatureTemplate + ", at position: " + posx + ", " + posy + ", kingdom: " + 
      
        Kingdoms.getNameFor(kingdom) + ", deity: " + deity + " due to " + ex
        .getMessage(), ex);
    }
  }
  ...
  public static final boolean useItem(@Nullable Item source, Item target, Creature performer, float counter)
  {
    ...
    Server.getInstance().broadCastAction(performer
      .getName() + " uses " + performer.getHisHerItsString() + " " + target.getName() + "!", performer, 5);
    if (target.getTemplateId() == 332)
    {
      ...
      LocatePlayerQuestion lpq = new LocatePlayerQuestion(performer, "Locate a soul", "Which soul do you wish to locate?", target.getWurmId(), true, 100.0D);
      lpq.sendQuestion();
    ...
    else if (target.getTemplateId() == 330)
    {
      if (Server.rand.nextInt(8) == 0) {
        spawnCreature(2, performer.getPosX() - 4.0F + Server.rand.nextFloat() * 8.0F, performer
          .getPosY() - 4.0F + Server.rand.nextFloat() * 8.0F, performer.isOnSurface(), performer
          .getKingdomId());
      }
    ...
    {
      if (Server.rand.nextInt(8) == 0) {
        spawnCreature(1, performer.getPosX() - 4.0F + Server.rand.nextFloat() * 8.0F, performer.getPosY() - 4.0F + Server.rand
          .nextFloat() * 8.0F, performer.isOnSurface(), performer.getKingdomId());
      }
      ...
      if (v == null)
      {
        if (Server.rand.nextInt(2) == 0)
        {
          ...
          if (item != null)
          {
            ...
            performer.getCommunicator().sendNormalServerMessage("You shake the ear, and out pops " + item
              .getNameWithGenus() + "!");
        ...
        {
          spawnCreature(3, performer.getPosX() - 4.0F + Server.rand.nextFloat() * 8.0F, performer
            .getPosY() - 4.0F + Server.rand.nextFloat() * 8.0F, performer.isOnSurface(), performer
            .getKingdomId());
        ...
        performer.getCommunicator().sendNormalServerMessage("You shake the ear, but nothing happens here at " + v
          .getName() + ".");
      }
    }
    ...
    else if (target.getTemplateId() == 336)
    {
      try
      {
        if (performer.getDeity() != null)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You feel the strength of " + 
            Deities.getDeity(2).name + " within you.");
        }
        else
        {
    ...
    else if (target.getTemplateId() == 338)
    {
      if (Server.rand.nextInt(8) == 0)
      {
        spawnCreature(4, performer.getPosX() - 4.0F + Server.rand.nextFloat() * 8.0F, performer.getPosY() - 4.0F + Server.rand
          .nextFloat() * 8.0F, performer.isOnSurface(), performer.getKingdomId());
        target.setAuxData((byte)(target.getAuxData() - 1));
      }
      ...
      {
        target.setAuxData((byte)(target.getAuxData() - 1));
        Phantasms.doImmediateEffect(100.0D, performer);
      }
    ...
    else if (target.getTemplateId() == 339)
    {
                ...
                if (vtile != null)
                {
                  ...
                  for (int c = 0; c < crets.length; c++) {
                    if ((!crets[c].isUnique()) && (!crets[c].isInvulnerable()))
                    {
                      ...
                      if (soul.skillCheck(crets[c].isChampion() ? 40.0D : 30.0D, 0.0D, false, 1.0F) < 0.0D)
                      {
                      ...
                      else
                      {
                        ...
                        crets[c].getCommunicator()
                          .sendSafeServerMessage("A sudden pain enters your body! You grit your teeth and escape to the darkest, innermost corner of your soul. There you barely escape death.");
                      }
                    }
                    else
                    {
      ...
      else
      {
        ...
        if (soul.skillCheck(performer.isChampion() ? 30.0D : 25.0D, 0.0D, false, 1.0F) < 0.0D)
        {
          performer.getCommunicator().sendAlertServerMessage("A sudden pain enters your body.");
          performer.die(false, target.getName());
          return true;
        }
        Server.getInstance().broadCastAction("A pulse surges through the air. " + performer
          .getName() + " activates the " + target.getName() + "!", performer, 15);
    ...
    else if (target.getTemplateId() == 340)
    {
      try
      {
        ...
        if (performer.getDeity() != null)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You feel the power of " + 
            Deities.getDeity(4).name + " descend on you.");
    ...
    else if (target.getTemplateId() == 329)
    {
      try
      {
        ...
        if (performer.getDeity() != null)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You feel the light of " + 
            Deities.getDeity(1).name + " shine on you.");
      ...
      if ((mark) && (p.isWithinDistanceTo(performer, 24.0F)))
      {
        ...
        if (p.isMarkedByOrb())
        {
          ...
          if (deal)
          {
            ...
            p.addWoundOfType(performer, (byte)6, 21, false, 1.0F, false, 30000.0D, 0.0F, 50.0F, false, false);
          }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\ArtifactBehaviour.class
