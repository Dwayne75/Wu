  ...
  implements ItemTypes, ActionTypes, SoundNames, MiscConstants
{
  ...
  static void emoteAt(short emote, Creature performer, Creature receiver)
  {
    ...
    else if (emote == 2030)
    {
      ...
      performerString = StringUtil.format("You tell %s to follow.", new Object[] { rname });
      receiverString = StringUtil.format("%s tells you to follow.", new Object[] { pname });
      bcastString = StringUtil.format("%s tells %s to follow %s.", new Object[] { pname, rname, performer.getHimHerItString() });
    }
    else if (emote == 2031)
    {
      emoteSound = "sound.emote.goodbye";
      performerString = StringUtil.format("You say goodbye to %s.", new Object[] { rname });
      receiverString = StringUtil.format("%s says goodbye to you.", new Object[] { pname });
      bcastString = StringUtil.format("%s says goodbye to %s.", new Object[] { pname, rname });
    }
    else if (emote == 2032)
    {
      emoteSound = "sound.emote.lead";
      performerString = StringUtil.format("You ask %s to lead the way.", new Object[] { rname });
      receiverString = StringUtil.format("%s asks you to lead the way.", new Object[] { pname });
      bcastString = StringUtil.format("%s asks %s to lead the way.", new Object[] { pname, rname });
    }
    else if (emote == 2033)
    {
      emoteSound = "sound.emote.that.way";
      performerString = StringUtil.format("You tell %s to go that way.", new Object[] { rname });
      receiverString = StringUtil.format("%s tells you to go that way.", new Object[] { pname });
      bcastString = StringUtil.format("%s tells %s to go that way.", new Object[] { pname, rname });
    }
    else if (emote == 2034)
    {
      emoteSound = "sound.emote.wrong.way";
      performerString = StringUtil.format("You tell %s to not go that way.", new Object[] { rname });
      receiverString = StringUtil.format("%s tells you to not go that way.", new Object[] { pname });
      bcastString = StringUtil.format("%s tells %s to not go that way.", new Object[] { pname, rname });
    }
    else if (emote == 2004)
    {
    ...
    if (receiver.hasTrait(10)) {
      if ((Server.rand.nextInt(6) == 0) || (Servers.localServer.testServer))
      {
        ...
        Server.getInstance().broadCastAction(bcastString, performer, receiver, 5);
        performer.getCommunicator().sendNormalServerMessage(performerString);
        if (performer.isWithinDistanceTo(receiver.getPosX(), receiver.getPosY(), receiver
          .getPositionZ() + receiver.getAltOffZ(), 5.0F, 0.0F)) {
          if (Blocking.getBlockerBetween(receiver, performer, 5) == null) {
            try
            {
              CombatEngine.addWound(receiver, performer, (byte)3, performer
                .getBody().getRandomWoundPos(), 4000.0D, performer.getArmourMod(), "bite", null, 0.0F, 0.0F, false, false, false, false);
            }
            catch (Exception ex)
            {
  ...
  private static void emoteAt(short emote, Creature performer, String pname, String rname, @Nullable Item item, boolean isBodyPart)
  {
    ...
    else if (emote == 2030)
    {
      ...
      if (isBodyPart)
      {
        performerString = StringUtil.format("You tell %s %s to follow.", new Object[] { ownerString, item.getName() });
        bcastString = StringUtil.format("%s tells %s %s to follow %s lead.", new Object[] { pname, possessionString, item.getName(), performer
          .getHisHerItsString() });
      }
      else
      {
        performerString = StringUtil.format("You command %s to follow.", new Object[] { detString });
        bcastString = StringUtil.format("%s commands %s to follow %s.", new Object[] { pname, rname, performer.getHimHerItString() });
      }
    }
    else if (emote == 2031)
    {
      ...
      if (isBodyPart)
      {
        performerString = StringUtil.format("You say goodbye to %s.", new Object[] { item.getName() });
        bcastString = StringUtil.format("%s say goodbye to %s.", new Object[] { pname, item.getName() });
      }
      else
      {
        performerString = StringUtil.format("You say goodbye to %s.", new Object[] { detString });
        bcastString = StringUtil.format("%s say goodbye to %s.", new Object[] { pname, rname });
      }
    }
    else if (emote == 2032)
    {
      ...
      if (isBodyPart)
      {
        performerString = StringUtil.format("You tell %s to lead the way.", new Object[] { item.getName() });
        bcastString = StringUtil.format("%s tells %s to lead the way.", new Object[] { pname, item.getName() });
      }
      else
      {
        performerString = StringUtil.format("You tell %s to lead the way.", new Object[] { detString });
        bcastString = StringUtil.format("%s tells %s to lead the way.", new Object[] { pname, rname });
      }
    }
    else if (emote == 2033)
    {
      ...
      if (isBodyPart)
      {
        performerString = StringUtil.format("You tell %s to go that way.", new Object[] { item.getName() });
        bcastString = StringUtil.format("%s tells %s to go that way.", new Object[] { pname, item.getName() });
      }
      else
      {
        performerString = StringUtil.format("You tell %s to go that way.", new Object[] { detString });
        bcastString = StringUtil.format("%s tells %s to go that way.", new Object[] { pname, rname });
      }
    }
    else if (emote == 2034)
    {
      ...
      if (isBodyPart)
      {
        performerString = StringUtil.format("You tell %s to not go that way.", new Object[] { item.getName() });
        bcastString = StringUtil.format("%s tells %s to not go that way.", new Object[] { pname, item.getName() });
      }
      else
      {
        performerString = StringUtil.format("You tell %s to not go that way.", new Object[] { detString });
        bcastString = StringUtil.format("%s tells %s to not go that way.", new Object[] { pname, rname });
      }
    }
    else if (emote == 2007)
  ...
  static void emote(short emote, Creature performer)
  {
    ...
    else if (emote == 2030)
    {
      ...
      bcastString = StringUtil.format("%s commands someone or something to follow %s.", new Object[] { pname, performer.getHimHerItString() });
    }
    else if (emote == 2031)
    {
      performerString = "You stare vacantly into the sky and whisper goodbye.";
      bcastString = StringUtil.format("%s stares vacantly into the sky and whispers goodbye.", new Object[] { pname });
    }
    else if (emote == 2032)
    {
      performerString = "You gaze into the sky and ask the gods for guidance.";
      bcastString = StringUtil.format("%s turns %s gaze into the sky and asks for guidance.", new Object[] { pname, performer
        .getHisHerItsString() });
    }
    else if (emote == 2033)
    {
      performerString = "Yes this is the way, I'm sure of it. You mumble quietly.";
      bcastString = StringUtil.format("%s mumbles something.", new Object[] { pname });
    }
    else if (emote == 2034)
    {
      performerString = "With tear filled eyes you look around you, you were so sure this was the way home.";
      bcastString = StringUtil.format("%s looks around with tear filled eyes.", new Object[] { pname });
    }
    else if (emote == 2006)
    {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Emotes.class
