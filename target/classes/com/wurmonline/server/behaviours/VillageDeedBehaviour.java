  ...
  extends ItemBehaviour
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item target)
  {
    ...
    toReturn.addAll(getBehavioursForPapers(performer, target));
    return toReturn;
  }
    ...
    if (ownerId == performer.getWurmId())
    {
      ...
      if (villageId <= 0)
      {
        if ((target.getTemplateId() != 663) && (target.getTemplateId() != 862))
        {
          ...
          toReturn.add(Actions.actionEntrys['ǒ']);
  ...
  public boolean action(Action act, Creature performer, Item target, short action, float counter)
  {
      ...
      if (!target.isNewDeed())
      {
        if (target.isVillageDeed()) {
          try
          {
            ...
            performer.getCommunicator().sendNormalServerMessage("This is the village deed for " + village
              .getName() + ". You should replace it with the new version.");
          }
          ...
          try
          {
            ...
            performer.getCommunicator().sendNormalServerMessage("This is the homestead deed for " + stead
              .getName() + ". You should replace it with the new version.");
          }
          catch (NoSuchVillageException nss)
          {
        ...
        {
          Village village = Villages.getVillage(target.getData2());
          performer.getCommunicator().sendNormalServerMessage("This is the settlement deed for " + village
            .getName() + ".");
        }
        catch (NoSuchVillageException nss)
        {
    ...
    else if (action == 77)
    {
      ...
      if (checkPapersOk(performer, target)) {
    ...
    {
      if ((!target.isNewDeed()) && (!Servers.localServer.testServer) && 
        (target.getTemplateId() != 862))
      {
        ...
        if (target.isOldDeed())
        {
          ...
          else if (performer.getWurmId() == target.getOwnerId())
          {
            ...
            if (left > 0L)
            {
              ...
              if (!lsw.addMoney(performer.getWurmId(), performer.getName(), left, "Refund " + target
                .getWurmId())) {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\VillageDeedBehaviour.class
