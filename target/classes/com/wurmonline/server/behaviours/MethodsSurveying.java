  ...
  implements MiscConstants
{
  public static boolean planBridge(Action act, Creature performer, Item source, Creature helper, @Nullable Item target, short action, float counter)
  {
    if (!Features.Feature.CAVE_BRIDGES.isEnabled())
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage(helper
          .getName() + " must be on the surface to help you plan a bridge.");
        return true;
      }
    }
    ...
    if (performer.getBuildingId() != -10L)
    {
      ...
      try
      {
        ...
        if (plannedStructure.isTypeBridge()) {
          performer.getCommunicator().sendNormalServerMessage("You cannot design a bridge as your mind keeps reverting back to the bridge \"" + plannedStructure
          
            .getName() + "\" that you are currently constructing.");
        } else if (plannedStructure.isFinalized()) {
          performer.getCommunicator().sendNormalServerMessage("You cannot design a bridge as your mind keeps reverting back to the house \"" + plannedStructure
    ...
    if (counter == 1.0F)
    {
      ...
      performer.sendActionControl(Actions.actionEntrys['ɽ'].getVerbString(), true, time);
    ...
    if ((helper != null) && (!insta) && (act.getTickCount() > 5) && (act.getTickCount() < 12) && 
      (!isHoldingRangePole(helper)))
    {
    ...
    if ((!done) && (act.currentSecond() % timePerStage == 0))
    {
      ...
      double rotRads = Math.atan2(performer.getStatus().getPositionY() - hY, performer
        .getStatus().getPositionX() - hX);
      ...
      case 1: 
        if (insta) {
          performer.getCommunicator().sendNormalServerMessage("Start(x,y,h):(" + start
            .getX() + "," + start.getY() + "," + start.getH() + ") End(x,y,h):(" + end
            .getX() + "," + end.getY() + "," + end.getH() + ")");
        }
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("The tile border to your " + 
            dirAsString(performer.getStatus().getDir()) + " is level");
      ...
      case 5: 
        ...
        else if ((target != null) && (!isBorderLevel(performer, target)))
        {
        ...
        {
          if (helper != null) {
            performer.getCommunicator().sendNormalServerMessage("The tile border to your helpers " + 
              dirAsString(helper.getStatus().getDir()) + " is level");
          } else {
            performer.getCommunicator().sendNormalServerMessage("The tile border to your rangepole's " + 
              dirAsString(getDir(performer, target)) + " is level");
          }
      ...
      case 14: 
        ...
        if (pMsg.length() == 0)
        {
          ans = PlanBridgeChecks.checkForBuildingPermissions(performer, performer.getTileX(), performer
            .getTileY(), insta, performer.isOnSurface());
          done = ans.failed();
          qMsg = ans.qMsg();
          pMsg = ans.pMsg();
          if (pMsg.length() == 0)
          {
            if (helper != null) {
              ans = PlanBridgeChecks.checkForBuildingPermissions(performer, helper.getTileX(), helper
                .getTileY(), insta, performer.isOnSurface());
            } else {
              ans = PlanBridgeChecks.checkForBuildingPermissions(performer, target.getTileX(), target
                .getTileY(), insta, performer.isOnSurface());
            }
            done = ans.failed();
            qMsg = ans.qMsg();
  ...
  public static boolean survey(Action act, Creature performer, Item source, Creature helper, @Nullable Item target, short action, float counter)
  {
    ...
    else
    {
      ...
      {
        act.setTimeLeft(time);
        performer.sendActionControl(Actions.actionEntrys['ʀ'].getVerbString(), true, time);
        performer.getCommunicator().sendNormalServerMessage("You carefully place the dioptra on its tripod in front of you.");
  ...
  private static boolean isHoldingRangePole(Creature target)
  {
    try
    {
      return (((Player)target).getCurrentAction().getNumber() == 636) && 
        (subjectItem(target).getTemplateId() == 901);
    }
  ...
  private static Item subjectItem(Creature target)
    throws NoSuchActionException, NoSuchItemException
  {
    long subject = ((Player)target).getCurrentAction().getSubjectId();
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\MethodsSurveying.class
