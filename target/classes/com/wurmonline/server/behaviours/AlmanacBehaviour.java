  ...
  extends ItemBehaviour
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item target)
  {
    ...
    toReturn.addAll(getBehavioursForAlmanac(performer, null, target));
    return toReturn;
  }
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item target)
  {
    ...
    toReturn.addAll(getBehavioursForAlmanac(performer, source, target));
    return toReturn;
  }
  
  public boolean action(Action act, Creature performer, Item target, short action, float counter)
  {
    boolean[] ans = almanacAction(act, performer, null, target, action, counter);
    if (ans[0] != 0) {
      return ans[1];
    }
  ...
  {
    boolean[] ans = almanacAction(act, performer, source, target, action, counter);
    if (ans[0] != 0) {
      return ans[1];
    }
          ...
          if (dv != null)
          {
            if ((Math.abs(dv.getCenterX() - performer.getTileX()) < 1) && (Math.abs(dv.getCenterY() - performer.getTileY()) < 1))
            {
              ...
              if (counter == 1.0F)
              {
                ...
                time = Actions.getVariableActionTime(performer, performer.getSkills().getSkillOrLearn(10069), null, 0.0D, 250, 100, 2500);
              ...
              {
                Item cache = FragmentUtilities.createVillageCache((Player)performer, target, dv, performer
                  .getSkills().getSkillOrLearn(10069));
                if (cache != null)
                {
            ...
            performer.getCommunicator().sendNormalServerMessage("Reading details from the report, " + dv.getDeedName() + " looks like it may have been " + dv
              .getDistanceFrom(performer.getTileX(), performer.getTileY()) + " to the " + dv
              .getDirectionFrom(performer.getTileX(), performer.getTileY()) + ".");
          }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\AlmanacBehaviour.class
