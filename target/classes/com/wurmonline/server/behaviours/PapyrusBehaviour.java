  ...
  extends ItemBehaviour
{
  ...
  List<ActionEntry> getPapyrusBehaviours(Creature performer, @Nullable Item source, Item target)
  {
    ...
    if (source != null) {
      if ((source.isContainerLiquid()) || (source.getTemplateId() == 14) || 
        (source.getTemplateId() == 272))
      {
        ...
        if (target.getInscription() == null)
        {
          ...
          if (recipe != null) {
            toReturn.add(new ActionEntry((short)742, "Write recipe \"" + recipe
              .getName() + "\"", "writing recipe"));
          }
          if (player.getStudied() > 0)
          {
            WurmHarvestables.Harvestable harvestable = WurmHarvestables.getHarvestable(player.getStudied());
            if (harvestable != null) {
              toReturn.add(new ActionEntry((short)853, "Record " + harvestable
                .getName() + " info", "Recording info"));
            }
          }
        }
      }
      else if ((source.getTemplateId() == 176) && (performer.getPower() > 2))
      {
        if (target.getInscription() == null)
        {
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item target)
  {
    ...
    toReturn.addAll(getPapyrusBehaviours(performer, null, target));
    return toReturn;
  }
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item target)
  {
    ...
    toReturn.addAll(getPapyrusBehaviours(performer, source, target));
    return toReturn;
  }
  
  public boolean action(Action act, Creature performer, Item target, short action, float counter)
  {
    return performPapyrusAction(act, performer, null, target, action, counter);
  }
  
  public boolean action(Action act, Creature performer, Item source, Item target, short action, float counter)
  {
    return performPapyrusAction(act, performer, source, target, action, counter);
  }
  ...
  boolean performPapyrusAction(Action act, Creature performer, @Nullable Item source, Item target, short action, float counter)
  {
    ...
    if (action == 505)
    {
      if (source == null)
      {
        performer.getCommunicator().sendNormalServerMessage("You fumble with the " + target
          .getName() + " but you cannot figure out how it works.");
        return true;
      }
      if (!target.canHaveInscription())
      {
      ...
      if ((inscriptionData != null) && ((performer.getPower() < 2) || (target.getAuxData() != 0))) {
        if (inscriptionData.hasBeenInscribed())
        {
          performer.getCommunicator().sendNormalServerMessage("This " + target
            .getName() + " has already been used by " + inscriptionData.getInscriber() + ".");
          return true;
        }
      }
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("This " + target
          .getName() + " has a waxy surface and therefore cannot be used to write on.");
        return true;
      }
      if (source.getTemplateId() == 14)
      {
        performer.getCommunicator().sendNormalServerMessage("You push your hand against the " + target
          .getName() + " in hopes of producing an imprint.");
        Server.getInstance().broadCastAction(performer
          .getName() + " presses a hand against the " + target.getName() + " in hopes of producing an imprint.", performer, 5);
        target.setInscription("The dirty hand of " + performer
          .getName() + " vaguely appears on the " + target.getName() + ".", performer
          .getName());
        return true;
      }
      if (source.getTemplateId() == 272)
      {
        performer.getCommunicator().sendNormalServerMessage("You push the bloody face of " + source
          .getName() + " against the " + target.getName() + " in hopes of producing an imprint.");
        
        Server.getInstance().broadCastAction(performer
          .getName() + " pushes the bloody face of " + source.getName() + " against the " + target.getName() + " in hopes of producing an imprint.", performer, 5);
        target.setInscription("The smeared bloody faceprint of " + source.getName() + " vaguely appears on the " + target
          .getName() + ".", performer.getName());
        return true;
      }
      ...
      for (Iterator<Item> it = source.getItems().iterator(); it.hasNext();)
      {
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("You cannot inscribe with " + contained
            .getName() + ". You need to use something special for writing on " + target.getName() + ".");
          return true;
        }
        liquid = contained;
      }
      if (liquid == null)
      {
        performer.getCommunicator().sendNormalServerMessage("You need a colorant to be able to inscribe with the " + source
          .getName() + " on the " + target
          .getName() + ".");
        return true;
      }
      short numChars = 0;
      ...
      case 749: 
        ...
        numChars = (short)(int)(numChars + targetQlBonus * 2.0F);
        break;
      case 5: 
        ...
        break;
      case 78: 
        numChars = (short)(int)(4.0D + 0.5D * source.getQualityLevel() + 0.5D * inkQlBonus + 0.5D * targetQlBonus);
        numChars = (short)(numChars + target.getRarity() * 10);
        break;
      case 653: 
        numChars = (short)(int)(4.0D + 0.5D * source.getQualityLevel() + 0.5D * inkQlBonus + 0.5D * targetQlBonus);
        ...
        break;
      case 76: 
        numChars = (short)(int)(2.0D + 0.5D * source.getQualityLevel() + 0.5D * inkQlBonus + 0.5D * targetQlBonus);
        numChars = (short)(numChars + target.getRarity() * 10);
        break;
      default: 
        numChars = 0;
      ...
      if (numChars <= 0)
      {
        performer.getCommunicator().sendNormalServerMessage("You would only make a mess with the " + source
          .getName() + " on the " + target.getName() + ". Try to use a more delicate tool when inscribing.");
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " starts to inscribe with " + source.getName() + " on " + target.getNameWithGenus() + ".", performer, 5);
    ...
    if (action == 185)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("It is made from " + 
        ...
        if (performer.getPower() >= 5) {
          performer.getCommunicator().sendNormalServerMessage("Zoneid=" + target
            ...
            WurmId.getNumber(target.getWurmId()) + " origin=" + 
    ...
    if (action == 1)
    {
      ...
      if ((inscriptionData != null) && (inscriptionData.hasBeenInscribed() == true))
      {
        ...
        if (target.getQualityLevel() > 20.0F) {
          if (inscriptionData.getInscriber() != null) {
    ...
    if (action == 742) {
      return writeRecipe(act, performer, source, target, action, counter, inscriptionData);
    }
    if (action == 853) {
      return recordInfo(act, performer, source, target, action, counter, inscriptionData);
    }
    if (action == 743)
    {
    ...
    if ((action == 118) && (source.getTemplateId() == 1254) && (target.canHaveInscription()))
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("This " + target
          .getName() + " has a waxy surface and does not need more wax.");
        return true;
      }
      if ((inscriptionData == null) && (target.getAuxData() == 0))
      {
        ...
        {
          int time = 50;
          performer.getCommunicator().sendNormalServerMessage("You start to wax the " + target
            .getActualName() + ".");
          Server.getInstance().broadCastAction(performer
            .getName() + " starts to wax something.", performer, 5);
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("You finish waxing the " + target
            .getActualName() + ".");
          Server.getInstance().broadCastAction(performer
            .getName() + " stops waxing.", performer, 5);
  ...
  private boolean writeRecipe(Action act, Creature performer, @Nullable Item source, Item target, short action, float counter, InscriptionData inscriptionData)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("This " + target
        .getName() + " has some study results on and therefore cannot be used again.");
      return true;
    }
    ...
    for (Iterator<Item> it = source.getItems().iterator(); it.hasNext();)
    {
      ...
      if (!isValidColorant(contained.getTemplateId()))
      {
        performer.getCommunicator().sendNormalServerMessage("You cannot write a recipe with " + contained
          .getName() + ". You need to use something special for writing on " + target.getName() + ".");
        return true;
      }
      liquid = contained;
    ...
    if (liquid == null)
    {
      performer.getCommunicator().sendNormalServerMessage("You need a colorant to be able to write a recipe with the " + source
        .getName() + " on the " + target
        .getName() + ".");
      return true;
    }
    ...
    if (counter == 1.0F)
    {
      ...
      int time = 50;
      performer.getCommunicator().sendNormalServerMessage("You start to write the recipe for \"" + recipe
        .getName() + "\" on the " + target.getActualName() + ".");
      Server.getInstance().broadCastAction(performer
        .getName() + " starts to write a recipe down.", performer, 5);
    ...
    if (counter * 10.0F > act.getTimeLeft())
    {
      ...
      performer.getCommunicator().sendNormalServerMessage("You carefully finish writing the recipe \"" + recipe
        .getName() + "\" and sign it.");
      Server.getInstance().broadCastAction(performer
        .getName() + " stops writing.", performer, 5);
      if (liquid != null) {
  ...
  private boolean recordInfo(Action act, Creature performer, @Nullable Item source, Item target, short action, float counter, InscriptionData inscriptionData)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("This " + target
        .getName() + " already has some study results on and therefore cannot be used again.");
      return true;
    }
    ...
    for (Iterator<Item> it = source.getItems().iterator(); it.hasNext();)
    {
      ...
      if (!isValidColorant(contained.getTemplateId()))
      {
        performer.getCommunicator().sendNormalServerMessage("You cannot write a study report with " + contained
          .getName() + ". You need to use something special for writing on " + target.getName() + ".");
        return true;
      }
      liquid = contained;
    ...
    {
      int time = 50;
      performer.getCommunicator().sendNormalServerMessage("You start to write a report for \"" + harvestable
        .getName() + "\" on the " + target.getActualName() + ".");
      Server.getInstance().broadCastAction(performer
        .getName() + " starts to write a report.", performer, 5);
    ...
    if (counter * 10.0F > act.getTimeLeft())
    {
      ...
      float power = (float)forestry.skillCheck(diff + alc, target, 0.0D, false, skillMultiplier);
      float knowledge = (float)forestry.getKnowledge(0.0D);
      float modify = (100.0F - knowledge) * power / 1000.0F - alc / 10.0F;
      float newQL = Math.min(100.0F, Math.max(1.0F, knowledge + modify));
      
      performer.getCommunicator().sendNormalServerMessage("You carefully finish writing the report about \"" + harvestable
        .getName() + "\" and sign it.");
      Server.getInstance().broadCastAction(performer
        .getName() + " stops writing.", performer, 5);
      player.setStudied(0);
  ...
  static List<ActionEntry> getPapyrusBehavioursFor(Creature performer, Item source)
  {
    ...
    if ((source.canHaveInscription()) && (source.getParentId() != -10L))
    {
      ...
      if ((inscriptionData != null) && (source.getAuxData() == 1))
      {
        Recipe recipe = inscriptionData.getRecipe();
        if (recipe != null) {
          toReturn.add(new ActionEntry((short)744, "Add \"" + recipe
            .getName() + "\" to cookbook", "adding recipe"));
        }
      }
    }
    return toReturn;
  ...
  static boolean addToCookbook(Action act, Creature performer, Item source, Item target, short action, float counter)
  {
    ...
    if ((inscriptionData != null) && (source.getAuxData() == 1))
    {
      ...
      if (recipe != null)
      {
        ...
        {
          int time = 50;
          performer.getCommunicator().sendNormalServerMessage("You start to add the recipe " + source
            .getName() + " into your cookbook.");
          Server.getInstance().broadCastAction(performer
            .getName() + " starts to write a recipe down.", performer, 5);
          performer.sendActionControl("Writing " + recipe.getName(), true, 50);
          performer.getStatus().modifyStamina(-400.0F);
        ...
        if (counter * 10.0F > act.getTimeLeft())
        {
          ...
          {
            performer.getCommunicator().sendNormalServerMessage("You finish adding the " + source
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\PapyrusBehaviour.class
