    ...
    if (action == 169)
    {
      if (!target.isTraded())
      {
        ...
        try
        {
          ...
          {
            performer.getCommunicator().sendSafeServerMessage("You must carry the " + source
              .getName() + " to use it.");
            return done;
          }
          ...
          if ((!creation.isCreateOnGround()) && (performer.getInventory().insertItem(created)) && (performer.canCarry(created.getWeightGrams())))
          {
            ...
            Server.getInstance().broadCastAction(performer
              ...
              Math.max(3, created.getSizeZ() / 10));
            ...
            try
            {
              ...
              else
              {
                ...
                if (created.getTemplateId() == 37)
                {
                  Effect effect = EffectFactory.getInstance().createFire(created.getWurmId(), created
                    .getPosX(), created.getPosY(), created.getPosZ(), performer.isOnSurface());
              ...
              else if (created.isBoat())
              {
                ...
                case 543: 
                  ...
                  break;
                default: 
                  logger.fine("Was a boat but not of a known type so no achievement: templateId: " + created
                    .getTemplateId());
                }
              }
            }
        ...
        catch (FailedException localFailedException) {}catch (Exception ex)
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\UnfinishedItemBehaviour.class
