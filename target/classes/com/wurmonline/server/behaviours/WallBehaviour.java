  ...
  implements MiscConstants, ItemTypes
{
  ...
  private static boolean isValidModifyableWall(Wall wall, Item tool)
  {
    ...
    if (wall.getMaterial() == StructureMaterialEnum.PLAIN_STONE) {
      if ((templateId == 62) || (templateId == 63)) {
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Wall wall)
  {
    ...
    else if (source.getTemplateId() == 676) {
    ...
    if (performer.getPower() >= 2)
    {
      if (structure != null)
      {
        ...
        if ((structure.isFinalized()) && (wall.getType() != StructureTypeEnum.PLAN))
        {
          ...
          toReturn.add(Actions.actionEntrys['ʖ']);
          if (performer.getPower() >= 5) {
            toReturn.add(Actions.actionEntrys[90]);
          }
        }
        else if ((structure.isFinalized()) && (wall.getType() == StructureTypeEnum.PLAN) && 
          (performer.getPower() >= 4))
        {
          toReturn.add(Actions.actionEntrys['͢']);
        }
        else if (((source.getTemplateId() == 176) || (source.getTemplateId() == 315)) && 
    ...
    try
    {
      ...
      if (str.getRealKnowledge() > 21.0D) {
        if (!wall.isWallPlan())
        {
          ...
          if ((!structure.hasWalls()) && (!addedDestroy))
          {
    ...
    if (wall.isFinished()) {
      if (MethodsStructure.mayModifyStructure(performer, structure, wallTile, (short)683))
      {
        toReturn.add(Actions.actionEntrys['ʫ']);
        if ((wall.isPlainStone()) && ((source.getTemplateId() == 130) || (
          (source.isWand()) && (performer.getPower() >= 4)))) {
          toReturn.add(Actions.actionEntrys['͏']);
        } else if ((wall.isPlastered()) && ((source.getTemplateId() == 1115) || (
          (source.isWand()) && (performer.getPower() >= 4)))) {
          toReturn.add(new ActionEntry((short)847, "Remove render", "removing"));
        }
        if ((wall.isLRArch()) && ((source.getTemplateId() == 1115) || (
          (source.isWand()) && (performer.getPower() >= 4)))) {
          toReturn.add(Actions.actionEntrys['͐']);
        }
      }
    }
    return toReturn;
  ...
  private static final List<ActionEntry> addBuildMenu(Creature performer, Item source, Wall wall, boolean hasMarker)
  {
    ...
    for (StructureMaterialEnum material : materials)
    {
      ...
      List<WallEnum> wallTypes = WallEnum.getWallsByToolAndMaterial(performer, source, false, (hasMarker) || 
        (MethodsStructure.hasInsideFence(wall)), material);
      if (wallTypes.size() > 0)
      {
        hlist.add(new ActionEntry((short)-wallTypes.size(), Wall.getMaterialName(material), "building"));
        for (int i = 0; i < wallTypes.size(); i++) {
          mlist.add(((WallEnum)wallTypes.get(i)).createActionEntry());
        }
        Collections.sort(mlist);
        alist.add(mlist);
      }
    }
    Object toReturn = new LinkedList();
    int count;
    switch (hlist.size())
    {
    case 0: 
      break;
    case 1: 
      ((List)toReturn).add(new ActionEntry((short)-((List)alist.get(0)).size(), "Build", "building"));
      ((List)toReturn).addAll((Collection)alist.get(0));
      break;
    default: 
      ((List)toReturn).add(new ActionEntry((short)-hlist.size(), "Build", "building"));
      count = 0;
      for (List<ActionEntry> zlist : alist)
      {
        ((List)toReturn).add(hlist.get(count++));
        ((List)toReturn).addAll(zlist);
      }
    }
    return (List<ActionEntry>)toReturn;
  }
  ...
  public static final List<ActionEntry> addManage(Creature performer, Structure structure, Wall wall)
  {
    ...
    if (structure.isFinalized())
    {
      if (structure.mayManage(performer)) {
        permissions.add(Actions.actionEntrys['ʡ']);
      }
      if ((structure.mayShowPermissions(performer)) || (structure.isActualOwner(performer.getWurmId()))) {
        permissions.add(Actions.actionEntrys['ʘ']);
      }
      Door door = wall.getDoor();
      if ((door != null) && ((door.mayShowPermissions(performer)) || 
        (structure.mayManage(performer)) || (performer.getPower() > 1))) {
        permissions.add(Actions.actionEntrys['ʚ']);
      }
      if (structure.maySeeHistory(performer))
      {
        permissions.add(Actions.actionEntrys['ʳ']);
        if ((wall.isFinished()) && (door != null)) {
          permissions.add(Actions.actionEntrys['ʴ']);
        }
      }
      ...
      if (door != null) {
        if ((door.mayLock(performer)) && (door.hasLock())) {
          if (door.isLocked()) {
  ...
  private static String getConstructionMessage(Wall wall)
  {
    ...
    String genus;
    String type;
    switch (WallBehaviour.1.$SwitchMap$com$wurmonline$shared$constants$StructureTypeEnum[wall.getType().ordinal()])
    {
    case 1: 
      String genus = "a";
      type = "wall";
    ...
    String msg = StringUtil.format("You see %s %s under construction. The %s needs ", new Object[] { genus, type, type });
  ...
  static final void sendQlString(Communicator comm, Wall wall)
  {
    ...
    if ((comm.player != null) && (comm.player.getPower() > 0)) {
      try
      {
        ...
        comm.sendNormalServerMessage("Structure=" + wall.getStructureId() + ", owner=" + ownerName + " (" + struct
          .getOwnerId() + ")");
        comm.sendNormalServerMessage("Structure finished=" + struct.isFinished());
        if (comm.player.getPower() >= 5) {
          comm.sendNormalServerMessage("wall= " + wall.getNumber() + " start[" + wall.getStartX() + "," + wall
            .getStartY() + "] end=[" + wall.getEndX() + "," + wall.getEndY() + "] state=" + wall
            .getState() + " color=" + wall.getColor() + " material=" + wall.getMaterial() + " type=" + wall
            .getType() + " cover=" + wall.getCover() + " walltile=[" + wall.getTile().tilex + "," + 
            wall.getTile().tiley + "] finished=" + wall.getTile().getStructure().isFinished() + " isIndoor=" + wall
            .isIndoor() + " height=" + wall.getHeight() + " layer=" + wall.getLayer() + ")");
        }
      }
  ...
  public boolean action(Action act, Creature performer, Wall wall, short action, float counter)
  {
    ...
    if (action == 1)
    {
        ...
        if (structure != null)
        {
          ...
          for (Floor floor : floors) {
            if (floor.getFloorLevel() == 0) {
              if (performer.getPower() > 0) {
                comm.sendNormalServerMessage("State=" + floor.getState() + "  x=" + floor
                  .getTileX() + ", " + floor.getTileY() + " finished=" + floor.isFinished());
              }
            }
          }
          Door[] doors = structure.getAllDoors();
          for (int x = 0; x < doors.length; x++) {
            try
            {
              ...
              {
                if (performer.getPower() > 0) {
                  comm.sendNormalServerMessage("State=" + wall.getState() + " inner x=" + doors[x]
                    .getInnerTile().getTileX() + ", " + doors[x].getInnerTile().getTileY() + ", outer: " + doors[x]
                    .getOuterTile().getTileX() + ", y=" + doors[x]
                ...
                try
                {
                  ...
                  if (performer.getPower() >= 5) {
                    comm.sendNormalServerMessage("Lock WurmId=" + lock.getWurmId() + ", dam=" + lock
                      .getDamage());
                  }
                  if (wall.getColor() != -1) {
                    comm.sendNormalServerMessage("Colors: R=" + WurmColor.getColorRed(wall.getColor()) + ", G=" + 
                      WurmColor.getColorGreen(wall.getColor()) + ", B=" + 
                      WurmColor.getColorBlue(wall.getColor()) + ".");
                  }
                  if (doors[x].getLockCounter() > 0) {
                    comm.sendNormalServerMessage("The door is picked open and will shut in " + doors[x]
                      .getLockCounterTime());
      ...
      if (wall.getColor() != -1) {
        comm.sendNormalServerMessage("Colors: R=" + WurmColor.getColorRed(wall.getColor()) + ", G=" + 
          WurmColor.getColorGreen(wall.getColor()) + ", B=" + WurmColor.getColorBlue(wall.getColor()) + ".");
      }
      if (performer.getPower() >= 2)
      {
        if (structure != null)
        {
          comm.sendNormalServerMessage("State=" + wall.getState() + ", wall id=" + wall.getId() + ", structure id=" + wall
            .getStructureId() + " writid=" + structure.getWritId());
      ...
      if (action == 209)
      {
        if (performer.getCitizenVillage() != null)
        {
          if (wall.getTile() != null) {
            if (wall.getTile().getVillage() != null) {
        ...
        historyBuilding(performer, structure, wall);
      } else if (action == 692) {
        historyDoor(performer, wall, door);
  ...
  {
    if (structure != null) {
      if ((structure.isOwner(performer)) || (performer.getPower() > 0))
      {
  ...
  public boolean action(Action act, Creature performer, Item source, Wall wall, short action, float counter)
  {
    ...
    else
    {
      ...
      if (act.isBuildHouseWallAction())
      {
        ...
        if ((MethodsStructure.isCorrectToolForBuilding(performer, source.getTemplateId())) && 
          (targetWallType.isCorrectToolForType(source, performer))) {
          usesRightItem = true;
        }
        if (!usesRightItem)
        {
        ...
        else
        {
          ...
          if ((Math.abs(performer.getPosX() - (wall.getEndX() << 2)) > 8.0F) || (Math.abs(performer.getPosX() - (wall.getStartX() << 2)) > 8.0F) || 
            (Math.abs(performer.getPosY() - (wall.getEndY() << 2)) > 8.0F) || (Math.abs(performer.getPosY() - (wall.getStartY() << 2)) > 8.0F))
          {
          ...
          {
            if ((wall.isStone()) || (wall.isPlainStone()) || (wall.isSlate()) || (wall.isRoundedStone()) || 
              (wall.isPottery()) || (wall.isSandstone()) || (wall.isMarble())) {
        ...
        if ((counter * 10.0F > time) || (immediate))
        {
          ...
          try
          {
            ...
            if (((act.isBuildDoorAction()) || (act.isBuildDoubleDoorAction()) || (act.isBuildPortcullisAction()) || 
              (act.isBuildCanopyDoor())) && (orig.isFinished()))
            {
      ...
      else
      {
        if (action == 662)
        {
          if (performer.getPower() >= 2)
          {
            ...
            performer.getCommunicator().sendNormalServerMessage("Wall toggled and now is " + (wall
              .isIndoor() ? "Inside" : "Outside"));
            if (structure != null)
            {
              structure.updateStructureFinishFlag();
            }
  ...
  static final boolean toggleRenderWall(Creature performer, Item tool, Wall wall, Action act, float counter)
  {
    ...
    if ((wall.isPlainStone()) && (!insta)) {
      if (tool.getWeightGrams() < 20000)
      {
        performer.getCommunicator().sendNormalServerMessage("It takes 20kg of " + tool
          .getName() + " to render the " + wall.getName() + ".");
        return true;
      }
    }
    int time = 40;
    if (counter == 1.0F)
    {
      ...
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You start to " + render + " the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s starts to " + render + " the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
      return false;
    }
    time = act.getTimeLeft();
    if ((counter * 10.0F > time) || (insta))
    {
      ...
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You " + render + " the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s " + renders + " the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
      if ((wall.isPlainStone()) && (!insta)) {
  ...
  static final boolean toggleLeftRightArch(Creature performer, Item tool, Wall wall, Action act, float counter)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You start to move the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s starts to move the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
      return false;
    }
    time = act.getTimeLeft();
    if ((counter * 10.0F > time) || (insta))
    {
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You move the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s move the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
  ...
  static final boolean rotateWall(Creature performer, Wall wall, Action act, float counter)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You start to rotate the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s starts to rotate the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
      return false;
    }
    time = act.getTimeLeft();
    if ((counter * 10.0F > time) || (insta))
    {
      performer.getCommunicator().sendNormalServerMessage(StringUtil.format("You rotate the %s.", new Object[] { wall.getName() }));
      Server.getInstance().broadCastAction(
        StringUtil.format("%s rotates the %s.", new Object[] {performer.getName(), wall.getName() }), performer, 5);
  ...
  static final boolean modifyWall(Creature performer, Item tool, Wall wall, Action act, float counter)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " starts to " + modify + wall.getName() + ".", performer, 5);
      return false;
    }
    time = act.getTimeLeft();
    if (counter * 10.0F > time)
    {
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + modify + wall.getName() + ".", performer, 5);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\WallBehaviour.class
