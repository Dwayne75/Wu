  ...
  implements ItemMaterials
{
  ...
  public List<ActionEntry> getBehavioursFor(Creature performer, Item source, Item target)
  {
    ...
    try
    {
      ...
      {
        if (((source.getTemplateId() == 169) || (source.getTemplateId() == 36) || 
          (source.isLiquidInflammable()) || (source.isPaper())) && 
          (!source.isNewbieItem()) && (!source.isChallengeNewbieItem()) && 
        ...
        (!source.isNewbieItem()) && (!source.isChallengeNewbieItem()) && 
        (!source.isIndestructible()) && (!source.isComponentItem()) && (source.getTemplateId() != 1392)) {
  ...
  public boolean action(Action act, Creature performer, Item source, Item target, short action, float counter)
  {
    ...
    if (action == 117)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You cannot fuel " + target
          .getNameWithGenus() + " with " + source.getNameWithGenus() + " silly.");
        return true;
      }
      if (source.isMagicContainer())
      {
        performer.getCommunicator().sendNormalServerMessage("The " + source
          .getName() + " will not burn.");
        return true;
      }
      if ((source.isHollow()) && (!source.isEmpty(true)))
      {
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("The " + source
            .getName() + " must contain no items before using it to fuel the " + target
            .getName() + ".");
          return true;
        }
      }
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You cannot fuel " + target
          .getNameWithGenus() + " with " + source.getNameWithGenus() + ".");
        return true;
      }
      short newPTemp;
      if (target.getTemplateId() == 1243)
      {
          ...
          (!source.isIndestructible()))
        {
          ...
          if (result == null)
          {
            performer.getCommunicator().sendNormalServerMessage("You fuel the " + target
              .getName() + " with " + source.getNameWithGenus() + ".");
            Server.getInstance().broadCastAction(performer
              .getName() + " fuels the " + target.getName() + " with " + source
            ...
            {
              short maxTemp = 30000;
              newPTemp = (short)(int)Math.min(30000.0D, target.getTemperature() + newTemp);
              target.setTemperature(newPTemp);
            }
            Items.destroyItem(source.getWurmId());
            
            target.setAuxData((byte)(int)Math.min(127.0D, target.getAuxData() + newTemp / 100.0D));
          }
        }
      }
      else if (((source.isWood()) || (source.isCloth()) || (source.isMelting()) || (source.isLiquidInflammable()) || (source.isPaper())) && 
        ...
        if (result == null) {
          if (target.getTemplateId() != 74) {
            if (target.getTemperature() > 1000)
            {
              ...
              double newTemp = source.getWeightGrams() * Item.fuelEfficiency(source.getMaterial());
              target.setTemperature((short)(int)Math.min(30000.0D, target.getTemperature() + newTemp));
              for (Item item : source.getAllItems(true)) {
              ...
              if (target.getTemplateId() == 889) {
                target.setAuxData((byte)(int)Math.min(127.0D, target.getAuxData() + newTemp / 1000.0D));
              }
            }
            else
            {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\FireBehaviour.class
