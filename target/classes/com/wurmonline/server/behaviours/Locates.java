...
public final class Locates
{
  ...
  static void locateSpring(Creature performer, Item pendulum, Skill primSkill)
  {
    ...
    float difficulty = Server.rand.nextFloat() * (max + 3) * 30.0F;
    
    double result = Server.rand.nextFloat() * knowl * 10.0D;
    result -= difficulty;
    
    Server.getInstance().broadCastAction(performer
      .getName() + " lets out a mild sigh as " + performer.getHeSheItString() + " starts breathing again.", performer, 5);
    if (closest[0] == -1) {
      performer.getCommunicator().sendNormalServerMessage("The " + pendulum.getName() + " seems dead.");
    } else if (result > 0.0D)
    {
      if (max < 1) {
        performer.getCommunicator().sendNormalServerMessage("The " + pendulum
          .getName() + " now swings frantically! There is something here!");
      } else if (max < 2) {
        performer.getCommunicator().sendNormalServerMessage("The " + pendulum
          .getName() + " swings rapidly back and forth! You are close to a water source!");
      } else if (max < 3) {
        performer.getCommunicator().sendNormalServerMessage("The " + pendulum
          .getName() + " is swinging in a circle, there is probably a water source in the ground nearby.");
      } else if (max < 5) {
        performer.getCommunicator().sendNormalServerMessage("The " + pendulum
          .getName() + " is starting to move, indicating a flow of energy somewhere near.");
      } else if (result > 30.0D) {
        performer.getCommunicator().sendNormalServerMessage("You think you detect some faint tugs in the " + pendulum
          .getName() + ".");
      } else {
        performer.getCommunicator().sendNormalServerMessage("The " + pendulum.getName() + " seems dead.");
      }
  ...
  static void locateChamp(Creature performer, Item pendulum, Skill primSkill)
  {
    ...
    int dist = (int)(pendulum.getSpellLocChampBonus() / 100.0F * Zones.worldTileSizeX / 32.0F * getMaterialPendulumModifier(pendulum.getMaterial()));
    ...
    if (firstChamp != null)
    {
      ...
      if (primSkill.skillCheck(maxd / 10.0F, pendulum, 0.0D, false, 5.0F) > 0.0D)
      {
    ...
    else if (primSkill.skillCheck(10.0D, pendulum, 0.0D, false, 5.0F) > 0.0D)
    {
  ...
  static boolean locateTraitor(Creature performer, Item pendulum, Skill primSkill)
  {
    ...
    {
      int maxDist = (int)(pendulum.getSpellLocEnemyBonus() / 100.0F * Zones.worldTileSizeX / 16.0F * getMaterialPendulumModifier(pendulum.getMaterial()));
  ...
  private static final void sendFishFound(int targx, int targy, int fish, Creature performer, Skill primSkill, Item pendulum)
  {
    ...
    if (fish > 0)
    {
      ...
      int maxd = (int)Math.sqrt(dx * dx + dy * dy);
      double skillCheck = primSkill.skillCheck(maxd, pendulum, 0.0D, false, 5.0F);
      if (skillCheck > 0.0D)
      {
        ...
        if ((!champ) && (c.isPlayer()))
        {
          ...
          if (found) {
            if ((!c.isStealth()) || (dist <= 25))
            {
              ...
              else if (c.getBonusForSpellEffect((byte)29) <= 0.0F)
              {
                return c;
              }
            }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Locates.class
