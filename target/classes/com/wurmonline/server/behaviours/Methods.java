  ...
  implements MiscConstants, QuestionTypes, ItemTypes, CounterTypes, ItemMaterials, SoundNames, VillageStatus, TimeConstants, MonetaryConstants
{
  ...
  static void sendFoundVillageQuestion(Creature performer, Item deed)
  {
    ...
    if (!decliners.isEmpty())
    {
      ...
      for (Iterator<Village> it = decliners.keySet().iterator(); it.hasNext();)
      {
        Village vill = (Village)it.next();
    ...
    if ((deed.isNewDeed()) || (Servers.localServer.testServer) || (deed.getTemplateId() == 862))
    {
      ...
      {
        if ((village != null) && 
          (village.getCitizen(performer.getWurmId()).getRole() == village.getRoleForStatus((byte)2)))
        {
  ...
  static void sendExpandVillageQuestion(Creature performer, @Nullable Item deed)
  {
    try
    {
      ...
      try
      {
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("The settlement has been attacked, or been under siege recently. You need to wait " + 
  ...
  static void setVillageToken(Creature performer, Item token)
  {
    ...
    else
    {
      ...
      {
        logger.log(Level.WARNING, "Village: " + village
          .getId() + ", token: " + token.getWurmId() + " - " + iox.getMessage(), iox);
      ...
      {
        logger.log(Level.WARNING, "Village: " + village
          .getId() + ", token: " + token.getWurmId() + " - " + nsi.getMessage(), nsi);
        performer.getCommunicator().sendNormalServerMessage("A server error occured. Please report this.");
        return;
      }
      if (oldToken != null) {
  ...
  {
    boolean done = true;
    if ((!performer.isFriendlyKingdom(village.kingdom)) || 
      (village.isEnemy(performer.getCitizenVillage())))
    {
          ...
          if (nextDrain < 0L)
          {
            ...
            if (moneyDrained > 1L)
            {
              ...
              if (counter == 1.0F)
              {
                ...
                performer.getCommunicator().sendNormalServerMessage("You start to search for gold in the coffers of " + village
                  .getName() + ".");
                Server.getInstance().broadCastAction(performer
                  .getName() + " starts to rummage through the coffers of " + village.getName() + ", looking for coins.", performer, 5);
                
                performer.sendActionControl(Actions.actionEntrys['Ş']
              ...
              if ((counter * 10.0F > time) || (insta))
              {
                ...
                performer.getCommunicator().sendNormalServerMessage("You find " + change
                  .getChangeString() + " in the coffers of " + village.getName() + ".");
                
                Server.getInstance().broadCastAction(performer
                  .getName() + " proudly displays the " + change.getChangeString() + " " + performer
                  .getHeSheItString() + " found in the coffers of " + village
                  .getName() + ".", performer, 5);
                ...
                if ((village.plan != null) && (village.plan.getNumHiredGuards() >= 5)) {
                  if ((performer.isChampion()) && (village.kingdom != performer.getKingdomId()))
                  {
                    performer.modifyChampionPoints(3);
                    Servers.localServer.createChampTwit(performer.getName() + " drains " + village
                      .getName() + " and gains 3 champion points");
                  }
                }
                if ((Servers.localServer.isChallengeOrEpicServer()) && (Servers.localServer.HOMESERVER)) {}
                boolean enemyHomeServer = Servers.localServer.KINGDOM != performer.getKingdomId();
                if ((!Servers.localServer.HOMESERVER) || (enemyHomeServer)) {
                  MissionTriggers.activateTriggers(performer, -1, 350, token
                    .getWurmId(), 1);
                }
              }
            }
            ...
            {
              performer.getCommunicator().sendNormalServerMessage("There is no money to steal in the coffers of " + village
                .getName() + ".");
            }
          }
          else
          {
            performer.getCommunicator().sendNormalServerMessage("The coffer timelock has been activated. Try again in " + 
              Server.getTimeFor(nextDrain) + ".");
          }
        }
        else
        {
          performer.getCommunicator().sendNormalServerMessage("The coffers of " + village
            .getName() + " echo hollowly.");
        }
      }
    }
    else {
  ...
  public static void sendJoinVillageQuestion(Creature performer, Creature invited)
  {
    if (performer.getKingdomId() == invited.getKingdomId())
    {
      ...
      if (vill != null)
      {
        ...
        if (role.getStatus() == 2)
        {
          performer.getCommunicator().sendNormalServerMessage(invited
            .getName() + " is the mayor of " + vill.getName() + ". He can't join another settlement.");
          return;
        }
      }
      try
      {
        ...
        if ((village != null) && (village.acceptsNewCitizens()))
        {
          ...
          {
            performer.getCommunicator().sendNormalServerMessage(invited
              .getName() + " has negative reputation and may not join a settlement now.");
            return;
          }
          ...
          performer.getCommunicator().sendNormalServerMessage("You invite " + invited
            .getName() + " to join " + village.getName() + ".");
        }
        else
        {
          SimplePopup pp = new SimplePopup(performer, "Max citizens reached", "The settlement does not accept more citizens right now");
          
          pp.setToSend("Every settlement has a maximum amount of citizens depending on their size. You may unlimit the amount of allowed citizens in the citizen management or settlement management forms. As long as " + village
            .getName() + " has more than " + village
            
            .getMaxCitizens() + " player citizens your upkeep is doubled.");
    ...
    if (vill != null) {
      if (vill.mayDoDiplomacy(invited))
      {
        ...
        if (village != null) {
          ...
          {
            performer.getCommunicator().sendNormalServerMessage("You may not do diplomacy in the name of " + village
              .getName() + ". You cannot offer peace.");
            return;
          }
        }
      }
      ...
      {
        performer.getCommunicator().sendNormalServerMessage(invited
          .getName() + " may not do diplomacy in the name of " + vill.getName() + ". He cannot give you peace.");
  ...
  static void sendWarDeclarationQuestion(Creature performer, Village targetVillage)
  {
    if (targetVillage != null)
    {
      ...
      if (village != null) {
        if (village.mayDoDiplomacy(performer))
        {
          ...
          catch (NoSuchPlayerException nsp)
          {
            ...
            performer.getCommunicator()
              .sendNormalServerMessage("Failed to locate the player for that declaration.");
          }
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("You may not do diplomacy in the name of " + village
            .getName() + ". You cannot declare war.");
          return;
        }
      }
    }
  ...
  static final void sendAllianceQuestion(Creature performer, Creature target)
  {
    if ((!target.isFighting()) && (target.hasLink()))
    {
      ...
      aq.sendQuestion();
      performer.getCommunicator().sendNormalServerMessage("You send an elaborate invitation to form a high and mighty alliance to " + target
        .getName() + ".");
    }
    else
    {
    ...
    if (performer.getKingdomId() == target.getKingdomId()) {
      if ((!target.isFighting()) && (target.hasLink()))
      {
        ...
        performer.getCommunicator().sendNormalServerMessage("You ask " + target
          .getName() + " for permission to add " + target.getHimHerItString() + " to your friends list.");
      }
      else
      {
        performer.getCommunicator().sendNormalServerMessage(target.getName() + " does not answer questions right now.");
  ...
  public static final boolean discardSellItem(Creature performer, Action act, Item discardItem, float counter)
  {
    ...
    if (counter > 3.0F)
    {
      ...
      if (kingsShop.getMoney() > 100000L)
      {
        ...
        long value = discardItem.getCurrentQualityLevel() / 10L + 1L + percentMod;
  ...
  static final boolean disbandVillage(Creature performer, Item villageToken, float counter)
  {
    ...
    try
    {
      ...
      if (((vill == performer.getCitizenVillage()) && 
        (vill.isActionAllowed((short)348, performer))) || (insta)) {
        settings = true;
      }
      if (!vill.isDisbanding())
      {
        if (settings)
        {
          ...
          if (counter == 1.0F) {
            ...
            {
              int time = 3000;
              performer.sendActionControl(Actions.actionEntrys['Ŝ']
                .getVerbString(), true, 3000);
              performer.getCommunicator().sendNormalServerMessage("You start to disband the village of " + vill
                .getName() + ".");
              Server.getInstance().broadCastAction(performer
                .getName() + " starts to disband the village of " + vill.getName() + ".", performer, 5);
              
              vill.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> starts to disband the village of " + vill
                .getName() + "."));
          ...
          if ((counter > 300.0F) || (insta) || (vill.getMayor().getId() == performer.getWurmId()))
          {
            ...
            else
            {
              ...
              Village localVillage1;
              Village lAllie;
              if ((vill.getMayor().getId() == performer.getWurmId()) && 
                (vill.getDiameterX() < 30) && (vill.getDiameterY() < 30))
              {
                ...
                Server.getInstance().broadCastAction(performer
                  .getName() + " has set " + vill.getName() + " to disband immediately.", performer, 5);
                
                vill.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer
                  .getName() + "> has set " + vill.getName() + " to disband immediately."));
                Village[] allies = vill.getAllies();
                Village[] arrayOfVillage1 = allies;int i = arrayOfVillage1.length;
                for (localVillage1 = 0; localVillage1 < i; localVillage1++)
                {
                  lAllie = arrayOfVillage1[localVillage1];
                  
                  lAllie.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer
                    .getName() + "> has set " + vill.getName() + " to disband immediately."));
                }
              }
              else
              {
                ...
                performer.getCommunicator().sendNormalServerMessage(vill
                  .getName() + " will disband in " + "24 hours" + ".");
                ...
                Server.getInstance().broadCastAction(performer
                  .getName() + " has set " + vill.getName() + " to disband in " + "24 hours" + ".", performer, 5);
                
                vill.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer
                  .getName() + "> has set " + vill.getName() + " to disband in " + "24 hours" + "."));
                Village[] allies = vill.getAllies();
                Village[] arrayOfVillage2 = allies;localVillage1 = arrayOfVillage2.length;
                for (lAllie = 0; lAllie < localVillage1; lAllie++)
                {
                  Village lAllie = arrayOfVillage2[lAllie];
      ...
      if (vill.isDisbanding())
      {
        if (counter == 1.0F)
        {
          ...
          if ((citizVill != null) && (citizVill.equals(vill)))
          {
            int time = 300;
            performer.sendActionControl(Actions.actionEntrys['ŝ']
              .getVerbString(), true, 300);
            performer.getCommunicator().sendNormalServerMessage("You start to salvage the settlement of " + vill
              .getName() + ".");
            Server.getInstance()
              .broadCastAction(performer
              .getName() + " starts to salvage the settlement of " + vill.getName() + ".", performer, 5);
            
            vill.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> starts to salvage the settlement of " + vill
              .getName() + "."));
            try
            {
              Player player = Players.getInstance().getPlayer(vill.getDisbander());
              player.getCommunicator().sendAlertServerMessage(performer
                .getName() + " is trying to salvage the settlement of " + vill.getName() + "!");
            }
          ...
          {
            if (!insta) {
              performer.getCommunicator().sendNormalServerMessage("You need to be citizen to salvage the settlement of " + vill
                .getName() + ".");
            }
            toReturn = true;
          }
        ...
        if ((counter > 30.0F) || (insta))
        {
          ...
          try
          {
            try
            {
              Player player = Players.getInstance().getPlayer(vill.getDisbander());
              player.getCommunicator().sendAlertServerMessage(performer
                .getName() + " successfully salvaged the settlement of " + vill.getName() + "!");
            }
            ...
            vill.broadCastMessage(new Message(performer, (byte)3, "Village", "<" + performer.getName() + "> has salvaged the settlement of " + vill
              .getName() + "."));
          }
          catch (IOException iox)
          {
  ...
  static boolean transferPlayer(Creature performer, Creature target, Action act, float counter)
  {
    ...
    if (!done) {
      if (counter == 1.0F)
      {
        ...
        ((Player)target).sendTransfer(Server.getInstance(), targetserver.INTRASERVERADDRESS, 
          Integer.parseInt(targetserver.INTRASERVERPORT), targetserver.INTRASERVERPASSWORD, targetserver.id, tilex, tiley, true, false, target
          .getKingdomId());
        ((Player)target).transferCounter = 30;
        if (!target.equals(performer)) {
          performer.getLogger().log(Level.INFO, performer
            .getName() + " transferred " + target.getName() + " to " + targetserver.name + ".");
        }
        done = true;
      }
  ...
  {
    return kingAspirants.contains(Long.valueOf(performer.getWurmId()));
  }
  
  public static final void setAspiredKing(Creature performer)
  {
    kingAspirants.add(Long.valueOf(performer.getWurmId()));
  }
  ...
  public static final boolean aspireKing(Creature performer, byte kingdom, @Nullable Item item, Creature elector, Action act, float counter)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("You may not aspire to become the " + 
        King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
        Kingdoms.getNameFor(kingdom) + ".");
      done = true;
    }
    else if (current != null)
    {
      performer.getCommunicator().sendNormalServerMessage("There is already a " + current
        .getRulerTitle() + " of " + Kingdoms.getNameFor(kingdom) + "!");
      done = true;
    }
    if ((hasAspiredKing(performer)) || (performer.getPower() > 0)) {
    ...
    if (counter == 1.0F)
    {
      if (kingdom == 1) {
        if (jennElector != null)
        {
          performer.getCommunicator().sendNormalServerMessage("The " + elector
            .getName() + " is busy. You will have to wait for your turn.");
          done = true;
        }
      }
      if (kingdom == 3) {
      ...
      if (kingdom == 2) {
        if (molrStone != null)
        {
          performer.getCommunicator().sendNormalServerMessage("The " + item
            .getName() + " is occupied. You will have to wait for your turn.");
          done = true;
        }
      }
    }
    if (!done)
    {
      ...
      if (counter == 1.0F)
      {
        ...
        performer.sendActionControl(Actions.actionEntrys['š'].getVerbString(), true, 300);
      }
      else
      {
        ...
        if (act.currentSecond() == 5)
        {
          if (item != null)
          {
            ...
            Server.getInstance().broadCastAction(performer
              .getName() + " struggles with the " + item.getName() + ".", performer, 5);
          }
          if (elector != null)
          {
            if (kingdom == 1) {
            ...
            Server.getInstance().broadCastAction(elector
              .getName() + " watches " + performer.getName() + " intensely.", performer, 5);
        ...
        else if (act.currentSecond() == 10)
        {
          ...
          try
          {
            ...
            if (sk.getKnowledge(0.0D) > 21.0D)
            {
              ...
              if (item != null)
              {
                ...
                Server.getInstance().broadCastAction(performer
                  .getName() + " seems to make progress with the sword!", performer, 5);
              }
              if (elector != null) {
                if (kingdom == 1)
                {
                ...
                {
                  performer.getCommunicator().sendNormalServerMessage(elector
                    .getName() + " hisses and sways " + elector.getHisHerItsString() + " head back and forth in excitement!");
                  
                  Server.getInstance().broadCastAction(elector
                    .getName() + " hisses and sways " + elector.getHisHerItsString() + " head back and forth in excitement!", performer, 5);
                }
              }
            }
          }
          ...
          if (done)
          {
            ...
            if (item != null)
            {
              ...
              Server.getInstance().broadCastAction(performer
                .getName() + " shrugs in disappointment as the sword does not budge.", performer, 5);
            }
            if (elector != null) {
              if (kingdom == 1)
              {
                elector.submerge();
                performer.getCommunicator().sendNormalServerMessage(elector
                  .getName() + " silently disappears into the depths.");
                Server.getInstance().broadCastAction(elector
                  .getName() + " disappears into the depths in silence.", performer, 5);
              }
              else if (kingdom == 3)
              {
                performer.getCommunicator().sendNormalServerMessage(elector
                  .getName() + " ushers you away with some really threatening moves.");
                Server.getInstance()
                  .broadCastAction(elector
                  .getName() + " thwarts " + performer.getName() + " with some threatening moves.", performer, 5);
              }
            }
          }
        }
        else if (act.currentSecond() == 15)
        {
          ...
          if (elector != null)
          {
            ...
            Server.getInstance().broadCastAction(elector
              .getName() + " watches " + performer.getName() + " intensely again.", performer, 5);
            elector.turnTowardsCreature(performer);
          }
        }
          ...
          try
          {
            ...
            if (sk.getKnowledge(0.0D) > 21.0D)
            {
              ...
              if (item != null)
              {
                ...
                Server.getInstance()
                  .broadCastAction(performer
                  .getName() + " seems to make even more progress with the sword!", performer, 5);
              }
              if (elector != null) {
                if (kingdom == 1)
                {
        ...
        else if (act.currentSecond() == 25)
        {
          if (item != null)
          {
            ...
            Server.getInstance().broadCastAction(performer
              .getName() + " exerts all " + performer.getHisHerItsString() + " force on the sword.", performer, 5);
          }
          if (elector != null)
          {
            performer.getCommunicator().sendNormalServerMessage(elector
              .getName() + " seems to make up " + elector.getHisHerItsString() + " mind about you.");
            Server.getInstance().broadCastAction(elector
              .getName() + " watches " + performer.getName() + " intensely again.", performer, 5);
            elector.turnTowardsCreature(performer);
          }
        ...
        else if (act.currentSecond() >= 30)
        {
          ...
          if ((Server.rand.nextInt(randomint) == 0) || (performer.getPower() >= 3))
          {
            ...
            if (item != null)
            {
              performer.getCommunicator().sendNormalServerMessage("The sword gets loose and disappears! You are the new " + 
              
                King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                Kingdoms.getNameFor(kingdom) + "!");
              Server.getInstance().broadCastAction("The sword gets loose! " + performer
                .getName() + " is the new " + 
                King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                Kingdoms.getNameFor(kingdom) + "!", performer, 10);
            }
            if (elector != null)
            {
              elector.turnTowardsCreature(performer);
              if (kingdom == 1)
              {
                ...
                elector.playAnimation("regalia", false);
                performer.getCommunicator().sendNormalServerMessage(elector
                  .getName() + " hands you the royal regalia! You are the new " + 
                  King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                  Kingdoms.getNameFor(kingdom) + "!");
                Server.getInstance().broadCastAction(elector
                  .getName() + " hands the royal regalia to " + performer.getName() + "! " + performer
                  .getHeSheItString() + " is the new " + 
                  King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                  Kingdoms.getNameFor(kingdom) + "!", performer, 10);
              }
              else if (kingdom == 3)
              {
                performer.getCommunicator().sendNormalServerMessage(elector
                  .getName() + " hisses loudly and the royal regalia is handed to you from above! You are the new " + 
                  
                  King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                  Kingdoms.getNameFor(kingdom) + "!");
                Server.getInstance().broadCastAction(elector
                  .getName() + " hisses loudly in excitement! " + performer.getName() + " is the new " + 
                  King.getRulerTitle(performer.getSex() == 0, kingdom) + " of " + 
                  Kingdoms.getNameFor(kingdom) + "!", performer, 10);
              }
            }
          ...
          else
          {
            ...
            {
              performer.getCommunicator().sendNormalServerMessage("The " + item
                .getName() + " is stuck and won't budge.");
              Server.getInstance().broadCastAction(performer
                .getName() + " shrugs in disappointment as the " + item.getName() + " does not budge.", performer, 5);
            }
            if (elector != null) {
              if (kingdom == 1)
              {
                elector.submerge();
                performer.getCommunicator().sendNormalServerMessage("For no obvious reason you are rejected. " + elector
                  .getName() + " silently disappears into the depths.");
  ...
  public static void rewardRegalia(Creature creature)
  {
    ...
    if (inventory != null) {
      try
      {
        ...
        if (template == 1)
        {
          Item sceptre = ItemFactory.createItem(529, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          sceptre.setAuxData(kingdom);
          inventory.insertItem(sceptre, true);
          Item crown = ItemFactory.createItem(530, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          crown.setAuxData(kingdom);
          inventory.insertItem(crown, true);
          Item robes = ItemFactory.createItem(531, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
        ...
        {
          Item sceptre = ItemFactory.createItem(535, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          sceptre.setAuxData(kingdom);
          inventory.insertItem(sceptre, true);
          Item crown = ItemFactory.createItem(536, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          crown.setAuxData(kingdom);
          inventory.insertItem(crown, true);
          Item robes = ItemFactory.createItem(537, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          robes.setAuxData(kingdom);
          inventory.insertItem(robes, true);
        }
        ...
        {
          Item sceptre = ItemFactory.createItem(532, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          sceptre.setAuxData(kingdom);
          inventory.insertItem(sceptre, true);
          Item crown = ItemFactory.createItem(533, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          crown.setAuxData(kingdom);
          inventory.insertItem(crown, true);
          Item robes = ItemFactory.createItem(534, Server.rand.nextFloat() * 30.0F + 70.0F, creature
            .getName());
          robes.setAuxData(kingdom);
          inventory.insertItem(robes, true);
        }
  ...
  public static Item[] getBestReports(Creature creature, @Nullable Item container)
  {
    ...
    Item[] reportArr = (Item[])reports.values().toArray(new Item[reports.size()]);
  ...
  private static void addReport(Map<Byte, Item> reports, Item report)
  {
    ...
    {
      Item oldReport = (Item)reports.get(Byte.valueOf(report.getAuxData()));
      if (oldReport == null) {
        reports.put(Byte.valueOf(report.getAuxData()), report);
  ...
  private static boolean isNotAllowedMessage(Creature performer, Village village, Structure structure, short action, boolean inPerimeter)
  {
    if ((!performer.isOnPvPServer()) || (Servers.isThisAChaosServer()))
    {
      ...
      {
        String msg;
        if ((village != null) && (village.getGuards().length > 0))
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\Methods.class
