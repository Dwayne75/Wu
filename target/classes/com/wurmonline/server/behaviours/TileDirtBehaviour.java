    ...
    if ((action == 153) && (source.isSeed()))
    {
      ...
      if (source.isProcessedFood())
      {
        performer.getCommunicator().sendNormalServerMessage("The " + source
          .getName() + " is processed and cannot be planted.");
        return true;
      }
      ...
      try
      {
        ...
        if (isWaterPlant) {
          if (!Terraforming.isAllCornersInsideHeightRange(tilex, tiley, onSurface, (short)-1, (short)-4))
          {
      ...
      if (counter == 1.0F)
      {
        ...
        short time = (short)(int)(130.0D - farming.getKnowledge(source, 0.0D) - act.getRarity() - source.getRarity());
        ...
        if (type == Tiles.Tile.TILE_DIRT.id)
        {
          if (onSurface)
          {
            Server.setSurfaceTile(tilex, tiley, Tiles.decodeHeight(tile), Crops.getTileType(crop), 
              Crops.encodeFieldData(true, 0, crop));
            
            double diff = Crops.getDifficultyFor(crop);
            Skill farming = performer.getSkills().getSkillOrLearn(10049);
            farming.skillCheck(diff, 0.0D, false, 1.0F);
            Players.getInstance().sendChangedTile(tilex, tiley, onSurface, false);
            
            int resource = (int)(100.0D - farming.getKnowledge(0.0D) + source.getQualityLevel() + source.getRarity() * 20 + act.getRarity() * 50);
          ...
          {
            Server.caveMesh.setTile(tilex, tiley, Tiles.encode(Tiles.decodeHeight(tile), Crops.getTileType(crop), 
              Crops.encodeFieldData(true, 0, crop)));
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\TileDirtBehaviour.class
