  ...
  implements MiscConstants, QuestionTypes, ItemTypes, CounterTypes, ItemMaterials, SoundNames, VillageStatus, TimeConstants, AttitudeConstants
{
  ...
  static final void teleportSet(Creature performer, Item source, int tilex, int tiley)
  {
    if (source.getTemplateId() == 174)
    {
      ...
      if (tile != null)
      {
        ...
        performer
          .getCommunicator()
          .sendNormalServerMessage("You utter arcane words and make some strange gestures. You will teleport to where you are standing now using the wand.");
        
        Server.getInstance().broadCastAction(performer
          .getName() + " utters some arcane words and makes a few strange gestures with " + source
    ...
    else if (source.getTemplateId() == 525)
    {
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("The magic of the duelling ring interferes. You can not use the " + source
          .getName() + " here.");
        return;
      }
      if (performer.isInPvPZone())
      {
        performer.getCommunicator().sendNormalServerMessage("The magic of the pvp zone interferes. You can not use the " + source
          .getName() + " now.");
        return;
      }
      if ((Servers.localServer.PVPSERVER) && (EndGameItems.getEvilAltar() != null))
      {
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("The magic of this place interferes. You can not use the " + source
            .getName() + " here.");
          return;
        }
      }
      ...
      if (Tiles.isOreCave(Tiles.decodeType(tilenum))) {
        if (!performer.isOnSurface())
        {
          VolaTile tile = performer.getCurrentTile();
          if (tile != null) {
            if (tile.getStructure() == null) {
              if ((Math.abs(tilex - tile.tilex) <= 1) || (Math.abs(tiley - tile.tiley) <= 1))
              {
                tilex = tile.tilex;
                tiley = tile.tiley;
                ok = true;
                performer
                  .getCommunicator()
                  .sendNormalServerMessage("You knock the " + source
                  
                  .getName() + " in the cave wall a few times. You will teleport to where you are standing now using the stone.");
                
                Server.getInstance().broadCastAction(performer
                  .getName() + " knocks with " + source.getNameWithGenus() + " in the cave wall a few times.", performer, 5);
              }
            }
          }
        }
      ...
      if (!ok)
      {
        performer.getCommunicator().sendNormalServerMessage("You knock the " + source
          .getName() + " in the cave wall but nothing happens.");
        Server.getInstance().broadCastAction(performer
          .getName() + " knocks with " + source.getNameWithGenus() + " a few times.", performer, 5);
        return;
      }
    }
    else if (source.getTemplateId() == 524)
    {
      ...
      if ((performer.isOnPvPServer()) && (Zones.isWithinDuelRing(performer.getTileX(), performer.getTileY(), true) != null))
      {
        performer.getCommunicator().sendNormalServerMessage("There magic of the duelling ring interferes. You can not use the " + source
          .getName() + " here.");
        return;
      }
      if (performer.isInPvPZone())
      ...
      if ((Servers.localServer.PVPSERVER) && (EndGameItems.getEvilAltar() != null))
      {
        ...
        if (performer.isWithinDistanceTo(egi.getItem().getPosX(), egi.getItem().getPosY(), egi.getItem().getPosZ(), 50.0F))
        {
          performer.getCommunicator().sendNormalServerMessage("There magic of this place interferes. You can not use the " + source
            .getName() + " here.");
          return;
        }
      }
          ...
          if (tile.getStructure() == null) {
            if ((Math.abs(tilex - tile.tilex) <= 1) || (Math.abs(tiley - tile.tiley) <= 1))
            {
              ...
              if (Tiles.isTree(Tiles.decodeType(tilenum)))
              {
                ...
                performer
                  .getCommunicator()
                  .sendNormalServerMessage("You tap the " + source
                  
                  .getName() + " at the tree a few times. You will teleport to where you are standing now using the twig.");
                
                Server.getInstance().broadCastAction(performer
                  .getName() + " taps " + source.getNameWithGenus() + " at a tree few times.", performer, 5);
              }
            }
          }
        }
      ...
      if (!ok)
      {
        performer.getCommunicator().sendNormalServerMessage("You tap the " + source
          .getName() + " a few times but nothing happens.");
        Server.getInstance().broadCastAction(performer
          .getName() + " taps " + source.getNameWithGenus() + " a few times.", performer, 5);
        return;
      }
    }
    else
  ...
  static final void teleportCreature(Creature performer, Item source)
  {
    if ((source.getTemplateId() == 176) || (source.getTemplateId() == 315) || 
      (source.getTemplateId() == 174) || (source.getTemplateId() == 1027) || (source.getTemplateId() == 525) || 
      ...
      if ((performer.getVisionArea() != null) && (performer.getVisionArea().isInitialized()))
      {
        ...
        if ((tx >= 0) && (ty >= 0) && (tx < 1 << Constants.meshSize) && (ty < 1 << Constants.meshSize))
        {
          ...
          if (performer.getPower() <= 0)
          {
            ...
            {
              lInventoryItem = arrayOfItem1[localObject];
              if (lInventoryItem.isArtifact())
              {
                performer.getCommunicator().sendNormalServerMessage("The " + lInventoryItem
                  .getName() + " hums and disturbs the weave. You can not use the " + source
                  .getName() + " now.");
            ...
            Item[] arrayOfItem2 = bodyItems;localObject = arrayOfItem2.length;
            for (lInventoryItem = 0; lInventoryItem < localObject; lInventoryItem++)
            {
              lInventoryItem = arrayOfItem2[lInventoryItem];
              if (lInventoryItem.isArtifact())
              {
                performer.getCommunicator().sendNormalServerMessage("The " + lInventoryItem
                  .getName() + " hums and disturbs the weave. You can not use the " + source
            ...
            (source.getTemplateId() == 524))
          {
            ...
            {
              performer.getCommunicator().sendNormalServerMessage("There is a blocking enemy presence nearby. You can not use the " + source
                .getName() + " now.");
              
              return;
            }
            if ((performer.isOnPvPServer()) && 
              (Zones.isWithinDuelRing(performer.getTileX(), performer.getTileY(), true) != null))
            {
              performer.getCommunicator().sendNormalServerMessage("The magic of the duelling ring interferes. You can not use the " + source
                .getName() + " now.");
            ...
            {
              EndGameItem egi = EndGameItems.getEvilAltar();
              if (performer.isWithinDistanceTo(egi.getItem().getPosX(), egi.getItem().getPosY(), egi.getItem()
                .getPosZ(), 50.0F))
              {
                performer.getCommunicator().sendNormalServerMessage("There magic of this place interferes. You can not use the " + source
                  .getName() + " now.");
          ...
          else
          {
            ...
            localObject = inventoryItems;lInventoryItem = localObject.length;
            Item lInventoryItem;
            for (lInventoryItem = 0; lInventoryItem < lInventoryItem; lInventoryItem++)
            {
              lInventoryItem = localObject[lInventoryItem];
              if (lInventoryItem.getTemplateId() == 45) {
                if (lInventoryItem.getWeightGrams() >= lInventoryItem.getTemplate().getWeightGrams())
                {
            ...
            if (barid == -10L)
            {
              ...
              lInventoryItem = bodyItems;lInventoryItem = lInventoryItem.length;
              for (lInventoryItem = 0; lInventoryItem < lInventoryItem; lInventoryItem++)
              {
                Item lBodyItem = lInventoryItem[lInventoryItem];
                if (lBodyItem.getTemplateId() == 45) {
                  if (lBodyItem.getWeightGrams() >= lBodyItem.getTemplate().getWeightGrams())
                  {
            ...
            if (port)
            {
              ...
              if (performer.startTeleporting())
              {
                  ...
                  (source.getTemplateId() == 524))
                {
                  ...
                  Server.getInstance().broadCastAction(performer
                    .getName() + " uses a " + source.getName() + ".", performer, 5);
                }
  ...
  {
    double newrot = Math.atan2((targetY << 2) + 2 - (int)performer.getStatus().getPositionY(), (targetX << 2) + 2 - 
  ...
  static boolean track(Creature performer, int tilex, int tiley, int tile, float counter)
  {
    ...
    if (Tiles.decodeHeight(tile) > 0)
    {
      try
      {
        ...
        int time = Actions.getStandardActionTime(performer, tracking, null, 0.0D);
        if (counter == 1.0F)
        {
        ...
        {
          tracking.skillCheck(tracking.getKnowledge(0.0D), 0.0D, false, counter / 2.0F);
          done = true;
          Track[] tracks = trackZone.getTracksFor(trackTilex, trackTiley, 3);
          if (tracks.length > 0)
          {
            int max = Math.max(30, (int)tracking.getKnowledge(0.0D));
            int min = Math.min((int)tracking.getKnowledge(0.0D), max);
            if (performer.getPower() > 0) {
              min = 100;
            }
            ...
            for (int x = 0; x < min; x++) {
              if (!tracks[x].getCreatureName().equals(performer.getName()))
              {
                ...
                if (performer.getPower() > 0) {
                  performer.getCommunicator().sendNormalServerMessage(tracks[x]
                    .getCreatureName() + " (" + tracks[x].getId() + " [" + direction + "] " + hours + " hrs");
                } else {
                  performer.getCommunicator().sendNormalServerMessage("You find tracks of " + tracks[x]
                    .getCreatureName() + " leading " + direction + " done " + timeString);
                }
              }
              else
              {
  ...
  static boolean absorb(Creature performer, int tilex, int tiley, int tile, float counter, Action act)
  {
    ...
    if (Tiles.decodeHeight(tile) > 0)
    {
      ...
      if (counter == 1.0F)
      {
        ...
        performer.sendActionControl(Actions.actionEntrys['ś'].getVerbString(), true, 100);
      ...
      if (act.currentSecond() == 5)
      {
        ...
        Server.getInstance().broadCastAction("Roots stretch out from the mycelium, entangling " + performer
          .getName() + ".", performer, 5);
      }
      else if (counter > 10.0F)
      {
        ...
        if ((wounds != null) && (wounds.getWounds().length > 0))
        {
          ...
          w[0].modifySeverity(60536);
        }
        if (performer.getDisease() > 0) {
  ...
  static final boolean firstAid(Creature performer, Item bandaid, Wound wound, float counter, Action act)
  {
    ...
    if ((bandaid.isHealingSalve()) && ((wound.isInternal()) || (wound.isBruise()) || (wound.isPoison())))
    {
      ...
      if (nums < wound.getNumBandagesNeeded()) {
        if ((performer instanceof Player))
        {
          performer.getCommunicator().sendNormalServerMessage("The " + bandaid
            .getName() + " will not cover the wound.", (byte)3);
          done = true;
        }
      }
      bandage = "smear";
    ...
    else if (!wound.isInternal())
    {
      ...
      if (nums < wound.getNumBandagesNeeded()) {
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("The " + bandaid
            .getName() + " does not cover the wound.", (byte)3);
          done = true;
        }
      }
      if (wound.isBandaged())
      ...
      {
        performer.getCommunicator().sendNormalServerMessage("You can't use " + bandaid
          .getNameWithGenus() + " to bind the wound!", (byte)3);
        done = true;
      }
    }
    else
    ...
    else if (!done)
    {
      ...
      if (counter == 1.0F)
      {
        ...
        performer.sendActionControl(Actions.actionEntrys['Ä'].getVerbString(), true, time);
        
        performer.getCommunicator().sendNormalServerMessage("You start to " + bandage + " the wound.");
        if (wound.getCreature() == performer)
        {
          Server.getInstance().broadCastAction(performer
            .getName() + " starts tending " + performer.getHisHerItsString() + " wounds.", performer, 5);
        }
        else
        {
          wound.getCreature().getCommunicator().sendNormalServerMessage(performer.getName() + " starts tending your wounds.");
          Server.getInstance().broadCastAction(performer.getName() + " starts tending " + targetName + "'s wounds.", performer, wound
            .getCreature(), 5);
        }
        if (act.getRarity() != 0) {
      ...
      if ((counter > 5.0F) && (counter * 10.0F > time))
      {
        ...
        power = firstaid.skillCheck(wound.getSeverity() / 1000.0F, bandaid, 0.0D, false, counter);
        if (act.getRarity() > 0) {
        ...
        if (power > 0.0D)
        {
          performer.getCommunicator().sendNormalServerMessage("You " + bandage + " the wound with " + bandaid
            .getNameWithGenus() + ".");
          if (wound.getCreature() == performer)
          {
            Server.getInstance().broadCastAction(performer
              .getName() + " " + bandage + "s " + performer.getHisHerItsString() + " wound with " + bandaid
              .getNameWithGenus() + ".", performer, 5);
          }
          else
          {
            wound.getCreature().getCommunicator().sendNormalServerMessage(performer
              .getName() + " " + bandage + "s the wound with " + bandaid.getNameWithGenus() + ".");
            
            Server.getInstance().broadCastAction(performer
              .getName() + " " + bandage + "s " + targetName + "'s wound with " + bandaid
              .getNameWithGenus() + ".", performer, wound.getCreature(), 5);
            if (((wound.getCreature() instanceof Player)) && (
              (performer.getDeity() == null) || (!performer.getDeity().isHateGod()))) {
              performer.maybeModifyAlignment(1.0F);
            }
          }
          wound.setBandaged(true);
          wound.modifySeverity((int)Math.min(-wound.getSeverity() / 10.0F, 
          
            -(3275.0F * rarity + power * ((performer.getCultist() != null) && (performer.getCultist().healsFaster()) ? 100.0F : 50.0F))));
          
          performer.achievement(536);
        }
        else if ((!wound.isInternal()) && (!wound.isBruise()) && (power < -90.0D) && 
          (bandaid.getRarity() == 0) && (act.getRarity() == 0))
        {
          performer.getCommunicator().sendNormalServerMessage("You try to " + bandage + " the wound with " + bandaid
            .getNameWithGenus() + " but only make it worse!");
          if (wound.getCreature() == performer)
          {
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to " + bandage + " " + performer.getHisHerItsString() + " wound with " + bandaid
              .getNameWithGenus() + " but only makes it worse.", performer, 5);
          }
          else
          {
            wound.getCreature().getCommunicator().sendNormalServerMessage(performer
              .getName() + " tries to " + bandage + " the wound with " + bandaid
              .getNameWithGenus() + " but only makes it worse.");
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to " + bandage + " " + targetName + "'s wound with " + bandaid
              .getNameWithGenus() + " but only makes it worse.", performer, wound
              .getCreature(), 5);
          }
        ...
        else
        {
          ...
          if (wound.getCreature() == performer)
          {
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to " + bandage + " " + performer.getHisHerItsString() + " wound with " + bandaid
              .getNameWithGenus() + " but fails.", performer, 5);
          }
          else
          {
            wound.getCreature().getCommunicator().sendNormalServerMessage(performer
              .getName() + " tries to " + bandage + " the wound with " + bandaid
              .getNameWithGenus() + " but fails.");
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to " + bandage + " " + targetName + "'s wound with " + bandaid
              .getNameWithGenus() + " but fails.", performer, wound.getCreature(), 5);
          }
        }
        done = true;
      }
  ...
  static final boolean treat(Creature performer, Item cover, Wound wound, float counter, Action act)
  {
    ...
    else if (!done)
    {
      ...
      if (counter == 1.0F)
      {
        time = (int)(3.0D * (wound.getSeverity() / 2000.0F + (100.0D - firstaid.getKnowledge(0.0D) / 2.0D)));
        act.setTimeLeft(time);
        performer.sendActionControl(Actions.actionEntrys['Ä'].getVerbString(), true, time);
        ...
        {
          Server.getInstance().broadCastAction(performer
            .getName() + " starts treating " + performer.getHisHerItsString() + " wounds.", performer, 5);
        }
        else
        {
          wound.getCreature().getCommunicator().sendNormalServerMessage(performer.getName() + " starts treating your wounds.");
          Server.getInstance().broadCastAction(performer
            .getName() + " starts treating " + wound.getCreature().getNameWithGenus() + "'s wounds.", performer, 5);
        }
        if (act.getRarity() != 0) {
          performer.playPersonalSound("sound.fx.drumroll");
        }
      ...
      {
        Methods.sendSound(performer, "sound.work.firstaid.bandage");
        power = firstaid.skillCheck(wound.getSeverity() / 1000.0F, cover, 0.0D, false, counter);
        if (act.getPower() > 0.0F) {
          power = Math.max(power + act.getRarity() * 10, act.getRarity() * 10);
        }
        if ((power > 0.0D) || (Server.rand.nextInt(100) < 20))
        {
          performer.getCommunicator().sendNormalServerMessage("You treat the wound with " + cover
            .getNameWithGenus() + ".");
          if (wound.getCreature() == performer)
          {
            Server.getInstance().broadCastAction(performer
              .getName() + " treats " + performer.getHisHerItsString() + " wound.", performer, 5);
          }
          else
          {
            wound.getCreature().getCommunicator().sendNormalServerMessage(performer
              .getName() + " treats the wound with " + cover.getNameWithGenus() + " " + cover
              .getDescription().toLowerCase() + ".");
            Server.getInstance().broadCastAction(performer
              .getName() + " treats " + wound.getCreature().getName() + "'s wound with " + cover
              .getNameWithGenus() + ".", performer, 5);
            if (((wound.getCreature() instanceof Player)) && (
              (performer.getDeity() == null) || (!performer.getDeity().isHateGod()))) {
              performer.maybeModifyAlignment(1.0F);
          ...
          if (!wound.isAcidWound()) {
            wound.setHealeff((byte)(int)(cover.getAuxData() + act.getPower() + cover.getRarity()));
          } else if (cover.getTemplateId() == 128) {
            if (cover.getWeightGrams() > wound.getSeverity() / 10.0F)
            {
              wound.setType((byte)4);
              wound.setHealeff((byte)(int)(18.0F + act.getPower() + cover.getRarity()));
            }
            else
            {
        ...
        else if ((power < -90.0D) && (!cover.isLiquid()) && (cover.getRarity() == 0) && (act.getRarity() == 0))
        {
          performer.getCommunicator().sendNormalServerMessage("You try to treat the wound with " + cover
            .getNameWithGenus() + " but only make it worse!");
          if (wound.getCreature() == performer)
          {
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to treat " + performer.getHisHerItsString() + " wound but only makes it worse.", performer, 5);
          }
          else
          {
            wound.getCreature().getCommunicator().sendNormalServerMessage(performer
              .getName() + " tries to treat the wound with " + cover.getName() + " but only makes it worse.");
            
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to treat " + wound.getCreature().getName() + "'s wound with " + cover
              .getNameWithGenus() + " but only makes it worse.", performer, 5);
          }
          wound.modifySeverity(3275);
        }
        else
        {
          ...
          if (wound.getCreature() == performer) {
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to treat " + performer.getHisHerItsString() + " wound with " + cover
              .getNameWithGenus() + " but fails.", performer, 5);
          } else {
            Server.getInstance().broadCastAction(performer
              .getName() + " tries to treat " + wound.getCreature().getName() + "'s wound with " + cover
  ...
  static boolean shear(Action act, Item source, Creature performer, Creature target, float counter)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      time = Actions.getSlowActionTime(performer, animalHusbandry, source, 0.0D);
      
      act.setTimeLeft(time);
      performer.sendActionControl(Actions.actionEntrys['ʆ'].getVerbString(), true, time);
      
      performer.getCommunicator().sendNormalServerMessage("You start shearing the " + target
        .getName() + ".");
      Server.getInstance().broadCastAction(performer
        .getName() + " starts shearing the " + target.getName() + ".", performer, 5);
    }
    else
    {
      time = act.getTimeLeft();
    ...
    if (counter * 10.0F > time)
    {
      ...
      int knowledge = (int)animalHusbandry.getKnowledge(0.0D);
      float diff = getShearingDifficulty(target.getStatus().age, knowledge);
      double power = animalHusbandry.skillCheck(diff, source, 0.0D, false, counter);
      ...
      try
      {
        ...
        Item wool = ItemFactory.createItem(921, Math.max(1.0F, Math.min(100.0F, ql * modifier)), (byte)69, act
          .getRarity(), null);
        ...
        performer.getCommunicator().sendNormalServerMessage("You finish shearing the " + target
          .getName() + ".");
        Server.getInstance().broadCastAction(performer
          .getName() + " shears the " + target.getName() + ".", performer, 5);
      }
  ...
  static boolean milk(Action act, Item tool, Creature performer, Creature target, float counter)
  {
    ...
    if (cont.length < 2)
    {
      if (cont.length == 1)
      {
        if ((cont[0].getTemplateId() != 142) && 
          (cont[0].getTemplateId() != 1012) && 
          (cont[0].getTemplateId() != 1013))
        {
          performer.getCommunicator().sendNormalServerMessage("The milk would be destroyed. Empty the " + tool
            .getName() + " first.");
          return true;
        }
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("You cannot mix types of milk. Empty the " + tool
            .getName() + " first.");
          return true;
        }
      }
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + tool
        .getName() + " is already full and therefore will not be able to contain any milk.");
      return true;
    }
    ...
    if (counter == 1.0F)
    {
      if (tool.getFreeVolume() < 1000) {
        performer.getCommunicator().sendNormalServerMessage("The " + tool
          .getName() + " will not be able to contain all of the milk.");
      }
      int maxSearches = calcMaxPasses(target, skill.getKnowledge(0.0D), tool);
      ...
      performer.sendActionControl(Actions.actionEntrys['ř'].getVerbString(), true, (int)totalTime);
      performer.getStatus().modifyStamina(-500.0F);
    }
    else
    {
    ...
    if (counter * 10.0F >= act.getNextTick())
    {
      ...
      int diff = getShearingDifficulty(target.getStatus().age, knowledge);
      ...
      double power = skill.skillCheck(diff, tool, 0.0D, false, counter / searchCount);
      try
      {
        ...
        if (searchCount < maxSearches)
        {
          ...
          {
            performer.getCommunicator().sendNormalServerMessage("The " + tool
              .getName() + " is full, and cannot fit anymore milk in.");
            Server.getInstance().broadCastAction(performer
              .getName() + " milks the " + target.getName() + ".", performer, 5);
            return true;
          }
          if (tool.getFreeVolume() < 1000) {
            performer.getCommunicator().sendNormalServerMessage("The " + tool
      ...
      if (toReturn)
      {
        performer.getCommunicator().sendNormalServerMessage("You finish milking the " + target
          .getName() + ".");
        Server.getInstance().broadCastAction(performer
          .getName() + " milks the " + target.getName() + ".", performer, 5);
      }
  ...
  public static final boolean catchFlies(Creature performer, Creature target, Item jar, short action, Action act, float counter)
  {
    ...
    if (!jar.isFlyTrap())
    {
      performer.getCommunicator().sendNormalServerMessage("You cannot catch flies with that! You need honey or vinegar in the " + jar
        .getName() + ".");
      return true;
    }
    if ((target.canBeGroomed()) || (counter > 1.0F))
    {
      ...
      int time = 20;
      target.shouldStandStill = true;
      int maxSearches = (int)((breeding.getKnowledge(0.0D) + 28.0D) / 27.0D) + 1;
      if (counter == 1.0F)
      {
        act.setNextTick(time);
        act.setTickCount(1);
        ...
        performer.sendActionControl(Actions.actionEntrys['Ǝ'].getVerbString(), true, act
          .getTimeLeft());
        performer.getStatus().modifyStamina(-500.0F);
        return false;
      }
      ...
      if (counter * 10.0F > act.getNextTick())
      {
        ...
        if (breeding.skillCheck(target.getBaseCombatRating(), jar, 0.0D, false, counter / 2.0F) > 0.0D)
        {
          try
          {
            target.setLastGroomed(System.currentTimeMillis());
            
            int knowledge = (int)breeding.getKnowledge(0.0D);
            float ql = knowledge - Server.rand.nextFloat() * (knowledge / 5);
            ...
            Server.getInstance().broadCastAction(performer
              .getName() + " puts something in a jar.", performer, 5);
        ...
        else
        {
          performer.getCommunicator().sendNormalServerMessage("The " + target
            .getName() + " shys away and interrupts the action.", (byte)3);
  ...
  private static int calcMaxPasses(Creature creature, double currentSkill, Item tool)
  {
    ...
    return Math.min(getCreatureAgeFactor(creature), (int)(currentSkill + 28.0D) / 27 + bonus);
  }
  
  static final void sendSetKingdomQuestion(Creature performer, Item wand)
  {
    if (((wand.getTemplateId() == 315) || (wand.getTemplateId() == 176)) && 
      (performer.getPower() >= 2))
    {
  ...
  {
    if (performer.isPlayer()) {
      if (((Player)performer).lastSentQuestion > 0)
      {
  ...
  {
    if ((performer.getDeity() != null) && ((performer.getFaith() == 30.0F) || (performer.getPower() > 0)) && 
      (!performer.isPriest()))
    {
  ...
  static final boolean tame(Action act, Creature performer, Creature target, Item food, float counter)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + target
        .getName() + " seems focused on " + target.getLeader().getName() + " and ignores your attempts.");
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + target
        .getName() + " is too restrained and ignores your attempts.");
      return true;
    }
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + target
        .getName() + " is busy fighting " + target.opponent.getName() + ".");
      return true;
    }
    if (target.isReborn())
    {
    ...
    if ((performer.getCurrentTile() != null) && (target.getCurrentTile() != null)) {
      if (performer.getCurrentTile().getStructure() != target.getCurrentTile().getStructure())
      {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + target
        .getName() + " sniffs the air and looks too nervous to focus.");
      return true;
    }
    ...
    if (counter == 1.0F)
    {
      ...
      time = Actions.getSlowActionTime(performer, taming, food, 0.0D) / 2;
      act.setTimeLeft(time);
    ...
    if ((Server.rand.nextInt(10) == 0) && (!target.isFighting()) && (!target.isDominated()) && 
      (target.isAggHuman()))
    {
    ...
    if (counter * 10.0F > time)
    {
      ...
      if (target.getLoyalty() > 99.0F)
      {
        ...
        performer.getCommunicator().sendNormalServerMessage(target
          .getName() + " is already tame, so you don't learn anything new.");
      }
      float difficultyBonus = target.getBaseCombatRating();
      
      double bonus = Math.max(0.0D, ownsoul.skillCheck(soul.getKnowledge() + 5.0D, -difficultyBonus, dryrun, counter));
      if (performer.getDeity() == Deities.getDeity(1)) {
        if ((performer.getFaith() > 20.0F) && (performer.getFavor() >= 20.0F)) {
          bonus += 20.0D;
        }
      }
      double power = taming.skillCheck(soul.getKnowledge() + difficultyBonus, bonus, dryrun, counter);
      if (power > 0.0D)
      {
        double maxpower = taming.getKnowledge(0.0D);
      ...
      else
      {
        ...
        if (power < -50.0D)
        {
          if (!target.isFighting())
          {
            ...
            target.setTarget(performer.getWurmId(), true);
            performer.getCommunicator().sendNormalServerMessage(target
              .getName() + " becomes aggravated and attacks you!");
          }
          if ((target.isDominated()) && (target.getDominator() == performer)) {
  ...
  static void tameEffect(Creature performer, Creature target, double power, boolean dryrun, double maxpower)
  {
    ...
    if (performer.getPet() != null)
    {
      if (performer.getPet() != target)
      {
        performer.getCommunicator().sendNormalServerMessage("You can't keep control over " + performer
          .getPet().getNameWithGenus() + " as well.");
    ...
    {
      target.setLoyalty((float)Math.min(maxpower, target.getLoyalty() + power / 10.0D));
      target.getStatus().setLastPolledLoyalty();
      performer.getCommunicator()
        .sendNormalServerMessage("You create a stronger bond with " + target.getName() + ".");
    }
  ...
  static final void petGiveAway(Creature dominator, Creature receiver)
  {
    if ((receiver instanceof Player))
    {
      if (dominator.getPet() != null) {
        if (DbCreatureStatus.getIsLoaded(dominator.getPet().getWurmId()) == 1)
        {
      ...
      {
        dominator.getCommunicator().sendNormalServerMessage(receiver
          .getName() + " does not accept pets now. " + 
          LoginHandler.raiseFirstLetter(receiver.getHeSheItString()) + " needs to type /invitations in a chat window.");
        ...
        if (receiver.getPet() == null)
        {
          if (dominator.getPet() != null) {
            try
            {
              ...
              {
                dominator.getCommunicator().sendNormalServerMessage("You control that creature with your will. It can not be given away."); return;
              }
              Skill taming;
              try
              {
              ...
              {
                logger.log(Level.WARNING, dominator
                  .getPet().getName() + " no soul strength when " + receiver.getName() + " tames.");
                
                receiver.getCommunicator().sendNormalServerMessage(dominator
                  .getPet().getName() + " seems untamable.");
                return;
              }
              Skill ownsoul = null;
              try
              ...
              double bonus = Math.max(0.0D, ownsoul.skillCheck(soul.getKnowledge(0.0D), 0.0D, dryrun, 1.0F));
              double power = taming.skillCheck(soul.getKnowledge(0.0D) + difficultyBonus, bonus, dryrun, 1.0F);
              if (power > 0.0D)
              {
                ...
                dominator.getCommunicator().sendNormalServerMessage("You give away your pet to " + receiver
                  .getName() + ".");
                receiver.getCommunicator().sendNormalServerMessage(dominator
                  .getName() + " gives you " + dominator.getHisHerItsString() + " " + receiver
                  .getPet().getName());
              }
              else if (power < -30.0D)
              {
                if (!dominator.getPet().isFighting())
                {
                  ...
                  receiver.getCommunicator().sendNormalServerMessage(dominator
                    .getPet().getName() + " becomes aggravated and attacks you!");
                }
                dominator.getCommunicator().sendAlertServerMessage(dominator
                  .getPet().getName() + " is tame no more.");
                dominator.getPet().setDominator(-10L);
                dominator.setPet(-10L);
              }
              else
              {
                dominator.getCommunicator().sendAlertServerMessage(receiver
                  .getName() + " fails to take control over the " + dominator
                  .getPet().getName() + ".");
              }
            }
            catch (Exception ex)
            {
              logger.log(Level.WARNING, dominator
                .getName() + " and " + receiver.getName() + ": " + ex.getMessage(), ex);
            }
          }
  ...
  static boolean findCaveExit(Creature performer, Creature orderer)
  {
    ...
    if (!performer.isOnSurface()) {
      ...
      else
      {
        ...
        {
          orderer.getCommunicator().sendNormalServerMessage(performer
            .getName() + " found cave exit at " + tilePosX + ", " + tilePosY);
          try
          {
            PathFinder pf = new PathFinder();
            Path path = pf.findPath(performer, performer.getTileX(), 
              (int)performer.getStatus().getPositionY() >> 2, tilePosX, tilePosY, performer.isOnSurface(), 10);
            if (path != null) {
              while (!path.isEmpty()) {
                try
                {
                  PathTile p = path.getFirst();
                  ItemFactory.createItem(344, 100.0F, (p.getTileX() << 2) + 2, 
                    (p.getTileY() << 2) + 2, 180.0F, performer.isOnSurface(), (byte)0, -10L, null);
                  path.removeFirst();
                }
                catch (FailedException fe)
                {
  ...
  {
    boolean done = true;
    if ((Servers.localServer.PVPSERVER) && (performer.getBodyControlSkill().getRealKnowledge() < 21.0D))
    {
        ...
        for (VolaTile lVolaTile : set)
        {
          ...
          for (lCret : crets) {
            if ((lCret != performer) && (lCret.getPower() <= 0))
            {
              ...
              performer.getCommunicator()
                .sendNormalServerMessage("You are too close to other creatures now.", (byte)3);
              return true;
            }
          }
        }
      }
      performer.sendActionControl(Actions.actionEntrys[''].getVerbString(), true, 
        (int)Math.max(50.0D, (35.0D - performer.getBodyControl()) * 10.0D));
      performer.getCommunicator().sendNormalServerMessage("You try to hide.");
    }
    else if (counter > Math.max(5.0D, 35.0D - performer.getBodyControl()))
    {
      done = true;
      ...
      if (t != null)
      {
        double mod = getStealthTerrainModifier(performer, t.tilex, t.tiley, performer.isOnSurface());
        if (Server.rand.nextFloat() * 100.0F < mod)
        {
          ...
          try
          {
            ...
            Object wornl = lhand.getItems();
            if (wornl != null) {
              for (Item i : (Set)wornl) {
            ...
            if (!found)
            {
              ...
              Object wornr = rhand.getItems();
              if (wornr != null) {
                for (Item i : (Set)wornr) {
                  if ((i.getTemplateId() == 297) && (i.getMaterial() == 7))
                  {
                    performer.achievement(93);
                    break;
  ...
  static final boolean unbrand(Creature performer, Creature target, Item source, Action act, float counter)
  {
    ...
    if (!source.isOnFire())
    {
      performer.getCommunicator().sendNormalServerMessage("The " + source
        .getName() + " must be very hot if you're going to remove a brand with it.", (byte)3);
      
      return true;
    }
    if ((target.isLeadable(performer)) && (performer.getCitizenVillage() != null)) {
      if (performer.isWithinTileDistanceTo(target.getTileX(), target.getTileY(), 
        (int)(target.getStatus().getPositionZ() + target.getAltOffZ()) >> 2, 3)) {
        if ((target.getCurrentVillage() != null) && (target.getCurrentVillage() == performer.getCitizenVillage()))
        {
          ...
          if (brand != null) {
            try
            {
              ...
              if (villageBrand == performer.getCitizenVillage())
              {
                ...
                {
                  Server.getInstance().broadCastAction(performer
                    .getName() + " prepares to remove brand on " + target.getNameWithGenus() + ".", performer, 5);
                  
                  performer.getCommunicator().sendNormalServerMessage("You prepare to remove brand on the " + target
                    .getName() + ".");
                ...
                {
                  Server.getInstance().broadCastAction(performer
                    .getName() + " promptly raises " + performer.getHisHerItsString() + " " + source
                    .getName() + ".", performer, 5);
                  
                  performer.getCommunicator().sendNormalServerMessage("Determined, you raise your " + source
                    .getName() + ".");
                  return false;
                }
                if (counter > 10.0F)
                {
                  ...
                  Server.getInstance().broadCastAction("The " + target
                    .getName() + " makes a horrible sound as " + performer.getName() + " removes the brand.", performer, 5);
                  
                  performer.getCommunicator().sendNormalServerMessage("The " + target
                    .getName() + " makes a horrible sound as you press your " + source
                    .getName() + " against it and remove the brand.");
  ...
  static final boolean brand(Creature performer, Creature target, Item source, Action act, float counter)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The " + source
        .getName() + " must be very hot if you're going to brand with it.", (byte)3);
      return true;
    }
    if ((target.isLeadable(performer)) && (performer.getCitizenVillage() != null)) {
      if (performer.isWithinTileDistanceTo(target.getTileX(), target.getTileY(), 
        ...
        if ((target.getCurrentVillage() != null) && (target.getCurrentVillage() == performer.getCitizenVillage()))
        {
          ...
          {
            if ((!Servers.isThisAPvpServer()) && 
              (target.getCurrentVillage().getMaxCitizens() <= Creatures.getInstance().getBranded(performer.getCurrentVillage().getId()).length))
            {
            ...
            {
              Server.getInstance().broadCastAction(performer
                .getName() + " prepares to brand " + target.getNameWithGenus() + ".", performer, 5);
              
              performer.getCommunicator().sendNormalServerMessage("You prepare to brand the " + target
                .getName() + ".");
            ...
            else if (counter > 10.0F)
            {
              ...
              SoundPlayer.playSound(target.getDeathSound(), target, 1.0F);
              Server.getInstance().broadCastAction("The " + target
                .getName() + " makes a horrible sound as " + performer.getName() + " brands it.", performer, 5);
              
              performer.getCommunicator().sendNormalServerMessage("The " + target
                .getName() + " makes a horrible sound as you press your " + source.getName() + " against it.");
  ...
  static final boolean naturalBreed(Creature breeder, Creature target, Action act, float counter)
  {
    ...
    if (breeder.mayMate(target)) {
      if (!target.isPregnant()) {
        ...
        {
          if (!breeder.isWithinDistanceTo(target.getPosX(), target.getPosY(), target
            .getPositionZ() + target.getAltOffZ(), 3.0F, 0.0F)) {
            return true;
          }
          ...
          if (counter == 1.0F)
          {
            Server.getInstance().broadCastAction("The " + breeder
              .getName() + " and the " + target.getName() + " gets to it!", breeder, 5);
          }
          else if (counter > 30 + Server.rand.nextInt(20))
          {
            ...
            if (Server.rand.nextInt(30) == 0)
            {
              if (breeder.getSex() == 1)
              {
                ...
                Server.getInstance().broadCastAction("Something seems to have happened between the " + breeder
                  .getName() + " and the " + target
                  .getName() + "!", breeder, 5);
              }
              ...
              Server.getInstance().broadCastAction("The " + breeder
                .getName() + " and the " + target.getName() + " stops whatever they were doing.", breeder, 5);
            }
  ...
  static final boolean breed(Creature performer, Creature target, short action, Action act, float counter)
  {
    ...
    if (performer.getFollowers().length == 1)
    {
      ...
      if (breeder == null)
      {
        performer.getCommunicator().sendNormalServerMessage("Which creature should breed with the " + target
          .getName() + "?", (byte)3);
        return true;
      }
      ...
      if (breeder.mayMate(target))
      {
        if (!target.isPregnant())
        {
          if (!breeder.isPregnant())
          {
              ...
              .getPositionZ() + target.getAltOffZ(), 3.0F, 0.0F))
            {
              ...
              if (result != null)
              {
                performer.getCommunicator().sendNormalServerMessage("The creatures are separated by the " + result
                  .getFirstBlocker().getName() + ".", (byte)3);
            ...
            else
            {
              ...
              performer.sendToLoggers(breeder.getName() + " at " + breeder.getPosX() + "," + breeder.getPosY() + ", " + target
                .getName() + " at " + target.getPosX() + "," + target.getPosY() + ".");
            ...
            {
              performer.getCommunicator().sendNormalServerMessage("The " + breeder
                .getName() + " doesn't seem to be in the mood right now.", (byte)3);
              return true;
            }
            ...
            if (counter == 1.0F)
            {
              ...
              Server.getInstance().broadCastAction(performer
                .getName() + " does " + performer.getHisHerItsString() + " job. The " + breeder
                .getName() + " and the " + target.getName() + " get intimate!", performer, 5);
              
              performer.getCommunicator().sendNormalServerMessage("The " + breeder
              
                .getName() + " and the " + target
                
                .getName() + " get intimate." + (performer
                
                .getPower() >= 5 ? target.getBreedCounter() + ", " + breeder
                .getBreedCounter() : ""));
              if (performer.isPlayer()) {
                performer.sendActionControl(Actions.actionEntrys['Ż']
                  .getVerbString(), true, act.getTimeLeft());
              }
            }
            else if ((performer.getPower() >= 5) || (counter > act.getTimeLeft() / 10))
            {
              ...
              if (breeding.skillCheck(breeder.getBaseCombatRating(), 0.0D, false, performer
                .getPower() >= 5 ? 20.0F : counter) > 0.0D)
              {
                if (breeder.getSex() == 1)
                {
                  breeder.mate(target, performer);
                  performer.getCommunicator().sendNormalServerMessage("The " + breeder
                    .getName() + " will probably give birth in a while!", (byte)2);
                }
                else if (target.getSex() == 1)
                {
              ...
              {
                performer.getCommunicator().sendNormalServerMessage("The " + breeder
                  .getName() + " shys away and interrupts the action.");
              }
          ...
          {
            performer.getCommunicator().sendNormalServerMessage("The " + breeder
              .getName() + " is already pregnant.", (byte)3);
          }
        }
        else {
        ...
        performer.getCommunicator().sendNormalServerMessage("The " + breeder
          .getName() + " may not mate with " + target.getName() + ".", (byte)3);
      }
    }
    else
    {
  ...
  static final boolean ride(Creature performer, Creature target, short action)
  {
    ...
    if ((action == 331) || (action == 332)) {
      if (performer.getVehicle() == -10L)
      {
        if ((target.getDominator() != null) && (performer.getKingdomId() != target.getDominator().getKingdomId()))
        {
          performer.getCommunicator().sendNormalServerMessage("The " + target
            .getName() + " refuses to let you embark.", (byte)3);
          return true;
        }
        if (target.getHitched() != null)
        {
          performer.getCommunicator().sendNormalServerMessage("The " + target
            .getName() + " is too occupied to be ridden at the moment.", (byte)3);
          return true;
        }
        if (performer.isWithinDistanceTo(target.getPosX(), target.getPosY(), target
          .getPositionZ() + target.getAltOffZ(), 8.0F))
        {
          if (target.getStrengthSkill() < 20.0D)
          {
            performer.getCommunicator().sendNormalServerMessage("The " + target
              .getName() + " is too weak to ride.", (byte)3);
            return true;
          }
          ...
          {
            performer.getCommunicator().sendNormalServerMessage("The " + target
              .getName() + " looks confused by its equipment and refuses to move.", (byte)3);
            return true;
          }
            ...
            if (action == 331)
            {
              if (target.getHitched() != null)
              {
                ...
                performer.getCommunicator().sendNormalServerMessage(
                  StringUtil.format("You can not control the %s while it is in front of the %s.", new Object[] {target
                  .getName(), 
                  Vehicle.getVehicleName(target.getHitched()) }), (byte)3);
              ...
              if (performer.getDraggedItem() != null)
              {
                ...
                performer.getCommunicator().sendNormalServerMessage(
                  StringUtil.format("You may not drag %s while trying to control the %s.", new Object[] {performer
                  .getDraggedItem().getNameWithGenus(), vehicName }), (byte)3);
              ...
              if (VehicleBehaviour.mayDriveVehicle(performer, target))
              {
                if (VehicleBehaviour.canBeDriverOfVehicle(performer, vehicle))
                {
                  ...
                  for (int x = 0; x < vehicle.seats.length; x++) {
                    ...
                    {
                      float r = -(target.getStatus().getRotation() + 180.0F) * 3.1415927F / 180.0F;
                      ...
                      BlockingResult result = Blocking.getBlockerBetween(performer, performer
                        ...
                        .getPositionZ(), target.getPositionZ(), performer
                        .isOnSurface(), target.isOnSurface(), false, 4, -1L, performer
                        .getBridgeId(), target.getBridgeId(), false);
                      if (result != null)
                      {
                        performer.getCommunicator().sendNormalServerMessage("The " + result
                          .getFirstBlocker().getName() + " is in the way. You can not reach the seat.", (byte)3);
                      ...
                      if ((target.getTemplateId() == 12) || 
                        (target.getTemplateId() == 42) || 
                        (target.getTemplateId() == 49)) {
                        performer.achievement(545);
                      }
                      if (target.getTemplateId() != 21) {
                        break;
                      }
                      performer.achievement(594); break;
                    }
                  }
                  if (!addedDriver)
                  {
                    ...
                    performer.getCommunicator().sendNormalServerMessage(
                      StringUtil.format("You may not %s the %s as a %s right now. The space is occupied.", new Object[] { vehicle.embarkString, vehicName, vehicle.pilotName }), (byte)3);
                  }
                }
                else
                {
                  ...
                  performer.getCommunicator().sendNormalServerMessage(
                    StringUtil.format("You are not agile enough to control the %s. You need %.2f in %s.", new Object[] { vehicName, 
                    
                    Float.valueOf(vehicle.skillNeeded), 
                    SkillSystem.getNameFor(104) }), (byte)3);
                }
              }
              else
              {
                ...
                performer.getCommunicator().sendNormalServerMessage(
                  StringUtil.format("You are not allowed to %s the %s as a %s.", new Object[] { vehicle.embarkString, vehicName, vehicle.pilotName }), (byte)3);
              }
            }
            else if (action == 332)
            {
              if (VehicleBehaviour.mayEmbarkVehicle(performer, target))
              {
                if (performer.getDraggedItem() == null)
                {
                  ...
                  for (int x = 0; x < vehicle.seats.length; x++) {
                    if ((!vehicle.seats[x].isOccupied()) && (vehicle.seats[x].type == 1))
                    {
                      ...
                      BlockingResult result = Blocking.getBlockerBetween(performer, performer
                        .getPosX(), performer.getPosY(), newposx, newposy, performer
                        .getPositionZ(), target.getPositionZ(), performer.isOnSurface(), target
                        .isOnSurface(), false, 4, -1L, performer.getBridgeId(), target
                        .getBridgeId(), false);
                      if (result == null)
                      {
                        addedPassenger = true;
                  ...
                  if (!addedPassenger)
                  {
                    ...
                    performer.getCommunicator().sendNormalServerMessage(
                      StringUtil.format("You may not %s the %s as a passenger right now.", new Object[] { vehicle.embarkString, vehicName }));
                    if (wallInWay) {
                ...
                else
                {
                  ...
                  performer.getCommunicator().sendNormalServerMessage(
                    StringUtil.format("You may not %s the %s while dragging something.", new Object[] { vehicle.embarkString, vehicName }), (byte)3);
                }
              }
              else
              {
                ...
                performer.getCommunicator()
                  ...
                  StringUtil.format("You are not allowed to %s the %s.", new Object[] { vehicle.embarkString, vehicName }), (byte)3);
              }
            }
          }
          else
  ...
  public static final float getStealthTerrainModifier(Creature performer, int tilex, int tiley, boolean surfaced)
  {
      ...
      for (int y = -1; y <= 1; y++)
      {
        ...
        if (surfaced) {
          if ((tilex > 0) && (tilex < Zones.worldTileSizeX - 1) && (tiley > 0) && (tiley < Zones.worldTileSizeY - 1))
          {
            ...
            Tiles.Tile theTile = Tiles.getTile(type);
            if (theTile.isTree()) {
              if (FoliageAge.getAgeAsByte(data) > FoliageAge.MATURE_TWO.getAgeId()) {
                mod += 5.0F;
              }
            }
          }
    ...
    if (!surfaced) {
      mod = (float)(mod + (10.0D + performer.getBodyControl()));
    } else if (WurmCalendar.isNight()) {
      mod = (float)(mod + (20.0D + performer.getBodyControl()));
    } else if (WurmCalendar.isMorning()) {
      mod = (float)(mod + (5.0D + performer.getBodyControl() / 4.0D));
    }
  ...
  public static final boolean groom(Creature performer, Creature target, Item groomTool, short action, Action act, float counter)
  {
    ...
    if (target.canBeGroomed())
    {
      ...
      if (counter == 1.0F)
      {
        time = Actions.getSlowActionTime(performer, breeding, groomTool, 0.0D);
        act.setTimeLeft(time);
      ...
      {
        toReturn = true;
        if (breeding.skillCheck(target.getBaseCombatRating(), groomTool, 0.0D, false, performer
          .getPower() >= 5 ? 20.0F : counter / 2.0F) > 0.0D)
        {
          target.setLastGroomed(System.currentTimeMillis());
          Server.getInstance().broadCastAction(performer
            .getName() + " finishes tending to the " + target.getName() + ".", performer, 5);
          performer.getCommunicator().sendNormalServerMessage("You have now tended to " + target
            .getName() + " and " + target.getHeSheItString() + " seems pleased.");
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\behaviours\MethodsCreatures.class
