...
public class EpicMission
{
  ...
  public EpicMission(int entityid, int scenarioId, String name, String scenarioName, int mission, byte missionType, int difficulty, float progress, int server, long time, boolean load, boolean isCurrent)
  {
    ...
    this.rewards = generateRewardsString();
  ...
  private String generateRewardsString()
  {
    ...
    EpicMissionEnum missionEnum = EpicMissionEnum.getMissionForType(getMissionType());
    if (missionEnum == null) {
      return "Unknown";
    }
    int karmaPerPart = missionEnum.getBaseKarma();
    int karmaSplit = missionEnum.getKarmaBonusDiffMult() * getDifficulty();
    int sleepPerPart = missionEnum.getBaseSleep();
    int sleepSplit = missionEnum.getSleepBonusDiffMult() * getDifficulty();
    int numReq = EpicServerStatus.getNumberRequired(getDifficulty(), missionEnum);
    if (karmaPerPart > 0) {
    ...
    MissionTrigger[] triggers = MissionTriggers.getMissionTriggers(getMissionId());
    ...
    if (karmaSplit > 0) {
      if (missionEnum.isKarmaMultProgress()) {
        sb.append(" as well as " + (int)Math.ceil(karmaSplit / numReq) + " karma per " + (item ? "item" : creature ? "creature" : "action"));
    ...
    if (Servers.isThisAnEpicServer()) {
      sb.append(". The movement timer for " + Deities.getEntityName(this.epicEntityId) + " will also be reduced by " + 
        EpicMissionEnum.getTimeReductionForMission(getMissionType(), getDifficulty()) / 3600000L + " hours.");
    }
    return sb.toString();
  }
  ...
  public boolean equals(Object obj)
  {
    ...
    if ((this.epicEntityId != other.epicEntityId) || (this.missionId != other.missionId) || (this.serverId != other.serverId)) {
      return false;
    }
    return true;
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\epic\EpicMission.class
