  ...
  implements MiscConstants, TimeConstants
{
  ...
  static void setupMissionItemTemplates()
  {
    ...
    for (ItemTemplate lTemplate : templates) {
      if ((lTemplate.isMissionItem()) && (!lTemplate.isUseOnGroundOnly()) && (!lTemplate.isNoTake()) && (!lTemplate.unique) && (!lTemplate.artifact) && 
        (!lTemplate.isRiftLoot()) && 
        (lTemplate.getTemplateId() != 737) && (lTemplate.getTemplateId() != 683) && 
  ...
  public static final EpicMission[] getEpicMissionsForKingdomTemplate(byte kingdomTemplateId)
  {
    ...
    if (toRet.size() > 0) {
      return (EpicMission[])toRet.toArray(new EpicMission[toRet.size()]);
    }
  ...
  public static final EpicMission[] getEpicMissionsForDeity(int deityNum)
  {
    ...
    for (EpicMission em : epicMissions) {
      if (em.isCurrent()) {
        if (em.getEpicEntityId() == deityNum) {
          toRet.add(em);
        }
      }
    }
      ...
      for (int x = p.getTileX(); x < p.getTileX() + sizeX; x++) {
        for (int y = p.getTileY() - sizeY; y < p.getTileY(); y++) {
          if (Server.rand.nextInt(Math.max(1, sizeX / 2)) == 0)
          {
            ...
            if (isTree)
            {
              if ((theTile.isMyceliumTree()) || (theTile.isNormalTree())) {
                if (FoliageAge.getAgeAsByte(data) > FoliageAge.MATURE_ONE.getAgeId()) {
  ...
  private final void linkMission(Mission m, MissionTrigger trig, TriggerEffect effect, byte missionType, int difficulty)
  {
    ...
    EpicMission mp = new EpicMission((int)m.getOwnerId(), currentScenario.getScenarioNumber(), m.getName(), currentScenario.getScenarioName(), m.getId(), missionType, difficulty, 1.0F, Servers.localServer.id, System.currentTimeMillis() + this.maxTimeSecs * 1000L, false, true);
  ...
  {
    return (ItemTemplate)itemplates.get(Server.rand.nextInt(itemplates.size()));
  }
  ...
  private static final void destroySpecificMission(EpicMission em)
  {
    ...
    for (Mission m : missions) {
      if ((m.getCreatorType() == 2) && (m.getOwnerId() == em.getEpicEntityId())) {
        if (!m.isInactive())
        {
          MissionPerformer[] allperfs;
          if (failed)
          {
            allperfs = MissionPerformed.getAllPerformers();
          ...
          for (Object t : triggers) {
            if (((MissionTrigger)t).getMissionRequired() == m.getId())
            {
              TriggerEffects.destroyEffectsForTrigger(((MissionTrigger)t).getId());
              ((MissionTrigger)t).destroy();
            }
          }
          for (Object e : effects) {
            if (((TriggerEffect)e).getMissionId() == m.getId()) {
              ((TriggerEffect)e).destroy();
            }
          }
        }
      }
  ...
  public static final void pollExpiredMissions()
  {
    ...
    {
      mis = Missions.getMissionWithId(m.getMissionId());
      if (m.isCurrent()) {
        deities.add(Integer.valueOf(m.getEpicEntityId()));
      }
      if ((m.isCurrent()) && (System.currentTimeMillis() > m.getExpireTime()))
      {
        ...
        if (!Servers.localServer.EPIC) {
          if ((dietyNum > 0) && (dietyNum <= 4))
          {
            ...
            if (getCurrentScenario() != null) {
              es.generateNewMissionForEpicEntity(dietyNum, Deities.getDeityName(dietyNum), Math.max(1, m.getDifficulty() - 2), 604800, 
                getCurrentScenario().getScenarioName(), getCurrentScenario()
                .getScenarioNumber(), 
                getCurrentScenario().getScenarioQuest(), true);
            }
          }
        }
      }
      else if ((mis != null) && (!Servers.localServer.EPIC) && (m.isCurrent()) && 
        (System.currentTimeMillis() > mis.getLastModifiedAsLong() + 302400000L))
      {
        if (m.getMissionProgress() < 33.0F)
        {
    ...
    Object entityMap = Deities.getEntities();
    for (Object entry : ((Map)entityMap).entrySet())
    {
      boolean found = false;
      for (Integer i : deities) {
        if (((Integer)((Map.Entry)entry).getKey()).intValue() == i.intValue()) {
          found = true;
        }
      }
      if (!found)
      {
        EpicMission m = getEpicMissionForEntity(((Integer)((Map.Entry)entry).getKey()).intValue());
        if (m != null)
        {
          destroyLastMissionForEntity(((Integer)((Map.Entry)entry).getKey()).intValue());
          
          WcEpicStatusReport wce = new WcEpicStatusReport(WurmId.getNextWCCommandId(), false, ((Integer)((Map.Entry)entry).getKey()).intValue(), m.getMissionType(), m.getDifficulty());
          wce.sendToLoginServer();
          storeLastMissionForEntity(m.getEpicEntityId(), m);
        }
      }
  ...
  public void generateNewMissionForEpicEntity(int epicEntityId, String epicEntityName, int baseDifficulty, int maxTimeSeconds, String scenarioNameId, int scenarioIdentity, String questReasonString, boolean destroyPreviousMission)
  {
    ...
    while ((error) && (attempt++ < 10))
    {
      ...
      {
        e = getValrei().getEntity(epicEntityId);
        ...
        else if ((backupDifficultyMap == null) || (!backupDifficultyMap.containsKey(Integer.valueOf(epicEntityId))))
        {
          baseDifficulty = 1;
        ...
        else
        {
          if (baseDifficulty == -2)
          {
            baseDifficulty = ((Integer)backupDifficultyMap.get(Integer.valueOf(epicEntityId))).intValue();
            if (Server.rand.nextInt(Math.max(1, baseDifficulty)) == 0) {
              baseDifficulty++;
            }
          }
          ...
          {
            baseDifficulty = ((Integer)backupDifficultyMap.get(Integer.valueOf(epicEntityId))).intValue() - 1;
          }
      ...
      if (!destroyPreviousMission) {
        for (EpicMission m : getCurrentEpicMissions()) {
          if ((m.getEpicEntityId() == epicEntityId) && (System.currentTimeMillis() < m.getExpireTime()) && 
            (m.getMissionProgress() < 100.0F)) {
            return;
          }
        }
      }
      ...
      if (((newMission.getMissionType() == 121) || (newMission.getMissionType() == 122) || 
        (newMission.getMissionType() == 123)) && (deityNumber == 4) && (!Servers.localServer.EPIC) && 
      ...
      {
        int tries = 0;
        while ((newMission == null) && (tries < 30))
        {
      ...
      else
      {
        ...
        case 102: 
        case 103: 
          creationResponse = createBuildStructureMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 104: 
        case 105: 
          creationResponse = createMSRitualMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 106: 
        case 107: 
          creationResponse = createCutTreeMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 108: 
          creationResponse = createGTRitualMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 109: 
          creationResponse = createMISacrificeMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 110: 
          creationResponse = createGenericSacrificeMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 111: 
          creationResponse = createCreateItemMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 112: 
        case 113: 
          creationResponse = createGiveItemMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 114: 
        case 115: 
        case 116: 
          creationResponse = createSlayCreatureMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 117: 
        case 118: 
        case 119: 
          creationResponse = createSlayTraitorMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 120: 
          creationResponse = createDestroyGTMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 121: 
        case 122: 
        case 123: 
          creationResponse = createSacrificeCreatureMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
          break;
        case 124: 
          creationResponse = createSlayTowerGuardsMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, targetKingdom);
        }
  ...
  private String createBuildStructureMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    try
    {
      ...
      TriggerEffect effect = initializeTriggerEffect(epicEntityId, epicEntityName);
      ...
      String missionInstruction = epicEntityName + generateNeedString(baseDifficulty) + "you to construct " + targetTemplate.getNameWithGenus() + '.' + ' ' + requirement + ' ' + location;
      
      MissionTrigger trig = initializeMissionTrigger(epicEntityId, epicEntityName);
      ...
      linkMission(m, trig, effect, newMission.getMissionType(), baseDifficulty);
  ...
  private String createGenericRitualMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, Item ritualTargetItem, String targetName)
  {
    ...
    ActionEntry e = com.wurmonline.server.behaviours.Actions.actionEntrys[action];
    ...
    StringBuilder sbuild = new StringBuilder(epicEntityName + generateNeedString(baseDifficulty) + numbers + " of you to " + actionString + " at the " + targetName);
    
    MissionTrigger trig = initializeMissionTrigger(epicEntityId, epicEntityName);
    if (baseDifficulty > 1)
    {
      ItemTemplate it = getRandomItemTemplateUsed();
  ...
  private String createMSRitualMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    return createGenericRitualMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, ritualTargetItem, ritualTargetItem.getName());
  }
  ...
  private String createGTRitualMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    return createGenericRitualMission(m, newMission, epicEntityId, epicEntityName, baseDifficulty, favoredKingdomId, ritualTargetItem, targetName);
  }
  ...
  private String createMISacrificeMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    try
    {
      ...
      StringBuilder sbuild = new StringBuilder(epicEntityName + generateNeedString(baseDifficulty) + "you to sacrifice " + required + " of the hidden " + itemName + " which can be found by investigating and digging in the wilderness.");
  ...
  private String createGenericSacrificeMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    try
    {
      ...
      while (!found)
      {
        ...
        if ((usedTemplate.getSizeZ() <= altar.getSizeZ()) && (usedTemplate.getSizeY() <= altar.getSizeY()) && 
          (usedTemplate.getSizeX() <= altar.getSizeX()))
        {
    ...
    StringBuilder sbuild = new StringBuilder(epicEntityName + generateNeedString(baseDifficulty) + "you to sacrifice " + required + itemName + ".");
  ...
  private String createCutTreeMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    while (randVillage == null)
    {
      ...
      else
      {
        ...
        int[] position = getTileOutsideVillage(direction, randVillage.getPerimeterSize() + baseDifficulty * baseDifficulty, randVillage
          .getPerimeterSize() + baseDifficulty * baseDifficulty, randVillage);
        ...
        else
        {
          ...
          StringBuilder sbuild = new StringBuilder(epicEntityName + generateNeedString(baseDifficulty) + "you to " + "cut down " + "the " + treeName + ' ' + dirProximityString);
          ...
          try
          {
            float xTree = (position[0] << 2) + 4.0F * TerrainUtilities.getTreePosX(position[0], position[1]);
            float yTree = (position[1] << 2) + 4.0F * TerrainUtilities.getTreePosY(position[0], position[1]);
  ...
  private String createCreateItemMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    try
    {
      usedTemplate = ItemTemplateFactory.getInstance().getTemplate(entries[Server.rand
        .nextInt(entries.length)].getObjectCreated());
    ...
    StringBuilder sbuild = new StringBuilder(epicEntityName + generateNeedString(baseDifficulty) + "you to create " + required + ' ' + itemName + '.');
  ...
  private String createGiveItemMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    String prefix = (required == 1) && (!usedTemplate.getName().endsWith("s")) ? "a " : StringUtilities.isVowel(usedTemplate.sizeString.charAt(0)) ? "an " : usedTemplate.sizeString.equals("") ? "a " : StringUtilities.isVowel(usedTemplate.getName().charAt(0)) ? "an " : "";
    if (Villages.getVillages().length == 0) {
    ...
    while (randVillage == null)
    {
      ...
      {
        byte direction = (byte)Server.rand.nextInt(8);
        int[] position = getTileOutsideVillage(direction, randVillage.getDiameterX() * baseDifficulty, randVillage
          .getDiameterY() * baseDifficulty, randVillage);
        if ((position[0] < 0) || (position[1] < 0))
        {
          randVillage = null;
        }
        else
        {
          ...
          try
          {
            ...
            byte sex = ct.getSex();
            Creature target = Creature.doNew(cid, false, (position[0] << 2) + 2, (position[1] << 2) + 2, Server.rand
              .nextFloat() * 360.0F, 0, "Avatar of " + epicEntityName, sex, favoredKingdomId, (byte)0, false);
            ...
            StringBuilder sbuild = new StringBuilder();
            sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + ' ' + required + " of you to bring and give " + prefix + itemName + " to " + target
  ...
  private String createSlayCreatureMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    case 116: 
      for (CreatureTemplate t : templates) {
        if (!t.isMissionDisabled()) {
          if (((Servers.localServer.PVPSERVER) && (t.isFromValrei) && (t.getTemplateId() != 68)) || (
    ...
    StringBuilder sbuild = new StringBuilder();
    sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + "you to slay " + required + (forceChamps ? " champion " : " ") + creatureName + " that have appeared across the land.");
  ...
  private String createSlayTraitorMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    case 119: 
      for (CreatureTemplate t : templates) {
        if (!t.isMissionDisabled()) {
          if (((Servers.localServer.PVPSERVER) && (t.isFromValrei)) || (
            (t.isEpicMissionTraitor()) && (t.getBaseCombatRating() > 7.0F)))
          {
            possibleTemplates.add(t);
            if (!Servers.localServer.PVPSERVER) {
              forceChamps = true;
    ...
    Effect traitorEffect = EffectFactory.getInstance().createGenericEffect(spawnedTraitor.getWurmId(), "traitor", spawnedTraitor
      .getPosX(), spawnedTraitor.getPosY(), spawnedTraitor.getPositionZ() + spawnedTraitor.getHalfHeightDecimeters() / 10.0F, spawnedTraitor
      .isOnSurface(), -1.0F, spawnedTraitor.getStatus().getRotation());
    ...
    StringBuilder sbuild = new StringBuilder();
    sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + "you to slay the traitor" + (forceChamps ? " champion " : " ") + usedTemplate
      .getName() + " seen fleeing from Valrei to these lands. It was last spotted in the " + 
  ...
  private String createSacrificeCreatureMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    case 121: 
      for (CreatureTemplate t : templates) {
        if ((!t.isMissionDisabled()) && (!t.isSubmerged())) {
          if ((t.isHerbivore()) && (!t.isBabyCreature())) {
            possibleTemplates.add(t);
          }
        }
    ...
    StringBuilder sbuild = new StringBuilder();
    sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + "you to sacrifice " + required + " " + creatureName + " that " + (required > 1 ? "have appeared across the land" : "has appeared in the world") + ". Use a sacrificial knife on them inside the domain of " + epicEntityName + " after weakening them to at least half of their health.");
  ...
  private String createSlayTowerGuardsMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    StringBuilder sbuild = new StringBuilder();
    sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + "you to slay " + required + " " + creatureName + " to help cleanse the lands.");
  ...
  private String createDestroyGTMission(Mission m, EpicMissionEnum newMission, int epicEntityId, String epicEntityName, int baseDifficulty, byte favoredKingdomId, byte targetKingdom)
  {
    ...
    {
      targetTower = (Item)Zones.getGuardTowers().get(Server.rand.nextInt(Zones.getGuardTowers().size()));
      if (targetTower != null) {
    ...
    StringBuilder sbuild = new StringBuilder();
    sbuild.append(epicEntityName + generateNeedString(baseDifficulty) + "you to destroy " + targetTower
      .getName() + " which can be found in the " + getAreaString(targetTower.getTileX(), targetTower.getTileY()) + ".");
    m.setInstruction(sbuild.toString());
  ...
  private static int[] getTileOutsideVillage(byte direction, int distanceX, int distanceY, Village v)
  {
    ...
    while (((tileX < 0) || (tileY < 0)) && (tileAttempts++ < 50))
    {
      ...
      if (vt == null)
      {
        ...
        byte data = Tiles.decodeData(tileToCheck);
        Tiles.Tile theTile = Tiles.getTile(type);
        if ((theTile.isTree()) && (!theTile.isEnchanted())) {
          if ((FoliageAge.getAgeAsByte(data) > FoliageAge.MATURE_ONE.getAgeId()) && 
  ...
  {
    int playerMax = (int)(Servers.localServer.PVPSERVER ? difficulty * 1.5D : difficulty * 2.5D);
    switch (newMission.getMissionType())
    {
    case 101: 
    case 102: 
    ...
    case 122: 
    case 123: 
      return Math.max(1, (int)Math.min(difficulty * difficulty, difficulty * 2.5D));
    case 124: 
  ...
  private static final byte[] getEnemyKingdoms(int epicEntityId, byte favoredKingdom)
  {
    ...
    if (favoredKingdom != 0)
    {
      ...
      for (Kingdom lElement : karr) {
        if (lElement.getTemplate() != favoredKingdom) {
          if (!lElement.isAllied(favoredKingdom)) {
            toReturn.add(Byte.valueOf(lElement.getId()));
          }
        }
      }
    }
  ...
  private static final String[] missionAdjectives = { "last ", "horrendous ", "shining ", "first ", "scary ", "mysterious ", "enigmatic ", "important ", "strong ", "massive ", "gigantic ", "heavy ", "light ", "bright ", "deadly ", "dangerous ", "marked ", "fantastic ", "imposing ", "paradoxical ", "final " };
  private static final String[] missionNames = { "whisper", "gesture", "tears", "laughter", "horror", "mystery", "enigma", "celebration", "word", "run", "challenge", "test", "jest", "joke", "need", "quest", "trip", "folly", "lesson", "journey", "adventure" };
  private static final String[] missionFor = { " to ", " for ", " from ", " in honour of ", " of ", " to help ", " in aid of ", " in service of " };
  ...
  public static EpicMission getMISacrificeMission()
  {
    ...
    return (EpicMission)matchingMissions.get(Server.rand.nextInt(matchingMissions.size()));
  }
  ...
  public static EpicMission[] getTraitorMissions()
  {
    matchingMissions.clear();
    for (EpicMission m : getCurrentEpicMissions()) {
      if ((m.getMissionType() == 117) || 
        (m.getMissionType() == 118) || 
        (m.getMissionType() == 119)) {
        matchingMissions.add(m);
      }
    }
    if (matchingMissions.isEmpty()) {
  ...
  public static Creature[] getCurrentTraitors()
  {
    ...
    if (missions != null)
    {
      ...
      return (Creature[])creatureList.toArray(new Creature[creatureList.size()]);
    }
    return null;
  }
  ...
  {
    for (MissionTrigger mt : ) {
      if (mt.getOnActionPerformed() == 47) {
        if (mt.getTarget() == creatureId)
        {
  ...
  {
    for (MissionTrigger mt : ) {
      if (mt.getOnActionPerformed() == 491) {
        if (mt.getTarget() == creatureId)
        {
          EpicMission mis = getEpicMissionForMission(mt.getMissionRequired());
          if (mis != null)
        ...
        try
        {
            ...
            (template.isSwimming()) && (height < -2.0F) && (height > -30.0F)))
          {
            ...
            if ((t.getStructure() == null) && (t.getVillage() == null))
            {
              ...
              if ((sex == 0) && (!template.keepSex) && 
                (Server.rand.nextBoolean())) {
                sex = 1;
              }
              byte ctype = champion ? 99 : 0;
              Creature toReturn = Creature.doNew(template.getTemplateId(), false, tx * 4 + 2, ty * 4 + 2, Server.rand
                .nextFloat() * 360.0F, 0, template.getName(), sex, (byte)0, ctype, false, 
  ...
  {
    EpicEntity entity = getValrei().getEntity(epicEntityId);
    if (entity != null)
    {
    ...
    else
    {
      ...
      backupDifficultyMap.put(Integer.valueOf(epicEntityId), Integer.valueOf(em.getDifficulty()));
    }
  }
  ...
  public static EpicMission getRitualMissionForTarget(long targetId)
  {
    ...
    return getEpicMissionForMission(((MissionTrigger)allTriggers.get(0)).getMissionRequired());
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\epic\EpicServerStatus.class
