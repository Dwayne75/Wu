  ...
  implements CreatureTemplateIds, MiscConstants
{
  ...
  static final void promoteImmortal(long deityNumber)
  {
    if (Servers.localServer.LOGINSERVER) {
      if (HexMap.VALREI.elevateDemigod(deityNumber)) {}
    }
  }
  ...
  private static void disease(long deityNumberSaved)
  {
    ...
    for (int x = 0; x < players.length; x++) {
      if ((friendlyKingdom == 0) || (players[x].getKingdomTemplateId() != friendlyKingdom)) {
        if ((players[x].getDeity() == null) || (players[x].getDeity().getNumber() != deityNumberSaved))
        {
    ...
    for (int x = 0; x < players.length; x++) {
      if (players[x].getKingdomTemplateId() == friendlyKingdom) {
        try
        {
          ...
          old.setKnowledge(old.getKnowledge() + (100.0D - old.getKnowledge()) * toIncrease, false);
        }
  ...
  private static void slay(long deityNum)
  {
    ...
    else
    {
      ...
      while (!found)
      {
        ...
        int p = rand.nextInt(players.length);
        if ((!players[p].isDead()) && (players[p].isFullyLoaded()) && (players[p].getVisionArea() != null)) {
          if (players[p].getKingdomTemplateId() != friendlyKingdom) {
            if ((players[p].getDeity() != null) && (players[p].getDeity().getNumber() != deityNum))
            {
              if (((deityNum != 1L) || (players[p].getDeity().getNumber() != 3)) && ((deityNum != 3L) || 
                (players[p].getDeity().getNumber() != 1)))
              {
  ...
  public static void spawnDefenders(long deityId, int creatureTemplateId)
  {
    if ((!Servers.isThisATestServer()) && (
      (Servers.localServer.isChallengeOrEpicServer()) || (Servers.isThisAChaosServer())))
    {
      ...
      try
      {
        ...
        if (friendlyKingdom != 0)
        {
          ...
          if (k != null) {
            if ((k.lastConfrontationTileX > 1) && (k.lastConfrontationTileY > 1)) {
        ...
        if (deity != null) {
          if ((deity.lastConfrontationTileX > 1) && (deity.lastConfrontationTileY > 1)) {
            for (int a = 0; a < rand.nextInt(7) + 1; a++)
            {
    ...
    try
    {
      ...
      if (players.length > 10) {
        for (int x = 0; x < maxplayers; x++)
        {
          ...
          if ((players[playint].getPositionZ() > -1.0F) || (
            (ctemplate.isSwimming()) && (players[playint].getPositionZ() < -4.0F))) {
            if (players[playint].getKingdomTemplateId() != friendlyKingdom) {
              if (!players[playint].isFriendlyKingdom(friendlyKingdom))
              {
      ...
      if (!Servers.isThisATestServer())
      {
        ...
        while ((summoned < maxSummoned) && (tries < 5000))
        {
          ...
          if (((onlyAtHome) && (Zones.getKingdom(centerx, centery) == friendlyKingdom)) || 
            (Zones.getKingdom(centerx, centery) != friendlyKingdom)) {
            for (int x = 0; x < 10; x++)
            {
              int tx = Zones.safeTileX(centerx - 5 + rand.nextInt(10));
              int ty = Zones.safeTileY(centery - 5 + rand.nextInt(10));
  ...
  private static void spawnCreatureAt(int tilex, int tiley, CreatureTemplate ctemplate, byte friendlyKingdom)
  {
    if (ctemplate != null) {
      try
      {
        ...
        Creature.doNew(ctemplate.getTemplateId(), false, tilex * 4 + 2, tiley * 4 + 2, rand.nextFloat() * 360.0F, 0, ctemplate
          .getName(), sex, friendlyKingdom, ctype, false);
      }
      catch (Exception ex)
      {
        logger.log(Level.WARNING, ex.getMessage(), ex);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\epic\Effectuator.class
