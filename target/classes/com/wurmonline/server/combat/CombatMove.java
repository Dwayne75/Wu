  ...
  implements CombatConstants, MiscConstants, SoundNames
{
  ...
  {
    return (CombatMove)moves.get(Integer.valueOf(number));
  }
        ...
        if (tile != null)
        {
          ...
          if (crets.length > 0) {
            for (int l = 0; l < crets.length; l++) {
              if ((crets[l] != creature) && (!crets[l].isUnique())) {
                if (((crets[l].isPlayer()) && (crets[l].getPower() <= 0)) || (crets[l].isDominated())) {
        ...
        if (tile != null)
        {
          ...
          if (crets.length > 0) {
            for (int l = 0; l < crets.length; l++) {
              if (crets[l] != creature) {
                if ((!crets[l].isUnique()) && (
                  ((crets[l].isPlayer()) && (crets[l].getPower() <= 0)) || 
                  (crets[l].isDominated())))
                {
                  Server.getInstance().broadCastAction(creature
                    .getNameWithGenus() + replace(this.actionString, "@defender", crets[l].getNameWithGenus()), creature, 5, true);
  ...
  private final void doubleFist(Creature creature)
  {
    ...
    Server.getInstance().broadCastAction(creature
      .getNameWithGenus() + replace(this.actionString, "@hisher", creature.getHisHerItsString()), creature, 5, true);
    ...
    try
    {
      ItemFactory.createItem(344, 20.0F, (x1 << 2) + 2, (y1 << 2) + 2, attAngle, creature.isOnSurface(), (byte)0, creature
        .getBridgeId(), creature.getName());
    }
    catch (Exception ex)
    {
    ...
    if (tile != null)
    {
      ...
      if (crets.length > 0) {
        for (int l = 0; l < crets.length; l++) {
          if (crets[l] != creature) {
            if ((!crets[l].isUnique()) && (
              ((crets[l].isPlayer()) && (crets[l].getPower() <= 0)) || (crets[l].isDominated()))) {
              crets[l].addWoundOfType(creature, this.woundType, 1, true, 1.0F, true, damage, 0.0F, 0.0F, false, false);
            }
          }
        }
    ...
    try
    {
      ItemFactory.createItem(344, 20.0F, (x2 << 2) + 2, (y2 << 2) + 2, attAngle, creature.isOnSurface(), (byte)0, creature
        .getBridgeId(), creature.getName());
    }
    catch (Exception ex)
    {
  ...
  private final void breathAcid(Creature creature)
  {
    ...
    if (tilearr != null)
    {
      ...
      for (int y = 0; y < 5; y++) {
        ...
        {
          if (tilearr[num] > 0) {
            breathAcid(Zones.getOrCreateTile(x1 + x, y1 + y, creature.isOnSurface()), creature);
          }
          num++;
        }
      }
    ...
    if (t != null)
    {
      ...
      if (crets.length > 0) {
        for (int l = 0; l < crets.length; l++) {
          if (crets[l] != creature) {
            if ((!crets[l].isUnique()) && (
              (crets[l].isPlayer()) || (crets[l].isDominated()))) {
  ...
  private final void breatheFire(Creature creature)
  {
    ...
    if (tilearr != null)
    {
      ...
      for (int y = 0; y < 5; y++) {
        ...
        {
          if (tilearr[num] > 0) {
            breatheFire(Zones.getOrCreateTile(x1 + x, y1 + y, creature.isOnSurface()), creature);
          }
          num++;
        }
      }
  ...
  private void breatheFire(VolaTile t, Creature creature)
  {
    ...
    try
    {
      ItemFactory.createItem(520, 20.0F, (t.getTileX() << 2) + 2, (t.getTileY() << 2) + 2, Server.rand
        .nextFloat() * 180.0F, t.isOnSurface(), (byte)0, creature.getBridgeId(), "");
    }
    catch (Exception ex)
    {
      logger.log(Level.WARNING, ex.getMessage(), ex);
  ...
  {
    if ((creature.opponent != null) && ((creature.opponent.getPower() <= 0) || (Servers.isThisATestServer()) || 
      (creature.opponent.isDominated())))
    {
      ...
      int targetY = Zones.safeTileY(y + 10 - Server.rand
        .nextInt(20));
      ...
      float damage = this.basedam + Server.rand.nextFloat() * this.basedam;
      Server.getInstance().broadCastAction(creature
        .getNameWithGenus() + replace(this.actionString, "@defender", creature.opponent.getNameWithGenus()), creature, 5, true);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\combat\CombatMove.class
