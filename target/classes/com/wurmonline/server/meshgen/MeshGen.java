...
public final class MeshGen
{
  ...
  public MeshGen(int level1, MeshGenGui.Task task)
    throws Exception
  {
    this.level = level1;
    this.width = (1 << level1);
  ...
  public MeshGen(Random random, int aLevel, MeshGenGui.Task task)
    throws Exception
  {
    this(aLevel, task);
  ...
  protected void setData(float[] data, MeshGenGui.Task task)
    throws Exception
  {
    ...
    float mapSizeMod = this.width * 0.035F;
    ...
    for (int x = 0; x < this.width; x++)
    {
      ...
      for (int y = 0; y < this.width; y++)
      {
        ...
        float subtracted = (1.0F - getDirtSlope(x, y) * mapSizeMod) * 0.02F * heightModifier / 3.0F;
        if ((x == 3274) && (y == 1425)) {
          System.out.println("dslope=" + getDirtSlope(x, y) + ", subtracted=" + subtracted + " heightmod=" + heightModifier + " height " + x + "," + y + "=" + this.height[x][y]);
        }
      ...
      for (int y = 0; y < this.width; y++)
      {
        ...
        for (int xx = 0; xx < 2; xx++) {
          for (int yy = 0; yy < 2; yy++) {
            if (getGroundHeight(x + xx, y + yy) < getHeight(x + xx, y + yy))
            {
              if ((x == 3274) && (y == 1425)) {
                System.out.println("ggh=" + getGroundHeight(x + xx, y + yy) + ", gh=" + getHeight(x + xx, y + yy) + ": rock false");
              }
              rock = false;
            }
          }
    ...
    for (int x = 0; x < this.width; x++)
    {
      ...
      for (int y = 0; y < this.width; y++) {
        if ((this.textures[x][y] == Tiles.Tile.TILE_CLIFF.id) || (this.textures[x][y] == Tiles.Tile.TILE_ROCK.id)) {
          if (getGroundHeight(x, y) < getHeight(x, y)) {
  ...
  {
    float hs1 = Math.abs(getHeight(x - 1, y) - getHeight(x, y));
    float hs2 = Math.abs(getHeight(x + 1, y) - getHeight(x, y));
    float vs1 = Math.abs(getHeight(x, y - 1) - getHeight(x, y));
    float vs2 = Math.abs(getHeight(x, y + 1) - getHeight(x, y));
  ...
  public void dropADirt(boolean forceDrop, Random random)
  {
    for (int x = 0; x < this.width; x++) {
      for (int y = 0; y < this.width; y++) {
        if ((forceDrop) || (getHeight(x, y) > -1.0F))
        {
          Point p = findDropTile(x, y, random);
          incHeight(p.getX(), p.getY());
        }
      }
    }
  }
  ...
  {
    setHeight(x, y, this.height[(x & this.width - 1)][(y & this.width - 1)] + 1.0E-4F);
  }
  ...
  private Point findDropTile(int tileX, int tileY, Random random)
  {
    ...
    short h = (short)(int)(getHeight(tileX, tileY) * 1000.0F * 10.0F);
    for (int xx = 1; xx >= -1; xx--) {
      for (int yy = 1; yy >= -1; yy--)
      {
        short th = (short)(int)(getHeight(tileX + xx, tileY + yy) * 1000.0F * 10.0F);
    ...
    if (slopes.size() > 0)
    {
      ...
      return findDropTile(((Point)slopes.get(r)).getX(), ((Point)slopes.get(r)).getY(), random);
    }
  ...
  public final void createReeds()
  {
    ...
    for (int xk = 0; xk < this.width - 1; xk++) {
      ...
      {
        float theight = getHeight(xk, yk) * 1000.0F;
  ...
  private void blotch(byte id, long amount, int spread, int sizeScale, boolean underwater, boolean nearRock, boolean waterOrNoWater, boolean shallowOnly, Random random)
  {
    ...
    for (int i = 0; i < amount; i++)
    {
      ...
      else
      {
        ...
        ok = ((waterOrNoWater) || (!isWater(xo, yo))) && (!isRock(xo, yo));
      }
      if (ok)
      {
        ...
        for (int j = 0; j < size / 5; j++)
        {
          ...
          for (int k = 0; k < size / 5; k++)
          {
            ...
            for (int xk = x; xk < x + sizeScale / 2 + 1; xk++) {
              ...
              {
                boolean tileSubmerged = isWater(xk, yk);
                if ((underwater) && (!tileSubmerged)) {
                  k++;
                }
                if (((tileSubmerged) && (!underwater)) || (isRock(xk, yk))) {
                  break;
                }
                float theight = getHeight(xk, yk);
                if (shallowOnly)
                {
                  if (theight * 1000.0F > -1.0F) {
                    setTile(id, xk & this.width - 1, yk & this.width - 1, random);
                  }
                }
                else {
                  setTile(id, xk & this.width - 1, yk & this.width - 1, random);
  ...
  private void setTile(byte id, int x, int y, Random random)
  {
    ...
    {
      float theight = getHeight(x, y) * 1000.0F;
      if (theight < -2.0F) {
  ...
  private void exposeClay(long amount, Random random)
  {
    ...
    for (int i = 0; i < amount; i++)
    {
      ...
      if ((below) || (above)) {
        fail = true;
      } else if (this.height[xo][yo] < 0.0F) {
        if (this.height[(xo + w)][(yo + h)] < 0.0F) {
          fail = true;
        }
      }
      if (!fail) {
  ...
  public void generateTextures(Random random, MeshGenGui.Task task)
  {
    ...
    blotch(Tiles.Tile.TILE_DIRT.id, wsqd / 3, 2, 1, false, true, false, false, random);
    task.setNote(11, "    peat.");
    blotch(Tiles.Tile.TILE_PEAT.id, wsqd / 4, 3, 2, false, false, false, false, random);
    task.setNote(12, "    steppe.");
    blotch(Tiles.Tile.TILE_STEPPE.id, wsqd / 17 / 1, 4, 8, false, false, false, false, random);
    task.setNote(13, "    desert.");
    blotch(Tiles.Tile.TILE_SAND.id, wsqd / 15 / 1, 3, 8, false, false, false, false, random);
    task.setNote(14, "    tundra.");
    blotch(Tiles.Tile.TILE_TUNDRA.id, wsqd / 18 / 1, 3, 7, false, false, false, false, random);
    task.setNote(15, "    moss.");
    blotch(Tiles.Tile.TILE_MOSS.id, wsqd / 5, 1, 2, false, false, false, false, random);
    task.setNote(16, "    gravel.");
    blotch(Tiles.Tile.TILE_GRAVEL.id, wsqd / 3, 5, 2, false, true, false, true, random);
    
    task.setNote(17, "    underwater sand.");
    blotch(Tiles.Tile.TILE_SAND.id, wsqd * 4, 2, 3, true, false, true, false, random);
    task.setNote(18, "    marsh.");
    blotch(Tiles.Tile.TILE_MARSH.id, wsqd / 2, 4, 5, true, false, false, true, random);
    task.setNote(19, "    reeds and kelp.");
    blotch(Tiles.Tile.TILE_REED.id, wsqd / 2, 2, 3, true, false, false, true, random);
    ...
    for (int i = 0; i < this.width * this.width * 1; i++)
    {
      ...
      if ((random.nextFloat() < 0.04F) && (isTreeCapable(x, y, 5)))
      {
    ...
    for (int i = 0; i < this.width * 16; i++)
    {
      ...
      if ((tex.isBush()) || (tex.isTree())) {
        makeForest(x, y, tex, this.textureDatas[x][y], random);
      }
    }
    task.setNote(92, "  Adding grass plains...");
    
    blotch(Tiles.Tile.TILE_GRASS.id, wsqd / 2 / 1, 3, 20, false, false, false, false, random);
    
    task.setNote(95, "  Exposing some tar...");
    exposeTar(wsqd, random);
    task.setNote(96, "  Exposing some clay...");
    exposeClay(wsqd, random);
  ...
  private void makeForest(int x, int y, Tiles.Tile theTile, byte data, Random random)
  {
    ...
    for (int i = 0; i < sqrw * 10; i++)
    {
      ...
      for (int j = 0; j < maxForestSize; j++)
      {
        ...
        if (isTreeCapable(xx, yy, scarcity))
        {
          addTree(x, y, xx, yy, random);
          count++;
        }
      }
      if (count == 0) {
  ...
  public BufferedImage getImage(int layer, MeshGenGui.Task task)
  {
    ...
    for (int x = 0; x < lWidth; x++)
    {
      ...
      for (int y = lWidth - 1; y >= 0; y--)
      {
        ...
        if (layer == 0)
        {
          node = getHeight(x + xo, y + yo);
          
          node2 = getHeight(x + 1 + xo, y + 1 + yo);
        }
        else
        {
          node = getGroundHeight(x + xo, y + yo);
          
          node2 = getGroundHeight(x + 1 + xo, y + 1 + yo);
        }
        byte tex = this.textures[(x + xo)][(y + yo)];
        
        float hh = node;
        
        float h = (node2 - node) * 1500.0F / 256.0F * (1 << this.level) / 128.0F + hh / 2.0F + 1.0F;
        h *= 0.4F;
        ...
        int altTarget = y - (int)(getHeight(x, y) * 1000.0F / 4.0F);
  ...
  protected int[] getData(MeshGenGui.Task task)
  {
    ...
    try
    {
      for (y = 0; y < this.width; y++)
      {
        ...
        {
          float lHeight = getHeight(x, y) * 1000.0F;
  ...
  protected int[] getRockData(MeshGenGui.Task task)
  {
    ...
    for (int y = 0; y < this.width; y++)
    {
      ...
      for (int x = 0; x < this.width; x++)
      {
        float lHeight = getGroundHeight(x, y);
        byte tex = 0;
        byte texdata = 0;
        
        data[(x + (y << this.level))] = Tiles.encode(lHeight * 1000.0F, 0, 0);
      }
    }
    return data;
  }
  ...
  public void setData(MeshIO meshIO, MeshIO meshIO2, MeshGenGui.Task task)
  {
    ...
    for (int x = 0; x < this.width; x++)
    {
      ...
      for (int y = 0; y < this.width; y++)
      {
        ...
        setHeight(x, y, Tiles.decodeHeightAsFloat(tile) / 1000.0F);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\meshgen\MeshGen.class
