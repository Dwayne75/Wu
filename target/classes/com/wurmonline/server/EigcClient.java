...
public final class EigcClient
{
  ...
  {
    logger.log(Level.INFO, "Setting client " + getClientId() + " to player name " + newPlayerName + " reason=" + reason);
    
    this.currentPlayerName = newPlayerName;
    if ((this.currentPlayerName == null) || (this.currentPlayerName.length() == 0)) {
      setLastUsed(System.currentTimeMillis());
    } else {
      setLastUsed(Long.MAX_VALUE);
    }
    if (System.currentTimeMillis() > getExpiration()) {
      Eigc.modifyUser(getClientId(), "proximity", Long.MAX_VALUE);
    }
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\EigcClient.class
