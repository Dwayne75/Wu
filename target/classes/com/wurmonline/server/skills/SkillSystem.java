  ...
  implements TimeConstants
{
  ...
  static
  {
    ...
    addSkillTemplate(new SkillTemplate(1016, "Weapon smithing", Servers.localServer
      .isChallengeServer() ? 4000.0F : 20000.0F, new int[] { 1015 }, 1209600000L, (short)2));
  ...
  {
    String name = (String)skillNames.get(Integer.valueOf(skillNum));
    if (name != null) {
      return name;
    }
    return "unknown";
  ...
  public static float getDifficultyFor(int skillNum, boolean priest)
  {
    ...
    if (template.getType() == 0) {
      if ((priest) && (template.isPriestSlowskillgain)) {
  ...
  private static void addSkillTemplate(SkillTemplate template)
  {
    ...
    skillNames.put(Integer.valueOf(template.getNumber()), template.getName());
    namesToSkill.put(template.getName().toLowerCase(), Integer.valueOf(template.getNumber()));
  ...
  {
    return skillnums[com.wurmonline.server.Server.rand.nextInt(templates.size())].intValue();
  }
  ...
  {
    Integer i = (Integer)namesToSkill.get(name.toLowerCase());
    if (i == null) {
      return -1;
    }
    return i.intValue();
  ...
  public static SkillTemplate getSkillTemplateByIndex(int index)
  {
    ...
    return (SkillTemplate)templateList.get(index);
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\skills\SkillSystem.class
