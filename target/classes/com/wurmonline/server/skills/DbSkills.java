      ...
      try
      {
        if (WurmId.getType(this.id) == 0)
        {
          ...
          while (rs.next())
          {
            ...
            if ((!this.skills.containsKey(Integer.valueOf(skill.getNumber()))) || 
              (skill.getMinimumValue() > ((Skill)this.skills.get(Integer.valueOf(skill.getNumber()))).getMinimumValue())) {
              this.skills.put(Integer.valueOf(skill.getNumber()), skill);
            }
          }
        }
    ...
    throws SQLException
  {
    ...
    try
    {
      if (this.id != -10L)
      {
        ...
        else if (WurmId.getType(this.id) == 1)
        {
          dbcon = DbConnector.getCreatureDbCon();
        }
        else
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\skills\DbSkills.class
