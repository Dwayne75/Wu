  ...
  implements MiscConstants
{
  ...
  public static void planBridge(Creature performer, byte dir, byte bridgeType, boolean arched, String bridgePlan, int steepness, Point start, Point end, String bridgeName)
  {
    ...
    if (dir == 0)
    {
      ...
      for (int y = start.getY(); y <= end.getY(); y++)
      {
        ...
        {
          byte ndir = rdir;
          BridgeConstants.BridgeType bridgetype = getBridgeType(dir, parts[yy], end.getX(), start.getX(), x, ndir);
          if ((!bridgetype.isAbutment()) && (!bridgetype.isBracing())) {
            if (onLeft(dir, end.getX(), start.getX(), x, ndir)) {
              ndir = (byte)((ndir + 4) % 8);
            }
          }
          try
          {
            ...
            BridgePart bridgePart = new DbBridgePart(bridgetype, x, y, hts[yy], 1.0F, performer.getStructure().getWurmId(), BridgeConstants.BridgeMaterial.fromByte(bridgeType), ndir, slope, northExit, -1, southExit, -1, (byte)0, layer);
    ...
    else
    {
      ...
      for (int x = start.getX(); x <= end.getX(); x++)
      {
        ...
        {
          byte ndir = rdir;
          BridgeConstants.BridgeType bridgetype = getBridgeType(dir, parts[xx], end.getY(), start.getY(), y, ndir);
          if ((!bridgetype.isAbutment()) && (!bridgetype.isBracing())) {
            if (onLeft(dir, end.getY(), start.getY(), y, ndir)) {
              ndir = (byte)((ndir + 4) % 8);
            }
          }
          try
          {
            ...
            BridgePart bridgePart = new DbBridgePart(bridgetype, x, y, hts[xx], 1.0F, performer.getStructure().getWurmId(), BridgeConstants.BridgeMaterial.fromByte(bridgeType), ndir, slope, -1, eastExit, -1, westExit, (byte)0, layer);
  ...
  public static int[] calcHeights(Creature performer, byte dir, byte bridgeType, boolean arched, String bridge, int steepness, Point start, Point end)
  {
    ...
    if (arched)
    {
      if (bridgeType == BridgeConstants.BridgeMaterial.ROPE.getCode())
      {
        ...
        else
        {
          ...
          float totalSag = steepness / 100.0F;
          float sagDistance = lenp1 * 4.0F * totalSag;
          double scaleCosh = 5.0E-5D;
          double scaleFactor = sagDistance / 5.0E-5D;
          int htDiff = end.getH() - start.getH();
          float htd = htDiff / 10.0F;
          ...
          for (int x = 0; x < lenp1; x++) {
            if (borders[x] >= 0) {
              scale[x] = (borders[x] / lowBorder);
            } else {
              scale[x] = (-borders[x] / (hiBorder + odd));
            }
          }
          for (int x = 0; x < lenp1; x++) {
            floatSag[x] = (scaleFactor * Math.cosh(scale[x] / 100.0F) - scaleFactor - sagDistance);
          }
          for (int x = 0; x < lenp1; x++) {
    ...
    float slope = (start.getH() - end.getH()) / bridge.length();
    int ht = end.getH();
    float sd = 0.0F;
    if (dir == 0)
    {
  ...
  public static int[] calcArch(Creature performer, int maxSlope, int len, Point start, Point end)
  {
    ...
    for (int i = 0; i <= len; i++)
    {
      ahtL[i] = ((int)(highest[(i * 2)] * factor));
      ahtS[i] = (start.getH() + ahtL[i]);
      ahtR[i] = ((int)(highest[((len - i) * 2)] * factor));
      ahtE[i] = (end.getH() + ahtR[i]);
    }
    ahtL[(len + 1)] = ((int)(highest[((len + 1) * 2)] * factor));
    ...
    {
      hts[(i + 1)] = ((int)(start.getH() + (shts[i] - slopeLeftAdjustment) * factor));
    ...
    {
      hts[(len - i - 1)] = ((int)(end.getH() + (ehts[i] - slopeRightAdjustment) * factor));
  ...
  private static Wall getWall(VolaTile checkedTile, int floorlevel, int startX, int startY, int endX, int endY)
  {
    for (Wall wall : checkedTile.getWalls()) {
      if (wall.getFloorLevel() == floorlevel) {
        if ((wall.getStartX() == startX) && (wall.getStartY() == startY) && 
          (wall.getEndX() == endX) && (wall.getEndY() == endY)) {
          return wall;
        }
      }
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\structures\PlanBridgeMethods.class
