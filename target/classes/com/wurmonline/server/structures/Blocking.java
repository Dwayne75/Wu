  ...
  implements MiscConstants
{
  ...
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), target.getPosX(), target
      .getPosY(), performer.getPositionZ(), target.getPosZ(), performer.isOnSurface(), target.isOnSurface(), true, 5, target
      .getWurmId(), performer.getBridgeId(), target.getBridgeId(), 
  ...
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), target.getPosX(), target
      .getPosY(), performer.getPositionZ(), target.getPosZ(), performer.isOnSurface(), target.isOnSurface(), false, blockingType, target
      .getWurmId(), performer.getBridgeId(), target.getBridgeId(), 
      (performer.followsGround()) || ((target.getFloorLevel() == 0) && (target.getBridgeId() == -10L)));
  }
  ...
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), 2 + 
      Tiles.decodeTileX(floor.getId()) * 4, 2 + Tiles.decodeTileY(floor.getId()) * 4, performer.getPositionZ(), floor
      .getMinZ() - 0.25F, performer.isOnSurface(), floor.isOnSurface(), false, 
      !performer.isOnSurface() ? 7 : blockingType, floor
      .getId(), performer.getBridgeId(), -10L, false);
  }
  ...
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), 2 + Tiles.decodeTileX(target) * 4, 2 + 
      Tiles.decodeTileY(target) * 4, performer.getPositionZ(), 
      Zones.getHeightForNode(Tiles.decodeTileX(target), Tiles.decodeTileY(target), targetSurfaced ? 0 : -1) + 
      Tiles.decodeFloorLevel(target) * 3 + ceilingHeight, performer
      .isOnSurface(), targetSurfaced, false, (!performer.isOnSurface()) && (blockingType != 8) ? 7 : blockingType, target, sourceBridgeId, targetBridgeId, false);
  }
  ...
  public static final BlockingResult getBlockerBetween(Creature performer, Wall target, int blockingType)
  {
    ...
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), tpx, tpy, performer
      .getPositionZ(), target.getMinZ(), performer.isOnSurface(), target.isOnSurface(), false, blockingType, target
      .getId(), performer.getBridgeId(), -10L, false);
  }
  ...
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), target.getPosX(), target.getPosY(), performer
      .getPositionZ() + performer.getHalfHeightDecimeters() / 10.0F, target
      .getPositionZ() + target.getHalfHeightDecimeters() / 10.0F, performer.isOnSurface(), target.isOnSurface(), true, 4, target
      .getWurmId(), performer.getBridgeId(), target.getBridgeId(), false);
  }
  
  public static final BlockingResult getBlockerBetween(Creature performer, Creature target, int blockingType)
  {
    return getBlockerBetween(performer, performer.getPosX(), performer.getPosY(), target.getPosX(), target.getPosY(), performer
      .getPositionZ(), target.getPositionZ(), performer.isOnSurface(), target.isOnSurface(), false, blockingType, target
      .getWurmId(), performer.getBridgeId(), target.getBridgeId(), (performer.followsGround()) || 
      (target.followsGround()));
  }
  ...
  public static final BlockingResult getBlockerBetween(@Nullable Creature creature, float startx, float starty, float endx, float endy, float startZ, float endZ, boolean surfaced, boolean targetSurfaced, boolean rangedAttack, int typeChecked, boolean test, long target, long sourceBridgeId, long targetBridgeId, boolean followGround)
  {
    ...
    while (!found)
    {
      ...
      {
        if ((!surfaced) && ((!isTransition) || (!targetSurfaced))) {
          if ((typeChecked != 1) && (typeChecked != 2) && (typeChecked != 3))
          {
            ...
            if (result != null)
            {
              if (((typeChecked != 7) && (typeChecked != 8)) || 
                (result.getFirstBlocker().getTileX() != endtilex) || (result.getFirstBlocker().getTileY() != endtiley)) {
                return result;
              }
              result = null;
            }
  ...
  public static final BlockingResult isDiagonalRockBetween(Creature creature, int startx, int starty, int endx, int endy)
  {
    ...
    {
      int northTile = Server.caveMesh.getTile(Zones.safeTileX(startx), 
        Zones.safeTileY(starty - 1));
      int southTile = Server.caveMesh.getTile(Zones.safeTileX(startx), 
        Zones.safeTileY(starty + 1));
      int westTile = Server.caveMesh.getTile(Zones.safeTileX(startx - 1), 
        Zones.safeTileY(starty));
      int eastTile = Server.caveMesh.getTile(Zones.safeTileX(startx + 1), 
        Zones.safeTileY(starty));
      if (endx < startx)
      {
        if (endy < starty)
        {
          ...
          int nw = Server.caveMesh.getTile(Zones.safeTileX(startx - 1), 
            Zones.safeTileY(starty - 1));
        ...
        if (endy > starty)
        {
          ...
          int sw = Server.caveMesh.getTile(Zones.safeTileX(startx - 1), 
            Zones.safeTileY(starty + 1));
      ...
      else
      {
        if (endy < starty)
        {
          ...
          int ne = Server.caveMesh.getTile(Zones.safeTileX(startx + 1), 
            Zones.safeTileY(starty - 1));
        ...
        if (endy > starty)
        {
          ...
          int se = Server.caveMesh.getTile(Zones.safeTileX(startx + 1), 
            Zones.safeTileY(starty + 1));
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\structures\Blocking.class
