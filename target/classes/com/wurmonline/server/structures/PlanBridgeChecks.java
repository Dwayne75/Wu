  ...
  implements MiscConstants
{
  ...
  public static PlanBridgeCheckResult checkPlanWidth(Point start, Point end, byte dir, int layer)
  {
    ...
    if (dir == 0)
    {
      Structure startBuilding = Structures.getStructureForTile(start.getX(), start.getY() - 1, layer == 0);
  ...
  public static PlanBridgeCheckResult checkForHighways(Point start, Point end, byte dir, int layer)
  {
    ...
    if (dir == 0) {
      for (int x = start.getX(); x <= end.getX(); x++)
      {
        ...
        if ((startOffset == 0) && (isRoadTile(startType))) {
          if (MethodsHighways.onHighway(x, start.getY(), layer == 0))
          {
        ...
        if ((start.getY() != end.getY()) && (endOffset == 0) && (isRoadTile(endType))) {
          if (MethodsHighways.onHighway(x, y, layer == 0))
          {
            String pMsg = "Cannot build a bridge on a highway.";
            return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
          }
      ...
      for (int y = start.getY(); y <= end.getY(); y++)
      {
        ...
        int startOffset = start.getH() - startHeight;
        if ((startOffset == 0) && (isRoadTile(startType))) {
          if (MethodsHighways.onHighway(start.getX(), y, layer == 0))
          {
            String pMsg = "Cannot build a bridge on a highway.";
            return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
          }
        ...
        int endHeight = Tiles.decodeHeight(encodedEndTile);
        int endOffset = end.getH() - endHeight;
        if ((start.getX() != end.getX()) && (endOffset == 0) && (isRoadTile(endType))) {
          if (MethodsHighways.onHighway(x, y, layer == 0))
          {
            String pMsg = "Cannot build a bridge on a highway.";
            return new PlanBridgeCheckResult(true, "Cannot build a bridge on a highway.", "");
  ...
  public static PlanBridgeCheckResult checkClearance(Point start, Point end, byte dir, boolean insta, int layer)
  {
    ...
    {
      float slope = (end.getH() - start.getH()) / len;
  ...
  private static PlanBridgeCheckResult checkForSettlements(Creature performer, int x, int y, boolean insta, boolean onSurface)
  {
    ...
    if (vill != null) {
      if (!vill.isActionAllowed((short)116, performer, false, 0, 0))
      {
        if (insta) {
          qMsg = " (@ " + x + "," + y + ")";
        }
      ...
      for (int x = start.getX(); x <= end.getX(); x++) {
        for (int y = start.getY(); y < end.getY(); y++)
        {
          ...
          if (vill == null)
          {
            ...
            if (perim != null) {
              if (!perim.isCitizen(performer)) {
                if (!Methods.isActionAllowed(performer, (short)116, x, y))
                {
                  String qMsg = "";
                  if (insta) {
                    qMsg = " (@ " + x + "," + y + ")";
  ...
  {
    Structure building = Structures.getStructureForTile(x, y, onSurface);
    if ((building != null) && (building.isTypeHouse())) {
      if (!building.isGuest(performer))
      {
        String qMsg = "";
        if (insta) {
          qMsg = " (@ " + x + "," + y + ")";
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\structures\PlanBridgeChecks.class
