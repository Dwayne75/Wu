  ...
  implements ItemTypes, ItemMaterials, MiscConstants
{
  ...
  private double minimumSkill = 0.0D;
  ...
  public float getDifficultyFor(Item source, Item target, Creature performer)
    throws NoSuchTemplateException
  {
    Item realSource = source;
    if ((source.getTemplateId() == this.objectTarget) && (target.getTemplateId() == this.objectSource)) {
    ...
    try
    {
      ...
      if ((hasMinimumSkillRequirement()) && (getMinimumSkillRequirement() > primSkill.getKnowledge(0.0D))) {
        return 0.0F;
      }
    }
    ...
    if (secondarySkill != null) {
      bonus = Math.max(1.0D, secondarySkill.getKnowledge(realSource, 0.0D) / 10.0D);
    }
    float chance = 0.0F;
    if (template.isRune())
    {
  ...
  protected final void checkSaneAmounts(Item realSource, int sourceWeightToRemove, Item realTarget, int targetWeightToRemove, ItemTemplate template, Creature performer, boolean advancedItem)
    throws NoSuchItemException
  {
    if (((this.depleteSource) && (sourceWeightToRemove <= 0)) || ((this.depleteTarget) && (targetWeightToRemove <= 0)))
    {
      performer.getCommunicator().sendNormalServerMessage("The " + realSource
        .getName() + " or the " + realTarget.getName() + " contains too little material to create " + template
        .getNameWithGenus() + ". You need to find more.");
      
      throw new NoSuchItemException("Too little material.");
    }
    if ((!advancedItem) && (((this.depleteSource) && (this.depleteTarget)) || ((this.depleteEqually) && 
      (realSource.getWeightGrams(false) + realTarget.getWeightGrams(false) < template.getWeightGrams()))))
    {
      if ((realSource.isCombine()) || (realSource.isLiquid()) || (realTarget.isLiquid()) || (realTarget.isCombine())) {
        performer.getCommunicator().sendNormalServerMessage("The " + realSource
          .getName() + " and the " + realTarget.getName() + " contains too little material to create " + template
          .getNameWithGenus() + ".  Try to combine any of them with a similar object to get larger pieces.");
      } else {
        performer.getCommunicator().sendNormalServerMessage("The " + realSource
          .getName() + " and the " + realTarget.getName() + " contains too little material to create " + template
          .getNameWithGenus() + ". You need to find larger parts.");
      }
      throw new NoSuchItemException("Too little material.");
    }
    if ((!advancedItem) && (((this.depleteSource) && (this.depleteTarget)) || ((this.depleteEqually) && 
      ((realTarget.isCombine()) || (realTarget.isLiquid())) && ((realSource.isCombine()) || (realSource.isLiquid())))))
    {
    ...
    if (realSource.getWeightGrams(false) < sourceWeightToRemove)
    {
      if (realSource.isCombine()) {
        performer.getCommunicator().sendNormalServerMessage("The " + realSource
          ...
          .getNameWithGenus() + ".  Try to combine it with a similar object to get a larger amount.");
      } else {
        performer.getCommunicator().sendNormalServerMessage("The " + realSource
          .getName() + " contains too little material to create " + template
          .getNameWithGenus() + ".");
    ...
    if ((realSource.isLiquid()) && (realTarget.isLiquid()))
    {
      ...
      if ((sourceMax < 1) || (targetMax < 1) || (sourceMax / targetMax > 2) || (targetMax / sourceMax > 2))
      {
        ...
        if (targetMax < 1) {
          ...
          if (sourceMax < targetMax) {
            performer.getCommunicator().sendNormalServerMessage("You need to add more " + realSource
              .getName() + " or remove some " + realTarget.getName() + ".");
          } else {
            performer.getCommunicator().sendNormalServerMessage("You need to add more " + realTarget
              .getName() + " or remove some " + realSource.getName() + ".");
          }
        }
      ...
      if (template.isLiquid())
      {
        ...
        if (parent != null) {
          if (parent.getVolume() < realSource.getWeightGrams() + realTarget.getWeightGrams()) {
            performer.getCommunicator().sendNormalServerMessage("Not all the liquid will fit in the " + parent
              .getName() + " so some will be lost.");
          }
        }
      }
    }
  ...
  public int getSourceWeightToRemove(Item realSource, Item realTarget, ItemTemplate template, boolean advancedEntry)
  {
    ...
    {
      weightToRemove = getDepleteFromSource();
    }
    ...
    else if (template.isLiquid())
    {
      ...
      {
        nums = getTemplateNumbers(realSource, realTarget, template);
        weightToRemove = nums * realSource.getTemplate().getWeightGrams();
      }
      else if (this.depleteSource)
      {
        nums = getTemplateNumbersForSource(realSource, template);
    ...
    else if ((this.depleteSource) && (this.depleteTarget) && ((realTarget.isCombine()) || (realTarget.isLiquid())) && (
    ...
    else if (this.depleteSource)
    {
      ...
      if (this.depleteTarget)
      {
        if ((!advancedEntry) && (!realTarget.isCombine()) && (realSource.isCombine())) {
          weightToRemove = Math.max(realSource.getTemplate().getWeightGrams(), template
            .getWeightGrams() - realTarget.getWeightGrams());
        }
        if ((realSource.getTemplateId() == 9) && 
          (realSource.getWeightGrams() > realSource.getTemplate().getWeightGrams() * 0.7F)) {
  ...
  public int getTargetWeightToRemove(Item realSource, Item realTarget, ItemTemplate template, boolean advancedEntry)
  {
    ...
    {
      weightToRemove = getDepleteFromTarget();
    }
    else if (this.depleteEqually)
    {
      if (advancedEntry) {
    ...
    else if (template.isLiquid())
    {
      ...
      if ((this.depleteTarget) && (this.depleteSource)) {
        ...
        nums = getTemplateNumbersForTarget(realTarget, template);
      } else {
        nums = 0;
      }
      ...
      (realSource.isCombine()) || (realSource.isLiquid())))
    {
      ...
      weightToRemove = nums * (realTarget.getTemplate().getWeightGrams() + getExtraWeight(template));
      int weightCap = (int)((template.getWeightGrams() + getExtraWeight(template)) * 1.5F);
    ...
    else if (this.depleteTarget)
    {
      if (!realTarget.isCombine())
      {
        ...
        {
          if (advancedEntry) {
            weightToRemove = realTarget.getTemplate().getWeightGrams() + getExtraWeight(template);
          } else {
            weightToRemove = template.getWeightGrams() + getExtraWeight(template);
          }
        }
        else {
          weightToRemove = realTarget.getTemplate().getWeightGrams();
      ...
      else if (this.depleteSource)
      {
        if (advancedEntry) {
          ...
          weightToRemove = template.getWeightGrams() - realSource.getTemplate().getWeightGrams() + getExtraWeight(template);
        }
      }
      else {
        weightToRemove = template.getWeightGrams() + getExtraWeight(template);
  ...
  {
    return 
      Math.max(1, realTarget.getWeightGrams() / (realTarget.getTemplate().getWeightGrams() + getExtraWeight(template)));
  }
  
  protected final int getTemplateNumbersForSource(Item realSource, ItemTemplate template)
  {
    return 
      Math.max(1, realSource.getWeightGrams() / (realSource.getTemplate().getWeightGrams() + getExtraWeight(template)));
  }
  ...
  protected final int getTemplateNumbers(Item realSource, Item realTarget, ItemTemplate template)
  {
    return Math.max(1, 
      ...
      Math.min(realSource.getWeightGrams() / realSource.getTemplate().getWeightGrams(), realTarget.getWeightGrams() / (realTarget
      .getTemplate().getWeightGrams() + getExtraWeight(template))));
  }
  ...
  public abstract Item run(Creature paramCreature, Item paramItem, long paramLong, float paramFloat)
    throws FailedException, NoSuchSkillException, NoSuchItemException;
  
  abstract CreationEntry cloneAndRevert();
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\CreationEntry.class
