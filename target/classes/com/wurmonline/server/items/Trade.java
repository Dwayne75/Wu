  ...
  implements MiscConstants
{
  ...
  private boolean makeTrade()
  {
    ...
    if ((this.creatureOneRequestWindow.hasInventorySpace()) && (this.creatureTwoRequestWindow.hasInventorySpace()))
    {
      ...
      if ((diff > 0) && ((this.creatureOne instanceof Player))) {
        if (!this.creatureOne.canCarry(diff))
        {
      ...
      if ((diff > 0) && ((this.creatureTwo instanceof Player))) {
        if (!this.creatureTwo.canCarry(diff))
        {
      ...
      if (ok)
      {
        ...
        if (shop != null) {
          if (this.shopDiff != 0L)
          {
            ...
            {
              if (shop.getTax() > 0.0F) {
                if ((!shop.isPersonal()) && (citizenVillage != null) && (this.creatureOne.citizenVillage != citizenVillage)) {
                  if (citizenVillage.plan != null)
                  {
                    setTax(((float)this.shopDiff * shop.getTax()));
                    logger.log(Level.INFO, this.creatureOne.getName() + " and " + this.creatureTwo.getName() + " adding " + 
                      getTax() + " tax to " + citizenVillage.getName());
                    citizenVillage.plan.addMoney(getTax());
                    shop.addTax(getTax());
                  }
                }
              }
              if (this.creatureOne.getPower() >= 5) {
                this.creatureOne.getCommunicator().sendNormalServerMessage("Adding " + (this.shopDiff - 
                  getTax()) + " to shop");
              }
            ...
            if (this.creatureOne.getPower() >= 5) {
              this.creatureOne.getCommunicator().sendNormalServerMessage("Shop setting money " + (shop
                .getMoney() + this.shopDiff - getTax()) + " (" + shop.getMoney() + " before).");
            }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\Trade.class
