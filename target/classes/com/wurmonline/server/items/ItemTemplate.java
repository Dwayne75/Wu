  ...
  implements MiscConstants, ItemMaterials, ItemTypes, ItemSizes, Comparable<ItemTemplate>
{
  ...
  ItemTemplate(int aTemplateId, int aSize, String aName, String aPlural, String aItemDescriptionSuperb, String aItemDescriptionNormal, String aItemDescriptionBad, String aItemDescriptionRotten, String aItemDescriptionLong, short[] aItemTypes, short aImageNumber, short aBehaviourType, int aCombatDamage, long aDecayTime, int aCentimetersX, int aCentimetersY, int aCentimetersZ, int aPrimarySkill, byte[] aBodySpaces, String aModelName, float aDifficulty, int aWeight, byte aMaterial, int aValue, boolean aIsPurchased)
  {
    ...
    int[] sizes = { aCentimetersX, aCentimetersY, aCentimetersZ };
    Arrays.sort(sizes);
    ...
    setSizeString();
    assignTypes(aItemTypes);
    assignTemplateTypes();
    checkHolyItem();
    if (this.weight > 2000) {
      setFragmentAmount(Math.max(3, this.weight / 750));
    }
  }
  ...
  {
    if (getTemplateId() == 1238) {
      return getName();
    }
    return MaterialUtilities.getMaterialString(getMaterial());
  }
  ...
  {
    return (getTemplateId() == 319) || (getTemplateId() == 1029);
  }
  ...
  {
    return getName().compareTo(aItemTemplate.getName());
  }
  ...
  {
    if ((this.templateId == 1333) || (this.templateId == 1334)) {
      return true;
    }
    return false;
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\ItemTemplate.class
