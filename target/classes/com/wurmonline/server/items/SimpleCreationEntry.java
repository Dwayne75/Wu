  ...
  implements SoundNames, TimeConstants
{
  ...
  {
    CreationEntry toReturn = new SimpleCreationEntry(this.primarySkill, this.objectTarget, this.objectSource, this.objectCreated, this.destroyTarget, this.useCapacity, this.percentageLost, this.minTimeSeconds, this.destroyBoth, this.createOnGround, getCategory());
    return toReturn;
  }
    ...
    throws FailedException, NoSuchSkillException, NoSuchItemException, IllegalArgumentException
  {
    ...
    else if (this.objectCreated == 847)
    {
      ...
      if (realSource.getTemplateId() == 302) {
        if (creatureId != 12)
        {
    ...
    {
      int creatureId = realSource.getData2();
      if (realSource.getTemplateId() == 313) {
        if (creatureId != 14)
        {
    ...
    if (create)
    {
      ...
      if ((!this.createOnGround) && (template.isHollow()))
      {
        ...
        {
          performer.getCommunicator().sendSafeServerMessage("The " + realTarget
      ...
      if ((!realSource.isWeapon()) || (Weapon.getSkillPenaltyForWeapon(realSource) > 0.0D)) {
        try
        {
      ...
      if (counter == 1.0F)
      {
        ...
        {
          if (this.primarySkill == 10041) {
            if (realSource.getTemperature() < 3500)
            {
        ...
        int sourceWeightToRemove = getSourceWeightToRemove(realSource, realTarget, template, false);
        int targetWeightToRemove = getTargetWeightToRemove(realSource, realTarget, template, false);
        if (this.objectCreated == 652) {
        ...
        checkSaneAmounts(realSource, sourceWeightToRemove, realTarget, targetWeightToRemove, template, performer, false);
        ...
        try
        {
          time = Actions.getItemCreationTime(150, performer, primSkill, this, realSource, realTarget, template
            .isMassProduction());
        }
        catch (NoSuchTemplateException nst)
        {
      ...
      else
      {
        try
        {
          ...
          if (act.mayPlaySound()) {
            sendSound(performer, this.objectSource, realTarget, this.objectCreated);
          }
        }
        catch (NoSuchActionException nsa)
        {
      ...
      if (counter * 10.0F > time)
      {
        ...
        if (secondarySkill != null) {
          bonus += Math.max(-10.0D, secondarySkill.skillCheck(template.getDifficulty(), 0.0D, false, counter / 10.0F));
        }
        ...
        if (((Servers.localServer.entryServer) && (template.newbieItem)) || 
          (template.getTemplateId() == 156)) {
          bonus += 100.0D;
        }
        if (template.isRune())
        {
          ...
          {
            if (((performer.getDeity().isMountainGod()) && (this.objectCreated == 1289)) || 
        ...
        try
        {
          double imbueEnhancement = 1.0D + 0.23047D * realSource.getSkillSpellImprovement(primSkill.getNumber()) / 100.0D;
          float qlevel = (float)(power * imbueEnhancement);
          ...
          if ((this.objectCreated == 37) && (
            (this.objectSource == 169) || (this.objectTarget == 169))) {
            itq = 1.0F;
          }
          ...
          if ((qlevel > 0.0F) || ((template.isTutorialItem()) && (Server.rand.nextInt(100) < 90.0F)))
          {
            if ((template.isRepairable()) && (!template.isImproveItem)) {
              itq = Math.max(1.0F, Math.min(itq, (float)(primSkill.getKnowledge(0.0D) / 5.0D)));
            }
            if ((realSource.getMaterial() == 56) || 
              (realSource.getMaterial() == 57))
            {
            ...
            {
              performer.getCommunicator().sendNormalServerMessage("You cut the " + realTarget
                .getName() + " into five labels.");
              Server.getInstance().broadCastAction(performer
                .getName() + " cuts the " + realTarget.getName() + " into five labels.", performer, 5);
              for (int x = 0; x < 5; x++) {
                try
                {
              ...
              if (template.isLiquid()) {
                if ((this.depleteSource) && (this.depleteTarget))
                {
                  if (mixedLiquids)
                  {
                    ...
                    newItem.setWeight(newWeight, false);
                    newItem.setSizes(Math.min(template.getSizeX(), realSource.getSizeX()), 
                      Math.min(template.getSizeY(), realSource.getSizeY()), 
                      Math.min(template.getSizeZ(), realSource.getSizeZ()));
                  }
                  else
                  {
                    int nums = getTemplateNumbers(realSource, realTarget, template);
                    newWeight = Math.min(sourceWeightToRemove + targetWeightToRemove, nums * template
                      .getWeightGrams());
                    newItem.setWeight(newWeight, false);
                    if (template.isLiquid()) {
                      newItem.setSizes(Math.min(template.getSizeX(), realSource.getSizeX()), 
                      ...
                      newItem.setSizes(
                        (int)Math.min(template.getSizeX() * (nums < 3 ? nums : 2.0F + nums / 3.0F), realSource
                        .getSizeX()), 
                        (int)Math.min(template.getSizeY() * Math.max(1.0F, nums / 3.0F), realSource
                        .getSizeY()), 
                        (int)Math.min(template.getSizeZ() * Math.max(1.0F, nums / 3.0F), realSource
                        .getSizeZ()));
                    }
                  }
                }
                ...
                {
                  int nums = getTemplateNumbersForSource(realSource, template);
                  ...
                  if (template.isLiquid()) {
                    ...
                    newItem.setSizes(
                      (int)Math.min(template.getSizeX() * (nums < 3 ? nums : 2.0F + nums / 3.0F), realSource
                      .getSizeX()), 
                      (int)Math.min(template.getSizeY() * Math.max(1.0F, nums / 3.0F), realSource.getSizeY()), 
                      (int)Math.min(template.getSizeZ() * Math.max(1.0F, nums / 3.0F), realSource.getSizeZ()));
                  }
                }
                else if (this.depleteTarget)
                {
                  int nums = getTemplateNumbersForTarget(realTarget, template);
                  newWeight = Math.min(targetWeightToRemove, nums * template.getWeightGrams());
                  newItem.setWeight(newWeight, false);
                  if (template.isLiquid()) {
                    newItem.setSizes(Math.min(template.getSizeX(), realTarget.getSizeX()), 
                      Math.min(template.getSizeY(), realTarget.getSizeY()), 
                      Math.min(template.getSizeZ(), realTarget.getSizeZ()));
                  } else {
                    newItem.setSizes(
                      (int)Math.min(template.getSizeX() * (nums < 3 ? nums : 2.0F + nums / 3.0F), realTarget
                      .getSizeX()), 
                      (int)Math.min(template.getSizeY() * Math.max(1.0F, nums / 3.0F), realTarget.getSizeY()), 
                      (int)Math.min(template.getSizeZ() * Math.max(1.0F, nums / 3.0F), realTarget.getSizeZ()));
                  }
                }
              }
              ...
              if (newItem != null)
              {
                ...
                if ((newItem.getWeightGrams() <= 0) || (newItem.deleted))
                {
                  ...
                  if (sendFailedMessage) {
                    performer.getCommunicator().sendNormalServerMessage("You failed to create " + template
                      .getNameWithGenus() + ".");
                  }
                }
              }
              if (newItem != null)
              {
                ...
                if ((extraWeight > 10) && (!realTarget.isLiquid()))
                {
                  ...
                  if (itc != -1) {
                    try
                    {
                      Item scrap = ItemFactory.createItem(itc, realTarget.getCurrentQualityLevel() / 10.0F, material, (byte)0, performer
                        .getName());
          ...
          else
          {
            ...
            if ((!realTarget.isKey()) && (!realTarget.isBodyPart())) {
              if ((realTarget.isRepairable()) || (realTarget.getTemplateId() == 288)) {
                ...
                (!realTarget.isLiquid()) && 
                (realTarget.getQualityLevel() >= 2.0F) && (realTarget.getQualityLevel() > primSkill.getKnowledge(0.0D))) {
            ...
            if ((!realSource.isKey()) && (!realSource.isBodyPart())) {
              if ((realSource.isRepairable()) || (realSource.getTemplateId() == 288)) {
                ...
                (!realSource.isLiquid()) && 
                (realSource.getQualityLevel() >= 2.0F) && (realSource.getQualityLevel() > primSkill.getKnowledge(0.0D))) {
          ...
          if ((realTarget.getTemplateId() == 385) && 
            (realTarget.getWeightGrams() <= 24000)) {
            ...
            if (parent != null)
            {
              ...
              if ((!newItem.deleted) && (newItem.getParentId() == -10L))
              {
                performer.getCommunicator().sendNormalServerMessage("Not all the " + newItem
                  .getName() + " would fit in the " + parent.getName() + ".");
                Items.decay(newItem.getWurmId(), newItem.getDbStrings());
                newItem = null;
              }
            }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\SimpleCreationEntry.class
