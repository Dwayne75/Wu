  ...
  implements TimeConstants, MiscConstants, CounterTypes, Comparable<WurmMail>
{
    ...
    for (Set<WurmMail> mailset : mails.values()) {
      for (WurmMail m : mailset) {
        if (m.isExpired())
        {
          ...
          {
            logger.log(Level.INFO, "Deleting expired rejected mail " + m
              .getItemId() + " sent to " + m.getReceiver() + " from " + m
              .getSender());
            toDelete.add(m);
          }
        }
  ...
  {
    return mailsByItemId.get(Long.valueOf(itemId)) != null;
  }
  ...
  {
    Set<WurmMail> set = (Set)mails.get(new Long(receiverid));
  ...
  {
    Long receiver = (Long)mailsByItemId.get(new Long(itemId));
    if (receiver == null) {
      return -10L;
    }
  ...
  public static final WurmMail getWurmMailForItem(long itemId)
  {
    ...
    Set<WurmMail> set = getMailsFor(receiver.longValue());
    if (set != null) {
      for (WurmMail m : set) {
        if (m.itemId == itemId) {
          return m;
  ...
  public static final WurmMail[] getAllMail()
  {
    ...
    return (WurmMail[])sent.toArray(new WurmMail[sent.size()]);
  }
  ...
  public static final void loadAllMails()
    throws IOException
  {
  ...
  {
    return getName().compareTo(otherWurmMail.getName());
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\WurmMail.class
