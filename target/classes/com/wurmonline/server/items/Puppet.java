  ...
  implements MiscConstants, TimeConstants
{
  ...
  public static final boolean sendConversationString(Action act, Item puppetOne, Item puppetTwo, Creature performerOne, Creature performerTwo, int counter)
  {
    ...
    if ((puppeteering.skillCheck(counter * (3.0F - 0.2F * puppetOne.getRarity() - 0.2F * puppetTwo.getRarity()) + (courierOne + courierTwo) / 4.0F, puppetOne, 0.0D, false, 5.0F) > 0.0D ? 1 : 0) == 0)
    {
    ...
    else if (counter < toUse.length)
    {
      ...
      performerOne.getCommunicator()
        .sendNormalServerMessage("Your " + puppetOne.getName() + ": '" + toUse[counter] + "'");
          ...
          try
          {
            ...
            try
            {
              ...
              armour.setDamage(armour.getDamage() + 
                Math.max(0.05F, Math.min(1.0F, 7000.0F * 
                ArmourTemplate.getArmourDamageModFor(armour, (byte)3) / 1200000.0F * armour
                .getDamageModifier())));
            }
            ...
            CombatEngine.addWound(null, performer, (byte)3, pos, 5000 + Server.rand.nextInt(10000), armourMod, "bites", null, 20.0F, Server.rand
              .nextInt((int)poisdam), false, false, false, false);
          }
          catch (Exception ex)
          {
  ...
  {
    Long last = (Long)puppetmasters.get(Long.valueOf(performer.getWurmId()));
    if ((last == null) || (System.currentTimeMillis() - last.longValue() > 900000L)) {
      return true;
    }
    return false;
  }
  ...
  {
    puppetmasters.put(Long.valueOf(performer.getWurmId()), Long.valueOf(System.currentTimeMillis()));
  }
  
  public static final long getLastPuppeteered(long performerId)
  {
    Long last = (Long)puppetmasters.get(Long.valueOf(performerId));
    if (last == null) {
      return 0L;
    }
    if (System.currentTimeMillis() - last.longValue() > 900000L) {
      return 0L;
    }
  ...
  {
    puppetmasters.put(Long.valueOf(performerId), Long.valueOf(lastPuppeteered));
  }
}


/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\items\Puppet.class
