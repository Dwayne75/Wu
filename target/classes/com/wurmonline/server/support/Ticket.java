  ...
  implements MiscConstants, ProtoConstants, TimeConstants
{
  ...
  {
    this(Tickets.getNextTicketNo(), System.currentTimeMillis(), aPlayerWurmId, aPlayerName, aCategoryCode, 
    
      Servers.getLocalServerId(), true, 0L, (byte)0, (byte)1, "", aDescription
      .replace('"', '\''), true, (short)0, "", "", (byte)0, false, false, false, (byte)0, true);
  ...
  {
    if (!this.ticketActions.containsKey(Long.valueOf(aActionNo)))
    {
    ...
    return (TicketAction)this.ticketActions.get(Long.valueOf(aActionNo));
  }
  ...
  public void autoForwardToGM()
  {
    if ((this.stateCode == 0) && 
      ...
      (this.ticketDate < System.currentTimeMillis() - 900000L)) {
  ...
  {
    addNewTicketAction(action, byWhom, note, visibilityLevel, (byte)0, (byte)0, (byte)0, (byte)0, (byte)0, (byte)0);
  }
  ...
  public void addNewTicketAction(byte action, String byWhom, String note, byte visibilityLevel, byte aQualityOfServiceCode, byte aCourteousCode, byte aKnowledgeableCode, byte aGeneralFlags, byte aQualitiesFlags, byte aIrkedFlags)
  {
    ...
    setAckFor(action, visibilityLevel);
    switch (action)
    {
    case 1: 
      this.stateCode = 4;
  ...
  {
    if ((this.description.length() <= 400) || (getStateCode() != 5)) {
      return getDescription();
    }
    if (getTrelloCardId().length() == 0) {
      return "Possible Client Hack: Soon in a trello near you!";
    }
    return "Possible Client Hack: see trello https://trello.com/c/" + getTrelloCardId();
  }
  ...
  public String getTrelloName()
  {
    ...
    return 
      ...
      "#" + this.ticketId + " " + Tickets.convertTime(this.ticketDate) + " " + this.playerName + " : " + abbreviateCategory(this.categoryCode) + " (" + getServerAbbreviation() + ") " + lastAction + " " + this.responderName;
  }
  ...
  public String getTrelloFeedbackTitle()
  {
    if (hasFeedback())
    {
      TicketAction ta = getFeedback();
      return "#" + this.ticketId + " " + 
        Tickets.convertTime(this.closedDate) + " S:" + ta
        .getQualityOfServiceCode() + " C:" + ta
        .getCourteousCode() + " K:" + ta
        .getKnowledgeableCode() + " G:" + ta
        .getGeneralFlagString() + " Q:" + ta
        .getQualitiesFlagsString() + " I:" + ta
        .getIrkedFlagsString();
    }
    return "";
  }
  ...
  public String getTicketSummary(Player player)
  {
    ...
    if (player.mayHearMgmtTalk()) {
      return 
        ...
        "#" + this.ticketId + " " + Tickets.convertTime(this.ticketDate) + " " + this.playerName + " : " + abbreviateCategory(this.categoryCode) + (this.serverId == Servers.getLocalServerId() ? " " : new StringBuilder().append(" (").append(getServerAbbreviation()).append(")").toString()) + " " + getLastAction() + ((this.levelCode <= 1) || (player.mayMute()) ? " " + this.responderName : "");
    }
    return 
    
      "#" + this.ticketId + " " + Tickets.convertTime(this.ticketDate) + " : " + getLastAction();
  }
  ...
  {
    if (hasFeedback()) {
      return (TicketAction)this.ticketActions.get(Long.valueOf(this.refFeedback));
    }
    return null;
  }
  ...
  public TicketAction[] getTicketActions(Player player)
  {
    ...
    for (Map.Entry<Long, TicketAction> entry : this.ticketActions.entrySet()) {
      if (((TicketAction)entry.getValue()).isActionShownTo(player)) {
        playerTicketAction.put(entry.getKey(), entry.getValue());
      }
    }
    return (TicketAction[])playerTicketAction.values().toArray(new TicketAction[playerTicketAction.size()]);
  }
  ...
  public TicketAction[] getDirtyTicketActions()
  {
    ...
    for (Map.Entry<Long, TicketAction> entry : this.ticketActions.entrySet()) {
      if (((TicketAction)entry.getValue()).isDirty()) {
        dirtyTicketAction.put(entry.getKey(), entry.getValue());
      }
    }
    return (TicketAction[])dirtyTicketAction.values().toArray(new TicketAction[dirtyTicketAction.size()]);
  }
  ...
  public long getLatestActionDate()
  {
    ...
    for (Map.Entry<Long, TicketAction> entry : this.ticketActions.entrySet()) {
      if (((TicketAction)entry.getValue()).getDate() > newestAction) {
        newestAction = ((TicketAction)entry.getValue()).getDate();
      }
    }
    return newestAction;
  }
  ...
  public boolean isTicketShownTo(Player player)
  {
    ...
    if ((this.categoryCode != 11) && (player.getWurmId() == this.playerWurmId)) {
      return true;
    }
    return false;
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\support\Ticket.class
