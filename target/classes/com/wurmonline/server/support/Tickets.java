  ...
  implements CounterTypes, TimeConstants
{
  ...
  {
    if (tickets.containsKey(Integer.valueOf(newTicket.getTicketId())))
    {
      Ticket oldTicket = (Ticket)tickets.get(Integer.valueOf(newTicket.getTicketId()));
      
      oldTicket.update(newTicket.isGlobal(), newTicket.getClosedDate(), newTicket.getStateCode(), newTicket
        .getLevelCode(), newTicket.getResponderName(), newTicket.getDescription(), newTicket
        .getRefFeedback(), newTicket.getAcknowledged());
      return oldTicket;
    }
    tickets.put(Integer.valueOf(newTicket.getTicketId()), newTicket);
    if (saveit) {
      newTicket.save();
    }
    return newTicket;
  ...
  public static Ticket[] getTickets(Player player)
  {
    ...
    for (Map.Entry<Integer, Ticket> entry : tickets.entrySet()) {
      if (((Ticket)entry.getValue()).isTicketShownTo(player)) {
        playerTickets.put(entry.getKey(), entry.getValue());
      }
    }
    return (Ticket[])playerTickets.values().toArray(new Ticket[playerTickets.size()]);
  }
  
  public static void acknowledgeTicketUpdatesFor(Player player)
  {
    for (Map.Entry<Integer, Ticket> entry : tickets.entrySet()) {
      ((Ticket)entry.getValue()).acknowledgeTicketUpdate(player);
    }
  }
  ...
  public static Ticket[] getTicketsChangedSince(long aDate)
  {
    Map<Integer, Ticket> playerTickets = new HashMap();
    for (Map.Entry<Integer, Ticket> entry : tickets.entrySet()) {
      if (((Ticket)entry.getValue()).hasTicketChangedSince(aDate)) {
        playerTickets.put(entry.getKey(), entry.getValue());
      }
    }
    return (Ticket[])playerTickets.values().toArray(new Ticket[playerTickets.size()]);
  ...
  public static long getLatestActionDate()
  {
    ...
    for (Map.Entry<Integer, Ticket> entry : tickets.entrySet()) {
      if (latestTicketDate < ((Ticket)entry.getValue()).getLatestActionDate()) {
        latestTicketDate = ((Ticket)entry.getValue()).getLatestActionDate();
      }
    }
  ...
  public static Ticket[] getDirtyTickets()
  {
    ...
    return (Ticket[])dirtyTickets.values().toArray(new Ticket[dirtyTickets.size()]);
  }
  ...
  public static Ticket[] getArchiveTickets()
  {
    ...
    return (Ticket[])archiveTickets.values().toArray(new Ticket[archiveTickets.size()]);
  }
  
  public static Ticket getTicket(int ticketNo)
  {
    return (Ticket)tickets.get(Integer.valueOf(ticketNo));
  }
  
  public static void removeTicket(int ticketNo)
  {
    tickets.remove(Integer.valueOf(ticketNo));
  }
  ...
  private static void dbLoadAllTickets()
  {
    ...
    try
    {
      ...
      while (rs.next())
      {
        ...
        getTicket(aTicketId).addTicketAction(new TicketAction(aTicketId, aActionNo, aAction, aActionDate, aByWhom, aNote, aVisibilityLevel, isDirty, aQualityOfServiceCode, aCourteousCode, aKnowledgeableCode, aGeneralFlags, aQualitiesFlags, aIrkedFlags, aTrelloLink));
      }
      DbUtilities.closeDatabaseObjects(ps, rs);
    }
    catch (SQLException sqx)
  ...
  {
    TicketToSend ticketToSend = (TicketToSend)ticketsToSend.pollFirst();
    while (ticketToSend != null)
    {
      ticketToSend.send();
      ticketToSend = (TicketToSend)ticketsToSend.pollFirst();
    }
    TicketToUpdate ticketToUpdate = (TicketToUpdate)ticketsToUpdate.pollFirst();
  ...
  {
    for (Ticket ticket : tickets.values()) {
      if ((ticket.getArchiveState() == 0) && (ticket.isClosed()) && 
        (ticket.getClosedDate() < System.currentTimeMillis() - 604800000L))
      {
  ...
  public static final boolean checkForFlooding(long playerId)
  {
    int count = 0;
    for (Ticket ticket : tickets.values()) {
      if ((ticket.getPlayerId() == playerId) && (ticket.isOpen()) && 
        (ticket.getTicketDate() > System.currentTimeMillis() - 10800000L)) {
        count++;
      }
    }
    return count >= 3;
  ...
  public static final void sendRequiresAckMessage(Player player)
  {
    for (Ticket ticket : tickets.values()) {
      if (ticket.getPlayerId() == player.getWurmId()) {
        if (ticket.isWaitingAcknowledgement())
        {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\support\Tickets.class
