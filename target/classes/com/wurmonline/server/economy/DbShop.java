  ...
  extends Shop
{
  ...
  public void setOwner(long newOwnerId)
  {
    if (newOwnerId != this.ownerId)
    {
      ...
      {
        logger.log(Level.WARNING, "Failed to update trader owner " + this.ownerId + " for " + this.wurmid + ": " + sqx
          .getMessage(), sqx);
      }
      finally
      {
  ...
  public void setMerchantData(int _numberOfItems, long _whenEmpty)
  {
    if ((_numberOfItems != this.numberOfItems) || (_whenEmpty != this.whenEmpty))
    {
      ...
      {
        logger.log(Level.WARNING, "Failed to update merchant data " + this.numberOfItems + "," + this.whenEmpty + " for " + this.wurmid + ": " + sqx
          .getMessage(), sqx);
      }
      finally
      {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\economy\DbShop.class
