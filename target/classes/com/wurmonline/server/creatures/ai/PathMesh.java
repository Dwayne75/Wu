...
final class PathMesh
{
  ...
  PathMesh(int startx, int starty, int endx, int endy, boolean surf, int borderSize, int[] aMesh)
  {
    ...
    for (int x = 0; x < this.sizex; x++) {
      ...
      {
        this.pathables[x][y] = new PathTile(this.borderStartX + x, this.borderStartY + y, lMesh[(this.borderStartX + x | this.borderStartY + y << Constants.meshSize)], this.surfaced, (byte)(this.surfaced ? 0 : -1));
        if (logger.isLoggable(Level.FINEST)) {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\creatures\ai\PathMesh.class
