...
final class StaticPathMeshNPC
{
  ...
  StaticPathMeshNPC(int startx, int starty, int endx, int endy, boolean surf, int borderSize, int[] aMesh)
  {
    ...
    for (int x = 0; x < this.sizex; x++) {
      ...
      {
        pathables[x][y] = new PathTile(this.borderStartX + x, this.borderStartY + y, lMesh[(this.borderStartX + x | this.borderStartY + y << Constants.meshSize)], this.surfaced, (byte)(this.surfaced ? 0 : -1));
        if (logger.isLoggable(Level.FINEST)) {
  ...
  PathTile[] getAdjacent(PathTile p)
  {
    ...
    if (minOneX >= this.borderStartX) {
      surrPathables[3] = getPathTile(minOneX, y);
    }
    if (plusOneX < this.borderEndX) {
      surrPathables[1] = getPathTile(plusOneX, y);
    }
    if (minOneY >= this.borderStartY) {
      surrPathables[0] = getPathTile(x, minOneY);
    }
    if (plusOneY < this.borderEndY) {
      surrPathables[2] = getPathTile(x, plusOneY);
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\creatures\ai\StaticPathMeshNPC.class
