  ...
  implements MiscConstants, Comparable<Delivery>
{
  ...
  public Delivery(long deliveryId, byte state, long collectionWaystoneId, long containerId, int crates, long senderId, long senderCost, long receiverId, long receiverCost, long deliveryWaystoneId, long wagonerId, long tsExpiry, long tsWaitingForAccept, long tsAcceptedOrRejected, long tsDeliveryStarted, long tsPickedUp, long tsDelivered)
  {
    ...
    addDelivery(this);
    if (containerId != -10L) {
      containerDelivery.put(Long.valueOf(containerId), this);
    }
  }
  
  public int compareTo(Delivery otherDelivery)
  {
    if (getWhenAcceptedOrRejected() == otherDelivery.getWhenAcceptedOrRejected()) {
      return 0;
    }
    if (getWhenAcceptedOrRejected() < otherDelivery.getWhenAcceptedOrRejected()) {
      return -1;
  ...
  public void setTimingOut()
  {
    ...
    try
    {
      Player player = Players.getInstance().getPlayer(getSenderId());
      player.getCommunicator().sendServerMessage("Delivery to " + 
        getReceiverName() + " timed out, was not accepted in time.", 255, 127, 127);
    }
    catch (NoSuchPlayerException localNoSuchPlayerException) {}
    try
    {
      Player player = Players.getInstance().getPlayer(getReceiverId());
      player.getCommunicator().sendServerMessage("Delivery from " + 
        getSenderName() + " timed out, was not accepted in time.", 255, 127, 127);
    }
  ...
  public void setDelivered()
  {
    ...
    if ((wagoner != null) && (wagoner.getVillageId() != -1)) {
      try
      {
        ...
        int deedCut = (int)(wagonersCut * 0.2F);
  ...
  {
    String desc = "goods from " + getSenderName() + " to " + getReceiverName();
    switch (this.state)
    {
    case 1: 
  ...
  void checkPayment(boolean force)
  {
    ...
    case 0: 
      ...
      break;
    case 12: 
      boolean paid = pay(this.receiverId, getReceiverName(), this.receiverCost, "Refund goods cost for delivery " + 
        getDeliveryId() + " as cancelled.", 
        getSenderName() + " has cancelled the delivery as no wagoner.", (byte)9);
      if (!paid) {
        break;
      }
    case 9: 
      pay(this.senderId, getSenderName(), this.senderCost, "Refund delivery " + getDeliveryId() + " as cancelled.", "You have cancelled the delivery to " + 
        getReceiverName() + ".", (byte)10);
      break;
    case 5: 
      pay(this.senderId, getSenderName(), this.senderCost, "Refund delivery " + getDeliveryId() + " as rejected.", "Your delivery to " + 
        getReceiverName() + " has been rejected.", (byte)8);
      break;
    case 6: 
      pay(this.senderId, getSenderName(), this.senderCost, "Refund delivery " + getDeliveryId() + " as timed out.", "Your delivery to " + 
        getReceiverName() + " has timed out.", (byte)11);
      break;
    case 4: 
      int deliveryCost = this.senderCost == 0L ? getCrates() * 100 : 0;
      pay(this.senderId, getSenderName(), this.receiverCost - deliveryCost, "Delivery " + getDeliveryId() + " paid.", "Your delivery to " + 
        getReceiverName() + " has been paid.", (byte)7);
      break;
    }
  }
  ...
  {
    return (Delivery)deliveryQueue.get(Long.valueOf(deliveryId));
  }
  ...
  public static final void addDelivery(long collectionWaystoneId, long containerId, int crates, long senderId, long senderCost, long receiverId, long receiverCost, long wagonerId)
  {
    ...
    long deliveryId = dbCreateDelivery((byte)0, collectionWaystoneId, containerId, crates, senderId, senderCost, receiverId, receiverCost, -10L, wagonerId, tsExpiry, tsWaitingForAccept, 0L, 0L, 0L, 0L);
  ...
  public static final Delivery[] getWaitingDeliveries(long playerId)
  {
    ...
    return (Delivery[])deliverySet.toArray(new Delivery[deliverySet.size()]);
  }
  ...
  {
    return (Delivery)containerDelivery.get(Long.valueOf(containerId));
  }
  ...
  public static final Delivery canViewDelivery(Item container, Creature creature)
  {
    for (Delivery delivery : deliveryQueue.values()) {
      if (delivery.getContainerId() == container.getWurmId()) {
        if (delivery.canViewDelivery()) {
          if ((delivery.getSenderId() == creature.getWurmId()) || (creature.getPower() >= 2)) {
            return delivery;
          }
        }
      }
  ...
  public static final boolean canUnSealContainer(Item container, Creature creature)
  {
    ...
    if (delivery != null) {
      if ((!delivery.canViewDelivery()) || (delivery.getWagonerId() == -10L)) {
        if ((delivery.getSenderId() == creature.getWurmId()) || (creature.getPower() >= 2)) {
          return true;
        }
      }
  ...
  {
    for (Delivery delivery : deliveryQueue.values()) {
      if (delivery.getDeliveryWaystoneId() == waystoneId) {
        if (delivery.isPreDelivery()) {
          return true;
        }
      }
    }
  ...
  public static void freeContainer(long containerId)
  {
    ...
    if (delivery != null)
    {
      ...
      containerDelivery.remove(Long.valueOf(containerId));
    }
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\creatures\Delivery.class
