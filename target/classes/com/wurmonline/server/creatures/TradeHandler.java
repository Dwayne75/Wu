  ...
  implements MiscConstants, ItemTypes, MonetaryConstants
{
  ...
  TradeHandler(Creature aCreature, Trade _trade)
  {
    ...
    if (this.shop.isPersonal())
    {
      ...
      if (this.ownerTrade) {
        this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
          .getName() + " says, 'Welcome back, " + this.trade.creatureOne.getName() + "!'");
      } else {
        this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
          .getName() + " says, 'I will not buy anything, but i can offer these things.'");
      }
    }
    else
    {
      ...
      if (this.shop.getMoney() <= 1L)
      {
        if (this.trade.creatureOne.getPower() >= 3) {
          this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
            .getName() + " says, 'I only have " + this.shop.getMoney() + " and can't buy anything right now.'");
        } else {
          this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
            .getName() + " says, 'I am a bit low on money and can't buy anything right now.'");
        }
      }
      else if (this.shop.getMoney() < 100L)
      {
        if (this.trade.creatureOne.getPower() >= 3) {
          this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
            .getName() + " says, 'I only have " + new Change(this.shop.getMoney()).getChangeShortString() + ".'");
        } else {
          this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
            .getName() + " says, 'I am a bit low on money, but let's see what you have.'");
        }
      }
      else if (this.trade.creatureOne.getPower() >= 3) {
        this.trade.creatureOne.getCommunicator().sendSafeServerMessage(aCreature
          .getName() + " says, 'I have " + new Change(this.shop.getMoney()).getChangeShortString() + ".'");
      }
    }
  }
  ...
  void addItemsToTrade()
  {
    if (this.trade != null)
    {
      ...
      for (int x = 0; x < itarr.length; x++)
      {
        ...
        Set<Item> its = (Set)this.itemMap.get(Integer.valueOf(templateId));
        if (its == null) {
          its = new HashSet();
        }
        its.add(itarr[x]);
        ...
        this.itemMap.put(Integer.valueOf(templateId), its);
      }
      if (!this.shop.isPersonal())
      {
        ...
        if ((Features.Feature.NAMECHANGE.isEnabled()) && (!foundDeclaration)) {
          try
          {
            ...
            Item item = Creature.createItem(843, 60 + Server.rand.nextInt(40));
        ...
        if ((Features.Feature.WAGONER.isEnabled()) && (!foundWagonerContract)) {
          ...
          {
            Item inventory = this.creature.getInventory();
            Item item = Creature.createItem(1129, 60 + Server.rand.nextInt(40));
            inventory.insertItem(item);
            newMerchandise = true;
          }
          catch (Exception ex)
        ...
        if (newMerchandise) {
          this.trade.creatureOne.getCommunicator().sendSafeServerMessage(this.creature
            .getName() + " says, 'Oh, I forgot I have some new merchandise. Let us trade again and I will show them to you.'");
        }
      }
  ...
  public int getTraderSellPriceForItem(Item item, TradingWindow window)
  {
    ...
    int numberSold = 1;
    if (window == this.trade.getCreatureOneRequestWindow()) {
    ...
    return (int)Math.max(2.0D, localPrice * this.shop.getPriceModifier());
  }
  ...
  public int getTraderBuyPriceForItem(Item item)
  {
    ...
    int price = 1;
    List<Item> itlist = (List)this.currentDemandMap.get(Integer.valueOf(item.getTemplateId()));
    int extra = 1;
    if (itlist == null)
    {
      if (logger.isLoggable(Level.FINEST)) {
    ...
    if (logger.isLoggable(Level.FINEST)) {
      logger.finest("localBuyPrice for " + item.getName() + "=" + price + " extra=" + extra + " price for extra+1=" + 
    ...
    for (int x = 0; x < whatPlayerWants.length; x++) {
      ...
      else
      {
        ...
        if ((this.shop.isPersonal()) && (whatPlayerWants[x].getPrice() > 0))
        {
          ...
          this.pdemand = ((this.pdemand + localPrice));
        }
        else
        {
          ...
          try
          {
            if ((!this.shop.isPersonal()) || (this.shop.usesLocalPrice()))
            {
              ...
              Set<Item> itlist = (Set)this.purchaseMap.get(Integer.valueOf(tid));
              if (itlist == null) {
                itlist = new HashSet();
              }
              itlist.add(whatPlayerWants[x]);
              this.purchaseMap.put(Integer.valueOf(tid), itlist);
              ItemTemplate temp;
              if (whatPlayerWants[x].isCombine())
              {
            ...
            if (logger.isLoggable(Level.FINEST)) {
              logger.finest("LocalSellPrice for " + whatPlayerWants[x].getName() + "=" + localPrice + " mod=" + this.shop
                .getPriceModifier() + " sum=" + Math.max(2.0D, localPrice * this.shop.getPriceModifier()));
            }
            this.pdemand = ((this.pdemand + Math.max(2.0D, localPrice * this.shop.getPriceModifier())));
          }
      ...
      else
      {
        ...
        if (this.fullPriceItems.isEmpty())
        {
          ...
          if (whatIWant[x].isCombine()) {
            try
            {
              temp = ItemTemplateFactory.getInstance().getTemplate(whatIWant[x]
                .getTemplateId());
              for (Item i : itlist) {
                extra += Math.max(1, i.getWeightGrams() / temp.getWeightGrams());
              }
  ...
  private void suckInterestingItems()
  {
    ...
    if (!this.shop.isPersonal())
    {
      ...
      for (int x = 0; x < setItems.length; x++)
      {
        ...
        List<Item> itlist = (List)this.currentDemandMap.get(Integer.valueOf(templateId));
        if (itlist == null) {
          itlist = new LinkedList();
        }
        itlist.add(setItems[x]);
        this.currentDemandMap.put(Integer.valueOf(templateId), itlist);
      }
      int templateId = -10;
      ...
      for (int x = 0; x < offItems.length; x++) {
        if ((!offItems[x].isArtifact()) && (offItems[x].isPurchased()) && (offItems[x].getLockId() == -10L))
        {
          ...
          if ((offItems[x] == parent) || (parent.isViewableBy(this.creature))) {
            ...
            {
              this.trade.creatureOne.getCommunicator().sendSafeServerMessage(this.creature
                .getName() + " says, 'Please empty the " + offItems[x].getName() + " first.'");
            }
            else
            {
    ...
    else if (this.ownerTrade)
    {
      ...
      {
        this.trade.creatureOne.getCommunicator().sendNormalServerMessage(this.creature
          .getName() + " says, 'I cannot add more items to my stock right now.'");
      }
      else
      {
        ...
        else
        {
          targetWin.startReceivingItems();
          for (int x = 0; x < offItems.length; x++) {
            if ((offItems[x].getTemplateId() != 272) && (offItems[x].getTemplateId() != 781) && 
              (!offItems[x].isArtifact()) && (!offItems[x].isRoyal()) && (
              ...
              if (size > maxPersonalItems)
              {
                ...
                (offItems[x].isHollow()) && (!offItems[x].isEmpty(true)) && 
                (!offItems[x].isSealedByPlayer())))
              {
                if ((offItems[x].isLockable()) && (offItems[x].isLocked())) {
                  this.trade.creatureOne.getCommunicator().sendSafeServerMessage(this.creature
                    .getName() + " says, 'I don't accept locked items any more. Sorry for the inconvenience.'");
                } else {
                  this.trade.creatureOne.getCommunicator().sendSafeServerMessage(this.creature
                    .getName() + " says, 'Please empty the " + offItems[x].getName() + " first.'");
                }
        ...
        else if (!this.waiting)
        {
          ...
          if ((!this.fullPriceItems.isEmpty()) && (this.hasOtherItems))
          {
            for (Item lItem : this.fullPriceItems) {
              this.trade.creatureOne.getCommunicator().sendSafeServerMessage(this.creature
                .getName() + " says, 'Sorry, " + this.trade.creatureOne.getName() + ". I must charge full price in coin value for the " + lItem
                .getName() + ".'");
            }
            this.waiting = true;
          }
          else
          {
            ...
            else if (diff < 0L)
            {
              ...
              {
                change = this.trade.getCreatureTwoRequestWindow().getAllItems();toSend = change.length;
                for (String str1 = 0; str1 < toSend; str1++)
                {
                  Item i = change[str1];
                  if (!i.isCoin())
                  {
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\creatures\TradeHandler.class
