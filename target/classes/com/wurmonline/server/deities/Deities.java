  ...
  implements MiscConstants, TimeConstants
{
  ...
  {
    deities.remove(Integer.valueOf(number));
  }
  
  public static Deity getDeity(int number)
  {
    return (Deity)deities.get(Integer.valueOf(number));
  }
  ...
  {
    String entityName = (String)valreiNames.get(Integer.valueOf(entityNumber));
    if (entityName != null) {
      for (Deity d : deities.values()) {
  ...
  public static final int translateEntityForDeity(int deityNumber)
  {
    ...
    if (d != null) {
      for (Map.Entry<Integer, String> entry : valreiNames.entrySet()) {
        if (((String)entry.getValue()).equalsIgnoreCase(d.getName())) {
          return ((Integer)entry.getKey()).intValue();
        }
      }
    }
  ...
  {
    valreiNames.put(Integer.valueOf(number), name);
  }
  ...
  public static final boolean isNameOkay(String aName)
  {
    ...
    for (Deity d : deities.values()) {
      if (d.getNumber() < 100) {
        if (lName.equals(d.name.toLowerCase())) {
          return false;
        }
      }
    }
    if ((lName.equals("jackal")) || (lName.equals("valrej")) || (lName.equals("valrei")) || (lName.equals("seris")) || 
      (lName.equals("sol")) || (lName.equals("upkeep")) || (lName.equals("system")) || (lName.equals("village")) || 
      (lName.equals("team")) || (lName.equals("local")) || (lName.equals("combat")) || (lName.equals("friends")) || 
      (lName.equals("nogump")) || (lName.equals("uttacha")) || (lName.equals("pharmakos")) || (lName.equals("walnut"))) {
      return false;
    }
    return true;
  }
    ...
    try
    {
      ...
      while (rs.next())
      {
        ...
        if (d > 0)
        {
          ...
          Float f = (Float)faithMap.get(Integer.valueOf(d));
          if (f == null)
          {
            f = new Float(faith);
          ...
          Deity deity = (Deity)deities.get(Integer.valueOf(d));
          if (deity != null) {
            deity.setActiveFollowers(deity.getActiveFollowers() + 1);
          }
          faithPlayers += 1.0F;
        }
        byte kdom = rs.getByte("KINGDOM");
        if (kdom == 1) {
          Kingdoms.activePremiumJenn += 1;
        } else if (kdom == 3) {
          Kingdoms.activePremiumHots += 1;
        } else if (kdom == 2) {
          Kingdoms.activePremiumMolr += 1;
        }
        Kingdoms.getKingdom(kdom).activePremiums += 1;
        Kingdoms.getKingdom(kdom).countedAtleastOnce = true;
      }
      try
      {
        for (Map.Entry<Integer, Float> me : faithMap.entrySet())
        {
          Deity deity = getDeity(((Integer)me.getKey()).intValue());
  ...
  private static final void loadDeities()
    throws IOException
  {
    Connection dbcon = null;
    PreparedStatement ps = null;
  ...
  public static final Deity ascend(int newid, String name, long wurmid, byte sex, byte power, float attack, float vitality)
  {
    ...
    DbDeity deity = new DbDeity(newNum, name, (byte)0, sex, power, 0.0D, 505 + rand.nextInt(4), 0, attack, vitality, true);
  ...
  public static final Deity getRandomHateDeity()
  {
    ...
    for (Iterator localIterator = deities.values().iterator(); localIterator.hasNext();)
    {
      d = (Deity)localIterator.next();
    ...
    if (exists)
    {
      Object toRet = new LinkedList();
      for (Deity d : deities.values()) {
        if ((d.isHateGod()) && (d.number != 4)) {
          ((LinkedList)toRet).add(d);
        }
      }
      return (Deity)((LinkedList)toRet).get(Server.rand.nextInt(((LinkedList)toRet).size()));
    }
    return null;
  }
  ...
  {
    DbDeity deity = new DbDeity(1, "Fo", (byte)100, (byte)0, (byte)5, 0.0D, 505, 0, 7.0F, 5.0F, true);
    addDeity(deity);
    deity.setFavoredKingdom((byte)1);
    try
    {
    ...
    deity = new DbDeity(2, "Magranon", (byte)70, (byte)0, (byte)4, 0.0D, 507, 0, 6.0F, 6.0F, true);
    ...
    deity = new DbDeity(3, "Vynora", (byte)70, (byte)1, (byte)4, 0.0D, 508, 0, 5.0F, 7.0F, true);
    ...
    deity = new DbDeity(4, "Libila", (byte)-100, (byte)1, (byte)4, 0.0D, 506, 0, 6.0F, 6.0F, true);
  ...
  {
    valreiPositions.put(Integer.valueOf(deityId), Integer.valueOf(hexPosition));
  }
  
  public static final Integer getPosition(int deityId)
  {
    return (Integer)valreiPositions.get(Integer.valueOf(deityId));
  }
  ...
  {
    valreiStatuses.put(status, Integer.valueOf(deityId));
  }
  ...
  public static final String getEntityName(int deityId)
  {
    ...
    String n = (String)valreiNames.get(Integer.valueOf(deityId));
    if (n == null) {
      n = "";
    }
    return n;
  ...
  public static final String getRandomStatusFor(int deityId)
  {
    ...
    for (Map.Entry<String, Integer> status : valreiStatuses.entrySet()) {
      if (((Integer)status.getValue()).intValue() == deityId) {
  ...
  public static final String getRandomStatus()
  {
    ...
    {
      num = Server.rand.nextInt(valreiStatuses.size());
      
      x = 0;
      for (Map.Entry<String, Integer> status : valreiStatuses.entrySet())
      {
        if (x >= num) {
          return (String)status.getKey();
        }
        x++;
      }
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\deities\Deities.class
