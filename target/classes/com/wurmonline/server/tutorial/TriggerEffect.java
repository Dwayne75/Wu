  ...
  implements CounterTypes, MiscConstants, Comparable<TriggerEffect>, TimeConstants
{
  ...
  public final boolean setMissionState(Creature performer, MissionPerformed mperf, boolean setFinishedEpic)
  {
    ...
    {
      float newstate = Math.min(100.0F, mperf.getState() + getMissionStateChange());
      ...
      if (!mperf.isFailed())
      {
        ...
        if ((mission.getMaxTimeSeconds() > 0) && 
          (System.currentTimeMillis() > mperf.getFinishTimeAsLong(mission.getMaxTimeSeconds())))
        {
          mperf.setState(-1.0F, performer.getWurmId());
          String miss = Server.getTimeFor(System.currentTimeMillis() - mperf
            .getFinishTimeAsLong(mission.getMaxTimeSeconds()));
  ...
  {
    if (((getTextDisplayed() != null) && (getTextDisplayed().length() > 0)) || (
      (getTopText() != null) && (getTopText().length() > 0)))
    {
      ...
      pop.setToSend(getTextDisplayed());
      
      pop.setTop(getTopText());
      pop.sendQuestion();
      return true;
    }
    return false;
  ...
  private static void destroyInventoryItems(Item inventory, Creature performer)
  {
    ...
    {
      if (inventoryItems[i].isTraded()) {
        if (performer.getTrade() != null) {
  ...
  {
    performer.sendToLoggers("Running effect " + this.name + " state effect " + getMissionStateChange() + " on mission " + this.missionId, (byte)2);
    if (!this.inActive)
    {
      boolean sendSound = true;
      if ((performer.isPlayer()) && (this.creatorType != 2))
      {
        if (sendTriggerDescription(performer)) {
      ...
      if ((sendSound) && (getSoundName() != null) && (getSoundName().length() > 0)) {
        SoundPlayer.playSound(getSoundName(), performer, 1.5F);
      }
      if (getMissionStateChange() != 0.0F)
      {
        if (setMissionState(performer, perf, setFinished)) {
          if ((this.destroysTarget) && (this.creatorType != 3)) {
        ...
        if (this.creatorType == 2)
        {
          ...
          if (em != null) {
            if (em.isCurrent())
            {
              performer.sendToLoggers("Effect " + this.name + " state effect " + getMissionStateChange() + " on Epic mission " + em
                .getEntityName(), (byte)2);
              em.updateProgress(em.getMissionProgress() + getMissionStateChange());
              ...
              if ((Deities.getFavoredKingdom(deityNum) == performer.getKingdomTemplateId()) || (!Servers.localServer.EPIC)) {
                if ((missionEnum != null) && (!EpicMissionEnum.isMissionKarmaGivenOnKill(missionEnum)))
                {
                  ...
                  {
                    int karmaGained = (int)Math.ceil(karmaSplit / 100.0F * getMissionStateChange());
                    if (karmaGained > 0)
                    {
                      ...
                      if (getMissionStateChange() * 10.0F > 10.0F) {
                        logger.log(Level.INFO, "Added karma " + karmaGained + " to " + performer.getName() + " for mission " + em
                          .getMissionId() + " for " + em.getEntityName() + " state change=" + 
                          getMissionStateChange() + " to " + em.getMissionProgress());
                      }
                    }
                  }
                }
              ...
              if (em.isCompleted())
              {
                logger.log(Level.INFO, "Mission is complete! " + performer.getName() + " mission " + em
                  .getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf
                  .getMissionId());
                if (performer.isPlayer()) {
                  ((Player)performer).setLastTrigger(this.id);
                }
                sendTriggerDescription(performer);
                ...
                if (m != null)
                {
                  ...
                  if ((Deities.getFavoredKingdom(deityNum) == performer.getKingdomTemplateId()) || (!Servers.localServer.EPIC))
                  {
                    ...
                    baseKarma = missionEnum.getBaseKarma();
                    baseSleep = missionEnum.getBaseSleep();
                    int karmaSplit = missionEnum.getKarmaBonusDiffMult() * em.getDifficulty();
                    int sleepSplit = missionEnum.getSleepBonusDiffMult() * em.getDifficulty();
                    
                    ArrayList<Player> totalNearby = new ArrayList();
                    for (p : Players.getInstance().getPlayers()) {
                      if ((Deities.getFavoredKingdom(deityNum) == p.getKingdomTemplateId()) || (!Servers.localServer.EPIC)) {
                        if (p.isWithinDistanceTo(performer, 300.0F)) {
                          totalNearby.add(p);
                        }
                      }
                    }
                    Object allParticipants = new ArrayList();
                    MissionPerformer[] allperfs = MissionPerformed.getAllPerformers();
                    MissionPerformer[] arrayOfMissionPerformer1 = allperfs;Player p = arrayOfMissionPerformer1.length;
                    MissionPerformed thisMission;
                    for (Object localObject1 = 0; localObject1 < p; localObject1++)
                    {
                      MissionPerformer mp = arrayOfMissionPerformer1[localObject1];
                      
                      thisMission = mp.getMission(m.getId());
                      if (thisMission != null)
                      {
                        thisMission.setState(100.0F, mp.getWurmId());
                        PlayerInfo pinf = PlayerInfoFactory.getPlayerInfoWithWurmId(mp.getWurmId());
                        if (pinf != null) {
                          ((ArrayList)allParticipants).add(pinf);
                        }
                      }
                    }
                    ...
                    {
                      karmaGainedNearby = (int)Math.ceil(karmaSplit / totalNearby.size());
                      karmaSplit = 0;
                    }
                    ...
                    for (localObject1 = totalNearby.iterator(); ((Iterator)localObject1).hasNext();)
                    {
                      Player p = (Player)((Iterator)localObject1).next();
                      if (sleepGainedNearby > 0) {
                        p.getSaveFile().addToSleep((int)(Math.min(30, sleepGainedNearby) * 60L));
                      }
                      if (karmaGainedNearby > 0)
                      {
                        ...
                        if (p.isPaying())
                        {
                          ...
                          ids.add(Long.valueOf(p.getWurmId()));
                          values.add(Integer.valueOf(p.getScenarioKarma()));
                        }
                      }
                    }
                    int karmaGained = baseKarma;
                    if (karmaSplit > 0) {
                      karmaGained += (int)Math.ceil(karmaSplit / ((ArrayList)allParticipants).size());
                    }
                    int sleepBonusGain = 0;
                    if (sleepSplit > 0) {
                      sleepBonusGain += (int)Math.ceil(sleepSplit / ((ArrayList)allParticipants).size());
                    }
                    for (PlayerInfo pinf : (ArrayList)allParticipants)
                    {
                      ...
                      if (karmaGained > 0)
                      {
                        ...
                        if (pinf.isPaying())
                        {
                          ...
                          ids.add(Long.valueOf(pinf.wurmId));
                          values.add(Integer.valueOf(pinf.getScenarioKarma()));
                        }
                      }
                      if (Servers.localServer.PVPSERVER) {
                        ...
                        {
                          logger.log(Level.WARNING, getName() + ": failed to reset rank decay timer " + pinf.getRank(), iox);
                        }
                      }
                    ...
                    if (Servers.localServer.EPIC)
                    {
                      ...
                      {
                        idsToSend[x] = ((Long)ids.get(x)).longValue();
                        valuesToSend[x] = ((Integer)values.get(x)).intValue();
                      }
                  ...
                  for (MissionTrigger t : triggers) {
                    ...
                    {
                      logger.log(Level.INFO, performer.getName() + "Destroying triggers for  mission " + em
                        .getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf
                        .getMissionId());
                ...
                logger.log(Level.INFO, performer.getName() + "Destroying effect for  mission " + em
                  .getMissionId() + " for " + em.getEntityName() + ", performer mission is " + perf
                  .getMissionId());
                
                destroy();
                logger.log(Level.INFO, performer.getName() + " Sending report for " + em.getMissionId() + " for " + em
                  .getEntityName() + ", performer mission is " + perf
                  .getMissionId());
                ...
                if (!Servers.localServer.EPIC)
                {
                  ...
                  if ((deityNumber > 0) && (deityNumber <= 4))
                  {
                    ...
                    es.generateNewMissionForEpicEntity(deityNumber, entityName, em.getDifficulty() + 1, 604800, em
                      .getScenarioName(), EpicServerStatus.getCurrentScenario().getScenarioNumber(), "You must really do this for " + entityName + " because yeah you know.", true);
                  }
                }
              }
            }
      ...
      if ((this.rewardItem > 0) && (this.creatorType != 3))
      {
        ...
        for (int x = 0; x < this.rewardNumbers; x++) {
          try
          {
            Item toCreate = ItemFactory.createItem(this.rewardItem, this.rewardQl, this.itemMaterial, (byte)0, performer
              .getName());
            if (this.newbieItem) {
            ...
            insertItem(performer, toCreate, x);
          }
      ...
      if (this.existingItemReward > 0L) {
        try
        {
          ...
          insertItem(performer, toCreate, 0);
      ...
      if ((this.rewardSkillNum > 0) && (this.creatorType != 3))
      {
        ...
        if (skills != null) {
          try
          {
            ...
            reward = this.rewardSkillVal;
            if (this.rewardSkillVal < 1.0F)
            {
              double diff = 100.0D - existing;
      ...
      if (getSpecialEffectId() > 0) {
        if ((this.teleportX > 0) || (this.teleportY > 0)) {
          SpecialEffects.getEffects()[getSpecialEffectId()].run(performer, this.teleportX, this.teleportY, this.teleportLayer);
        } else {
          try
          {
            SpecialEffects.getEffects()[getSpecialEffectId()].run(performer, target, this.rewardNumbers);
          }
          catch (Exception ex)
          {
      ...
      if (getAchievementId() > 0) {
        performer.achievement(getAchievementId());
      }
      if ((getModifyTileX() > 0) || (getModifyTileY() > 0)) {
        if (getNewTileType() != 0) {
          if ((getNewTileType() == Tiles.Tile.TILE_CAVE_WALL.id) || (getNewTileType() == Tiles.Tile.TILE_CAVE_WALL_ORE_IRON.id))
          {
            ...
            {
              int ntile = Server.caveMesh.getTile(getModifyTileX(), getModifyTileY());
              if (Tiles.decodeType(ntile) == Tiles.Tile.TILE_CAVE_WALL.id)
              {
                Server.caveMesh.setTile(
                  getModifyTileX(), 
                  getModifyTileY(), 
                  Tiles.encode(Tiles.decodeHeight(ntile), Tiles.Tile.TILE_CAVE_WALL_ORE_IRON.id, 
                  Tiles.decodeData(ntile)));
          ...
          {
            int otile = Server.surfaceMesh.getTile(getModifyTileX(), getModifyTileY());
            if (Tiles.decodeType(otile) != Tiles.Tile.TILE_HOLE.id)
            {
              VolaTile t = Zones.getOrCreateTile(getModifyTileX(), getModifyTileY(), true);
              if (t != null) {
                if (t.getStructure() == null)
                {
                  Server.setSurfaceTile(getModifyTileX(), getModifyTileY(), 
                    Tiles.decodeHeight(otile), (byte)getNewTileType(), getNewTileData());
      ...
      if ((this.spawnTileX > 0) || (this.spawnTileY > 0)) {
        try
        {
          ...
          {
            CreatureTemplate ct = CreatureTemplateFactory.getInstance().getTemplate(getCreatureSpawn());
            byte sex = ct.getSex();
            if (!ct.keepSex) {
              sex = Server.rand.nextBoolean() ? 1 : 0;
            }
            byte ttype = getCreatureType();
            byte age = (byte)getCreatureAge();
            try
            {
              reward = Creature.doNew(ct.getTemplateId(), true, this.spawnTileX * 4 + 2, this.spawnTileY * 4 + 2, performer
                .getStatus().getRotation() - 180.0F, performer.getLayer(), ct.getName(), sex, (byte)0, ttype, false, age);
            }
            catch (Exception cex)
            {
      ...
      if (getMissionToActivate() > 0)
      {
        Mission mis = Missions.getMissionWithId(getMissionToActivate());
        if (mis != null) {
          mis.setInactive(false);
        }
      }
      if (getMissionToDeActivate() > 0)
      {
        Mission mis = Missions.getMissionWithId(getMissionToDeActivate());
        if (mis != null) {
          mis.setInactive(true);
        }
      }
      if (getTriggerToActivate() > 0)
      {
        MissionTrigger trg = MissionTriggers.getTriggerWithId(getTriggerToActivate());
        if (trg != null) {
          trg.setInactive(false);
        }
      }
      if (getTriggerToDeActivate() > 0)
      {
        MissionTrigger trg = MissionTriggers.getTriggerWithId(getTriggerToDeActivate());
        if (trg != null) {
          trg.setInactive(true);
        }
      }
      if (getEffectToActivate() > 0)
      {
        TriggerEffect eff = TriggerEffects.getTriggerEffect(getEffectToActivate());
        if (eff != null) {
          eff.setInactive(false);
        }
      }
      if (getEffectToDeActivate() > 0)
      {
        TriggerEffect eff = TriggerEffects.getTriggerEffect(getEffectToDeActivate());
        if (eff != null) {
      ...
      if ((this.teleportX > 0) || (this.teleportY > 0)) {
        if (getSpecialEffectId() <= 0) {
          if ((performer.getCurrentTile() != null) && (!performer.isTeleporting()))
          {
  ...
  void destroyTarget(long targetId)
  {
    ...
    if ((WurmId.getType(targetId) == 2) || (WurmId.getType(targetId) == 6) || 
      (WurmId.getType(targetId) == 19) || (WurmId.getType(targetId) == 20))
    {
  ...
  private void insertItem(Creature performer, Item toCreate, int x)
  {
    ...
    else
    {
      try
      {
        ...
        if (x < 1) {
          performer.getCommunicator().sendSafeServerMessage("A " + container
            .getName() + " now contains " + toCreate.getName() + ".");
        }
      }
      catch (NoSuchItemException nsi)
      {
  ...
  {
    return getName().compareTo(aTriggerEffect.getName());
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\tutorial\TriggerEffect.class
