  ...
  implements MiscConstants
{
  ...
  {
    this.missionsPerformed.put(Integer.valueOf(mp.getMissionId()), mp);
  }
  
  public MissionPerformed getMission(int mission)
  {
    return (MissionPerformed)this.missionsPerformed.get(Integer.valueOf(mission));
  }
  ...
  {
    return isMissionPerformed(mission, 100.0F);
  }
  
  boolean isMissionPerformed(int mission, float state)
  {
    MissionPerformed m = (MissionPerformed)this.missionsPerformed.get(Integer.valueOf(mission));
    if (m != null) {
  ...
  public static final void sendEpicMissionPvPServer(EpicMission mission, Creature creature, Communicator comm)
  {
    ...
    if ((mis != null) && ((!mis.isHidden()) || (comm.player.getPower() >= 2))) {
      if (Deities.getFavoredKingdom(mission.getEpicEntityId()) == creature.getKingdomTemplateId())
      {
        ...
        comm.sendMissionState(-mission.getMissionId(), name, mis.getInstruction(), mis
          .getMissionCreatorName(), mission
          .getMissionProgress(), mis.getLastModifiedAsLong(), mission.getEndTime(), mission
          .getExpireTime(), false, mis
          .getDifficulty(), mis.getRewards());
      }
      else
      {
  ...
  public void sendAllMissionPerformed(Communicator comm)
  {
    if (comm != null) {
      for (MissionPerformed mp : this.missionsPerformed.values()) {
        if (((mp.getState() > -1.0F) && (mp.getState() < 100.0F)) || 
          (mp.getEndTime() >= System.currentTimeMillis() - 7257600000L)) {
          if (!mp.isInactivated())
          {
            ...
            if (mis != null)
            {
              if ((!mis.isHidden()) || (comm.player.getPower() >= 2))
              {
                ...
                comm.sendMissionState(mp.getWurmId(), name, mis.getInstruction(), mis.getMissionCreatorName(), mp
                  .getState(), mp.getStartTime(), mp.getEndTime(), mp
                  .getFinishTimeAsLong(mis.getMaxTimeSeconds()), mis.hasSecondChance(), mis
                  .getDifficulty(), mis.getRewards());
              }
            }
            else {
              comm.sendMissionState(mp.getWurmId(), "Unknown", "N/A", "N/A", mp.getState(), mp.getStartTime(), mp
                .getEndTime(), 0L, false, (byte)-10, "Unknown");
            }
          }
        }
      }
  ...
  void sendUpdatePerformer(MissionPerformed mp)
  {
    try
    {
      ...
      else
      {
        ...
        perf.getCommunicator().sendMissionState(mp.getWurmId(), name, instruction, creatorName, mp.getState(), mp
          .getStartTime(), mp.getEndTime(), maxTime, mayBeRestarted, difficulty, rewards);
      }
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\tutorial\MissionPerformer.class
