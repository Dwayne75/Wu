  ...
  void run(Creature performer, MissionTrigger[] trigs, int counter)
  {
    ...
    if (trigs.length > 0)
    {
      ...
      for (int x = 0; x < trigs.length; x++) {
        if (!trigs[x].isInactive()) {
          if (!doneActions.contains(Integer.valueOf(trigs[x].getOnActionPerformed())))
          {
            ...
            if (mis != null)
            {
              ...
              if ((!Servers.isThisAChaosServer()) && (Servers.localServer.PVPSERVER)) {
                if (Deities.getFavoredKingdom(mis.getEpicEntityId()) != performer.getKingdomTemplateId())
                {
                  skip = true;
              ...
              if ((trigs[x].getSeconds() == counter) || ((counter == 1) && (trigs[x].getSeconds() == 0)))
              {
                ...
                if ((mission != null) && (!mission.isInactive()))
                {
                  ...
                  if (trigs[x].getStateRequired() == 0.0F) {
                    if (mp == null)
                    {
                      mp = MissionPerformed.startNewMission(trigs[x].getMissionRequired(), performer
                        .getWurmId(), 1.0F);
                      sendStartPopup = true;
                    ...
                    else
                    {
                      ...
                      if (mperf == null)
                      {
                        MissionPerformed.startNewMission(trigs[x].getMissionRequired(), performer
                          .getWurmId(), 1.0F);
                        sendStartPopup = true;
                        performer
                          .sendToLoggers("Starting mission. No existing mission.", (byte)2);
                      }
                    }
                  }
                  if (mp != null)
                  {
                    ...
                    runEffect(performer, mp, trigs[x].getMissionRequired(), trigs[x], sendStartPopup, doneActions);
                  }
                }
                else
                {
  ...
  public final void runEffect(Creature performer, MissionPerformer mp, int missionId, MissionTrigger trigger, boolean sendStartPopup, Set<Integer> doneActions)
  {
    ...
    if ((mperf != null) && ((mperf.isStarted()) || (mperf.isFailed()))) {
      if (mission != null)
      {
        ...
        if (secondChance) {
          if (trigger.getStateRequired() == 0.0F)
          {
        ...
        if (!mperf.isFailed())
        {
          ...
          if (mis != null) {
            if (mis.getMissionProgress() < 100.0F) {
            ...
            (System.currentTimeMillis() > mperf.getFinishTimeAsLong(mission.getMaxTimeSeconds())))
          {
            ...
            (trigger.isTriggered(mperf.getState(), true)) || ((sendStartPopup) && 
            (mperf.getState() == 1.0F) && (trigger.getStateRequired() == 0.0F)))
          {
            ...
            {
              eff[e].effect(performer, mperf, trigger.getTarget(), setEpicFinished, trigger
                .getOnActionPerformed() != 142);
              doneActions.add(Integer.valueOf(trigger.getOnActionPerformed()));
              this.triggered = true;
          ...
          if (sendStartPopup) {
            if ((mission.getInstruction() != null) && (mission.getInstruction().length() > 0)) {
              ...
              {
                performer.sendToLoggers("Not sending mission start popup for mission " + trigger
                  .getMissionRequired() + " since it is already finished.", (byte)2);
              }
            }
          }
        }
      ...
      else
      {
        ...
        if (performer.getPower() > 0) {
          performer.getCommunicator().sendNormalServerMessage("The trigger " + trigger
            .getName() + " was made inactive because it was not connected to any mission.");
        }
      }
    }
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\tutorial\TriggerRun.class
