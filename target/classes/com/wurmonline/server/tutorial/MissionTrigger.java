  ...
  implements MiscConstants, Comparable<MissionTrigger>, CounterTypes
{
  ...
  {
    if (getStateEnd() > getStateRequired()) {
      return getStateRequired() + " to " + getStateEnd();
    }
    return Float.toString(getStateRequired());
  }
  ...
  public boolean isTriggered(float missionState, boolean checkActive)
  {
    ...
    {
      if ((missionState >= getStateRequired()) && (missionState <= getStateEnd())) {
        stateOk = true;
      }
    }
    else if (getStateRequired() == missionState) {
      stateOk = true;
    }
    if (stateOk)
    {
  ...
  public Spawnpoint getSpawnPoint()
  {
    ...
    else if (WurmId.getType(targetId) == 3)
    {
      ...
      if (x > Zones.worldTileSizeX)
      {
        ...
        setOnTargetId(newTarg);
        update();
        logger.log(Level.INFO, "Updated mission trigger " + getName() + " to " + x + "," + y + " from " + oldx + "," + y);
      }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\tutorial\MissionTrigger.class
