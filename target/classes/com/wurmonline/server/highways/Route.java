...
public class Route
{
  ...
  public Route(Node startNode, byte direction, int id)
  {
    ...
    this.id = id;
    addCost(startNode.getWaystone(), direction);
  }
  ...
  public void AddCatseye(Item catseye, boolean atFront, byte direction)
  {
    ...
    addCost(catseye, direction);
  }
  
  private void addCost(Item marker, byte direction)
  {
    float thiscost = 1.0F;
    if ((direction == 2) || (direction == 8) || (direction == 32) || (direction == Byte.MIN_VALUE)) {
      thiscost = 1.414F;
    }
      ...
      for (int x = 0; x < highwayPath.size(); x++) {
        if (highwayPath.get(x) == this)
        {
          ...
          for (int y = x; y < highwayPath.size(); y++) {
            distance += ((Route)highwayPath.get(y)).getDistance();
          }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\target\classes\com\wurmonline\server\highways\Route.class
