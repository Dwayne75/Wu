...
public final class MeshIO
{
  ...
  private short[] maxHeightCoord = { -1, -1 };
  public final int[] data;
  private FileChannel fileChannel;
  private ByteBuffer tmpBuf;
  private final IntBuffer tmpBufInt;
  private boolean[] rowDirty = new boolean[''];
  private int rowId = 0;
  ...
  private MeshIO(ByteBuffer header)
    throws IOException
  {
    readHeader(header);
  ...
  private void writeHeader(ByteBuffer header)
    throws IOException
  {
  ...
  public static MeshIO createMap(String filename, int level, int[] data)
    throws IOException
  {
    ...
    if (logger.isLoggable(Level.FINER)) {
      logger.finer("Header capacity: " + header.capacity() + ", header.limit: " + header.limit() + ", header.position: " + header
        .position());
    }
  ...
  public static MeshIO open(String filename)
    throws IOException
  {
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.fine("Header capacity: " + header.capacity() + ", header.limit: " + header.limit() + ", header.position: " + header
        .position());
    }
  ...
  @Deprecated
  public final void saveTile(int x, int y)
    throws IOException
  {
    this.tmpBuf.clear();
  ...
  public final void saveFullRows(int y, int rows)
    throws IOException
  {
  ...
  public final int saveAllDirtyRows()
    throws IOException
  {
    long lStart = System.nanoTime();
  ...
  public final boolean saveNextDirtyRow()
    throws IOException
  {
    boolean lRowWasDirty = false;
    if (this.rowDirty[this.rowId] != 0)
    {
      lRowWasDirty = true;
      long lStart = System.nanoTime();
  ...
  public void calcDistantTerrain()
  {
    ...
    for (int xT = 0; xT < this.size / 16; xT++) {
      ...
      {
        int[] counts = new int['Ā'];
        for (int x = xT * 16; x < xT * 16 + 16; x++) {
          for (int y = yT * 16; y < yT * 16 + 16; y++)
          {
            int type = Tiles.decodeType(getTile(x, y)) & 0xFF;
            counts[type] += 1;
          }
        }
        int mostCommon = 0;
  ...
  {
    return 
      "MeshIO [Size: " + this.size + ", linesPerRow: " + this.linesPerRow + ", rowId: " + this.rowId + ", size_level: " + this.size_level + "]@" + hashCode();
  }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\mesh\MeshIO.class
