...
public class ValreiFight
{
  ...
  class FightEntity
  {
    ...
    {
      return rollSkill(skillId, -1);
    }
    
    public float rollSkill(int skillId, int bonusSkillId, float skillBuffed)
    {
      return rollSkill(skillId, bonusSkillId, 3.0F, skillBuffed);
    }
    
    public float rollSkill(int skillId, int bonusSkillId)
    {
      return rollSkill(skillId, bonusSkillId, 3.0F, 0.0F);
    }
    ...
    public Point getTargetMove(boolean towards, FightEntity other)
    {
      ...
      if (ValreiFight.test1.getY() != ValreiFight.test2.getY())
      {
        ...
        for (int i = 0; i < 3; i++)
        {
          ...
          {
          case 0: 
            newY += (ValreiFight.test2.getY() == ValreiFight.test1.getY() ? 0 : ValreiFight.test2.getY() > ValreiFight.test1.getY() ? -1 : 1);
            if ((newY != ValreiFight.test1.getY()) && 
              (ValreiFight.this.isMoveValid(this, newX, newY))) {
              return new Point(newX, newY);
            }
            break;
          case 1: 
            newX += (ValreiFight.test2.getX() == ValreiFight.test1.getX() ? 0 : ValreiFight.test2.getX() > ValreiFight.test1.getX() ? -1 : 1);
            if ((newX != ValreiFight.test1.getX()) && 
              (ValreiFight.this.isMoveValid(this, newX, newY))) {
              return new Point(newX, newY);
            }
            break;
          case 2: 
            ...
            if (((newX != ValreiFight.test1.getX()) || (newY != ValreiFight.test1.getY())) && 
              (ValreiFight.this.isMoveValid(this, newX, newY))) {
              return new Point(newX, newY);
            }
            break;
    ...
    public byte getDeitySpell(FightEntity defender)
    {
      ...
      if ((getFavor() >= 30.0F) && (getHealth() < 75.0F) && (defender.getHealth() > getHealth())) {
        preferredType = 0;
      } else if ((getFavor() >= 50.0F) && (getAttackBuffed() == 0.0F)) {
        if (defender.entityBase.getCurrentSkill(101) > defender.entityBase.getCurrentSkill(103)) {
          preferredType = 4;
        }
      }
    ...
    {
      byte preferredType = 1;
      if ((getKarma() >= 60.0F) && (getHealth() < 75.0F) && (defender.getHealth() > getHealth()))
      {
        float attackHigh = Math.max(defender.entityBase.getCurrentSkill(102), defender.entityBase
          .getCurrentSkill(104));
        float spellHigh = Math.max(defender.entityBase.getCurrentSkill(105), defender.entityBase
          .getCurrentSkill(106));
        if ((attackHigh > spellHigh) && (getPhysDefBuffed() == 0.0F)) {
          preferredType = 2;
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\server\epic\ValreiFight.class
