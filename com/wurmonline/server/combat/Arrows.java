  ...
  implements MiscConstants, ItemMaterials, SoundNames, TimeConstants
{
  ...
  {
    NOT,  SHIELD,  EVASION,  HIT,  HIT_NO_DAMAGE,  TREE,  FENCE_STONE,  FENCE_TREE,  GROUND;
    
    private ArrowHitting() {}
  }
  
  public static enum ArrowDestroy
  {
    NOT,  NORMAL,  WATER,  BREAKS,  DO_NOTHING;
    
    private ArrowDestroy() {}
  }
  ...
  public static final void hitCreature(Item projectile, Item source, Creature performer, Creature defender, int damage, ArrowDestroy arrowDestroy)
  {
    try
    {
      arrows.add(new Arrows(projectile, performer, defender, -1, -1, ArrowHitting.HIT, arrowDestroy, source, damage, 1.0F, 0.0F, defender
  ...
  public static final void shootCreature(Item shooter, Creature defender, int damage)
  {
    ...
    try
    {
      ...
      {
        if (parriedShield) {
          arrows.add(new Arrows(arrow, defender, ArrowHitting.SHIELD, ArrowDestroy.NORMAL, shooter, 0.0D, defender
            .getBody().getRandomWoundPos(), proj));
        } else {
          arrows.add(new Arrows(arrow, defender, ArrowHitting.HIT, ArrowDestroy.NORMAL, shooter, damage, defender
            .getBody().getRandomWoundPos(), proj));
        }
      }
      catch (Exception e)
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\server\combat\Arrows.class
