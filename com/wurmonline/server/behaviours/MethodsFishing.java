...
import com.wurmonline.shared.constants.FishingEnums.RodType;
  ...
  implements MiscConstants
{
  ...
  static boolean fish(Creature performer, Item source, int tilex, int tiley, int tile, float counter, Action act)
  {
    ...
    if (counter == 1.0F)
    {
      ...
      case 0: 
        ...
        performer.getCommunicator().sendFishStart(minRadius, maxRadius, rodType, rodMaterial, reelType.getTypeId(), reelMaterial, floatType
          .getTypeId(), baitType.getTypeId());
      ...
      performer.sendActionControl(Actions.actionEntrys[' '].getVerbString(), true, defaultTimer);
      return false;
    }
    byte currentPhase = (byte)act.getTickCount();
  ...
  private static boolean processNetPhase(Creature performer, byte currentPhase, Item net, Action act, int tilex, int tiley, Skill fishing)
  {
    ...
    FishRow fr = caughtFish(performer, posx, posy, net);
    if (fr != null)
    {
  ...
  private static boolean processSpearPhase(Creature performer, byte currentPhase, Item spear, Action act, Skill fishing)
  {
    ...
    case 26: 
      Creature fish = act.getCreature();
      testMessage(performer, "You launch the spear at " + fish.getName() + ".", " @fx:" + 
        (int)fish.getPosX() + " fy:" + (int)fish.getPosY());
  ...
  public static Point[] getSpecialSpots(int tilex, int tiley, int season)
  {
    ...
    return (Point[])specialSpots.toArray(new Point[specialSpots.size()]);
  }
  ...
  public static FishRow[] getFishTable(Creature performer, float posX, float posY, Item rod)
  {
    ...
    FishEnums.FishData[] arrayOfFishData1 = FishEnums.FishData.values();int i = arrayOfFishData1.length;
    FishEnums.FishData fd;
    for (FishEnums.FishData localFishData1 = 0; localFishData1 < i; localFishData1++)
    {
      fd = arrayOfFishData1[localFishData1];
      ...
      {
        float fishChance = fd.getChance(knowledge, rod, fishingReel, fishingLine, fishingFloat, fishingHook, fishingBait, posX, posY, performer
          .isOnSurface());
    ...
    if (totalChances > 99.0F)
    {
      ...
      FishEnums.FishData[] arrayOfFishData2 = FishEnums.FishData.values();localFishData1 = arrayOfFishData2.length;
      for (fd = 0; fd < localFishData1; fd++)
      {
        FishEnums.FishData fd = arrayOfFishData2[fd];
  ...
  public static boolean showFishTable(Creature performer, Item source, int tileX, int tileY, float counter, Action act)
  {
    ...
    double fishingSkill = performer.getSkills().getSkillOrLearn(10033).getKnowledge(0.0D);
    float posx = (tileX << 2) + 2;
    float posy = (tileY << 2) + 2;
    ...
    {
      FishRow localFishRow1;
      FishRow fishChance;
      if ((act.justTickedSecond()) && (act.getSecond() == 15))
      {
        FishRow[] fishChances = getFishTable(performer, posx, posy, source);
        FishRow topChance = null;
        FishRow[] arrayOfFishRow1 = fishChances;int i = arrayOfFishRow1.length;
        for (localFishRow1 = 0; localFishRow1 < i; localFishRow1++)
        {
          fishChance = arrayOfFishRow1[localFishRow1];
        ...
        performer.getCommunicator().sendNormalServerMessage("The most common fish around here that you think you might catch with the " + source.getName() + " is a " + topChance
          .getName() + ".");
      }
      else if ((act.justTickedSecond()) && (act.getSecond() == 25))
      {
        FishRow[] fishChances = getFishTable(performer, posx, posy, source);
        FishRow topChance = null;
        Object otherChances = new ArrayList();
        FishRow[] arrayOfFishRow2 = fishChances;localFishRow1 = arrayOfFishRow2.length;
        for (fishChance = 0; fishChance < localFishRow1; fishChance++)
        {
          FishRow fishChance = arrayOfFishRow2[fishChance];
          if (!FishEnums.FishData.fromName(fishChance.getName()).isSpecialFish())
          {
            float chance = fishChance.getChance();
            if (chance > 0.0F)
            {
              ...
              if (chance > (100.0D - fishingSkill) / 5.0D + 5.0D) {
                ((ArrayList)otherChances).add(fishChance);
              }
            }
          }
        }
        ((ArrayList)otherChances).remove(topChance);
        if ((((ArrayList)otherChances).isEmpty()) || (fishingSkill < 20.0D))
        {
        ...
        {
          String allOthers = "";
          for (int i = 0; i < ((ArrayList)otherChances).size(); i++)
          {
            if ((i > 0) && (i == ((ArrayList)otherChances).size() - 1)) {
              allOthers = allOthers + " and ";
            } else if (i > 0) {
              allOthers = allOthers + ", ";
            }
            allOthers = allOthers + ((FishRow)((ArrayList)otherChances).get(i)).getName();
          }
  ...
  {
    FishRow[] fishChances = getFishTable(performer, posX, posY, rod);
  ...
  public static boolean processSpearStrike(Creature performer, Action act, Skill fishing, Item spear, float posX, float posY)
  {
    ...
    if (dd > 1.0F)
    {
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " attempted to spear a fish and failed!", performer, 5);
    ...
    if (result <= 0.0F)
    {
      if ((result < -80.0F) && (performer.isWithinDistanceTo(posX, posY, 0.0F, 2.0F)))
      {
        ...
        byte foot = Server.rand.nextBoolean() ? 15 : 16;
        performer.getCommunicator().sendNormalServerMessage("You miss the " + fish
          .getName() + " and hit your own foot!");
        Server.getInstance().broadCastAction(performer
          .getName() + " spears their own foot.. how silly!", performer, 5);
        
        performer.addWoundOfType(null, (byte)2, foot, false, 1.0F, true, 400.0D * bodyStr.getKnowledge(0.0D), 0.0F, 0.0F, false, false);
      }
      else
      {
    ...
    else
    {
      ...
      Server.getInstance().broadCastAction(performer
        .getName() + " managed to spear a fish!", performer, 5);
  ...
  public static boolean processFishStrike(Creature performer, Action act, Skill fishing, Item rod)
  {
    ...
    {
      if ((act.getTickCount() != 4) && (act.getTickCount() != 2) && 
        (act.getTickCount() != 19))
      {
    ...
    Item fishingHook = fishingItems[3];
    Item fishingBait = fishingItems[4];
    float result = getDifficulty(performer, act, performer.getPosX(), performer.getPosY(), fishing, rod, fishingReel, fishingLine, fishingFloat, fishingHook, fishingBait, 0.0F, 40.0F);
    if ((Servers.isThisATestServer()) && ((performer.getPower() > 1) || (performer.hasFlag(51)))) {
  ...
  private static boolean processSpearMissed(Creature performer, Action act, Item spear)
  {
    ...
    float damMod = fdam * Math.max(0.1F, faid.getWeight() / 3000);
  ...
  {
    FishRow fr = caughtFish(performer, act.getPosX(), act.getPosY(), source);
    if (fr == null)
    {
    ...
    if (startCmd == 20)
    {
      ...
      Point4f mid = calcSpot(act.getPosX(), act.getPosY(), performer.getStatus().getRotation(), dist);
      Point4f start = calcSpot(mid.getPosX(), mid.getPosY(), angle, 10.0F);
      
      Point4f end = calcSpot(start.getPosX(), start.getPosY(), start.getRot(), 20.0F);
      try
      {
    ...
    if ((Servers.isThisATestServer()) && ((performer.getPower() > 1) || (performer.hasFlag(51))))
    {
      if (performer.getPower() >= 2) {
        testMessage(performer, "", performer.getName() + " @px:" + (int)performer.getPosX() + ",py:" + 
          (int)performer.getPosY() + ",pr:" + (int)performer.getStatus().getRotation());
      }
      if (startCmd == 20) {
  ...
  public static Creature makeFishCreature(Creature performer, Point4f start, String fishName, double ql, byte fishTypeId, float speedMod, Point4f end)
  {
    try
    {
      Creature fish = Creature.doNew(119, start.getPosX(), start.getPosY(), start.getRot(), performer
        .getLayer(), fishName, (byte)1, Server.rand.nextBoolean() ? 0 : (byte)0);
  ...
  private static int makeDeadFish(Creature performer, Action act, Skill fishing, byte fishTypeId, Item source, Item container)
  {
    ...
    if (fish == null)
    {
      ...
      double ql = getQL(performer, source, fishing, fishTypeId, act.getPosX(), act.getPosY(), false);
      ...
      weight = (int)(it.getWeightGrams() * (ql / 100.0D));
    }
    else
    {
      FishAI.FishAIData faid = (FishAI.FishAIData)fish.getCreatureAIData();
    ...
    if (weight == 0)
    {
      performer.getCommunicator().sendNormalServerMessage("You fumbled when trying to move the fish to " + container
        .getName() + ", and it got away.");
      return 0;
    }
    ...
    if (weight >= fd.getMinWeight())
    {
      try
      {
        ...
        Item deadFish = ItemFactory.createItem(fd.getTemplateId(), (float)ql, (byte)2, act
        ...
        {
          performer.getCommunicator().sendNormalServerMessage("You fumbled when trying to move the " + deadFish
            .getName() + " to " + container.getName() + ", and it got away.");
  ...
  private static float additionalDamage(Item item, float damMod, boolean flexible)
  {
    ...
    float typeMod = item.isMetal() ? 1.0F : item.isWood() ? 2.0F : 0.5F;
  ...
  private static double getQL(Creature performer, Item source, Skill fishing, byte fishTypeId, float posx, float posy, boolean noSkill)
  {
    ...
    double power = fishing.skillCheck(difficulty, null, 0.0D, noSkill, 10.0F);
    double ql = 10.0D + 0.9D * Math.max(Server.rand.nextFloat() * 10.0F, power);
    return ql;
  }
  ...
  private static boolean autoCast(Creature performer, Action act, Item rod)
  {
    ...
    {
      performer.getCommunicator().sendNormalServerMessage("The water is too shallow to fish.");
      testMessage(performer, "", performer
        .getTileX() + "=" + tilex + "," + performer.getTileY() + "=" + tiley);
      return true;
    }
  ...
  private static boolean processFishCasted(Creature performer, Action act, float castX, float castY, Item rod, boolean manualMode)
  {
    ...
    performer.sendActionControl(Actions.actionEntrys[' '].getVerbString(), true, 1800);
    Item[] fishingItems = rod.getFishingItems();
  ...
  private static boolean processFishBite(Creature performer, Action act, Item rod, Skill fishing)
  {
    ...
    Item fishingBait = fishingItems[4];
    FishEnums.FishData fd = faid.getFishData();
    float damMod = fd.getDamageMod() * Math.max(0.1F, faid.getWeight() / 3000);
    ...
    int skillBonus = (int)(fishing.getKnowledge(0.0D) / 3.0D);
        ...
        if (contents.length > 0) {
          for (Item item : contents) {
            if (item.getTemplateId() == templateId) {
              if ((material == 0) || (item.getMaterial() == material))
              {
  ...
  private static boolean doAutoReplace(Creature performer, Action act)
  {
    ...
    float knowledge = (float)fishing.getKnowledge(0.0D);
    ...
    if ((rod.getTemplateId() == 1346) && (reel == null) && (hasTacklebox))
    {
      if (knowledge >= 90.0F)
      {
        ...
        if (replaced)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You managed to put another " + reel.getName() + " in the " + rod
            .getName() + "!");
        }
        else
        {
    ...
    if ((line == null) && (hasTacklebox))
    {
      if ((knowledge > 70.0F) || (reel != null))
      {
        ...
        if (replaced)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You managed to put another " + line.getName() + " in the " + lineParent
            .getName() + "!");
        }
        else
        {
    ...
    if ((afloat == null) && (hasTacklebox)) {
      if (knowledge > 50.0F)
      {
        ...
        if (replaced)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You managed to put another " + floatName + " on the " + line
            .getName() + "!");
        }
        else
        {
    ...
    if ((hook == null) && (hasTacklebox)) {
      if (knowledge > 30.0F)
      {
        ...
        if (replaced)
        {
          ...
          performer.getCommunicator().sendNormalServerMessage("You managed to put another " + hook.getName() + " on the " + line
            .getName() + "!");
        }
        else
        {
    ...
    if (hook != null)
    {
      ...
      if ((bait == null) && (hasTacklebox)) {
        if (knowledge > 10.0F)
        {
          ...
          {
            bait = hook.getFishingBait();
            performer.getCommunicator().sendNormalServerMessage("You managed to put another " + bait.getName() + " on the " + hook
              .getName() + "!");
          }
          else
          {
  ...
  {
    for (Iterator localIterator1 = tacklebox.getItems().iterator(); localIterator1.hasNext();)
    {
      compartment = (Item)localIterator1.next();
  ...
  private static boolean processFishPull(Creature performer, Action act, Skill fishing, Item rod, boolean initial)
  {
    ...
    float pstr = (float)performer.getSkills().getSkillOrLearn(102).getKnowledge(0.0D);
    float strBonus = Math.max(1.0F, pstr - fstr);
    float stamBonus = faid.getBodyStamina() - 20.0F;
    float bonus = (strBonus + stamBonus) / 2.0F;
    
    float result = getDifficulty(performer, act, act.getPosX(), act.getPosY(), fishing, rod, fishingReel, fishingLine, fishingFloat, fishingHook, fishingBait, bonus, 10.0F);
    ...
    if ((adjusted < 0.0F) && (!isClam))
    {
      ...
      else
      {
        ...
        float rot = (float)(lNewrot * 57.29577951308232D) - 90.0F;
    ...
    else
    {
      ...
      double lNewrot = Math.atan2(performer.getPosY() - fish.getPosY(), performer.getPosX() - fish.getPosX());
      float rot = (float)(lNewrot * 57.29577951308232D);
      Point4f end;
      float speedMod;
      Point4f end;
      if (isClam)
    ...
    FishEnums.FishData fd = faid.getFishData();
    float fdam = fd.getDamageMod();
    float damMod = (float)(fdam * Math.max(0.10000000149011612D, faid.getWeight() > 6000 ? Math.pow(faid.getWeight() / 1000, 0.6D) : faid.getWeight() / 3000));
  ...
  private static boolean processFishLineSnapped(Creature performer, Action act, Item rod)
  {
    ...
    if (currentPhase == 7)
    {
      ...
      float fdam = fd.getDamageMod() * 5.0F;
      float damMod = fdam * Math.max(0.1F, weight / 3000);
  ...
  public static void fromClient(Creature performer, byte subCommand, float posX, float posY)
  {
    try
    {
      ...
      case 9: 
        if (phase != 0)
        {
          testMessage(performer, "Incorrect fishing subcommand", " (" + 
            fromCommand(subCommand) + ") for phase (" + fromCommand(phase) + ")");
          logger.log(Level.WARNING, "Incorrect fishing subcommand (" + fromCommand(subCommand) + ") for phase (" + 
            fromCommand(phase) + ")");
          return;
        }
        Item rod = act.getSubject();
  ...
  private static float[] getMinMaxRadius(Item rod, Item line)
  {
    ...
    float linelength = getSingleLineLength(line);
  ...
  {
    int[] bgRed = { 181, 34, 183, 192 };
    int[] bgGreen = { 230, 177, 141, 192 };
    int[] bgBlue = { 29, 0, 76, 192 };
    return new Color(bgRed[season], bgGreen[season], bgBlue[season]);
  }
  
  public static Point getSeasonOffset(int season)
  {
    int[] offsetXs = { 0, 128, 0, 128 };
    int[] offsetYs = { 0, 0, 128, 128 };
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\server\behaviours\MethodsFishing.class
