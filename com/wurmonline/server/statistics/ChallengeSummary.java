  ...
  public final void saveCurrentPersonalHtmlPage()
  {
    ...
    new Thread()
    {
      public void run()
      {
        ...
        try
        {
          ...
          for (ChallengePointEnum.ChallengeScenario scenario : ChallengePointEnum.ChallengeScenario.getScenarios()) {
            if (scenario.getNum() > 0)
            {
              ...
              if (summary != null)
              {
                output.write("<img src=\"" + summary.getRoundIcon() + "\" alt=\"round icon\"/><p><a href=\"../main" + summary
                  .getRound() + ".html\">" + summary.getRoundName() + "</a></p>\n\t");
                try
                {
                ...
                for (ChallengeScore score : summary.getScores()) {
                  if (score.getType() != 0) {
                    output.write("<TR class=\"gameDataTopTenTR\">\n\t\t\t<TD class=\"gameDataTopTenTDName\">" + 
                      ChallengePointEnum.ChallengePoint.fromInt(score.getType()).getName() + "</TD>\n\t\t\t<TD class=\"gameDataTopTenTDValue\">" + score
        ...
        {
          ChallengeSummary.logger.log(Level.WARNING, "Failed to close html file for " + ChallengeSummary.this.name, iox);
        }
        finally
        {
          try
      ...
      if (!writing)
      {
        ...
        new Thread()
        {
          public void run()
          {
            ...
            try
            {
              File aFile = ChallengeSummary.access$300();
              
              output = new BufferedWriter(new FileWriter(aFile));
              output.write("<!DOCTYPE html><HTML>\n\t<HEAD>\n\t<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><TITLE>Wurm Online Challenge Standings</TITLE>\n\t<link rel=\"stylesheet\" type=\"text/css\" href=\"http://www.wurmonline.com/css/gameData.css\" />\n\t</HEAD>\n\n<BODY id=\"body\" class=\"gameDataBody\">\n\t");
              output.write("<H1>Summary for " + ChallengePointEnum.ChallengeScenario.current.getName() + "</H1>\n\t<br>");
              output.write("<img src=\"" + ChallengePointEnum.ChallengeScenario.current.getUrl() + "\" alt=\"round icon\"/><p>" + ChallengePointEnum.ChallengeScenario.current
                .getDesc() + "</p>\n\t");
              try
              {
                output.write("<TABLE id=\"gameDataTable\">\n\t\t<TR>\n\t\t\t<TH>Challenge</TH>\n\t\t\t<TH>Leader</TH>\n\t\t\t<TH>Points</TH>\n\t\t\t<TH>Last Points</TH>\n\t\t\t<TH>Date</TH>\n\t\t</TR>\n\t\t");
              ...
              for (int x = 0; x < ChallengeSummary.topScores.length; x++) {
                ...
                {
                  scorerUrl = ChallengeSummary.getPlayerHomePageUrl(ChallengeSummary.topScorers[x]);
                  output.write("<TR class=\"gameDataTopTenTR\">\n\t\t\t<TD class=\"gameDataTopTenTDName\"><a href=\"" + 
                    ChallengeSummary.getHighScoreUrl(ChallengeSummary.topScores[x].getType()) + "\">" + 
                    ChallengePointEnum.ChallengePoint.fromInt(ChallengeSummary.topScores[x].getType()).getName() + "</a></TD>\n\t\t\t<TD class=\"gameDataTopTenTDName\"><a href=\"" + scorerUrl
                    
                    .toLowerCase() + "\">" + ChallengeSummary.topScorers[x] + "</a></TD>\n\t\t\t<TD class=\"gameDataTopTenTDValue\">" + 
                    
                    ChallengeSummary.topScores[x].getPoints() + "</TD>\n\t\t\n\t\t<TR>\n\t\t\t<TH>" + 
                    
                    ChallengeSummary.topScores[x].getLastPoints() + "</TH>\n\t\t\t<TH>" + new Date(
                    
                    ChallengeSummary.topScores[x].getLastUpdated()) + "</TH>\n\t\t</TR>\n\t\t");
                }
              }
              output.write("</TABLE>\n\n");
              output.write("</BODY>\n</HTML>");
              x = ChallengePointEnum.ChallengePoint.getTypes();String scorerUrl = x.length;
              for (String str1 = 0; str1 < scorerUrl; str1++)
              {
                ChallengePointEnum.ChallengePoint point = x[str1];
                if (point.getEnumtype() > 0) {
                  if (point.isDirty()) {
            ...
            {
              ChallengeSummary.logger.log(Level.WARNING, "Failed to close html file for main page", iox);
            }
            finally
            {
              try
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\server\statistics\ChallengeSummary.class
