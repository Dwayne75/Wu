  ...
  implements MiscConstants
{
  ...
  public LiquidResult getNewWeightGrams(Item container)
  {
    ...
    Map<Short, Liquid> liquids = new HashMap();
    for (Iterator localIterator = getAllIngredients(true).values().iterator(); localIterator.hasNext();)
    {
      ...
      if (in.getTemplate().isLiquid())
      {
        ...
        int loss = in.getLoss();
        liquids.put(Short.valueOf(id), new Liquid(id, name, ratio, loss));
      }
    }
    Ingredient in;
    short id;
    ...
    for (Item item : container.getItemsAsArray()) {
      if (item.isLiquid())
      {
        ...
        Liquid liquid = (Liquid)liquids.get(Short.valueOf(id));
        if (liquid == null)
        {
          short fgid = (short)item.getTemplate().getFoodGroup();
          liquid = (Liquid)liquids.get(Short.valueOf(fgid));
        }
        if (liquid != null)
        {
  ...
  public class LiquidResult
  {
    ...
    {
      this.errors.put(Short.valueOf(templateId), error);
    }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\wurmonline\server\items\Recipe.class
