  ...
  extends Protocol
{
      ...
      while (!done) {
        try
        {
          ...
          if (r.isContinuation())
          {
            ...
            if (first)
            {
              String s = ntlm.generateType1Msg(flags);
              first = false;
            }
            else
            {
    ...
    throws ProtocolException
  {
    if (this.saslAuthenticator == null) {
      ...
      {
        this.logger.log(Level.FINE, "Can't load SASL authenticator", ex); return;
      }
    }
    List v;
    ...
    throws ProtocolException
  {
    ...
    Response[] r;
    if (peek) {
      r = fetch(msgno, "BODY.PEEK[" + (section == null ? "]" : new StringBuffer().append(section).append("]").toString()));
    } else {
      r = fetch(msgno, "BODY[" + (section == null ? "]" : new StringBuffer().append(section).append("]").toString()));
    }
    notifyResponseHandlers(r);
  ...
  {
    this.ba = ba;
    Response[] r = fetch(msgno, (peek ? "BODY.PEEK[" : "BODY[") + (section == null ? "]<" : new StringBuffer().append(section).append("]<").toString()) + String.valueOf(start) + "." + String.valueOf(size) + ">");
  ...
  public RFC822DATA fetchRFC822(int msgno, String what)
    throws ProtocolException
  {
    Response[] r = fetch(msgno, "RFC822." + what);
    ...
    throws ProtocolException, SearchException
  {
    ...
    for (int i = 0; i < this.searchCharsets.length; i++) {
      if (this.searchCharsets[i] != null) {
        ...
        catch (IOException ioex) {}catch (ProtocolException pex)
        {
          throw pex;
        }
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\sun\mail\imap\protocol\IMAPProtocol.class
