        ...
        if (this.prefixes[i].length() == 0)
        {
          ...
          for (int j = knownURIs.length - 1; j >= 0; j--) {
            if (knownURIs[j].equals(oldUri))
            {
              ...
              assert (this.owner.knownUri2prefixIndexMap[j] == this.current.defaultPrefixIndex);
...
/* Location:              C:\Games\SteamLibrary\steamapps\common\Wurm Unlimited Dedicated Server\server.jar!\com\sun\xml\bind\v2\runtime\output\NamespaceContextImpl.class
